...
class _variables():
	def __init__(self,conn,values):
		...
		for var in values:
			try:
				self._cursor.execute(f"alter table globals add column 'var{var}' 'TEXT'")
			except:
				pass
			try:
				self._cursor.execute(f"alter table servers add column 'var{var}' 'TEXT'")
			except:
				pass
...
class dbd_bot():
	def __init__(self, info):
		...
		try:
			...
			print("Нет модуля ast, начинаем установку!")
	...
	def pycode(self, info):
		code=[]
		for n in info:
			if not n in ["name"]:
				code.append(info[n])
		self._functions[info["name"]]=code
...
def _start(client, bot):
	...
	async def replaces(line, message, guild, content, msg, vars, timerep, author):
		...
		while "$userDate"+bot._style[0] in line:
			if nowloop==line:
				break
			else:
				nowloop=line
				try:
					user = discord.utils.get(guild.members, id=int(line.split("$userDate"+bot._style[0])[-1].split(bot._style[1])[0]))
					line=line.replace("$userDate"+bot._style[0]+line.split("$userDate"+bot._style[0])[-1].split(bot._style[1])[0]+bot._style[1],str(user.created_at))
				except Exception as e:
					line=line.replace("$userDate"+bot._style[0]+line.split("$userDate"+bot._style[0])[-1].split(bot._style[1])[0]+bot._style[1],"none")
		while "$serverDate"+bot._style[0] in line:
			if nowloop==line:
				break
			else:
				nowloop=line
				try:
					server = discord.utils.get(client.guilds, id=int(line.split("$serverDate"+bot._style[0])[-1].split(bot._style[1])[0]))
					line=line.replace("$serverDate"+bot._style[0]+line.split("$serverDate"+bot._style[0])[-1].split(bot._style[1])[0]+bot._style[1],str(server.created_at))
				except Exception as e:
					line=line.replace("$serverDate"+bot._style[0]+line.split("$serverDate"+bot._style[0])[-1].split(bot._style[1])[0]+bot._style[1],"none")
		...
		while "$kot"+bot._style[0] in line:
			if nowloop==line:
				break
			else:
				nowloop=line
				try:
					line=line.replace("$kot"+bot._style[0]+line.split("$kot"+bot._style[0])[-1].split(bot._style[1])[0]+bot._style[1],str(line.split("$kot"+bot._style[0])[-1].split(bot._style[1])[0][::-1]))
				except Exception as e:
					line=line.replace("$kot"+bot._style[0]+line.split("$kot"+bot._style[0])[-1].split(bot._style[1])[0]+bot._style[1],"none")
					...
					for n, u in top:
						try:
							if topos<int(line.split("$leaderboard"+bot._style[0])[-1].split(bot._style[1])[0].split(bot._separator)[2]) and int(line.split("$leaderboard"+bot._style[0])[-1].split(bot._style[1])[0].split(bot._separator)[2])>=0:
								if line.split("$leaderboard"+bot._style[0])[-1].split(bot._style[1])[0].split(bot._separator)[0]=="server":
									t = discord.utils.get(client.guilds, id=int(n))
								elif line.split("$leaderboard"+bot._style[0])[-1].split(bot._style[1])[0].split(bot._separator)[0]=="global":
									t = discord.utils.get(client.get_all_members(), id=int(n))
								if t:
									topos+=1
									try:
										topstr+=str(line.split("$leaderboard"+bot._style[0])[-1].split(bot._style[1])[0].split(bot._separator)[3]).replace("$pos",str(topos)).replace("$name",str(t.name)).replace("$id",str(t.id)).replace("$value",str(u)).replace("$full",str(t)).replace("$discr",str(t.discriminator))+"\n"
									except:
					...
					try:
						...
						if line.startswith("$cooldown"+bot._style[0]):
							...
							def remember(text):
								bot._func=text
								return True
							if any(line.startswith(func) and remember(func) for func in bot._functions):
								def insert_returns(body):
									...
									return body
								fn_name = "_eval_expr"
								cmd = "\n	".join(bot._functions[bot._func])
								body = f"async def {fn_name}():\n	{cmd}"
								parsed = ast.parse(body)
								body = parsed.body[0].body
								body = insert_returns(body)
								env = { "arg":content, "message":message, "bot":client, 'discord': discord, 'commands': commands, 'ctx': ctx, '__import__': __import__ }
	...
	async def on_ready():
		...
		for guild in client.guilds:
			...
			try:
				if bot._db._cursor.execute(f"SELECT id FROM servers WHERE id = '{guild.id}'").fetchone() is None:
					bot._db._cursor.execute(f"INSERT INTO servers VALUES('{guild.id}'{adds})")
			except Exception as e:
				pass
			for var in bot._vars:
				try:
					if bot._db._cursor.execute(f"SELECT var{var} FROM servers WHERE id = '{guild.id}'").fetchone()[0] is None:
						bot._db._cursor.execute(f'UPDATE servers SET var{var}="{bot._vars[var]}" WHERE id="{guild.id}"')
				except Exception as e:
					pass
			for member in guild.members:
				bot._vars["id"]=f'{member.id}'
				try:
					if bot._db._cursor.execute(f"SELECT id FROM globals WHERE id = '{member.id}'").fetchone() is None:
						bot._db._cursor.execute(f"INSERT INTO globals VALUES('{member.id}'{adds})")
				except Exception as e:
					pass
				for var in bot._vars:
					try:
						if bot._db._cursor.execute(f"SELECT var{var} FROM globals WHERE id = '{member.id}'").fetchone()[0] is None:
							bot._db._cursor.execute(f'UPDATE globals SET var{var}="{bot._vars[var]}" WHERE id="{member.id}"')
					except Exception as e:
						pass
	...
	async def on_member_join(user):
		try:
			await use(user, "onJoin", "", "event")
		except Exception as e:
			pass
	@client.event
	async def on_message_delete(message):
		try:
			await use(message, "messageDelete", "", "event")
		except Exception as e:
			pass
