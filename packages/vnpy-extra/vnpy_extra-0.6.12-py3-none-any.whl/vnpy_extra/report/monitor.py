    ...
    def output_daily_report():
        ...
        for strategy_name, symbol_pos_daily_dic in strategy_symbol_pos_daily_dic.items():
            for symbol, pos_daily in symbol_pos_daily_dic.items():
                ...
                holding_list.append([
                    strategy_name, symbol, pos_daily.direction, pos_daily.volume,
                    pos_daily.avg_price,
        ...
        if len(holding_list) > 0:
            df = pd.DataFrame(
                ...
                columns=['策略名称', '合约', '持仓方向', '持仓手数', '持仓均价',
                         '持仓盈亏', '当日平仓盈亏', '累计平仓盈亏', '最新价格', '交易日', '最近成交时间'])
            df.to_csv(os.path.join(output_folder, f"各策略持仓状态_{date_2_str(trade_date)}.csv"), encoding='GBK')
