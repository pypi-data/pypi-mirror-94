...
class DownEmail:
    ...
    # 解析邮件,获取附件
    def get_attachment_dic(self, msg_in) -> Dict[str, TextIO]:
        attachment_dic = {}
        for part in msg_in.walk():
            ...
            if file_name:
                ...
                # att_file.close()
                attachment_dic[filename] = StringIO(data.decode())
            else:
                pass
                # 不是附件，是文本内容
    ...
    def run_ing(self, email_title, recent_n_days=2) -> Dict[str, TextIO]:
        ...
        try:
            ...
            for i in range(index, 0, -1):  # 倒序遍历邮件
                ...
                to_str = parseaddr(msg.get('to_str'))[1]
                cc_str = parseaddr(msg.get_all('cc_str'))[1]  # 抄送人
                subject = DownEmail.decode_str(msg.get('subject'))
                if email_title != subject:
                    continue
                print('from:%s,to:%s,cc_str:%s,subject:%s' % (from_str, to_str, cc_str, subject))
                # 获取邮件时间,格式化收件时间
                date_email = datetime.strptime(msg.get("Date")[0:24], '%a, %d %b %Y %H:%M:%S').date()
                # # 邮件时间格式转换
                # date2 = time.strftime("%Y-%m-%d", date_email)
                if date_email < date_n_days_ago:
                    break  # 倒叙用break
                    # continue # 顺叙用continue
                else:
                    # 获取附件
                    attachment_dic = self.get_attachment_dic(msg)
                    print(attachment_dic.keys())
...
def download_email_attachment(email_title, recent_n_days=3) -> Dict[str, TextIO]:
    try:
        # 输入邮件地址, 口令和POP3服务器地址:
        user = 'mmmaaaggg@163.com'
        # 此处密码是授权码,用于登录第三方邮件客户端
        password = 'CPFOOBJZRJOEDHSL'
