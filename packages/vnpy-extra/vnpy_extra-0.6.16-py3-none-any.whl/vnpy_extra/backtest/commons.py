...
def bulk_backtest_with_backtest_params_iter(
    ...
    for n, (engine_kwargs, param_values, strategy_stats_original) in backtest_params_iter:
        ...
        if multi_symbols:
            ...
            backtest_params_iter.set_description(" vs ".join(engine_kwargs["vt_symbols"]) + f" <{file_name_header}>")
        else:
            vt_symbol_str = engine_kwargs["vt_symbol"]
            backtest_params_iter.set_description(vt_symbol_str  + f" <{file_name_header}>")
    ...
    if backtest_params_count == 0:
        ...
        df_list = [
            pd.DataFrame(results_dic[_]).set_index(multi_valued_param_name_list).rename(columns=column_rename_dic) for _
            in keys_list]
        keys = ['_'.join([
        ...
        for key, df in zip(keys, df_list):
            ...
            df['short_name'] = ''
            df['shown_name'] = ''
            df[df['available']].to_csv(csv_file_path, encoding='GBK')
        ...
        try:
            ...
            result_df['backtest_status'] = StrategyBacktestStatusEnum.Unavailable.value
