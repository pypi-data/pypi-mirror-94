...
class BacktestingEngine(BacktestingEngineBase):
    ...
    def calculate_statistics(self, df: DataFrame = None, output=True):
        ...
        if df is None:
            ...
            if return_std:
                sharpe_ratio = np.round(daily_return / return_std * np.sqrt(240), 2)
            else:
                sharpe_ratio = 0
            ...
            return_drawdown_ratio = np.round(-total_return / max_ddpercent, 2)

        is_unavailable = (
                total_return <= 0
                or info_ratio < 0
                or daily_trade_count < 0.1  # 1/0.1 相当于10天交易一次
