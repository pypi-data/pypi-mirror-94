...
class StrategyBacktestStatusEnum(enum.IntEnum):
    ...
    RollingBacktest = 2  # 每日滚动更新回测状态，寻找合适的入场时机
    CompareTradeData = 3  # 策略已经启动，每日对比实盘交易数据与回测交易数据是否存在较大差异
...
class StrategyInfo(Model):
    ...
    class Meta:
        ...
        indexes = (
            # create a unique on module_name/strategy_class_name
...
class SymbolsInfo(Model):
    ...
    symbols: str = TextField(
        unique=True, help_text="合约列表，以数组形势进行存储，匹配原则是完全匹配，顺序也要保持一致")
...
class StrategyBacktestStats(Model):
    ...
    charts_data = JSONField(help_text="构建charts所需要的数据")
        ...
        try:
            with database.atomic():
                for data_dic in data_dic_list:
                    ...
                    where_clauses = [
                        getattr(StrategyBacktestStats, key) == val
