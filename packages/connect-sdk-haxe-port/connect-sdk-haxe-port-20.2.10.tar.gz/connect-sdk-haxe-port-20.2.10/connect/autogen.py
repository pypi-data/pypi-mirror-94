...
class connect_Env(connect_Base):
    ...
    _hx_fields = []
    _hx_methods = []
    _hx_statics = ["config", "loggers", "defaultQuery", "apiClient", "fulfillmentApi", "usageApi", "tierApi", "generalApi", "marketplaceApi", "subscriptionsApi", "ROOT_LOGGER", "initConfig", "loadConfig", "isConfigInitialized", "initLogger", "getLoggerForRequest", "copyLoggerConfig", "isLoggerInitialized", "initDefaultQuery", "isDefaultQueryInitialized", "getConfig", "getLogger", "getApiClient", "getFulfillmentApi", "getUsageApi", "getTierApi", "getGeneralApi", "getSubscriptionsApi", "getMarketplaceApi", "_reset", "_getDefaultQuery"]
    _hx_interfaces = []
    _hx_super = connect_Base
    ...
    def getLoggerForRequest(request):
        if ((request is not None) and ((Reflect.field(request,"id") is not None))):
            if (not connect_Env.loggers.exists(request.id)):
                originalConfig = Reflect.field(connect_Env.loggers.get("root"),"getInitialConfig")()
                requestLogger = connect_logger_Logger(connect_Env.copyLoggerConfig(originalConfig))
                handler = requestLogger.getHandlers().iterator()
                while handler.hasNext():
                    handler1 = handler.next()
                    handler1.formatter.setRequest(request.id)
                requestLogger.setFilenameForRequest(request)
                connect_Env.loggers.set(request.id,requestLogger)
            return connect_Env.loggers.get(request.id)
        if (not connect_Env.loggers.exists("root")):
            requestLogger = connect_logger_Logger(None)
            connect_Env.loggers.set("root",requestLogger)
        return connect_Env.loggers.get("root")

    @staticmethod
    def copyLoggerConfig(initialConfig):
        newConfig = connect_logger_LoggerConfig()
        ...
        newHandlers = connect_util_Collection()
        handler = initialConfig.handlers_.iterator()
        while handler.hasNext():
            handler1 = handler.next()
            newHandler = connect_logger_LoggerHandler(handler1.formatter.copy(),handler1.writer.copy())
...
class connect_api_ConnectHelper:
    ...
    def put(resource,id,suffix,body,currentRequest = None,logLevel = None):
        return connect_api_ConnectHelper.checkResponse(connect_api_ConnectHelper.connectSyncRequest("PUT",connect_api_ConnectHelper.parsePath(resource,id,suffix),connect_api_ConnectHelper.getHeaders(),None,None,body,None,None,None,currentRequest,logLevel))

    @staticmethod
    def post(resource,id = None,suffix = None,body = None,currentRequest = None,logLevel = None):
        return connect_api_ConnectHelper.checkResponse(connect_api_ConnectHelper.connectSyncRequest("POST",connect_api_ConnectHelper.parsePath(resource,id,suffix),connect_api_ConnectHelper.getHeaders(),None,None,body,None,None,None,currentRequest,logLevel))
    ...
    def delete(resource,id,suffix = None,currentRequest = None,logLevel = None):
        return connect_api_ConnectHelper.checkResponse(connect_api_ConnectHelper.connectSyncRequest("DELETE",connect_api_ConnectHelper.parsePath(resource,id,suffix),connect_api_ConnectHelper.getHeaders(),None,None,None,None,None,None,currentRequest,logLevel))

    @staticmethod
    def connectSyncRequest(method,path,headers,params = None,rqlParams = None,data = None,fileArg = None,fileName = None,fileContent = None,currentRequest = None,logLevel = None):
        if (rqlParams is None):
            rqlParams = False
        ...
        return connect_Env.getApiClient().syncRequestWithLogger(method,url,headers,data,fileArg,fileName,fileContent,None,logger,None)
...
class connect_api_FulfillmentApi(connect_Base):
    ...
    def listRequests(self,filters):
        return connect_api_ConnectHelper.get(connect_api_FulfillmentApi.REQUESTS_PATH,None,None,filters)
...
class connect_api_GeneralApi(connect_Base):
    ...
    def listAccounts(self,filters):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.ACCOUNTS_PATH,None,None,filters)
    ...
    def listAccountUsers(self,id):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.ACCOUNTS_PATH,id,"users")
    ...
    def listProducts(self,filters):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.PRODUCTS_PATH,None,None,filters,True)

    def getProduct(self,id):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.PRODUCTS_PATH,id)

    def listProductActions(self,id,filters):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.PRODUCTS_PATH,id,"actions",filters)
    ...
    def listProductItems(self,id,filters):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.PRODUCTS_PATH,id,"items",filters)

    def listProductParameters(self,id,filters):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.PRODUCTS_PATH,id,"parameters",filters)
    ...
    def listProductAgreements(self,id,filters):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.PRODUCTS_PATH,id,"agreements",filters)

    def listProductMedia(self,id,filters):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.PRODUCTS_PATH,id,"media",filters)
    ...
    def listCategories(self,filters):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.CATEGORIES_PATH,None,None,filters)
...
class connect_api_MarketplaceApi:
    ...
    def listAgreements(self,filters):
        return connect_api_ConnectHelper.get(connect_api_MarketplaceApi.AGREEMENTS_PATH,None,None,filters)
...
class connect_api_SubscriptionsApi(connect_Base):
    ...
    def listRecurringAssets(self,filters):
        return connect_api_ConnectHelper.get(connect_api_SubscriptionsApi.ASSETS_PATH,None,None,filters,True)
    ...
    def listBillingRequests(self,filters):
        return connect_api_ConnectHelper.get(connect_api_SubscriptionsApi.REQUESTS_PATH,None,None,filters,True)
...
class connect_api_TierApi(connect_Base):
    ...
    def listTierConfigRequests(self,filters):
        return connect_api_ConnectHelper.get(connect_api_TierApi.TCR_PATH,None,None,filters,True)
...
class connect_api_UsageApi(connect_Base):
    ...
    def listUsageFiles(self,filters):
        return connect_api_ConnectHelper.get(connect_api_UsageApi.USAGE_FILES_PATH,None,None,filters,True)
...
class connect_flow_FlowExecutor:
    ...
    _hx_methods = ["addStep", "executeRequest", "processSteps", "processStep", "callStepFunc", "processAbortOrEnd", "abort"]
...
class connect_flow_FlowLogger:
    ...
    _hx_methods = ["openFlowSection", "closeFlowSection", "cleanLogContext", "openRequestSection", "closeRequestSection", "openSetupSection", "closeSetupSection", "openStepSection", "closeStepSection", "writeStepInfo", "writeStepError", "writeStep", "getFormattedRequest", "getFormattedData", "getDataTable", "writeStepSkip", "writeStepSavedInConnect", "writeStepSavedLocally", "writeStepSaveFailed", "writeMigrationMessage", "writeException", "writeLoadedStepData", "setContextRequest"]
    ...
    def cleanLogContext(self):
        requestLogger = connect_Env.getLoggerForRequest(self.currentRequest)
        handler = requestLogger.getHandlers().iterator()
        while handler.hasNext():
            handler1 = handler.next()
            handler1.formatter.setRequest(None)
    ...
    def writeMigrationMessage(self,request):
        self.openRequestSection(request)
    ...
    def setContextRequest(self,requestId):
        handler = connect_Env.getLoggerForRequest(self.currentRequest).getHandlers().iterator()
        while handler.hasNext():
            handler1 = handler.next()
            handler1.formatter.setRequest(requestId)
...
class connect_logger_FileLoggerWriter(connect_Base):
    ...
    _hx_methods = ["setFilename", "getFilename", "writeLine", "getFile", "setFile", "copy"]
    _hx_statics = []
...
class connect_logger_ILoggerFormatter:
    ...
    __slots__ = ()
    _hx_methods = ["formatSection", "formatBlock", "formatCodeBlock", "formatList", "formatTable", "formatLine", "getFileExtension", "setRequest", "copy"]
...
class connect_logger_Logger(connect_Base):
    ...
    __slots__ = ("path", "level", "handlers", "sections", "maskedFields", "maskedParams", "regexMaskingList", "compact", "beautify", "defaultFilename", "intialConfig")
    _hx_fields = ["path", "level", "handlers", "sections", "maskedFields", "maskedParams", "regexMaskingList", "compact", "beautify", "defaultFilename", "intialConfig"]
    _hx_methods = ["getInitialConfig", "getPath", "getLevel", "isBeautified", "isCompact", "setFilename", "setFilenameForRequest", "getFilename", "getHandlers", "openSection", "closeSection", "writeBlock", "writeCodeBlock", "writeList", "writeTable", "write", "getMaskedFields", "getMaskedParams", "getRegExMaskingList", "log", "debug", "info", "notice", "warning", "error", "critical", "alert", "emergency", "_writeToHandler", "writeSections"]
...
class connect_logger_PlainLoggerFormatter(connect_Base):
    ...
    _hx_methods = ["setRequest", "formatSection", "formatPrefix", "formatBlock", "getPrefixedLines", "getUnprefixedLines", "formatCodeBlock", "formatList", "formatTable", "formatLine", "getFileExtension", "copy"]
    ...
    def setRequest(self,requestId):
        self.currentRequest = (requestId if ((requestId is not None)) else connect_logger_PlainLoggerFormatter.NO_REQUEST)
    ...
    def copy(self):
        return connect_logger_PlainLoggerFormatter()
...
class connect_util_Util:
    ...
    __slots__ = ()
    _hx_statics = ["replaceStrSensitiveData", "beautify", "beautifyObject", "maskFields", "maskParams", "isJson", "isJsonObject", "isJsonArray", "isArray", "isStruct", "createObjectDiff", "getLines", "boolToInt", "addIdsToObject", "compactArray"]
        ...
        if (Type.typeof(obj) == ValueType.TObject):
            ...
            while (_g < len(_g1)):
                ...
                if ((fieldName == "params") and Std.isOfType(value,list)):
                    ...
                    _g3 = _hx_local_2()
                    while (_g2 < len(_g3)):
                        param = (_g3[_g2] if _g2 >= 0 and _g2 < len(_g3) else None)
                        _g2 = (_g2 + 1)
                        if (((Type.typeof(param) == ValueType.TObject) and python_Boot.hasField(param,"id")) and python_Boot.hasField(param,"value")):
                            ...
                            if ((maskedParams.indexOf(Std.string(Reflect.field(param,"id"))) != -1) or isPassword):
                                ...
                                value1 = StringTools.lpad("","*",len(paramValue))
                                setattr(param,(("_hx_" + "value") if (("value" in python_Boot.keywords)) else (("_hx_" + "value") if (((((len("value") > 2) and ((ord("value"[0]) == 95))) and ((ord("value"[1]) == 95))) and ((ord("value"[(len("value") - 1)]) != 95)))) else "value")),value1)
                elif (((Type.typeof(value) == ValueType.TObject) or Std.isOfType(value,connect_util_Collection)) or Std.isOfType(value,list)):
                    value2 = connect_util_Util.maskParams(value)
                    setattr(obj,(("_hx_" + fieldName) if ((fieldName in python_Boot.keywords)) else (("_hx_" + fieldName) if (((((len(fieldName) > 2) and ((ord(fieldName[0]) == 95))) and ((ord(fieldName[1]) == 95))) and ((ord(fieldName[(len(fieldName) - 1)]) != 95)))) else fieldName)),value2)
        elif Std.isOfType(obj,list):
            arr = obj
            def _hx_local_5():
                ...
                return list(map(_hx_local_4,arr))
            return _hx_local_5()
        return obj
...
class haxe_macro_Access(Enum):
    ...
    _hx_constructs = ["APublic", "APrivate", "AStatic", "AOverride", "ADynamic", "AInline", "AMacro", "AFinal", "AExtern"]
...
class haxe_macro_TypeDefKind(Enum):
    ...
    @staticmethod
    def TDClass(superClass = None,interfaces= None,isInterface= None,isFinal= None):
        return haxe_macro_TypeDefKind("TDClass", 2, (superClass,interfaces,isInterface,isFinal))
