...
def configure(file_name):
    ...
    inputDfpath = str(os.path.join(os.path.split(os.path.dirname(os.path.dirname(os.getcwd())))[-2], 'csv', 'features_extracted',current_video + '.' + wfileType))
...
def load_frame(number, master, entry):
    ...
    max_size = 1080, 650
    IDlabelLoc  = []
    imageIn = cv2.imread(frames_in[current_frame_number])
