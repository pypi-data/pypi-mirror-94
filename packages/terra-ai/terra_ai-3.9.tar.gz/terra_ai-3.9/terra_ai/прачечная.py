...
def ввод_данных():
  ...
  if all(i in 'случайно' for i in rand_or_request):
    ...
    вес_мешков = np.random.randint(1, 6, len(количество_мешков)) #Масса каждого мешка
    
    новые_мешки = ['10:00', '15:00', '18:00']
    
    количество_новых_мешков = np.arange(10) #Количество мешков   
    вес_новых_мешков =  np.random.randint(2, 6, len(количество_новых_мешков))
...
def рассчитать_план_на_день(общее_число_ботов, количество_выживших, количество_эпох, коэфициент_мутаций):
  ...
  for it in range(epohs): # Проходим по всем эпохам
      ...
      if (it == 1000): # Меняем коэфициет мутации после 1000-ой эпохи
          mut = 0.05  
      if (it == 1500): # Меняем коэфициет мутации после 1000-ой эпохи
          mut = 0.02 
      ...
      print('Выполняется эпоха №' '{:>2} {:>2}'.format(it, ''), '{:>3}'.format(np.round(time.time() - curr_time, 1)), 'сек,', '{:>15}'.format('10 лучших ботов'), np.array2string(np.round(sval[0:10], 2).astype('int'),  precision=2,suppress_small=True))   
