...
def добавить_датасет_LabStory():
  ...
  description = input('Введите описание датасета: ')
  url = input('Ссылка на датасет: ')
  print()
  data = {
      "url": url,
      "name": name,
  ...
  d = datasets['data'][-1]
  датасет_id = d["id"]
  print(f'id добавленного датасета: {d["id"]}')
  print('-----------------------')    
...
def список_датасетов_LabStory():
  ...
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
    ...
    print('Код ошибки: ', json_response.status_code)
    print(json_response.text)
  print('Количество датасетов:', len(datasets['data']))
  print()
  for d in datasets['data']:
    print(f'Датасет: {d["name"]} (id {d["id"]}), {d["description"]}')
    print('Ссылка', d['url'])
    print('-----------------------')
...
def удалить_датасет_LabStory(id):
  ...
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
    ...
    print('Ошибка выполнения запроса')
    print('Код ошибки: ', json_response.status_code)
    print('нельзя удалить датасет который привязан к эксперименту')
    print(json_response.text)
...
def выбрать_датасет_LabStory(id):
  ...
  print('id датасета: ', датасет_id)
  print('Название датасета: ', datasets['name'])
  print('Описание датасета: ', datasets['description'])
  print('-----------------------')
    
# Создание задачи
def добавить_задачу_LabStory():
  if 'token' not in globals():
    print('Для выполнения данной функции, необходимо авторизироваться.')
    return
  global задача_id
...
def список_задач_LabStory():
  ...
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
    ...
    print('Код ошибки: ', json_response.status_code)
    print(json_response.text)
  print('Количество задач:', len(tasks['data']))
  print()
  for t in tasks['data']:
    print(f'Информация о задаче: название - {t["name"]} (id {t["id"]}), описание: {t["description"]}')
    print('Количество экспериментов', t['experiments_count'])
    print('-----------------------')
...
def выбрать_задачу_LabStory(id):
  ...
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
    ...
    print(json_response.text)
  задача_id = task['id']
  print('id задачи: ', задача_id)
  print('Название задачи: ', task['name'])
  print('Описание задачи: ', task['description'])
  print('Количетсво экспериментов', task['experiments_count'])
  print('-----------------------')
  
# Сохранение эксперимента
def сохранить_эксперимент_LabStory():
  if 'token' not in globals():
    print('Для выполнения данной функции, необходимо авторизироваться.')
    return
  experiment_description_list = ['Название модели', 'Описание модели', 'Комментарий', 'Тип обработки данных', 'Описание типа обработки данных', 'Скопируйте кодовую ячейку с моделью', 'хотите_присвоить_теги? (через запятую)']
  for i in range(len(experiment_description_list)):
    if i != len(experiment_description_list)-1:
      experiment_description_list[i] = input(f'Укажите {experiment_description_list[i].replace("_"," ")}: ')
    else:
      experiment_description_list[i] = input(f'Укажите {experiment_description_list[i].replace("_"," ")}: ').split(',')  

  model_history_list = []
  for key in history.history.keys():
    model_history_list.append(history.history[f'{key}'])  

  loss_float, metric_float, val_loss_float, val_metric_float = '', '', '', ''

  while type(loss_float) == str:
    lossBestValue = (input('Какое значение loss сохранить (укажите "мин" или "макс"): '))
    if lossBestValue == "мин":
      loss_float = min(model_history_list[0])
      val_loss_float = min(model_history_list[2])
    elif lossBestValue == "макс":
      loss_float = max(model_history_list[0])
      val_loss_float = max(model_history_list[2])   
    else:
      print(f'{lossBestValue} - неизвестное имя')
...
def сохранить_эксперимент_LabStory_terra_ai(experiment_dict):
  ...
  if experiment_dict['metrics'] == min:
    metric_float = min(model_history_list[0])
    val_metric_float = min(model_history_list[2])
  elif experiment_dict['metrics'] == max:
    metric_float = max(model_history_list[0])
    val_metric_float = max(model_history_list[2]) 
...
###############################
  ...
  for i in In:  
    if find and any(j in key_words for j in i):
      m_arc = i
...
###############################
  ...
  experiment = {
    ...
    "loss": model_history_list[0],
    "loss_float": loss_float,
    "val_loss": model_history_list[2],
    "val_loss_float": val_loss_float,
    "metric": model_history_list[1],
    "metric_float": metric_float,
    "val_metric": model_history_list[3],
...
# получаем id только что сохраненного эксперимента по id всех экспериментов
  ...
  while type(metric_float) == str:
    metricBestValue = (input('Какое значение metric сохранить (укажите "мин" или "макс"): '))
    if metricBestValue == "мин":
      metric_float = min(model_history_list[0])
      val_metric_float = min(model_history_list[2])
    elif metricBestValue == "макс":
      metric_float = max(model_history_list[0])
      val_metric_float = max(model_history_list[2])   
    else:
      print(f'{metricsBestValue} - неизвестное имя')     
  ...
  experiment = {
      ...
      "user_id": user_id,
      "dataset_id": датасет_id,
      "name_model": experiment_description_list[0],
      "loss": model_history_list[0],
      "loss_float": loss_float,
      "val_loss": model_history_list[2],
      "val_loss_float": val_loss_float,
      ...
      "val_metric": model_history_list[3],
      "val_metric_float": val_metric_float,
      "function_creating_model": experiment_description_list[5],
      "function_data_preprocessing": experiment_description_list[3],
      "description_model": experiment_description_list[1],
      "description_data_preprocessing": experiment_description_list[4],
      "comment": experiment_description_list[2],
      "tags": experiment_description_list[-1]
  }
  json_response = requests.post(post, json=experiment, headers=headers)
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
    print('Эксперимент успешно сохранен')
...
def список_экспериметов_LabStory():
  ...
  if json_response.status_code == 200: # Если пришел 200-ый код ответа (все хорошо)
    ...
    print('Код ошибки: ', json_response.status_code)
    print(json_response.text)
  print('Количество экспериментов:', len(experiments['data']))
  print()
  for e in experiments['data']:
    print('id эксперимента: ', e['id'])
    print('-----------------------')
...
def посмотреть_эксперимент_по_id_LabStory(id):
  ...
  columns = ['Дата создания', 'Метрика', 'Ошибка', 'Модель', 'Описание модели', 'Комментарий', 'Датасет']
  data = [[experiment['created_at'],
          experiment['metric_float'],
          experiment['loss_float'],
          experiment['name_model'],
...
def показать_изменение_маски_MNIST(number):
  numeric = np.array(imgs)[...,number]
  f, ax = plt.subplots(3,8,figsize=(28, 10))
  for i in range(3):
    for j in range(8):
      ax[i,j].imshow(numeric[i*10+j].reshape((28,28)), cmap='gray')
      ax[i,j].axis('off')
