...
def load_articles(article):
    try:
        ...
        cur  = conn.cursor() 
        try:
            page_title = article["Page_title"].replace("'","\'").replace('"','\"')
            page_id = article["page_id"]
            text = article["processed_text"].replace("'","\'").replace('"','\"')
            isredirect= article["isredirect"].replace("'","\'").replace('"','\"')
            redirect_title=article["redirect"].replace("'","\'").replace('"','\"')
