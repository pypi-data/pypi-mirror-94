...
class RustBackendGenerated:
    ...
    def filter_to_search(self, input: pb.SearchTerm) -> str:
        output = pb.String()
        output.ParseFromString(self._run_command(9, input))
        return output.val

    def normalize_search(self, val: str) -> str:
        input = pb.String(val=val)
        output = pb.String()
    ...
    def negate_search(self, val: str) -> str:
        input = pb.String(val=val)
        output = pb.String()
        output.ParseFromString(self._run_command(13, input))
        return output.val

    def concatenate_searches(self, *, sep: pb.ConcatenateSearchesIn.Separator.V, searches: Sequence[str]) -> str:
        input = pb.ConcatenateSearchesIn(sep=sep, searches=searches)
        output = pb.String()
        output.ParseFromString(self._run_command(14, input))
        return output.val

    def replace_search_term(self, *, search: str, replacement: str) -> str:
        input = pb.ReplaceSearchTermIn(search=search, replacement=replacement)
        output = pb.String()
    ...
    def graphs(self, *, search: str, days: int) -> bytes:
        ...
        return self._run_command(34, input)

    def get_graph_preferences(self) -> bytes:
        input = pb.Empty()
        return self._run_command(35, input)
    ...
    def get_config_bool(self, key: pb.ConfigBool.Key.V) -> bool:
        input = pb.ConfigBool(key=key)
        output = pb.Bool()
        output.ParseFromString(self._run_command(107, input))
        return output.val

    def set_config_bool(self, *, key: pb.ConfigBool.Key.V, value: bool) -> pb.Empty:
