...
class RustBackendGenerated:
    ...
    def unbury_cards_in_current_deck(self, mode: pb.UnburyCardsInCurrentDeckIn.ModeValue) -> pb.Empty:
        input = pb.UnburyCardsInCurrentDeckIn(mode=mode)
        output = pb.Empty()
        output.ParseFromString(self._run_command(22, input))
        return output

    def bury_or_suspend_cards(self, *, card_ids: Sequence[int], mode: pb.BuryOrSuspendCardsIn.ModeValue) -> pb.Empty:
    ...
    def schedule_cards_as_reviews(self, *, card_ids: Sequence[int], min_interval: int, max_interval: int) -> pb.Empty:
        input = pb.ScheduleCardsAsReviewsIn(card_ids=card_ids, min_interval=min_interval, max_interval=max_interval)
        output = pb.Empty()
    ...
    def graphs(self, *, search: str, days: int) -> bytes:
        ...
        return self._run_command(31, input)
    ...
    def get_stock_notetype_legacy(self, kind: pb.StockNoteTypeValue) -> bytes:
    ...
    def format_timespan(self, *, seconds: float, context: pb.FormatTimespanIn.ContextValue) -> str:
    ...
    def register_tags(self, *, tags: str, preserve_usn: bool, usn: int, clear_first: bool) -> bool:
        input = pb.RegisterTagsIn(tags=tags, preserve_usn=preserve_usn, usn=usn, clear_first=clear_first)
        output = pb.Bool()
