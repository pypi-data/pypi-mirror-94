...
def upload_new_model_version(model_name, env, folder, metadata):
    ...
    version = cdf.count_file_versions(filter, directory_prefix='/mlops') + 1
...
def download_model_version(model_name, env, folder, metadata={}):
    ...
    """
    external_id = cdf.find_model_version(model_name, env, metadata)
...
def set_up_model_artifact(artifact_folder, model_name):
    ...
    model_id = cdf.download_model_version(model_name, ENV, artifact_folder)
