...
class CliAtomicUserInteraction(IAtomicUserInteraction):
    ...
    def request_int(self, prompt: str, min: int = None, max: int = None):
        ...
        if min and max:
            prompt = prompt.replace(":", " ")
            prompt = prompt + f"[between {min} and {max} (inclusive)]:"
        elif min:
            prompt = prompt.replace(":", " ")
            prompt = prompt + f"[greater than or equal to {min}]:"
        elif max:
            prompt = prompt.replace(":", " ")
            prompt = prompt + f"[less than or equal to {max}]:"
    ...
    def request_enum(self, enum, prompt:str=None):
        ...
        while True:
            if issubclass(enum, Enum):
                ...
                if enum_num and enum_num in [i.value for i in enum]:
                    return enum(enum_num)
                elif not enum_num and inp in [i.name for i in enum]:
    ...
    @staticmethod
    def pretty_print_dataframe(df: pd.DataFrame, title: str = None, display_width: int = 2000, display_max_columns: int=2000):
        if title:
            print(title)

        with pd.option_context('display.max_rows', 500, 'display.max_columns', display_max_columns, 'display.width', display_width):
            print(f"{df}\n")
