...
class Condition(ABC):
    ...
    def _date_val(self, dt, date_format):
    ...
    def add_date_condition(
        ...
        result = And([self])
        if from_date:
            result &= date_field >= self._date_val(from_date, date_format)
        if to_date:
            if to_exclusive:
                result &= date_field < self._date_val(to_date, date_format)
            else:
                result &= date_field <= self._date_val(to_date, date_format)
    ...
    def add_daterange_overlap_condition(
        ...
        if from_date:
            ...
            result &= to_date_field >= self._date_val(from_date, date_format)
        if to_date:
            if not from_date_field:
                raise ValueError("Need from_date_field when to_date != None")
            if to_exclusive:
                result &= from_date_field < self._date_val(to_date, date_format)
            else:
                result &= from_date_field <= self._date_val(to_date, date_format)
        return result
    ...
    def split(
        ...
        def _split(c):
            ...
            if isinstance(c, FieldCondition):
                ...
                return c if field_str in field_set else None
    ...
    def parse(
        ...
        safe_dict = {"T": T, field_list_name: field_list, "And": And, "Or": Or}
...
class FieldCondition(Condition):
    ...
    def _query_val(self, repr=False):
        """
