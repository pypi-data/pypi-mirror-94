...
class Client:
    ...
    def salesforce_to_bq(self, salesforce_connection_string, salesforce_table, salesforce_columns, bq_table, bq_table_schema,
                         salesforce_condition=None, salesforce_include_deleted=False, write_preference='append'):
    ...
    def s3_to_gs(self, s3_connection_string, s3_bucket_name,
        ...
        logs.client.logger.info(f'Found {str(files)} files in S3')
    ...
    def s3_to_bq(self, s3_connection_string, bucket_name, object_name,
                 bq_table, auto_detect=True, separator=',',
                 skip_leading_rows=True, write_preference='append', schema=[]):
        """
        ...
        bq_client.upload_table(os.path.join('/tmp/', object_name),
                               bq_table, separator,
                               auto_detect, skip_leading_rows,
                               write_preference, schema)
