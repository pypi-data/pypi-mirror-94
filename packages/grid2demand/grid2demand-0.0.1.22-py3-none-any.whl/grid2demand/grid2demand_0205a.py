...
class Node:
    def __init__(self):
        ...
        self.production = 0
        self.attraction = 0
        self.boundary_flag = 0  # determine the node is boundary or not
        self.poi_id = ''
        self.activity_type = ''
...
def ReadNetworkFiles(input_folder=None):
    ...
    with open(node_filepath, errors='ignore') as fp:
        ...
        if poi_flag == 0:
            # print('poi_id is NOT defind in node.csv. Please check it!')
            logger.warning('poi_id is NOT defind in node.csv. It may cause empty demand volume and agent. Please '
                           'check node.csv!')
    ...
    with open(poi_filepath, errors='ignore') as fp:
        ...
        for line in reader:
            ...
            if area_meter > 90000:  # a simple benchmark to exclude extra large poi nodes
                poi.area = 0
            else:
                area_feet = area_meter * 10.7639104  # convert the area in square meters to square feet
                poi.area = area_feet  # convert the area unit of normal poi nodes to square feet
...
def GetPoiTripRate(trip_rate_folder=None,
    ...
    print('\nNo. of trip purpose used for grid2demand = ', g_trip_purpose)
    ...
    logger.info('No. of trip purpose used for grid2demand = ' + str(g_trip_purpose))
    logger.info('Total number of poi nodes with unmatched production rate = ' + str(
        g_number_of_unmatched_poi_production_rate))
    logger.info('Total number of poi nodes with unmatched attraction rate = ' + str(
...
def GetNodeDemand():
    ...
    for node in g_node_list:
        if node.boundary_flag == 0:  # node is not boundary
...
def RunGravityModel(trip_purpose=None, a=None, b=None, c=None):
    ...
    for i in range(g_number_of_nodes):
        if g_node_list[i].activity_type == 'poi' or g_node_list[i].boundary_flag == 1:
            ...
            error_flag += 1
    if error_flag == 0:
        # print("There is no node with activity_type = 'poi' or is_boundary = '1'. Please check node.csv!")
        logger.error("There is no node with activity_type = 'poi' or is_boundary = '1'. Please check node.csv!")
        sys.exit(0)
