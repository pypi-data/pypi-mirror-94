...
class Worker:
    ...
    def trigger_blobs(self, blob_names: List[str], params=None):
        guid = self._generate_id()
        request_ids = []
        for blob_name in blob_names:
            request_id = self._insert_blob_metadata(blob_name, params, guid)
            self._enqueue(request_id)
    ...
    def get_result(self, request_id):
        ...
        entity = self.table_client.get_entity(self.metadata_table_name, guid, request_hash, select="Result,Error,StackTrace,State,Offset")
    ...
    def get_results(self, guid):
        entities = list(
            self.table_client.query_entities(
                ...
                select="Result,Error,StackTrace,State,Offset"
            )
        )
