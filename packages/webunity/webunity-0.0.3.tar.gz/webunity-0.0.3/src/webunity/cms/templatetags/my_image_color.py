...
#  Based on https://github.com/wagtail/wagtail/blob/master/wagtail/images/templatetags/wagtailimages_tags.py

logger = logging.getLogger('image_color')


@register.tag(name="image_color")
def image_color(parser, token):
    #  Take :
    # - Wagtail Image
    # - Background color
    #  And filters for renditions

    bits = token.split_contents()[1:]
    image_expr = parser.compile_filter(bits[0])
    color_expr = parser.compile_filter(bits[1])
    ...
    if is_valid:
        return ImageNode(image_expr, color_expr, '|'.join(filter_specs), attrs=attrs, output_var_name=output_var_name)
    else:
        raise template.TemplateSyntaxError(
            "'image' tag should be of the form {% image self.photo max-320x200 [ custom-attr=\"value\" ... ] %} "
            "or {% image self.photo max-320x200 as img %}"
...
class ImageNode(template.Node):
    def __init__(self, image_expr, color_expr, filter_spec, output_var_name=None, attrs={}):
    ...
    def render(self, context):
        try:
            ...
            color_expr = self.color_expr.resolve(context)
        ...
        if color_expr and re.search(r'^#(?:[0-9a-fA-F]{3}){1,2}$', color_expr):
            logger.debug('Change bg color with %s' % str(color_expr))
            tag = 'bgcolor-%s' % color_expr.replace('#', '')
            self.filter_spec = tag + '|' + self.filter_spec
