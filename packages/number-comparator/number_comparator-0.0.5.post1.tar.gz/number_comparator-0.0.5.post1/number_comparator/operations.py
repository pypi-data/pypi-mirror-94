...
def averageFromList(l):
    "Extract numbers from a given list and return the average."
    if not isinstance(l, type(["dumb"])):
        raise TypeError(f"Expected lists, got %s" % l)
    ll = list()
    for i in l:
        if isinstance(i, (type(1), type(1.0))):
            ll.append(float(i))
