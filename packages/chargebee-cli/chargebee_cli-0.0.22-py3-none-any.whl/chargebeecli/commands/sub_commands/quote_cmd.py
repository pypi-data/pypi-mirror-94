...
from chargebeecli.processors.quotes.quote import Quote
from chargebeecli.util.theme_util import get_active_theme


@click.command("quote", help='endpoint to perform operation on [quote] resource')
@click.pass_context
...
def cmd(ctx, id, operation, columns, format, export_filename, export_format, export_path):
    Quote(export_format=export_format, export_path=export_path, file_name=export_filename,
