...
from chargebeecli.processors.transactions.transaction import Transaction
from chargebeecli.util.theme_util import get_active_theme


@click.command("txn", help='endpoint to perform operation on [Transaction] resource')
@click.pass_context
...
def cmd(ctx, id, operation, columns, format, export_filename, export_format, export_path):
    Transaction(export_format=export_format, export_path=export_path, file_name=export_filename,
