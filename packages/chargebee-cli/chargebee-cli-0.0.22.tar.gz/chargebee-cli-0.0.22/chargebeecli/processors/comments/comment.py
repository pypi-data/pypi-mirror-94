...
class Comment(Processor, Validator, ResponseFormatter, Exporter, Printer):
    ...
    def format(self):
        if self.to_be_formatted():
            self.tables = super(Comment, self).format(self.response, self.response_format, self.operation,
                                                      self.headers, 'comment', 'list')
        return self
