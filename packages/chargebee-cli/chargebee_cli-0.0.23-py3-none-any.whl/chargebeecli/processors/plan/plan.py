...
class Plan(Processor, Validator, ResponseFormatter, Exporter, Printer):
    ...
    def format(self):
        if self.to_be_formatted():
            self.tables = super(Plan, self).format(self.response, self.response_format, self.operation,
                                                   self.headers, 'plan', 'list')
        return self
