...
class UnbilledCharge(Processor, Validator, ResponseFormatter, Exporter, Printer):
    ...
    def format(self):
        if self.to_be_formatted():
            self.tables = super(UnbilledCharge, self).format(self.response, self.response_format, self.operation,
                                                             self.headers, 'unbilled_charge', 'list')
        return self
