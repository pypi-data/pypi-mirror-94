...
from chargebeecli.processors.cards.card import Card
from chargebeecli.util.theme_util import get_active_theme


@click.command("card", help='endpoint to perform operation on [card] resource')
@click.pass_context
...
def cmd(ctx, id , operation, columns, format, export_filename, export_format, export_path):
    Card(export_format=export_format, export_path=export_path, file_name=export_filename,
