...
from chargebeecli.processors.gifts.gifts import Gift
from chargebeecli.util.theme_util import get_active_theme


@click.command("gift", help='endpoint to perform operation on [gift] resource')
@click.pass_context
...
def cmd(ctx, id, operation, columns, format, export_filename, export_format, export_path):
    Gift(export_format=export_format, export_path=export_path, file_name=export_filename,
