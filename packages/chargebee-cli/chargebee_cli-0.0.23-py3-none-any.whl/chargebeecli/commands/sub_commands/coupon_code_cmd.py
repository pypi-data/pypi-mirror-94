...
from chargebeecli.processors.coupon_codes.coupon_code import CouponCode
from chargebeecli.util.theme_util import get_active_theme


@click.command("coupon_code", help='endpoint to perform operation on [coupon_code] resource')
@click.pass_context
...
def cmd(ctx, id, operation, columns, format, export_filename, export_format, export_path):
    CouponCode(export_format=export_format, export_path=export_path, file_name=export_filename,
