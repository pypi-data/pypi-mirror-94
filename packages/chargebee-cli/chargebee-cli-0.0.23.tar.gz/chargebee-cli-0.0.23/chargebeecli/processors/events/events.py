...
API_URI = '/api/v2/events'


class Event(Processor, Validator, ResponseFormatter, Exporter, Printer):
    ...
    def get_api_header(self):
        return ["api_version", "content", "event_type", "id", "object", "occurred_at", "source", "user",
                "webhook_status"]

    def process(self, ctx, operation, payload, resource_id):
        return super(Event, self).process(ctx, operation, payload, resource_id)
    ...
    def format(self):
        if self.to_be_formatted():
            self.tables = super(Event, self).format(self.response, self.response_format, self.operation,
                                                    self.headers, 'event', 'list')
        return self
