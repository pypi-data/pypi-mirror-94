...
from chargebeecli.processors.invoices.invoice import Invoice
from chargebeecli.util.theme_util import get_active_theme


@click.command("invoice", help='endpoint to perform operation on [invoice] resource')
@click.pass_context
...
def cmd(ctx, id, operation, columns, format, export_filename, export_format, export_path):
    Invoice(export_format=export_format, export_path=export_path, file_name=export_filename,
