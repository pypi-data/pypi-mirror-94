...
from chargebeecli.processors.comments.comment import Comment
from chargebeecli.util.theme_util import get_active_theme


@click.command("comment", help='endpoint to perform operation on [comment] resource')
@click.pass_context
...
def cmd(ctx, id, operation, columns, format, export_filename, export_format, export_path):
    Comment(export_format=export_format, export_path=export_path, file_name=export_filename,
