...
def read_sev(sev_dir, *, channel=0, event_name='', t1=0, t2=0, fs=0, ranges=None,
    ...
    t1 = np.ceil((t1/2.56e-6)) * 2.56e-6
    t2 = np.floor((t2/2.56e-6)) * 2.56e-6
    
    if isinstance(ranges, np.ndarray):
        ranges[0,:] = np.ceil(ranges[0,:]/2.56e-6) * 2.56e-6
        ranges[1,:] = np.floor(ranges[1,:]/2.56e-6) * 2.56e-6
...
upgrade to OpenEx v2.18 or above\n'''.format(file['name'],
    ...
    valid_time_range[0,:] = np.ceil(valid_time_range[0,:]/2.56e-6) * 2.56e-6
    valid_time_range[1,:] = np.floor(valid_time_range[1,:]/2.56e-6) * 2.56e-6
    ...
    for this_event in event_names:
        ...
        if export is None:
            if np.any([fff['gap_text'] != '' for fff in file_list_temp]):
                ...
                for jj in range(num_ranges):
                    data[varname]['data'][jj] = np.zeros((len(channels), absolute_end_sample[jj] - absolute_start_sample[jj] + np.uint64(1 + this_start_sample)), dtype=data_format)
            ...
            if current_name in sev_names:
                ...
                for jj in range(num_ranges):
                    ...
                    if export in ['csv', 'interlaced']:
                        ...
                        minSample = np.uint64(np.maximum(np.ceil((tlo_sample-td_sample)/sf),0))
                        maxSample = np.maximum(np.floor((thi_sample-td_sample)/sf),0)
        ...
        for sev_name in sev_names:
            ...
            if sev_name not in [data.streams[fff].name for fff in data.streams.keys()]:
                ...
                if d is not None: # exporting returns None
                    ...
                    data.streams[sev_name].start_time = np.ceil(t1 * d[sev_name].fs) / d[sev_name].fs
