...
def main():
    ...
    parser_project.add_argument(
        "-t", "--thug-life", action="store_true", help="Thug Life, press q to quit")
    ...
    if hasattr(args, "name"):
        if args.name == "project":
            if args.image_segment:
                segment.execute(args.image_segment)
            if args.thug_life:
                print("Currently in Progress, will be functional in next release.")
        if args.name == "resume":
            if args.download:
