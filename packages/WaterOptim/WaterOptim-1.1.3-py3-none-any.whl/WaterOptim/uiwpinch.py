...
class ProjetTree( wx.ScrolledWindow):
    def __init__(self,parent,main_frame):
        ...
        #     sys.exec_prefix
        im1 = os.path.join('', self.main_frame.icons.project_tree)
...
class subs_tab(wx.ScrolledWindow):
        def __init__(self, parent,main_frame):
            ...
            add_btn = wx.BitmapButton(self , 0,bitmap=wx.Bitmap(os.path.join(self.main_frame.icons.add), wx.BITMAP_TYPE_ANY))
            rmv_btn = wx.BitmapButton(self , 1,bitmap=wx.Bitmap(os.path.join(self.main_frame.icons.rmv), wx.BITMAP_TYPE_ANY))
            sizer = wx.BoxSizer(wx.VERTICAL)
...
class MainFrame(wx.Frame):
    def __init__(self):
        ...
        # DICT ================================================================
        self.config_filename=os.path.join(sys.exec_prefix,"Lib\site-package\WaterOptim",'config.json')
        self.dict_filename=os.path.join(sys.exec_prefix,"Lib\site-package\WaterOptim",'dict.json')
        ...
        # set icon
        icon = wx.Icon()
        im_ = os.path.join(self.icons.main_frame)
        ...
        menuExit.SetBitmap(wx.Bitmap(os.path.join(self.icons.logout)))
        self.menuNew.SetBitmap(wx.Bitmap(os.path.join(self.icons.new)))
        self.menuOpen.SetBitmap(wx.Bitmap(os.path.join(self.icons.open)))
        
        menuAbout= helpmenu.Append(wx.ID_ABOUT, self.dict.about.encode('cp1252'))
        menuAbout.SetBitmap(wx.Bitmap(os.path.join(self.icons.chat)))
        menuLicence= helpmenu.Append(wx.ID_INFO, self.dict.lic)
        menuLicence.SetBitmap(wx.Bitmap(os.path.join(self.icons.licence)))
        
        menuOptions=toolsmenu.Append(-1, self.dict.options)
        menuOptions.SetBitmap(wx.Bitmap(os.path.join(self.icons.setting)))
        ...
        self.ToolBar.AddTool(1, "Analyse Pinch", wx.Bitmap(os.path.join(self.icons.run1)), shortHelp="Analyse Pinch", ) 
        self.ToolBar.AddTool(2, "Sensitivity_Analysis", wx.Bitmap(os.path.join(self.icons.sensi)), shortHelp="Analyse de sensibilité\nEffet des paramètres de l'inventaire sur l'analyse Pinch", ) 
        self.ToolBar.AddTool(3, "Export to Excel", wx.Bitmap(os.path.join(self.icons.excel)), shortHelp="Convertir en Excel\nNon dispo dans la version actuelle", ) 
        self.ToolBar.AddTool(4, "Export to pdf", wx.Bitmap(os.path.join(self.icons.pdf)), shortHelp="Rapport d'analyse\nNon dispo dans la version actuelle", ) 
            ...
            if project and e.GetId()==2:
                class CellDialog(wx.Dialog):
                    def __init__(self, parent): 
                        ...
                        icon = wx.Icon()
                        im_ = os.path.join(parent.icons.sensi)
                        icon.CopyFromBitmap(wx.Bitmap(im_, wx.BITMAP_TYPE_ANY))
                        self.SetIcon(icon)
    ...
    def lic_manager(self,lic_notif=False):
        ...
        if path.exists(os.path.join(sys.exec_prefix,"wpinch.lic")):
            ...
            file.close()
            with open(os.path.join(os.path.dirname(sys.argv[0]),"inf.enc"), 'rb') as f:
