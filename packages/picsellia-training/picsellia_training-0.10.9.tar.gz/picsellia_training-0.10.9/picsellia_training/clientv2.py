...
import pxl_multithreading as mlt
import time
import zipfile
import pxl_exceptions as exceptions
from uuid import UUID, uuid4
from pxl_utils import is_uuid
import pxl_utils as utils
from pxl_urls_v2 import Urls as urls
from pathlib import Path
from functools import partial

class Client:
    def __init__(self, api_token, organization_id=None,  host="http://localhost:8000/sdk/v2/", interactive=True):
        """[summary]
        ...
        self.datalake = self.Datalake(self.host, self.auth, organization_id)
    ...
    class Experiment:
        ...
        def publish(self, name=None, id=None):
            ...
            try:
                r = requests.post(self.host + 'experiment/{}/publish'.format(id), data=data, headers=self.auth)
            except Exception:
                raise exceptions.NetworkError("Server is not responding, please check your host or Picsell.ia server status on twitter")
        ...
        def create(self, name=None, description='', id=None, dataset=None, source=None):
            """[summary]
        ...
        def create_data(self, id=None, name="", data={}, type=None):
            """[summary]
        ...
        def update_data(self, name=None, id=None, **kwargs):
            """[summary]
            ...
            data = json.dumps(kwargs)
            try:
                r = requests.patch(self.host + 'experiment/{}/data/{}'.format(id, name), data=data, headers=self.auth)
            except Exception:
                raise exceptions.NetworkError(r.json()["error"])
        ...
        def append_data(self, name=None, id=None, **kwargs):
            """[summary]
            ...
            data = json.dumps(kwargs)
            try:
                r = requests.post(self.host + 'experiment/{}/data/{}'.format(id, name), data=data, headers=self.auth)
            except Exception:
                raise exceptions.NetworkError(r.json()["error"])
        ...
        def log(self, name="", data={}, type=None, id=None, replace=False):
            assert self.id != None or id != None, "Please checkout an experiment or enter the desired experiment id"
            if self.id != None:
                id = self.id
            ...
            if stored == []:
                ...
                self.create_data(id, name, data=data, type=type)
            elif stored is not [] and replace:
                asset = stored[0]
                self.update_data(name, id, data=data, type=type)
            elif stored is not [] and not replace and type is 'line':
                asset = stored[0]
                self.append_data(name, id, data=data, type=type)
        ...
        def _create_or_update_file(self, file_name="", path="", id=None, **kwargs):
            ...
            if stored == []:
                self.create_file(id, file_name, kwargs["object_name"], kwargs["large"])
            else:
                asset = stored[0]
            ...
            if path != None:
                ...
                if filesize < 5*1024*1024:
                    ...
                    if name == 'model-latest':
                        object_name = os.path.join(id, '0', filename)
                    else:
                ...
                if name == 'config':
                    ...
                    object_name = os.path.join(id, '0', 'saved_model.zip')
                    self._send_large_file(path, name, object_name, None, id)
            return object_name
        ...
        def dl_annotations(self, id=None, option="all"):
            ...
            try:
                to_send = {"type": option}
                r = requests.get(self.host + 'experiment/{}/dl_annotations'.format(id),
    ...
    class Datalake:
        def __init__(self, host, auth, organization_id=None):
            """[summary]
        ...
        def push_dataset(self, name=None, description=None, tags=[], imgdir=None, annotations_path=None, format=None, only_annotations=False):
            ...
            if annotations_path is not None and format == "legacy" and not only_annotations:
                try:
                ...
                self.picture.fetch(tags=tags)

                self.dataset.create(name=name, pictures=self.picture.fetched_pictures)
                ...
                for annotations in dict_annotations["annotations"]:
                    pic_name = annotations["external_picture_url"]
                    self.dataset.add_annotation(dataset_id=dataset['dataset']['dataset_id'], picture_id=pic_name, new_picture=True, nb_instances=annotations["nb_labels"], 
                    duration=annotations["time_spent"], data=annotations["annotations"])
                ...
                for l in label_to_create:
                    self.dataset.create_labels(dataset_id=dataset['dataset']['dataset_id'], ann_type=l[0], name=l[1])
            ...
            if only_annotations:
                ...
                print("pushing annotations to an existing dataset")
        ...
        class Picture:
            ...
            def delete(self,):
                ...
                if len(self.fetched_pictures) < 1:
                    ...
                    try:
                        data = {
                            'to_delete': self.fetched_pictures
                        }
                        if self.organization_id == None:
                    ...
                    print(len(self.fetched_pictures), " assets deleted from Lake")
            ...
            def add_tag(self, tags=[]):
                if len(self.fetched_pictures) < 1:
                    ...
                    print(len(tags), " tags added to {} assets ".format(len(self.fetched_pictures)))
            ...
            def remove_tag(self, tags=[]):
                if len(self.fetched_pictures) < 1:
                    ...
                    try:
                        data = {
                            'tags': tags,
                            'to_delete_tag': self.fetched_pictures
                        }
                        if self.organization_id == None:
                    ...
                    print(len(tags), "tags deleted to {} assets ".format(len(self.fetched_pictures)))
        ...
        class Dataset:
            ...
            def delete_annotation(self, dataset_id=None, picture_id=None, all_annotations=False):
                """[summary]
    ...
    class Network:
        ...
        def __init__(self, api_token, host, network_id=None):
            """[summary]
        ...
        def store(self, organization="null", name="", path=None, zip=False):
            if path != None:
                if zip:
                    path = utils.zipdir(path)
                filesize = Path(path).stat().st_size
        ...
        def update_thumb(self, organization="null", name="", path=None):
            if path != None:
                filesize = Path(path).stat().st_size
                if filesize < 5*1024*1024:
                    filename = path.split('/')[-1]
...
if __name__ == '__main__':
    ...
    # clt = Client(api_token="a397928bc42894764b1144d35cb7c43f77866417")
    ...
    # x = clt.datalake.dataset.list()
    # print(clt.datalake.dataset.fetched_dataset['pictures'][0])
    # clt.datalake.picture.remove_tag(tags=['test1'])
    # pictures = clt.datalake.picture.fetch(quantity=1, tags=['tag2'])
    # clt.datalake.dataset.create(name='dataset2', description='this is a test2', pictures=pictures)
    # clt.datalake.dataset.new_version(name='dataset1', version='4th', pictures=pictures)
    # filepath = '/home/tibz/indexd.png'
    # clt.datalake.picture.upload(filepath=filepath, tags=['AAA', 'BBB'])

    api_token = "e5581d7b0d2b10de371018c3b320bd5e4cb7b89d"

    # dataset_id = "a371dfc0-3008-4818-911c-ec78f868f760"

    client = Client(api_token=api_token, host="https://beta.picsellia.com/sdk/v2/")

    import os 
    annotations_path = '/home/tibz/Downloads/Annotation_project_EPI_Detection_Project_downloaded_20210203-130732.json'
    
    client.datalake.push_dataset(name="HelmetDataset", tags=["epi", "colors"], annotations_path=annotations_path, imgdir=None, format="legacy")
    # for imgpath in os.listdir(dirpath):
    #     path = os.path.join(dirpath, imgpath)
    #     client.datalake.picture.upload(path, tags=["epi", "chantier"]) #.fetch(name="testSegmentation", version="first")

    # print(client.datalake.dataset.delete_annotation(all_annotations=True))
