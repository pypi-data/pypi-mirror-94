...
"""
def prepare_elections(experiment_id, approval=False):
    ...
    info = {}
    path = os.path.join("experiments", experiment_id, "controllers", "basic", 'meta.csv')
    ...
    path = os.path.join("experiments", experiment_id, "controllers", "basic", 'map.csv')
    file_ = open(path, 'r')
    
    family_name = [0 for _ in range(num_families)]
    family_special_1 = [0 for _ in range(num_families)]
    family_special_2 = [0 for _ in range(num_families)]
    ...
    for i in range(num_families):
        ...
        if family_name[i] in ["didi", "pl"]:
            family_special_1[i] = str(line[2])
            family_special_2[i] = str(line[3])
        else:
            family_special_1[i] = float(line[2])
            family_special_2[i] = float(line[3])
    ...
    id_ = 0
    for i in range(num_families):
        elections_type = family_name[i]
        special_1 = family_special_1[i]
        special_2 = family_special_2[i]
        for j in range(family_size[i]):
            elections_id = "core_" + str(id_)

            # for netflix
            if elections_type == "netflix" or elections_type == "formula":
                special_1 = -1

            #print("special: ", special)

            if approval:
                el.generate_approval_elections(experiment_id, elections_type, elections_id, num_elections,
                                 num_voters, num_candidates, special_1, special_2)
            else:
                el.generate_elections(experiment_id, elections_type, elections_id, num_elections,
                                 num_voters, num_candidates, special_1)

            id_ += 1
"""

"""
def prepare_elections(experiment_id, approval=False, folder=None, preflib=False):
    ...
    for i in range(model.num_families):
        ...
        for j in range(model.families[i].size):
            ...
            if approval:
                el.generate_approval_elections(experiment_id, elections_type, elections_id, num_elections,
                                               model.num_voters, model.num_candidates, special_1, special_2)
            elif preflib:
                el.generate_elections_preflib(experiment_id, elections_type, elections_id, num_elections,
...
def compute_distances_between_elections_old(experiment_id, metric_type="emd",
    ...
    #"""
    ctr = 0
    file_name = "experiments/" + experiment_id + "/results/distances/" + str(distance_name) + ".txt"
    file_ = open(file_name, 'w')
    file_.write(str(number_of_elections) + "\n")
    file_.write(str(num_families) + "\n")
    file_.write(str(len(results)) + "\n")
    ...
    """
    ctr = 0
    file_name = "experiments/" + experiment_id + "/results/distances/" + str(experiment_id) + "2.txt"
    file_ = open(file_name, 'w')
    file_.write(str(number_of_elections) + "\n")
    file_.write(str(num_families) + "\n")
    file_.write(str(len(results)) + "\n")
    ...
    for i in range(0, model.num_elections):
        ...
        for j in range(i + 1, model.num_elections):
            ...
            if j < starting_from:
                ...
                #result = metr.get_distance(model, i, j)
                result = metr.get_distance(model.elections[i], model.elections[j], distance_name=model.distance_name, metric_name=model.metric_name)
...
"""
def compute_distances_between_elections(experiment_id, metric_type='emd', distance_name='positionwise'):
    ...
    ctr = 0
    path = os.path.join(os.getcwd(), "experiments", experiment_id, "results", "distances", str(distance_name) + ".txt")
    with open(path, 'w') as txtfile:
        txtfile.write(str(model.num_elections) + '\n')
        txtfile.write(str(model.num_families) + '\n')
        txtfile.write(str(len(results)) + '\n')
        for i in range(model.num_elections):
            for j in range(i + 1, model.num_elections):
                txtfile.write(str(i) + ' ' + str(j) + ' ' + str(round(results[ctr],5)) + '\n')
                ctr += 1
"""
