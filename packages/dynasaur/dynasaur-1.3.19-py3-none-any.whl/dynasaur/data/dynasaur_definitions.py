...
class DynasaurDefinitions:
    ...
    def _parse_objects(self, json_objects):
        ...
        for json_object in json_objects[DefinitionConstants.OBJECTS]:
            ...
            elif json_object[JsonConstants.TYPE] == ObjectConstantsForData.JOINT:
                self._set_datacontainer_defintion(json_object, self._jntforc)
            ...
            elif json_object[JsonConstants.TYPE] == ObjectConstantsForData.SLEOUT:
                self._set_datacontainer_defintion(json_object, self._sleout)
            else:
                        ...
                        try:
                            if first:
                                ...
                                for json_object in json_objects:
                                    if DefinitionConstants.TITLE in json_object:
                                        object_order.append(DefinitionConstants.TITLE)
                                    elif DefinitionConstants.UNIT in json_object:
                                        object_order.append(DefinitionConstants.UNIT)
                                    elif DefinitionConstants.CRITERIA in json_object:
                                        object_order.append(DefinitionConstants.CRITERIA)
                                    elif DefinitionConstants.DATA_VIS in json_object:
                                        object_order.append(DefinitionConstants.DATA_VIS)
                                    elif DefinitionConstants.OBJECTS in json_object:
                                        object_order.append(DefinitionConstants.OBJECTS)
                            else:
                                new_json_file = json.load(fd)
                                for new_json_object in new_json_file:
                                    key = list(new_json_object.keys())[0]
                                    if key == DefinitionConstants.TITLE or key == DefinitionConstants.UNIT:
                                        if json_objects[object_order.index(key)][key] != new_json_object[key]:
                                            if key == DefinitionConstants.TITLE:
                                                exit("Exiting - Different def-file titles!")
                                            elif key == DefinitionConstants.UNIT:
                                                exit("Exiting - Different def-file units!")
                                    elif key == DefinitionConstants.CRITERIA or key == DefinitionConstants.DATA_VIS or key == DefinitionConstants.OBJECTS:
                                        json_objects[object_order.index(key)][key] = json_objects[object_order.index(key)][key] + new_json_object[key]
                                    else:
                                        self._logger.emit(LOGConstants.ERROR[0], "json object not understood:" + str(json_object))
                                        return
                        except ValueError as e:
                            self._logger.emit(LOGConstants.ERROR[0], LoggerERROR.print_statements[4] % e)
                            exit("Exiting - Invalid JSON file")  # Because Mutant, if GUI "exit()" comment
