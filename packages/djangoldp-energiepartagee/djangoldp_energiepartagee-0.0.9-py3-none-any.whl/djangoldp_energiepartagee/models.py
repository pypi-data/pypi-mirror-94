...
class Actor (Model):
    ...
    adhesiondate = models.DateField(blank=True, null=True, verbose_name="Adhérent depuis")
...
class Integrationstep(Model):
    actor = models.ForeignKey(Actor, on_delete=models.CASCADE, blank=True, null=True, verbose_name="Acteur", related_name="integrationsteps")
...
class Contribution(Model):
    ...
    contributionnumber = models.IntegerField(blank=True, null=True, verbose_name="Numéro de la cotisation")
...
def create_actor(sender, instance, created, **kwargs):
    if created:
        if not instance.contributions or instance.contributions.exists() == False:
            ...
            integrationstepsInstance = instance.integrationsteps.create(
                packagestep = False,
                adhspacestep = False,
                adhliststep = False,
                regionalliststep = False
            )
            integrationstepsInstance.save()
