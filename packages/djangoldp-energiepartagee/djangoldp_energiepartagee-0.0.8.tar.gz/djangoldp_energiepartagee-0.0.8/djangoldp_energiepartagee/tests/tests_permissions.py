...
# --- class Profile ---
    ...
    def test_super_user_can_list_profile(self):
        self.setUpLoggedInUser(is_superuser=True)
        another_user = self._get_random_user()
        another_profile = self._get_random_profile(another_user)
    ...
    def test_user_can_list_only_its_profile(self):
        self.setUpLoggedInUser(is_superuser=False)
        ownprofile = self._get_random_profile(self.user)
        another_user = self._get_random_user()
        another_profile = self._get_random_profile(another_user)
...
# --- class Integrationstep  ---
    ...
    def test_super_user_can_add_integrationstep(self):
        ...
        new_data = self._get_request_json(actor=actor.urlid)
    ...
    def test_super_user_can_change_integrationstep(self):
        ...
        self.assertEqual(integrationstep.actor, another_actor)   
...
# --- class RelatedActor
    ...
    # admin: view his own
    def test_admin_can_list_only_its_relatedactor(self):
        self.setUpLoggedInUser(is_superuser=False)
        actor = self._get_random_actor()
        relatedactor = self._get_relatedactor(user=self.user, actor=actor, role='admin')

        another_actor = self._get_random_actor()
        another_user = self._get_random_user()
        another_relatedactor = self._get_relatedactor(user=another_user, actor=actor, role='admin')

        response = self.client.get('/relatedactors/')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data['ldp:contains']), 1)

    # admin: change his own
    def test_admin_can_change_its_relatedactor(self):
        self.setUpLoggedInUser(is_superuser=False)
        actor = self._get_random_actor()
        relatedactor = self._get_relatedactor(user=self.user, actor=actor, role='admin')
    ...
    def test_authenticated_user_cannot_add_relatedactor(self):
        self.setUpLoggedInUser(is_superuser=False)
        user = self._get_random_user()
        actor = self._get_random_actor()
    ...
    def test_member_can_list_only_its_relatedactor(self):
        ...
        another_actor = self._get_random_actor()
        another_user = self._get_random_user()
        another_relatedactor = self._get_relatedactor(user=another_user, actor=actor, role='member')
