...
class SQLAlchemy:
    ...
    def setup(self, options: Dict = {}):
        ...
        """
        try:
            self._logger = options.get("logger", None)
            self._logger.info(f"Using db: started")
            yield self._setup(options=options)
            self._logger.info(f"Using db: completed")
        finally:
            self.dispose()

    def _setup(self, options: Dict = {}):
        self._conection_uri, self._engine = self._create_engine(options=options)
        self._setup_engine_logging(engine=self._engine, options=options)
        self._setup_engine_before_cursor_execute(engine=self._engine, options=options)
        self._setup_engine_after_cursor_execute(engine=self._engine, options=options)
        self._setup_engine_connection_connect(engine=self._engine, options=options)
        self._setup_engine_connection_begin(engine=self._engine, options=options)
        return self

    def _setup_engine_logging(self, engine: object = None, options: Dict = None):
    ...
    def _setup_engine_before_cursor_execute(self, engine=None, options: Dict = None):
        # Profile SQL query execution time
        @event.listens_for(engine, "before_cursor_execute")
        def before_cursor_execute(conn, cursor, statement, parameters, context, executemany):
            context._query_start_time = time.time()
            self._logger.debug("Start Query:\n%s" % statement)
    ...
    def _setup_engine_after_cursor_execute(self, engine=None, options: Dict = None):
        @event.listens_for(engine, "after_cursor_execute")
        def after_cursor_execute(conn, cursor, statement, parameters, context, executemany):
            total = time.time() - context._query_start_time
            self._logger.debug("Query Complete!")
            # Modification for StackOverflow: times in milliseconds
            self._logger.debug("Total Time: %.02fms" % (total * 1000))
    ...
    def reflect_schema(
        ...
        try:
            self._logger.info(f"Reflect db schema: started")
            ...
            self._logger.info(f"Reflect db schema: completed")
        except BaseException:
            self._logger.error(f"Reflect db schema: failed", exc_info=1)
            raise
    ...
    def _reflect_schema(
        ...
        self._SessionMaker = sessionmaker(bind=self._engine, autoflush=True, autocommit=False)

        # Force checking db classes & relationships are automapped and/or manually mapped
        self._logger.debug("Table(s) relationships:")
        from sqlalchemy.inspection import inspect

        for entity_cls in entity_classes:
            relations = inspect(entity_cls).relationships.items()
            for relation in relations:
                self._logger.debug(f"Table {entity_cls.__name__} has relationship: -> {relation}")
    ...
    def migrate(self, options: Dict = {}):
        ...
        """
        try:
            self._logger.info(f"Migrating db schema: started")
            self._migrate(options=options)
            self._logger.info(f"Migrating db schema: completed")
        except BaseException:
            self._logger.error(f"Migrating db schema: failed", exc_info=1)
            raise
    ...
    def vacuum(self, options: Dict = {}):
        ...
        """
        try:
            self._logger.info(f"Vacumming db: started")
            self._vacuum(options=options)
            self._logger.info(f"Vacumming db: completed")
        except BaseException as e:
            self._logger.error(f"Vacumming db: failed", exc_info=1)
    ...
    def backup(self, options: Dict = {}):
        ...
        """
        try:
            self._logger.info(f"Backup db: started")
            self._backup(options=options)
            self._logger.info(f"Backup db: completed")
        except BaseException:
            self._logger.error(f"Backup db: failed", exc_info=1)
            raise

    def _backup(self, options: Dict = {}):
        raise NotImplementedError("Must be implemented by sub class")

    def dispose(self):
        if self._engine is not None:
            self._dispose(engine=self._engine)
    ...
    def _new_session(self):
        ...
        return self._SessionMaker(bind=connection)
