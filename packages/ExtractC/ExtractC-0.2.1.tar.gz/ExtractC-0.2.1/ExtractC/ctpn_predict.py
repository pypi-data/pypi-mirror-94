...
from ExtractC.ctpn_model import CTPN_Model
from ExtractC.ctpn_utils import gen_anchor, bbox_transfor_inv, clip_box, filter_bbox,nms, TextProposalConnectorOriented
from ExtractC.ctpn_utils import resize
import config
...
# weights = 'https://drive.google.com/file/d/1I5eBLXzpGs9242A0syM0NuNBgaRKQrUY'
model = CTPN_Model()
...
def get_det_boxes(image,display = True, expand = True):
    ...
    image = image.astype(np.float32) - config.IMAGE_MEAN #quy định mảng hình ảnh float32
    image = torch.from_numpy(image.transpose(2, 0, 1)).unsqueeze(0).float() #tạo ra tensor từ ma trận chuyển vị
    ...
    with torch.no_grad():  
        ...
        if display:
            ...
            for i in text:
                ...
                i = [int(j) for j in i]
                cv2.line(image_c, (i[0] , i[1]-2), (i[2], i[3]-2), (0, 0, 255), 1)
