...
@adapter(ISecondaryMenu)
class SecondaryMenuControlpanelSerializeToJson(ControlpanelSerializeToJson):
    def __call__(self):
        json_data = super(
            SecondaryMenuControlpanelSerializeToJson, self
        ).__call__()
        conf = json_data["data"].get("secondary_menu_configuration", "")
        if conf:
            json_data["data"]["secondary_menu_configuration"] = json.dumps(
