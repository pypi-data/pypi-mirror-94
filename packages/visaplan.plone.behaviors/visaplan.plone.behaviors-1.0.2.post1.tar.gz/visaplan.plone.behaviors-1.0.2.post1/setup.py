...
"""Installer for the visaplan.plone.behaviors package."""
...
package_name = 'visaplan.plone.behaviors'
...
def valid_suffix(suffix):
    ...
    if disallowed:
        ...
        raise ValueError('Version suffix contains disallowed characters'
                         ' (%(disallowed)s)'
                         % locals())
    return suffix  # skip further checks for now
    chunks = suffix.split('.')
    chunk = chunks.pop(0)
...
# see as well --> src/visaplan/plone/behaviors/configure.zcml:
exclude_subpackages = (
        )
exclude_packages = []
...
setup_kwargs = dict(
    name=package_name,
    version=VERSION,
    description="Interfaces and behaviors for a family of Plone/Dexterity datatypes",
