...
def parse_arg():
    ...
    parser.add_argument("-c", "--config", action="append", help="configuration file.")
    ...
    parser.add_argument(
        ...
        metavar="OPERATION",
        choices=["create", "import", "plan", "apply"],
    )
...
def hmddops_merge_registry(p_db, p_schema):
...
def hmddops_import(p_db, p_schema):
    ...
    hmddops_merge_registry(p_db, p_schema)
...
def hmddops_apply(p_db, p_schema, p_args):
    ...
    # delete the work dir and recreate it
    work_dir = config["work_dir"].get()
    logger.info(f" Deleting dir : {work_dir} ...")
