...
class NiceTaskShower:
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback) -> None:
        ...
        click.echo(f' {word} in {minutes}:{seconds:02d} seconds.',)
...
def play(*, board_size: int, n_players: int, n_food_tiles: int, allow_shooting: bool,
    ...
    with server.ServerThread(host=host, port=port, quiet=True) as server_thread:
        ...
        if pre_train:
            with NiceTaskShower('Running naive games to generate history'):
            ...
            with NiceTaskShower('Running first training'):
            ...
            with NiceTaskShower('Running practice games and training on them') as nice_task_shower:
