...
def whatmeans(statement):
    ...
    # FIXME:
    >>> whatmeans("What are genes???")
    're'
    >>> whatmeans(None)
    ''
    >>> whatmeans("Nothing to see. Move right along.")
    ''
    """
    statement = statement or ""
    # FIXME: This pattern is broken and rarely matches anything!!
    pattern = (r"\b(what\s+(is|are)\s*(not|n't)?\s+(a|an|the|those|this|that)?)\s*\b"
               ...
               r"((tech|technology|science|biology|medicine|health|healthcare|genomic|"
               r"data[ ]?science|machine[ ]?learning|dl|ml|ds|it|is|ir|nlp)?[es])*\s*"
...
def extract_name(s):
    """ from a sentence identify and extract proper nouns

    >>> extract_name("Who is Elon Musk?")
    'Elon Musk'
    """
    name = []
    for word in nlp(s):
