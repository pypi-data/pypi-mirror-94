...
class CoreDeploy:
    ...
    def hasAddressInList(self, address: str) -> bool:
        mark = "list_address_{}".format(self.last_class)
        if mark not in self._contract_dict:
            return False
        try:
            v = self._contract_dict[mark].index(address)
            return True
        except ValueError:
            return False
    ...
    def pushAddress(self, address: str) -> bool:
        mark = "list_address_{}".format(self.last_class)
        if mark not in self._contract_dict:
            self._contract_dict[mark] = list()
        self._contract_dict[mark].append(address)
        return True
    ...
    def removeAddress(self, address: str) -> bool:
        ...
        if mark not in self._contract_dict:
            return False
        self._contract_dict[mark].remove(address)
        return True
    ...
    def iterList(self) -> iter:
        ...
        return iter(self._contract_dict[mark])
