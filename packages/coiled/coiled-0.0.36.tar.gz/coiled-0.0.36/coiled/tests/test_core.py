...
@pytest.mark.asyncio
async def test_backend_option_validity(cloud, cluster_configuration, cleanup):
...
async def test_password_login(sample_user):
    async with coiled.Cloud(
        ...
        password=sample_user.user.password,  # type: ignore
        asynchronous=True,
    ) as cloud:
...
async def test_create_and_update_cluster_configuration_validates(
    ...
    result = str(exc_info)
    assert "currently do not support" in result
...
async def test_account_options_and_overrides(
    account_with_options, cloud, cluster_configuration, cleanup
):
    await cloud.create_cluster(
        name="cluster",
        configuration=cluster_configuration,
    ...
    result = await cloud.list_clusters(account=account_with_options.account.slug)
    assert result["cluster-2"]["options"] == {"region": "us-west-2"}
