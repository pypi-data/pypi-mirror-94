...
async def test_update_software_environment_pip_doesnt_overwrite_conda(
    ...
    text = out.read()
    assert "conda" in text.lower()
    if strtobool(os.environ.get("TEST_AGAINST_AWS", "n")):
...
async def test_create_software_environment_pip_only_python_from_conda(
    ...
    assert f"python={v}" in result["env-1"]["conda"]["dependencies"]
...
async def test_software_environment_pip(cloud, cleanup, sample_user, tmp_path):
    ...
    assert result["env-1"]["conda"] is not None
    ...
    assert result["env-2"]["conda"] is not None
...
async def test_software_environment_post_build(cloud, cleanup, sample_user, tmp_path):
    ...
    assert results["env-1"]["post_build"] == post_build
    ...
    assert results["env-2"]["post_build"] == post_build
...
async def test_create_software_environment_another_account_fails(
    cloud, cleanup, sample_user, second_user
):
