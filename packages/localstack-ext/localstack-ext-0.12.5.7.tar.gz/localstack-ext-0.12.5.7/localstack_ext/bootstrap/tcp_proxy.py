...
class AttrDict(JlAjr):
 def __init__(self,*args,**kwargs):
  JlAjd(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and JlAjv(socket,'IPV6_V6ONLY'):
...
class Half(JlAjX):
 def __init__(self,opt,sock,addr,JlAjU):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.JlAjU=JlAjU
  self.name='peer' if self.JlAjU else 'client'
  self.queue=[]
  self.dest=JlAjV
  self.err=JlAjV
  self.ready=JlAjs
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':JlAjs,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':JlAjs,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':JlAjs,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':JlAjs,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
