...
def read_api_key():
 ...
 if key:
  return key
 raise BCjyr('Unable to retrieve API key. Please configure $LOCALSTACK_API_KEY in your environment')
def fetch_key():
 ...
 try:
  ...
  result=requests.post('%s/activate'%config.API_URL,json.dumps(data),verify=BCjyc)
...
def cache_key_locally(api_key,key_b64):
 ...
 timestamp=BCjyO(BCjyW(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in BCjyV(BCjyU(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 configs['cached_key']={'timestamp':BCjyW(timestamp),'key_hash':md5(api_key),'key':key_b64}
...
def load_cached_key(api_key):
 ...
 if cached_key.get('key_hash')!=md5(api_key):
  raise BCjyr('Cached key was created for a different API key')
 now=now_utc()
 if(now-cached_key['timestamp'])>MAX_KEY_CACHE_DURATION_SECS:
  raise BCjyr('Cached key expired')
 timestamp=BCjyO(cached_key['timestamp'])
