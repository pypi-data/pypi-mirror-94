...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':bEkXw(perm='m',auth=bEkXR,arg=bEkXR,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  thread=ShellCommandThread(cmd,outfile=subprocess.PIPE,env_vars=env_vars,quiet=bEkXr)
  thread.start()
  time.sleep(2)
 else:
  thread=do_start_ftp(port,asynchronous=bEkXR)
 time.sleep(2)
 wait_for_port_open(port,retries=10,sleep_time=1.5)
 TMP_THREADS.append(thread)
 return thread
def do_start_ftp(port,asynchronous=bEkXR):
...
def main():
 do_start_ftp(bEkXW(sys.argv[1]),asynchronous=bEkXr)
