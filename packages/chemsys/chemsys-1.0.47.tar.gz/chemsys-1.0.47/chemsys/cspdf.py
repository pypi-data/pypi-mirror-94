...
class guip:
    ...
    def fullguip():
        ...
        return chemnamesl, rxnnum, Initreactions, Eqlist, indvdf[0], ffpath[0], kk, kJtoJ(eaf), RR[0], pdf_path[0]
...
class symbolgenp:
    ...
    def rprod(Ci, a, Cj, b):
        term1 = symbolgenp.rterm(Ci, a)
        term2 = symbolgenp.rterm(Cj, b)
        term3 = Mul(term1, term2)
        return term3

    def initfunc(C, initlist, symsinit):
        reactants = []
        products = []
        for i, j in enumerate(initlist):
            ...
            for k, li in zip(C, Rvals):
                if li != 0:
                    term = symbolgenp.rterm(k, li)
                    rterms.append(term)
            for t in rterms:
                rtotal = Mul(rtotal, t)
            for m, n in zip(C, Pvals):
                if n != 0:
                    pterm = symbolgenp.rterm(m, n)
                    pterms.append(pterm)
            for tt in pterms:
    ...
    def gen(names, rxn, inits, eqs, intz):
        Cs, Ks, EAs, Ts = symbolgenp.symfunc(names, rxn)
        reacts, prods = symbolgenp.initfunc(inits, Cs)
        equats, latexss = symbolgenp.eqlist(eqs, reacts, prods)
        slat, dlat = symbolgenp.dislat(names, latexss, intz)
        Chem, ChemD, ChemW = symbolgenp.chemeq(Cs, rxn, inits)
        return Cs, reacts, prods, equats, slat, dlat, Chem, ChemD, ChemW

    def fullgenp(names, rxn, inits, eqs, intz, filepathf, kk, ea, r, namesl, pdfpathf):
        Cs, Ks, EAs, Ts = symbolgenp.symfunc(names, rxn)
        reacts, prods = symbolgenp.initfunc(Cs, inits, Cs)
        equats, latexss = symbolgenp.eqlist(eqs, reacts, prods)
        slat, dlat, dlatb = symbolgenp.dislat(names, latexss, intz)
        Chem, ChemD, ChemW = symbolgenp.chemeq(Cs, rxn, inits)
        Cs.append("T")
        RHS, RHSf = symbolgenp.rhseqs(equats, kk, ea, r)
        Jac, JacNumpy, JacMath, JacSimple, lm, latexmatrix = symbolgenp.jacobian(RHSf, Cs)
        JacS, JacNumpyS, JacMathS, JacSimpleS, lmS, latexmatrixS = symbolgenp.jacobian(RHS, Cs)
        symbolgenp.psave(namesl, dlat, filepathf, dlatb)
        symbolgenp.csave(Chem, filepathf)
        KS = [str(r"{}".format(Ks[i])) for i in range(len(Ks))]
        EAS = [str(r"{}".format(EAs[i])) for i in range(len(EAs))]
        EAK = KS.copy()
        EAK.extend(EAS)
        symbolgenp.fsave(filepathf, equats, dlat, Chem, ChemW, RHS, RHSf, Jac, JacNumpy, JacMath, JacSimple, lm, latexmatrix, JacS, JacNumpyS, JacMathS, JacSimpleS, lmS, latexmatrixS, Cs, EAK, names, pdfpathf)
