...
class RunClient:
    ...
    def __init__(
        ...
        self._run_uuid = (
            ...
            if not settings.CLIENT_CONFIG.is_offline
        ...
        self._run_data = polyaxon_sdk.V1Run(
            ...
            uuid=self._run_uuid,
            kind=V1RunKind.JOB if settings.CLIENT_CONFIG.is_offline else None,
            runtime=V1RunKind.JOB if settings.CLIENT_CONFIG.is_offline else None,
            is_managed=not settings.CLIENT_CONFIG.is_offline,
        )
    ...
    def update(
        ...
        if settings.CLIENT_CONFIG.is_offline:
            for k in data:
                setattr(self._run_data, k, data[k])
    ...
    def log_succeeded(self, message="Operation has succeeded"):
        ...
        """
        self.end(status=polyaxon_sdk.V1Statuses.SUCCEEDED, message=message)
    ...
    def log_stopped(self, message="Operation is stopped"):
        ...
        """
        self.end(status=polyaxon_sdk.V1Statuses.STOPPED, message=message)
    ...
    def _has_meta_key(self, key: str):
        return self.run_data and self.run_data.meta_info.get(key, False)
    ...
    def _sync_events_summaries(
        ...
        with get_files_in_path_context(current_events_path) as files:
            for f in files:
                ...
                event_rel_path = self.get_rel_asset_path(path=f)
    ...
    def persist_offline_run(self, artifacts_path: str):
        ...
        <blockquote class="info">
        N.B. When offline mode is enabled, this method is triggered automatically at the end.
        </blockquote>
        """
        if not settings.CLIENT_CONFIG.is_offline or not self.run_data:
            logger.info(
        ...
        if not self._lineages:
            logger.info("Persist offline run call did not fins any lineage data. ")
            return
    ...
    def load_offline_run(
        ...
        > **Note**: When offline mode is enabled, and the run uuid is provided,
            this method is triggered automatically to resume from the last checkpoint.
        """
        run_path = "{}/run_data.json".format(artifacts_path)
    ...
    def sync_offline_run(
        ...
        if not self.run_data:
            logger.warning(
                "Sync offline run call failed. " "Make sure that run_data is provided."
            )
            return
        ...
        if not self._lineages:
            logger.info("Sync offline run has no lineage data. ")
            return
        self.log_artifact_lineage([l for l in self._lineages.values()], async_req=False)
