...
class Run(RunClient):
    ...
    def __init__(
        ...
        self._track_logs = (
            track_logs if track_logs is not None else settings.CLIENT_CONFIG.is_offline
        )
        ...
        if not self._artifacts_path and settings.CLIENT_CONFIG.is_offline:
            self.set_artifacts_path(artifacts_path)

        if (
            self._artifacts_path
            and (settings.CLIENT_CONFIG.is_managed or settings.CLIENT_CONFIG.is_offline)
            and get_collect_artifacts(collect_artifacts)
        ):
        ...
        self._set_exit_handler(is_new_instance=False, auto_resume=auto_resume)
    ...
    def _persist_logs_history(self):
        if self._logs_history.logs and len(self._logs_history.logs) > 0:
            ...
            with open(logs_path, "w") as logs_file:
                logs_file.write(
                    "{}\n{}".format(
                        V1Logs.get_csv_header(), self._logs_history.to_csv()
                    )
                )
    ...
    def set_artifacts_path(self, artifacts_path: str = None):
        ...
        elif settings.CLIENT_CONFIG.is_offline:
            _artifacts_path = container_contexts.CONTEXT_MOUNT_OFFLINE_FORMAT.format(
                self.run_uuid
            )
        else:
    ...
    def _set_exit_handler(
        self, is_new_instance: bool = False, auto_resume: bool = False
    ):
        if settings.CLIENT_CONFIG.is_managed:
            self._register_wait()
        elif settings.CLIENT_CONFIG.is_offline or is_new_instance:
            self._start(auto_resume=auto_resume)
    ...
    def _post_create(self):
        ...
        if self._track_env:
            self.log_env()
        if self._track_logs is not False:
            setup_logging(add_logs=self._add_logs)

        self._set_exit_handler(is_new_instance=True)
    ...
    @check_no_op
    def _start(self, auto_resume: bool = False):
        if settings.CLIENT_CONFIG.is_offline:
            ...
            if self.run_data.status:
                if auto_resume:
                    self.log_status(
                        polyaxon_sdk.V1Statuses.RESUMING,
                        reason="OfflineOperation",
                        message="Operation is resuming",
                    )
