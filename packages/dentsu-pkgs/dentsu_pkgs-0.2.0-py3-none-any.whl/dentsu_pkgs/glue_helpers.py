...
def setValue(targetDF, readColumn, targetColumn, values, newValue):
    df = targetDF.withColumn(targetColumn, when(col(readColumn).isin(values), newValue).otherwise(col(targetColumn)))

    return df


def googleadsSetCountry(targetDF):
    df = targetDF.withColumn(
        "country",
        ...
        .when(col("account_id") == "5086309825", "Trinidad y Tobago")
...
def setLuminaId(targetDF, sourceColumn):
    luminaPattern = "(?<!\d)[0-9]{4}(?!\d)"
    df = targetDF.withColumn("campaign_name_lumina_regex", regexp_replace(col(sourceColumn), "202[01]", ""))
    df = df.withColumn("lumina_id", regexp_extract(col("campaign_name_lumina_regex"), luminaPattern, 0))

    return df


def setCountry(targetDF, searchColumn):
    countryPattern = "(?<![a-zA-Z])(AR|ARG|Argentina|Aruba|BO|Bahamas|Barbados|Bermudas|BS|CAY|CH|CL|CO|CR|Chile|Costa Rica|Curacao|DO|Dominican Republic|do|EC|ECU|Ecuador|El Salvador|GT|GrandCayman|Guatemala|HN|hn|Honduras|JA|JM|Jamaica|MX|Mexico|Mx|NI|Nicaragua|PA|PE|PER|PR|PY|Panama|Peru|Puerto Rico|RD|SV|SaintLucia|SaintVicent|sv|TT|Trinidad y Tobago|Trinidad&Tobago|URU|UY|Uruguay)(?![a-zA-Z])"
    df = targetDF.withColumn("country_regex", regexp_extract(col(searchColumn), countryPattern, 1))
    df = df.withColumn(
        "country",
        ...
        .when(col("country_regex") == "Bermudas", "Bermudas")
        .when(col("country_regex") == "BS", "Bermudas")
        .when(col("country_regex") == "CAY", "Islas Caiman")
        ...
        .when(col("country_regex") == "DO", "Republica Dominicana")
        .when(col("country_regex") == "Dominican Republic", "Republica Dominicana")
        .when(col("country_regex") == "do", "Republica Dominicana")
        ...
        .when(col("country_regex") == "GrandCayman", "Islas Caiman")
        ...
        .when(col("country_regex") == "RD", "Republica Dominicana")
        ...
        .when(col("country_regex") == "sv", "El Salvador")
        .when(col("country_regex") == "TT", "Trinidad y Tobago")
        .when(col("country_regex") == "Trinidad y Tobago", "Trinidad y Tobago")
        .when(col("country_regex") == "Trinidad&Tobago", "Trinidad y Tobago")
        ...
        .when(col("account_id") == "541440410049583", "Republica Dominicana"),
    )
    return df
