...
if __name__ == '__main__':
    ...
    def zero_one_init(model):
    state_dict = model.state_dict()
    for param_name, param in state_dict.items():
        if 'weight' in param_name:
            state_dict[param_name] = torch.ones_like(param)
        elif 'bias' in param_name:
            state_dict[param_name] = torch.zeros_like(param)
    ...
    # %%
    zero_one_init(dnn)
    zero_one_init(resnet)
    zero_one_init(densenet)

    # DNN
    x = torch.ones(n_input)
    n_neurons = np.array([n_input]+n_hidden_list)
    print('dnn output: %s, real answer: %s'%(dnn(x).item(), np.prod(n_neurons)))

    # Resnet
    # skip connections * leftover at the end
    skip_result = np.prod([s.in_features+np.prod(l) for s, l in zip(resnet.skip_layers, [n_neurons[i:i+skip] for i in range(0,len(n_neurons)-skip, skip)]) ])
    leftover_result = np.prod(n_hidden_list[-(len(n_hidden_list)%skip)-1:])
    resnet_answer = skip_result*leftover_result
    print('resnet output: %s, real answer: %s'%(resnet(x).item(), resnet_answer))
    # 55*56*9

    # Densenet
    densenet.skip_layers
    [n_neurons[i:i+skip] for i in range(0,len(n_neurons)-skip, skip)]
    n_neurons
    skip_result
    leftover_result = np.prod(n_hidden_list[-(len(n_hidden_list)%skip)-1:])
    densenet_answer = skip_result*leftover_result
    print('densenet output: %s, real answer: %s'%(densenet(x).item(), densenet_answer))
