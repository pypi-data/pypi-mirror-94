...
def scoring_possessions(df_boxscore):
    ...
    '''
    try:
        df_boxscore['mp'] = df_boxscore['mp'].astype(str).apply(lambda x:x.replace("Player Suspended","0.0"))
        df_boxscore['mins_played'] = df_boxscore['mp'].apply(lambda x: x.split(":")[0]).astype(float)
    except:
        df_boxscore['mp'] = df_boxscore['mp'].astype(str)
        df_boxscore['mins_played'] = df_boxscore['mp'].apply(lambda x: x.split(":")[0]).astype(float)
...
def individual_floor_percentage(df_boxscore):
    ...
    '''
    df_boxscore['indiv_floor_percent'] = 100*df_boxscore['scoring_pos']/df_boxscore['non_scoring_pos']+df_boxscore['scoring_pos']
