...
class ModbusConnection:     #Singleton
    ...
    def __init__(self):
        ...
        self.__connectionErrorRate__ = dict()
    ...
    def getModbusErrorRate(self, nodeId):
        try:
            return self.__connectionErrorRate__[nodeId]
        except:
            return None
    ...
    def __readregisters__(self, node, startAddress, registersToRead, type="holding"):
        with self.__mutex__:
            ...
            try:
                self.__connectionErrorRate__[node] = round(self.__connectionErrorCounter__[node] / self.__connectionCounter__[node], 2)
            except:
                logging.debug("[ModbusConnection] Node " + str(node) + ": Cannot calculate error rate.")
