...
def get_config():
    return utils.max_nodes_base, utils.max_gpus_base, utils.check_enterprise_license(), \
           utils.nodes_errmsg, utils.gpus_errmsg, utils.processes_errmsg,\
           utils.nodes_infomsg, utils.gpus_infomsg, utils.processes_infomsg
...
def read_from_snap_format(filename):
    with open(filename, "rb") as f:
        ...
        if(data_type == 0):
            ...
            # l2-sparse data
            raise Exception("Sparse data with implicit values not supported by numpy/scipy")
        else:
            # not recognized
...
def _is_data_snapml_partition_type(X):
    if ( X.__class__.__name__ in ["SparsePartition", "DensePartition", "ConstantValueSparsePartition"] ):
        return True
    return False
...
def _check_devicendarray_types(X,y,self):
    ...
    if(type(X).__name__ == "DeviceNDArray" or type(y).__name__ == "DeviceNDArray"):
        if ((type(X).__name__ != "DeviceNDArray") or (type(y).__name__ != "DeviceNDArray")):    
            raise TypeError(DEVICE_NDARRAY_MSG + ", X_train and y_train should be of same types")
        if (X.dtype != "float32" or y.dtype != "float32"):
            raise TypeError(DEVICE_NDARRAY_MSG + ", only float32 data type is supported")
        if(len(self.device_ids)>1):
            raise ValueError(DEVICE_NDARRAY_MSG + ", training on multiple GPUs is not supported")
        if(len(self.device_ids)==1 and self.device_ids[0]!=0):
            raise ValueError(DEVICE_NDARRAY_MSG + ", training on GPU id 0 is only supported")
        ...
        if(len(self.classes_) > 2 ):
            raise ValueError(DEVICE_NDARRAY_MSG + ", multi-class labels are not supported")
    return True
...
def type_matches(dest_type, value):
...
def _param_check(param_def, varname, value):
    ...
    for attr in param["attr"]:
        if "values" in attr:
            ...
            if not type_matches(eval(typestr), value):
                continue
            if attr.get("gt") is not None and value <= attr["gt"]:
                raise ValueError("Error: '{:s}' should be larger than {}.".format(varname, attr["gt"]))
            if attr.get("ge") is not None and value < attr["ge"]:
                raise ValueError("Error: '{:s}' should be larger or equal {}.".format(varname, attr["ge"]))
            if attr.get("lt") is not None and value >= attr["lt"]:
                raise ValueError("Error: '{:s}' should be smaller than {}.".format(varname, attr["lt"]))
            if attr.get("le") is not None and value > attr["le"]:
                raise ValueError("Error: '{:s}' should be smaller or equal {}.".format(varname, attr["le"]))
            return
        else:
            print("[Warning] Unknown attribute {}".format(attr))
    
    raise TypeError("Error: '{:s}' should be:{:s}, value: {:s}".format(varname, _param_print(param), str(value)))
