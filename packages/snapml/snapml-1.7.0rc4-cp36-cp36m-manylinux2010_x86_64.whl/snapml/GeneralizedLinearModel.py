...
from snapml.utils import _is_mpi_enabled, _check_devicendarray_types, _is_data_snapml_partition_type
...
class GeneralizedLinearModel(ABC):
    PredictVars = namedtuple('PredictVars', ['num_ex' ,'num_ft' ,'num_classes' ,
    ...
    def normalize_data(self, X, refit=False):
        ...
        if type(X).__name__ not in normalize_types or _mpi_enabled or (not self.normalize):
            return X

        if refit:
            self.scaler_ = StandardScaler() if type(X).__name__ == 'ndarray' else MaxAbsScaler()
            self.scaler_.fit(X)
    ...
    def apply_kernel(self, X, refit=False):
        ...
        if type(X).__name__ not in kernel_types or _mpi_enabled or (self.kernel == 'linear'):
            return X

        if refit:
            self.kernel_ = RBFSampler(gamma=self.gamma, n_components=self.n_components,
    ...
    def validate_num_ft(self, X, refit=False):
        ...
        if refit:
            ...
            if X.shape[1] != self.n_features_in_:
                raise ValueError("Predict was passed %d features, but model was trained with %d features"
