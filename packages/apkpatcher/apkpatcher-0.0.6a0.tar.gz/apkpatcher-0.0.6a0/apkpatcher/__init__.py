        ...
        with open(manifest_path, 'rb') as fp:
            ...
            while aml.hasnext():
                ...
                if header.type == ResTypes.RES_XML_START_ELEMENT_TYPE and body.nodename == 'manifest':
                    inserted = aml.insert()
                    inserted.writexmlstartelement('uses-premission',
                        {
                            "name" : permission
                        })
                    inserted.writexmlendelement('uses-premission',{})
                    break
...
################################################################################
    ...
    def sign_and_zipalign(self, apk_path):
        ...
        subprocess.call(f"{self.path_build_tools}zipalign 4 {tmp_target_file} {apk_path}", stderr=subprocess.STDOUT, shell=True)
...
.end method
    ...
    def create_lib_arch_folders(self):
        '''
    ...
    def insert_frida_lib(self, gadget_path : str, config_file_path=None, auto_load_script_path=None):
        '''
        ...
        '''
        arch_folders = self.create_lib_arch_folders()
...
################################################################################
    ...
    def patching(self, gadget_to_use, arch, output_file=None, user_certificate=False):
        '''
...
def main():
    ...
    if args.output_file:
        patcher.patching(args.gadget, args.arch,
