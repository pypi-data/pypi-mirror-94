...
class ResXMLTree:
    ...
    def insert_attr(self, ns, name, value):
        ns = ResourceRef(self._aml.stringpool, ns)
        name = ResourceRef(self._aml.stringpool, name)
        self._aml.stringpool.setattribute(name.value, value)
        attr = ResXMLTree_attribute(aml=self._aml).make(ns, self._aml.stringpool,
                            name,
                            value)
        attr._aml = self._aml
...
class AML:
    ...
    class ResourceMapChunk:
        ATTRS = eval(pkgutil.get_data('apkpatcher.pyaml.libaml', 'android-attrs.json'))
    ...
    class InsertedPlaceHolder:
        ...
        def writexmlstartelement(self, name, attrs, linenumber=0, ns='http://schemas.android.com/apk/res/android'):
            ...
            element = ResXMLTree.create(ResChunk.Header.create(ResTypes.RES_XML_START_ELEMENT_TYPE, 16, 0),
                                        ...
                                        attrExt, aml=self)
            androidns = ResourceRef(stringpool=stringpool, value=ns)
