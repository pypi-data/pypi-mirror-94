...
class M(UserDict):
    ...
    def __init__(self, GROUP="MISC", SORT="_", **kwargs):
        kwargs = {**kwargs, "GROUP": GROUP, "SORT": SORT}
    ...
    def __call__(self):
        """ Return non-Murd attributes """
        return {k: v for k, v in self.items() if k not in [group_key, sort_key]}
...
class M_management:
    ...
    def prime_ms(cls, ms: list) -> dict:
        ...
        return {cls.m_to_key(m): m for m in ms}
...
class Murd(M_management, Murd_json_file):
    ...
    def update(self, ms, murd=None):
        ...
        if is_iterable(murd):
            ...
            _, results = zip(*run_async(self.update, arg_sets))
            return sum(results)
    ...
    def read(
        ...
        if is_iterable(group):
            ...
            return [M(**m) for ms in results for m in ms]
        ...
        if murd is None or is_iterable(murd):
            ...
            return self.murds[murd].read(group, sort, min_sort, max_sort, limit, murd)
        ...
        results = [M(**loaded_murd[key]) for key in matched]
    ...
    def read_first(
        ...
        try:
            return sorted(self.read(group, sort, min_sort, max_sort, limit, murd),
                          key=lambda x: x['SORT'])[0]
        except IndexError:
