...
class TestSDK(unittest.TestCase):
    ...
    def test_roles(self):
        ...
        body = akeyless.UpdateRole(name=role_name, audit_access='self',
                analytics_access='self', token=token)
        self.api.update_role(body)

        body = akeyless.GetRole(name=role_name, token=token)
        role = self.api.get_role(body)
        self.assertEqual(len(role.rules.path_rules), 5)

        body = akeyless.UpdateRole(name=role_name, audit_access='none',
                token=token)
        self.api.update_role(body)

        body = akeyless.UpdateRole(name=role_name, analytics_access='none',
                token=token)
        self.api.update_role(body)

        body = akeyless.GetRole(name=role_name, token=token)
        role = self.api.get_role(body)
        self.assertEqual(len(role.rules.path_rules), 3)
