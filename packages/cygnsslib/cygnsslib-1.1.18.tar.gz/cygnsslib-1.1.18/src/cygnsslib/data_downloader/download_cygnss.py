    ...
    for i_try in np.arange(MAX_CONNECTIONS_TRIES):
        try:
            with EarthdataSession(username=auth[0], password=auth[1]) as session:
                with session.get(file_url, stream=True) as response:
                    ...
                    if file_size > free_disk_space:
                        raise IOError('No enough space in the disk. file size: {:d} MB, free space {:d}'.format(int(file_size / 1024/1024),
                                                                                                                int(free_disk_space / 1024 / 1024)))
                ...
                if url_md5_checksum is None:  # get md5 checksum
                    url_md5_checksum = session.get('{:s}.md5'.format(file_url)).content.strip().decode('utf').split()[0]
                if url_md5_checksum:  # if the md5 file available
                    file_checksum = checksum(out_file_temp)
                    if url_md5_checksum != file_checksum:
                        raise RuntimeError
                else:
                    print('MD5 checksum is not available for this file; {:s}'.format(file_name))
            ...
            if i_try < MAX_CONNECTIONS_TRIES:
                ...
                raise RuntimeError('a connection error was raised when trying to access {:s}'.format(file_url))
    ...
    for i_try in np.arange(MAX_CONNECTIONS_TRIES):
        try:
            ...
            if i_try < MAX_CONNECTIONS_TRIES:
                ...
                raise RuntimeError('a connection error was raised when trying to access {:s}'.format(cyg_folder_url))
...
def _get_md5_file(day_page_data, sc_num, cyg_folder_url, auth):
    ...
    for item in day_page_data:
        if tag in item:
            ...
            md5_file_full_url = '{:s}{:s}'.format(cyg_folder_url, file_name)
            for i_try in np.arange(MAX_CONNECTIONS_TRIES):
                try:
                    with EarthdataSession(username=auth[0], password=auth[1]) as session:
                    ...
                    if i_try < MAX_CONNECTIONS_TRIES:
                        ...
                        raise RuntimeError('a connection error was raised when trying to access {:s}'.format(md5_file_full_url))
                except requests.exceptions.RequestException:
                    if i_try < MAX_CONNECTIONS_TRIES:
                        podaac_usr, podaac_pass = _cyg_data_auth_err_handling()
                        auth = (podaac_usr, podaac_pass)
...
def download_cyg_file(data_year, data_day, sc_num, cyg_data_ver, cyg_data_lvl, out_folder, podaac_usr_pass, day_page_data=None, cyg_file_name=None,
    ...
    cyg_file_full_url = '{:s}{:s}'.format(cyg_folder_url, cyg_file_name)
    ...
    for sc_num in list_sc_num:
        ...
        if check_md5_exist_file:
            ...
            if cyg_file_name is None:  # No md5 ==> no cygnss file
                print("File doesn't exist in the PODAAC, year:{:04d}, day:{:03d}, sc_num: {:02d}".format(data_year, data_day, sc_num))
                continue
        ...
        if not download_file_tf:
            if disk_cyg_file_name is None:
                ...
                if file_size > CYG_MIN_FILE_SIZE:
                    print('{:s} file exist'.format(disk_cyg_file_name))
                else:
