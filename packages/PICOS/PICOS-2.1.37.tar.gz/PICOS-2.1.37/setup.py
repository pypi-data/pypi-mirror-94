...
try:
    from version import get_base_version, get_full_version
    ...
    with (Path(__file__).parent / Path("picos", ".version")).open() as file:
        HARDCODED_VERSION = file.read().strip().split(".")

    def get_base_version():
        """Standin for version.get_base_version."""
        return ".".join(HARDCODED_VERSION[:2])

    def get_full_version():
        """Standin for version.get_full_version."""
        return ".".join(HARDCODED_VERSION)


URL = "https://gitlab.com/picos-api/picos"


class sdist_with_hardcoded_full_version(sdist):
...
setup(
    ...
    cmdclass={"sdist": sdist_with_hardcoded_full_version},
)
