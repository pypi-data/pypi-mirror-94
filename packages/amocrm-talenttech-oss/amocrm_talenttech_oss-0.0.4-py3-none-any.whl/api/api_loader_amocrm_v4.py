...
class AmocrmApiLoader:
    ...
    def extract(self):
        ...
        if self.date_modified_from is not None:
            self.logger.info(f"Uploading data with after {self.date_modified_from}")
            params["filter[updated_at][from]"] = int(
                self.date_modified_from.timestamp()
            )
        has_next = True
        while has_next and cur_page < PAGE_NUMBER_MAX:
            self.logger.info(f"Extracting page №{cur_page}")
            ...
            if objects.status_code == 204:
                self.logger.warning(
                    "Warning! API sent no result {}".format(objects.status_code)
                )
                return
            ...
            if limit_from_url != self.batch_api:
                logging.warning(
                    f"Batch API: {self.batch_api} is too large, changing to default value {limit_from_url}"
                )
            ...
            cur_page += 1
        self.logger.info(
            "Total number of rows received from API is {}".format(self.rows_to_upload)
        )
