...
class MessagingHandler:
    ...
    def send_fault_message(self, queue, exception, stack_trace, req_message):
        try:
            body = MessagingHandler.__create_fault_message(queue, str(exception), stack_trace, req_message)
    ...
    @staticmethod
    def __create_fault_message(queue, exception_msg, stack_trace, req_message):
        res_message = {'messageId': str(uuid.uuid4())}
        if req_message is not None and 'requestId' in req_message:
            res_message['requestId'] = req_message['requestId']
        ...
        if req_message is not None and 'message' in req_message:
            fault_message['message'] = req_message['message']
