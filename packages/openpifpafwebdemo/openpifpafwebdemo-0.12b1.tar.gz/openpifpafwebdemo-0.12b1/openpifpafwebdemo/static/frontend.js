...
const feed_link = document.getElementById('feedlink');
const url = new URL(document.location.href);
function newImage() {
    return __awaiter(this, void 0, void 0, function* () {
        const data = yield camera.imageData();
        const response = yield fetch(backend_location + '/v1/human-poses' + url.search, {
            method: 'post',
            mode: 'cors',
            body: data.image,
        });
...
function loop_forever() {
    return __awaiter(this, void 0, void 0, function* () {
        let prev_image_pred = yield newImage();
        while (true) {
            let [image_pred, _, __] = yield Promise.all([
                newImage(),
