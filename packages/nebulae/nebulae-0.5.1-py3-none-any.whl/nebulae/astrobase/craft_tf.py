...
class Conv(Craft):
    def __init__(self, in_chs, out_chs, kernel: tuple, stride=1, padding=0, dilation=1, group=1,
        ...
        # self.conv.add_loss(lambda: regularizers.l2(1)(self.conv.kernel))
        # if bias:
        #     self.conv.add_loss(lambda: regularizers.l2(1)(self.conv.bias))
...
class Dense(Craft):
    def __init__(self, in_chs, out_chs, axis=-1,
        ...
        self.fc = layers.Dense(out_chs, use_bias=bias, kernel_initializer=w_init.iniz, bias_initializer=b_init.iniz)
                               # kernel_regularizer=regularizers.l2(1), bias_regularizer=regularizers.l2(1))
        # self.fc.add_loss(lambda: regularizers.l2(1)(self.fc.kernel))
        # if bias:
        #     self.fc.add_loss(lambda: regularizers.l2(1)(self.fc.bias))
...
class Embed(Craft):
    def __init__(self, ntoken, token_dim, scope='EMBED'):
        ...
        # self.embd.add_loss(lambda: regularizers.l2(1)(self.embd.embeddings))
...
class EMA(object):
    ...
    def swap(self):
        if tf.distribute.in_cross_replica_context():
            ...
            with tf.distribute.OneDeviceStrategy(device='/'+self.hull.engine.param['device'].lower()+':0').scope():
                self._optz.swap_weights()
