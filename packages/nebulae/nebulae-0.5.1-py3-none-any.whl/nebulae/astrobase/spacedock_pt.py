...
class SpaceCraftPT(nn.Module):
    def __init__(self, device, blueprint, scope='', lookup=None):
        ...
        self.dict = {'_INPUTS': torch.zeros(1, device=device),
                       '_PENALTY': torch.zeros(1, device=device)}
    ...
    def _assemble(self, component, assemblage=None, sub_scope='', virtual=False):
        ...
        if symbol == '>':
            ...
            for s, sym in enumerate(symbol):
                if virtual:# special conditions
                    if sym == 'penalty':
                        self.dict['_PENALTY'] = message[s].to(self.device)
                        message[s] = '_PENALTY'
                    elif sym == 'is_train':
            ...
            if assemblage is None:
                ...
                if symbol[0] == 'penalty':
                    ...
                    args = (self.dict[assemblage],) + args[1:] # replace '_INPUTS' with input tensor
                    self.dict[node_name] = component.component(*args)
                    self.ID.add(node_name)
                    # draw layout sheet
    ...
    def _finalize(self, first_call):
        if first_call:
            ...
            for k in self.terminator.keys():
                self.dict[k] = self.terminator[k].component(media=(self._vars, self.dict['_PENALTY']))
    ...
    def _allot(self, comp, sub_scope, is_optz):
        ...
        if is_optz: # is optimizer
            ...
            # store params and states
            for i in range(len(self._vars), len(self.comp)):
                global_comp_name = self.scope + ':' + self.module[-1][1] + '/' + str(i)
                self._vars[global_comp_name] = self.comp[i].parameters()
                self._states[global_comp_name] = self.comp[i].state_dict()
