...
class Generator(dock.Craft):
    def __init__(self, in_shape, concat_dim, scope='GEN'):
        ...
        self.fc_1 = dock.Dense(concat_dim, 128 * (H // 4) * (W // 4))
    ...
    def run(self, z, c, v):
        ...
        x = self.cat((z, c, v))
        x = self.fc_1(x)
        x = self.reshape(x, (-1, 128, self.H//4, self.W//4))
...
class InfoGAN(dock.Craft):
    def __init__(self, in_shape, category_dim, code_dim, latent_dim=128, scope='INFOGAN'):
        ...
        self.G = Generator(in_shape, category_dim + code_dim + latent_dim)
