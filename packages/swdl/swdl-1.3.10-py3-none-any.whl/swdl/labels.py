...
def round_timestamps(array: np.ndarray):
    ...
    array_index = np.round((array[:, 0] % CP_INTERVAL) * CP_FRAMERATE).astype(int)
...
class CPManager:
    ...
    def upload_data(self, cam_data: bytes, match_id: str, virtual_cam_id: int):
        ...
        time_indexes = (np_data[:, 0] / CP_INTERVAL).astype(int)
        time_idx_set = set(time_indexes)
        for time_id in time_idx_set:
