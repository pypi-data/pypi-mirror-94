...
class TestCliCertificate(unittest.TestCase):
    ...
    def _mock_protocol(self):
        mock_get_iot_provision_protocol_patch = patch("pytrustplatform.cli_certificate_main.get_iot_provision_protocol")
        # Make sure the patching is stopped at the end of the test
        self.addCleanup(mock_get_iot_provision_protocol_patch.stop)
        mock_get_iot_provision_protocol = mock_get_iot_provision_protocol_patch.start()
        mock_protocol = MagicMock()
        mock_get_iot_provision_protocol.return_value = mock_protocol, None
        mock_protocol.read_ecc_serialnumber.return_value = self.ecc_data["serial"]
        mock_protocol.read_ecc_slot.side_effect = self._read_ecc_slot_stub
        mock_protocol.read_public_key.return_value = self.ecc_data["devkey"]

        return mock_get_iot_provision_protocol, mock_protocol
    ...
    def test_create_from_csr(self, mock_build_device_cert):
        ...
        """
        _, mock_protocol = self._mock_protocol()
        ...
        mock_build_device_cert.assert_called_with(signer_ca_cert_file=SIGNER_CA_CERT_FILE,
    ...
    def test_read_ecc_serialnumber(self):
        ...
        with patch.object(sys, 'argv', testargs):
            with patch('sys.stdout', new=StringIO()) as mock_stdout:
                retval = main()
                self.assertTrue("Reading ECC serial number from kit" in mock_stdout.getvalue())
