...
def build_device_csr(protocol, csr_filename, common_name, force=False):
    ...
    # Get public key from target
    pubkey = protocol.read_public_key()
    ...
    # Send the digest for signing
    signature = protocol.sign_digest(csr_digest)
...
def build_device_cert(signer_ca_cert_file, signer_ca_key_file, protocol, csr_filename, cert_filename, force=False):
    ...
    device_csr = build_device_csr(protocol, csr_filename, common_name, force)
