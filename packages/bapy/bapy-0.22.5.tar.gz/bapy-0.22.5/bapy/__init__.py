...
class SemEnum(EnumDict):
    ...
    async def text(self, func: Callable, priority: Priority = Priority.LOW, /, *args, **kwargs) -> list:
        ...
        common = [f'{self.name=}', f'{priority.name=}', f'{task=}', f'{func.__qualname__=}', f'{self.status=}',
                  f'{running=}']
    ...
    async def sem(self, func: Callable, /, *args, priority: Priority = Priority.LOW,
        ...
        """
        Examples:
            >>> await Sem.OS.sem(executor, ipaddress.ip_address, '8.8.8.8')
        ...
        Returns:
            Any:
        """
        await _log.adebug(*await self.text(func, priority, prefix='Start'))
        try:
            ...
            caller = str()
            await _log.awarning(*await self.text(func, priority, suffix=repr(exception)))
        ...
        if priority == Priority.HIGH and self.value[Priority.LOW].locked():
            ...
            using = priority.name
        await _log.awarning(*await self.text(func, priority, f'{using=}', f'{caller=}', suffix='Waiting'))
        async with sem:
            add = f'{using=}', f'{caller=}'
            await _log.anotice(*await self.text(func, priority, *add, suffix='Acquired'))
            while True:
                try:
                    ic(kwargs)
                    ...
                    await _log.aspam(*await self.text(func, priority, prefix='Waiting for:', *add,
                                                      suffix=repr(exception)))
                    await nap.asleep()
                    continue
        await _log.ainfo(*await self.text(func, priority, *add, suffix='Released'))
        return rv
...
bapy = Package(filename=__file__, _version='0.22.5')
...
class IpPort:
    ...
    async def is_global_ip(ip: str, p: Path = None) -> Optional[bool]:
        ...
        """
        try:
            ip_address = await Sem.OS.sem(executor, ipaddress.ip_address, ip)
    ...
    async def ping(self, ip: str, p=None) -> OptionalBool:
        ...
        if await self.is_global_ip(ip, p):
            rc = await Sem.PING.sem(aiocmd, f'sudo ping -c {pings} {ip}', False, True)
            if rc == 0:
                rv = True
            elif rc == 2:
                rv = False
        ...
        if await self.ping(ip, p):
            try:
                if proto == PROTO_NAME.tcp:
                    ...
                    res = await Sem.NMAP.sem(aiocmd, f'nc -vnzu "{ip}" "{port}" > /dev/null 2>&1', True, True)
                if status:
...
class Mongo:
    ...
    _db: dict[bool, MongoDB] = datafield(default_factory=dict, init=False)
    ...
    def __post_init__(self):
        self._db = {key: None for key in [True, False]}
    ...
    # noinspection PyMethodParameters
    @decorator
    async def connect_try_aio(func, self, *args, **kwargs):
        task = asyncio.current_task().get_name()
        log = dict(rv=['db', 'col'], args=[], kwargs=[])
        await self.log.adebug(f'Start: {func.__qualname__}', f'{task=}', f'{args=}', f'{kwargs=}')
        rv = None
        while True:
            try:
                # noinspection PyCallingNonCallable
                rv = await func(self, *args, **kwargs)
                if func.__name__ in log['rv']:
                    await self.log.ainfo(f'{func.__qualname__=}', f'{task=}', fmi(args), fmi(kwargs), fmi(rv))
                break
            except self.mongo_exceptions as exception:
                await self.log.awarning(f'Waiting for connection: {func.__qualname__=}, {task=}, {args=}, '
                                        f'{repr(exception)}')
                Nap.MONGO.sleep()
                continue
        await self.log.adebug(f'End: {func.__qualname__}', f'{task=}', f'{args=}', f'{kwargs=}')
        return rv
    ...
    def db(self) -> MongoDB:
        ...
        func = MongoClientMotor if aio else MongoClientPy
        if not self._db[aio]:
            self._db[aio] = func(uri, **kwargs).get_database(self.db_name)
        return self._db[aio]
    ...
    def find(self, col: str = None, find: dict = None) -> MongoCursorPy:
        return self.col(col).find(find if find else dict())
...
class NmapOutput:
    ...
    class Host:
        starttime: int = int()
        endtime: int = int()
        status: defaultdict = datafield(default_factory=partial(defaultdict, dict))
        address: defaultdict = datafield(default_factory=partial(defaultdict, dict))
        hostnames: defaultdict = datafield(default_factory=partial(defaultdict, dict))
        extraports: Union[defaultdict, list] = datafield(default_factory=partial(defaultdict, dict))
        port: Union[defaultdict, list] = datafield(default_factory=partial(defaultdict, dict))

    # parsed: defaultdict = datafield(default_factory=defaultdict_value)
    runstats: defaultdict = datafield(default_factory=partial(defaultdict, dict))
    scaninfo: str = str()
    scanner: str = str()
    start: str = str()
    startstr: str = str()
    ...
    host: Union[defaultdict, list] = datafield(default_factory=partial(defaultdict, dict))

    # finished: Optional[dict] = partial(defaultdict_value, str)
    finished: dict = datafield(default_factory=dict)
    script: Union[NmapServiceScript, list, set] = datafield(default_factory=set)
    service: Union[NmapServiceScript, dict, set] = datafield(default_factory=set)
...
class NmapCodec:
    # parsed: defaultdict = datafield(default_factory=defaultdict_value)
    parsed: dict = datafield(default_factory=dict)
    host: Optional[Union[dict, list]] = datafield(default_factory=set)
    # finished: Optional[dict] = partial(defaultdict_value, str)
    finished: dict = datafield(default_factory=dict)
    script: Union[NmapServiceScript, list, set] = datafield(default_factory=set)
    service: Union[NmapServiceScript, dict, set] = datafield(default_factory=set)
...
class NmapCommand:
    ...
    name: str = str()
    os: bool = False
    # parsed: defaultdict = datafield(default_factory=partial(defaultdict_value, defaultdict))
    parsed: dict = datafield(default_factory=dict)
    pn: bool = True
    _port: NmapCommandPortTyping = str()
    # samples: defaultdict = datafield(default_factory=partial(defaultdict_value, defaultdict))
    ...
    # stdout: defaultdict = datafield(default_factory=partial(defaultdict_value, defaultdict), init=False)
    stdout: dict = datafield(default_factory=dict)
    rc: int = int()
    ...
    def command(self, ip: str = str(), port: NmapCommandPortTyping = None):
        ...
        self._command = f'sudo nmap -R -r --reason -oX - ' \
                        ...
                        f'{("-6 " if ipaddress.ip_address(self.ip).version == 6 else "-4 ") if self.ip else str()}' \
                        f'{self.ip} ' \
    ...
    async def run(self, ip: str = str(), name: str = str(), port: NmapCommandPortTyping = None):
        self.ip = ip
        self.name = name

        if self.samples:
            self.stdout = self.samples[self.name].get(ip, dict())
        else:
            self.stdout, self.stderr, self.rc = await aiocmd(self.command(ip=ip, port=port), onlyout=False, utf8=True)

        try:
            self.parsed = self.stdout if isinstance(self.stdout, dict) else xmltodict.parse(
                self.stdout, dict_constructor=dict, process_namespaces=True, attr_prefix='')['nmaprun']
            self.parsed = defaultdict(dict, self.parsed)
        except xml.parsers.expat.ExpatError as exception:
            self.stderr = f'{exception=}, {self.command=}, {self.ip=}'
            self.rc = 255
            await self.log.aerror(f'{exception=}', f'{self.stdout=}', f'{self.command=}', f'{self.ip=}')
