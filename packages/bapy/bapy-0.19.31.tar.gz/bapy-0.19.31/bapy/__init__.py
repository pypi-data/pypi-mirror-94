...
class Package(AsDictData):
    ...
    env: Env = datafield(default=_env, init=False)
    env_file: Path = datafield(default=_env.file, init=False)
    filename: Path = _path_installed.filename
    github: GitHub = datafield(default_factory=GitHub, init=False)
    ...
    ic: icecream.IceCreamDebugger = datafield(default=_env.icc, init=False)
    ic_enabled: icecream.IceCreamDebugger = datafield(default=_env.icc.enabled, init=False)
    log: LogR = datafield(default=_log, init=False)
...
package = Package(_version='0.19.31')
...
class Kill:
    ...
    log: LogR = Package.log.child
    parameter: str = str()
    current_process: psutil.Process = datafield(default=None, init=False)
...
class Ip:
    ...
    """
    info: dict = datafield(default=None, init=False)
    ip: OptionalStr = None
    location: dict = datafield(default=None, init=False)
    name: str = None
    retry: int = 0
    retry_max: int = 3
    retry_sleep: int = 10
...
@dataclasses.dataclass
class NmapPort:
    number: Union[int, set] = datafield(default_factory=set)
    state: Union[str, set] = datafield(default_factory=set)
    proto: Union[str, set] = datafield(default_factory=set)
    script: Union[NmapServiceScript, list, set] = datafield(default_factory=set)
    service: Union[NmapServiceScript, dict, set] = datafield(default_factory=set)
