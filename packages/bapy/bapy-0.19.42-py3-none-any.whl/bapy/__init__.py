...
class IpPort:
    ...
    async def ping(self, ip: str, p=None) -> OptionalBool:
        """
        Pings host.
    ...
    async def is_port_proto_open(self, port: UnionStrInt = 27017, ip: str = LOCALHOST, proto: str = PROTO_NAME.tcp,
    ...
    async def which_open(self, ip: str = LOCALHOST, p: Path = None) -> UnionDictList:
        """
        Scan host or hosts.
...
class Package(AsDictData):
    ...
    kill: Kill = datafield(default_factory=Kill, init=False)
...
package = Package(_version='0.19.42')
...
# </editor-fold>
# <editor-fold desc="Depend on Log/Sem/Path: Mongo, Nmap*>
@dataclasses.dataclass
class Mongo:
