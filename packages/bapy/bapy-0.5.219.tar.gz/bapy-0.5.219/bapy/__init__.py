...
class Package(AsDictData):
    ...
    git: GitHub = dataclasses.field(default_factory=GitHub, init=False)
    ...
    def __post_init__(self):
        ...
        if (p := Path(file).resolved) != Package.filename:
            ...
            self.git = GitHub(path=self.project, log=self.log)
            _path_installed.project if _path_installed.project else None
