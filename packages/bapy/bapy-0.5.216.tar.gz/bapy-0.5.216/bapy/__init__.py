...
# </editor-fold>
# <editor-fold desc="Constants, Literal & Data">
...
NapLiteral: Any = Literal['lock', 'mongo', 'rand', 'queue']
...
class Nap:
    ...
    def get_sleep(self, name: NapLiteral = 'mongo') -> Union[int, float]:
        return round(random.uniform(value[0], value[1]), 2) \
            if (value := getattr(self, name)) and isinstance(value, tuple) else value

    def nap(self, name: NapLiteral = 'mongo') -> None:
        time.sleep(self.get_sleep(name))

    async def anap(self, name: NapLiteral = 'mongo') -> None:
        await asyncio.sleep(self.get_sleep(name))
