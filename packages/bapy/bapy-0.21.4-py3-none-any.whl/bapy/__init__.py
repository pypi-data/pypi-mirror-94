...
class SemEnum(EnumDict):
    ...
    def sem(self, priority: Priority = Priority.LOW) -> asyncio.Semaphore:
        _log.debug('Start', f'{self.name=}', f'{priority.name=}')
        try:
            ...
            caller = str()
            _log.warning(f'{self.name=}', f'{priority.name=}', repr(exception))
        ...
        if priority == Priority.HIGH and self.value[Priority.LOW].locked():
            ...
            using = priority.name
        _log.warning(f'{self.name=}', f'{priority.name=}', f'{using=}', f'{self.status=}', f'{caller=}',
                     'Waiting')
        return rv
...
bapy = Package(filename=__file__, _version='0.21.4')
