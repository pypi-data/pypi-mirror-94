...
def unZip(filename: Optional[str] = None, newfilename: Optional[str] = None,
          newdirname: Optional[str] = None, dst_path: Optional[str] = None, **kwargs):
    '''
    解压文件 支持：*.zip *.tar.gz
    ...
    if dst_path not in [None, ''] and newfilename in [None, '']:
        if '.zip' == filename[-4:]:
            ...
            first_name = tarfile.open(f"{path.abspath(path.curdir)}/{download_filename}").getnames()[0].replace('/', '')
            tarfile.open(filename).extractall(path=dst_path)
        if newdirname and path.isdir(f'{path.abspath(dst_path)}/{first_name}'):
...
def download(url: Optional[str] = None, **kwargs):
    ...
    if url not in [None, '']:
        ...
        if 'unzip' in kwargs.keys() and kwargs.get('unzip'):
            if 'newfilename' in kwargs.keys() and 'det_path' not in kwargs.keys():
                unZip(filename=download_filename, newfilename=kwargs.get('newfilename'))
            elif 'dst_path' in kwargs.keys() and 'newfilename' not in kwargs.keys():
...
def jmeter_install(version: Optional[str] = 'latest', dest_dir: Optional[str] = '.'):
    ...
    latest = getWebList(url='https://mirror.bit.edu.cn/apache/jmeter/binaries/HEADER.html', match='Apache JMeter (.+?) requires')
    version = latest[0] if len(latest) > 0 and version == 'latest' else version
...
def allure_install(version: Optional[str] = 'latest', dest_dir: Optional[str] = '.'):
    ...
    name = get_osname()
    if name == 'windows':
        # download(url=f'https://github.com/allure-framework/allure2/releases/download/{version}/allure-{version}.zip',
        #          filename=f'allure-{version}.zip', newdirname='allure', dst_dir=dest_dir, unzip=True, del_src=True)
        ...
        download(url=f'https://repo.maven.apache.org/maven2/io/qameta/allure/allure-commandline/{version}/allure-commandline-{version}.tgz',
                 filename=f'allure-{version}.tgz', newdirname='allure', dst_dir=dest_dir, unzip=True, del_src=True)
    setEnv(name='ALLURE_HOME', value=f'{path.abspath(dest_dir)}/allure', addPath=True)

def svn_install(version: Optional[str] = 'latest', dest_dir: Optional[str] = '.'):
    ''' SVN安装 '''
    if version == 'latest':
        url = 'https://mirror.bit.edu.cn/apache/subversion'
