...
DEFAULT_PREFIX = getattr(settings, 'GOOGLE_CLOUD_TASKS_PREFIX', '')
...
class Task(metaclass=TaskMeta):
    ...
    _prefix = f'{DEFAULT_PREFIX}__' if DEFAULT_PREFIX else ''
...
class SubscriberTask(Task):
    ...
    def delay(self, **kwargs):
        return run_coroutine(
            ...
            topic_id=self._full_topic_name(name=self.topic_name),
    ...
    def subscription_name(self):
        if self._prefix:
            return self._prefix.replace('__', '')
        return self.name()
