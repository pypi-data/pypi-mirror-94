...
class Data_Driven(Memristor):
    ...
    def resistance(self, voltage):
        ...
        if voltage > 0:
            ...
            resistance_ = np.log(np.exp(self.eta * self.k_p * r_pn_eval) + np.exp(-self.eta * self.k_p * \
                            (self.A_p * (math.exp(self.t_p * abs(voltage)) - 1)) * self.time_series_resolution) * \
                            (np.exp(self.eta * self.k_p * (1 / self.g)) - np.exp(self.eta * self.k_p * r_pn_eval))) / self.k_p
            ...
            resistance_ = -np.log(np.exp(-self.eta * self.k_n * (1 / self.g) + self.eta * self.k_n * \
                            (self.A_n * (-1 + np.exp(self.t_n * abs(voltage)))) * self.time_series_resolution) - \
                            np.exp(-self.eta * self.k_n * r_pn_eval) * (-1 + np.exp(self.eta * self.k_n * (self.A_n * \
                            (-1 + np.exp(self.t_n * abs(voltage)))) * self.time_series_resolution))) / self.k_n
            if resistance_ < self.eta * r_pn_eval:
                return 1 / self.g
            else:
                return max(min(resistance_, self.r_off), self.r_on) # Artificially confine the resistance between r_on and r_off
    ...
    def plot_hysteresis_loop(self, duration=1e-3, voltage_signal_amplitude=1.5, voltage_signal_frequency=10e3, return_result=False):
    ...
    def plot_bipolar_switching_behaviour(self, voltage_signal_amplitude=1.5, voltage_signal_frequency=10e3, log_scale=True, return_result=False):
