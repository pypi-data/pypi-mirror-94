...
def page_add_riassunto():
    ...
    sec_filepath = str(UPLOAD_FOLDER.joinpath(sec_filename).resolve())
    file.save(sec_filepath)

    summary = Summary(request.form.get("nome"), request.form.get("descrizione"), int(utente.aid),
                      int(request.form["listamaterie"]), sec_filepath)

    db.session.add(summary)
    db.session.commit()

    commit = Commit("Riassunto aggiunto a Erre2.", int(summary.sid))
    db.session.add(commit)
    db.session.commit()

    requests.get(f"https://api.telegram.org/bot/{telegram_token}/sendMessage", params={
        "chat_id": group_chat_id,
        "text": f'Il riassunto "{summary.nome}" è stato caricato su Erre2.\n'
                f'<a href="{url_for("page_filter_course", cid=summary.corso_id, _external=True)}">Clicca qui per '
                f'visitare Erre2.</a>',
        "parse_mode": "HTML"
    })
    return redirect(url_for('page_administration'))
...
def page_update_riassunto(sid):
    ...
    summary = Summary.query.get_or_404(sid)

    if request.method == 'GET':
        return render_template("/administration/update.htm", riassunto=summary, utente=utente)
    ...
    commit = Commit(request.form.get('descrizione'), summary.sid)
    db.session.add(commit)
    db.session.commit()

    requests.get(f"https://api.telegram.org/bot/{telegram_token}/sendMessage", params={
        "chat_id": group_chat_id,
        "text": f'Il riassunto "{summary.nome}" è stato aggiornato.\n'
                f'Modifiche: {commit.descrizione}'
                f'<a href="{url_for("page_filter_course", cid=summary.corso_id, _external=True)}">Clicca qui per '
                f'visualizzarlo su Erre2</a>.',
        "parse_mode": "HTML"
    })
