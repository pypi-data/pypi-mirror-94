...
def is_backup_task_of_current_user(task, connection):
    ...
    has_backup_task = task_details and task_details.get('type', None) == 'backup'
    is_of_current_user = connection.auth is None or task_details.get('createdBy', None) == connection.auth.username
    return has_backup_task and is_of_current_user
...
def check_backup_needed(project, connection):
    ...
    last_modified_date = project.get('lastModifiedDate', None)
    if last_modified_date is not None:
        ...
        if (now - parsed_last_modified_date).days > 8 * 7:
            all_tasks = open_tasks
        else:
            closed_tasks = connection.tasks('closed', project_id, start=parsed_last_modified_date, end=now)
...
def backup(connection, user_backup_key, project=False,
    ...
    with ProjectContext(connection, project) as context:
        ...
        if download_backup:
            ...
            backup_progress = {'stage': '', 'progress': '0', 'status': ''}
            ...
            if backup_progress['status'] == 'completed':
                backup_file_path = path.join(download_folder, project, task_id + ".backup")
...
def wait_for_task(context, task_id, task_finished, backup_progress):
    try:
        while True:
            ...
            backup_progress['progress'] = backup_task.get('progressCompleted', 0)
