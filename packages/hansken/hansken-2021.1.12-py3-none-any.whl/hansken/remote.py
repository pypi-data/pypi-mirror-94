...
class Connection:
    ...
    def close(self, check_response=True):
        """
        Explicitly ends the session established with the remote.

        :param check_response: whether to check the response for the expected
            status code
        :return: self
        """
    ...
    def identity(self):
        """
        The identity uid for the current session.

        :return: the currently available uid of the form
            <username>@<identity-provider-hostname>, or `None`
        """
    ...
    def identity_uid(self):
        """
        Retrieves the current user's uid as seen by the remote gatekeeper.

        :return: uid of the form <username>@<identity-provider-hostname>, or
            `None`
        """
    ...
    def extract(self, project_id, image_id, type='index', key=fetch, tools=None, configuration=None, query=None):
        ...
        :param tools: the tools to be used; either a sequence of tool names or
            `None`, indicating to use all available tools (see also `.tools`)
...
class SearchResult(Iterable):
    ...
    def take(self, num, include=None):
        """
        Takes the first num traces from this result. Subsequent calls will
        produce batches of traces as if using slicing. Note that the remote
        may use a timeout on the underlying stream of traces, causing an
        error if traces are not being processed fast enough.

        :param num: amount of traces to take
        :param include: name (`str`) or names to be included in the result,
            see `.including`
        :return: a list of `Trace <hansken.trace.Trace>` or `namedtuple`
            instances, of at most `num` size
        :rtype: `list`
        """
        if isinstance(include, str):
            include = [include]
    ...
    def __iter__(self, close=False, include=None):
        if self._exhausted:
            raise ValueError('search result stream is exhausted, can only be read once '
...
class ProjectContext:
    ...
    def search_tracelets(self, tracelet_type, query=None, start=0, count=None, sort='id', select=select_all,
        ...
        iterator of `Tracelet <hansken.trace.Tracelet>` objects.
        ...
        :param incomplete_results: whether to allow results from an incomplete
            ...
            *not* allow results from an incomplete index)
        :return: a `Tracelet <hansken.trace.Tracelet>` stream (iterable once)
