...
from ijson.common import items as ijson_items, parse as ijson_parse
...
try:
    # ijson.basic_parse is fastest when backed by libyajl v2, let ijson figure out if that's installed
    from ijson.backends.yajl2 import basic_parse as ijson_basic_parse
    log.debug('using yajl v2 json stream parsing backend')
except ImportError:
    try:
        from ijson.backends.yajl import basic_parse as ijson_basic_parse
        log.debug('using yajl v1 json stream parsing backend')
    except ImportError:
        # no usable version of libyajl, use pure python fallback impl
        from ijson.backends.python import basic_parse as ijson_basic_parse
        log.debug('using pure python json stream parsing backend')
