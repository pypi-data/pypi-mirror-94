...
class ElectricityDataSource(AbstractDataSource):
    ...
    def get_config(self) -> DataSourceConfig:
        ...
        if self._featurizers:
            ...
            num_date_features = 0
            encodings = []
        encodings += [EncodingSpec(feature_index=len(encodings), num_vals=ElectricityDataSource.NUM_STATIONS)]

        return DataSourceConfig(feature_channels=1+num_date_features,  # station_id + any date features
                                forecast_channels=1,
                                encodings=encodings)
    ...
    def _load_data(self) -> None:
        ...
        # others report 26304, we natively read in
