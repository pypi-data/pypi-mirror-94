...
def unbatch_and_undo(batch: Mapping[str, torch.Tensor],
    ...
    if tf:
        return [tf.undo({k: v[i] for k, v in unbatched.items() if k not in [PAST_IND_KEY, FUTURE_IND_KEY]})
                for i in range(batch_size)]
    else:
        return [{k: v[i] for k, v in unbatched.items() if k not in [PAST_IND_KEY, FUTURE_IND_KEY]}
                for i in range(batch_size)]
