...
def _get_training_data(data: dict, data_settings: dict) -> Tuple[Any, Any, Any, Any]:
    ...
    """
    raw_column_names = data['x_raw_column_names']

    for data_key in ["X", "X_train", "X_valid"]:
        if data.get(data_key) is not None and not isinstance(data[data_key], pd.DataFrame):
            data[data_key] = pd.DataFrame(data[data_key], columns=raw_column_names)
...
def get_parameters_from_settings(file_name: str) -> tuple:
    ...
    dataset = data_preparer.prepare(automl_settings_obj)
