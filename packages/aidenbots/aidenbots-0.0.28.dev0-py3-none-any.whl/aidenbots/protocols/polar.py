...
def run(protocol):
    ...
    def dispense_on_side_of_well(instrument, volume, location):
    ...
    #         instrument.drop_tip()
    #
    def remove_bead_wash_buffer(volume, well):
        p300.flow_rate.aspirate = 5
        for _ in range(3):
            p300.aspirate((volume / 3), lb_plt[well].bottom(0.5).move(types.Point(x=2 * int(side[well]))))
    ...
    #
    def extraction_plate_well_wash(well):
        p300.move_to(lb_plt[well].top(-3))
        p300.dispense(50, lb_plt[well].top().move(types.Point(x=-3.2, y=0, z=-3)))
        p300.dispense(50, lb_plt[well].top().move(types.Point(x=+3.2, y=0, z=-3)))
        p300.dispense(50, lb_plt[well].top().move(types.Point(x=0, y=+3.2, z=-3)))
        p300.dispense(50, lb_plt[well].top().move(types.Point(x=0, y=-3.2, z=-3)))
        p300.move_to(lb_plt[well].top(-3))

    def elute_na_from_beads(well, volume=20):
        set_speed_profile(p300, 200, 50)
        p300.move_to(lb_plt[well].bottom().move(types.Point(z=5)))
        p300.dispense(20, lb_plt[well].bottom().move(types.Point(x=-2 * int(side[well]), y=0, z=5)))
        p300.blow_out(lb_plt[well].bottom().move(types.Point(x=-2 * int(side[well]), y=0, z=5)))
        for _ in range(5):
            p300.aspirate(volume, lb_plt[well].bottom(0.5))
            p300.dispense(volume, lb_plt[well].bottom().move(types.Point(x=-2 * int(side[well]), y=1.5, z=5)))
            p300.aspirate(volume, lb_plt[well].bottom())
            p300.dispense(volume, lb_plt[well].bottom().move(types.Point(x=-2 * int(side[well]), y=0, z=5)))
            p300.aspirate(volume, lb_plt[well].bottom())
            p300.dispense(volume, lb_plt[well].bottom().move(types.Point(x=-2 * int(side[well]), y=-1.5, z=5)))
        set_speed_profile(p300, 200, 200)
        p300.mix(10, volume * (3 / 4), lb_plt[well].bottom())
        p300.blow_out(lb_plt[well].bottom(5))

    def magbead_mixing(volume, well):
        set_speed_profile(p300, 400, 400)
        for _ in range(wash_buffer_reps):
            p300.aspirate(volume * (8 / 10), lb_plt[well].bottom(0.5))
            p300.dispense(volume * (8 / 10), lb_plt[well].bottom().move(types.Point(x=-1.75 * int(side[well]), y=1.5, z=10)))
            p300.aspirate(volume * (8 / 10), lb_plt[well].bottom(0.5))
            p300.dispense(volume * (8 / 10), lb_plt[well].bottom().move(types.Point(x=-1.75 * int(side[well]), y=0, z=10)))
            p300.aspirate(volume * (8 / 10), lb_plt[well].bottom(0.5))
            p300.dispense(volume * (8 / 10), lb_plt[well].bottom().move(types.Point(x=-1.75 * int(side[well]), y=-1.5, z=10)))
        p300.flow_rate.dispense = 50
        p300.blow_out(lb_plt[well].bottom(dispense_depth))
        exit_well(p300, lb_plt[well])

    def remove_extraction_buffer(well, volume):
        p300.flow_rate.aspirate = 25
        p300.aspirate(volume * (8 / 10), lb_plt[well].bottom().move(types.Point(x=(1 * int(side[well])), y=0, z=2)))
        protocol.delay(seconds=2)
        p300.aspirate((200 - (volume * (8 / 10))), lb_plt[well].bottom().move(types.Point(x=(1 * int(side[well])), y=0, z=0)))
        protocol.delay(seconds=1)
    ...
    protocol.pause()
    play_alert_sound('begin.mp3') #todo fix sound file
