...
def gen_lambda(binning,abnormal_values):
    ...
    for i in range(len(temp)):
        if i==0:
            lambda_list = 'lambda x: 0 if (x in abnormal_values) else %d if (x<=%f)'%(temp.loc[i,'coarse_bin']+1,temp.loc[i,'ub'])
...
def coarse_bin(df_train,df_valid,exclude_column,bin_num,abnormal_values):
...
#             print('No coarse binning for numeric variable %s as its unique values is less than 10'%(var))
            df_train_coarse_bin[var] = df_train[var]
        else:
