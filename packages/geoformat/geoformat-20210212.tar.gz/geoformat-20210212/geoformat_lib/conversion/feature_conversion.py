...
from geoformat_lib.filter.feature import feature_filter
            ...
            if _try_to_force_type is True:
                ...
                if _feature_list_fields_scan[_field_name_filter]['tmp_field_type']:
                    ...
                    if bytes in _feature_list_fields_scan[_field_name_filter]['tmp_field_type'] and float in \
                        ...
                        if _feature_list_fields_scan[_field_name_filter]['tmp_field_type'] - {int, float, bytes} == {str} or _feature_list_fields_scan[_field_name_filter]['tmp_field_type'] - {int, float, bytes} == set():
                            # if str is always hexadecimal then it's a bytes valid
                            if str in _feature_list_fields_scan[_field_name_filter]['tmp_field_type'] and _feature_list_fields_scan[_field_name_filter]['str_is_always_hexadecimal'] is False:
                                str_force = True
                            else:
                    ...
                    if _feature_list_fields_scan[_field_name_filter]['tmp_field_type'] - {float, str, int} == set() and\
                            _feature_list_fields_scan[_field_name_filter]['str_is_always_hexadecimal'] is True and int not in field_type_set:
                        field_type_set = {bytes}
                        str_force = False
...
def feature_list_to_geolayer(
    ...
    for feature in feature_list:
        geolayer_feature = copy.deepcopy(feature)
        geolayer_feature = feature_filter(
            feature=geolayer_feature,
            field_name_filter=field_name_filter,
            geometry_type_filter=geometry_type_filter,
            bbox_filter=bbox_filter,
            bbox=bbox_extent
        ...
        if geolayer_feature:
            ...
            if serialize:
                geolayer_feature = feature_serialize(geolayer_feature)
            _geolayer['features'][i_feat] = geolayer_feature
            i_feat += 1
