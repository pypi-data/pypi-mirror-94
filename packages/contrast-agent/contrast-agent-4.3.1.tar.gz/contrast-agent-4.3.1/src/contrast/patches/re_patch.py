...
def patch_re(re_module):
    ...
    patch_cls_or_instance(pattern_cls, MATCH, pattern_match_hook, force=True)
    patch_cls_or_instance(pattern_cls, SEARCH, pattern_search_hook, force=True)
    patch_cls_or_instance(pattern_cls, FINDALL, pattern_findall_hook, force=True)
    patch_cls_or_instance(pattern_cls, FINDITER, pattern_finditer_hook, force=True)

    if six.PY3:
        # fullmatch method is new in Py3
        patch_cls_or_instance(
            pattern_cls, FULLMATCH, pattern_fullmatch_hook, force=True
        )
...
def reverse_patches():
    ...
    patch_manager.reverse_patches_by_owner(pattern_cls, force=True)
    patch_manager.reverse_patches_by_owner(match_cls, force=True)
