...
from graia.application.message.elements.internal import App, Json, Plain, Quote, Source, Xml, Voice, Poke, FlashImage
    ...
    def detect_index(self, target_chain: MessageChain):
        ...
        for iter_item in local_iter:
            ...
            if isinstance(setting, SwitchParameter):  # 这里是已经被 catch 到了.
                ...
                afters = MessageChain.create(sum(
                ...
                if afters.startswith('"'):
                    ...
                    print(param_content, chain_frames[len(param_content_chain.split(self.delimiter, raw_string=True))+1:])
                    local_iter.continue_count += len(param_content_chain.split(self.delimiter, raw_string=True))-1
    ...
    async def beforeDispatch(self, interface: DispatcherInterface):
        try:
            ...
            if set([i.__class__ for i in message_chain.__root__]).intersection(
                ...
                raise ExecutionStop()
            interface.execution_contexts[-1].literature_detect_result = printer(self.detect_index(
                message_chain
            ))
        except:
            import traceback
...
if __name__ == "__main__":
    ...
    mc = MessageChain.create(
        [
            Plain('test n -gt "1 2 tsthd thsd ydj re7u  '), At(351453455), Plain(' " "arg arega er ae aghr ae rtyh'), At(656735757), Plain(' "')
        ]
    )
    l = Literature(
        "test",
