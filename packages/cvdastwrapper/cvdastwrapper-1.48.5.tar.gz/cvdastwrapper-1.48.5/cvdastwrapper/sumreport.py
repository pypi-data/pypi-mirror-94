...
import runonetest
import runconfig
from runconfig import inspecdir
from runconfig import dlogrespdname
from runconfig import dloginputdname
import reportutil
...
def moveToNoRetest(rval, api):
    ...
    if "no-retest" in rval["skipped"]:
        if api in rval["skipped"]["no-retest"] :
            for x in rval["skipped"]["no-retest"][api]:
                if x["reason"].find("Also") < 0 :
                    x["reason"] += ". Also test calls with same attack type has generated passed/failed outcome."
# end moveToNoRetest
...
#
#
def sumFailedTestsToCSV(csvfname, failedr):
    ...
    for ( spec, a ) in failedr.items():
        ...
        for ( api, t ) in a.items():
            ...
            if len(t[0]) > 0:
                allrows[0].extend(genCSVPRows(spec, apiname, "P1", t[0],apidef))
            if len(t[1]) > 0:
                allrows[1].extend(genCSVPRows(spec, apiname, "P2", t[1],apidef))
            if len(t[2]) > 0:
                allrows[2].extend(genCSVPRows(spec, apiname, "P3", t[2],apidef))
            if len(t[3]) > 0:
                allrows[3].extend(genCSVPRows(spec, apiname, "P4", t[3],apidef))
    ...
    for c in pcnts:
        ...
        n+=1
    csv +=", , , , ,\n"
    csv+="Specification Name, API Name, Priority, Test Info for Repro/Fix , Fuzz Attack Info,\n"
    csv+=tmpcsv
    f = open(csvfname, "w+")
    f.write(csv)
    f.close()
...
def sumperattack(csvfname, inspeclist=[], inattacklist=[]):
    ...
    rhtml += "<th colspan=\"4\">N of APIs with Issues By Priority</th>\n"
    ...
    rhtml += "</UL>\n"
    sumFailedTestsToCSV(csvfname, failedr)
