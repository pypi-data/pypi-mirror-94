...
def build_package():
    ...
    # getting appversion
    app = importlib.import_module(PROJECT_NAME)
    app_zip_name = f'{config["builder"].get("dist_name",PROJECT_NAME).lower().replace(" ", "_")}_v{".".join(str(x) for x in app.__version__)}.zip'
