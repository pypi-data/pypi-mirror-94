...
def create_index():
    ...
    names, numbers, ids = zip(
        *rocks.identify(missing, return_id=True, verbose=False))
    index = index.append(pd.DataFrame(
        {"name": names, "number": numbers, "id_": ids}))
...
def __query_ssoCards(ids, progressbar=False):
    ...
    try:
        response = requests.post(
            url=f"{url}mytargets", params=params, files=file_)
...
def pretty_print_card(ssoCard, minimal):
    ...
    if not minimal:
        ...
        console.print(
            Panel(minimal_card, title=f"({rock.number}) {rock.name}"))
