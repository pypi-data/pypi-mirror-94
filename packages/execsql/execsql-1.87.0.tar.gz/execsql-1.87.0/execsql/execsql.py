...
class WriteableZipfile(object):
	def __init__(self, zipfile_name, append=False):
		...
		if sys.version_info.major < 3 or (sys.version_info.major >=3 and sys.version_info.minor < 3):
			...
			self.zf = zipfile.ZipFile(zipfile_name, mode=zmode, compression=zipfile.ZIP_BZIP2)
		else:
			self.zf = zipfile.ZipFile(zipfile_name, mode=zmode, compression=zipfile.ZIP_BZIP2, compresslevel=9)
