...
__kernel void detect_vertical_lines_fixed_radius_maximum_metric(
...
)
{
    ...
    if (global_id_x < dim_x)
    {
        ...
        for (uint global_id_y = 0; global_id_y < dim_y; ++global_id_y)
        {
            ...
            for (uint i = 0; i < m; ++i)
            {
                max = fmax(fabs(vectors_x[global_id_x + (i * dim_x)] - vectors_y[global_id_y + (i * dim_y)]), max);
            }

            if (max < e)
            {
                if (global_id_x - global_id_y >= 0)
                {
                    matrix[(global_id_y / size) * dim_x + global_id_x] += (convert_uint(1) << (global_id_y % size));
                }
...
__kernel void detect_vertical_lines_radius_corridor_maximum_metric(
    __global const ${fp_type}* vectors_x,
    __global const ${fp_type}* vectors_y,
    const uint dim_x,
    const uint dim_y,
