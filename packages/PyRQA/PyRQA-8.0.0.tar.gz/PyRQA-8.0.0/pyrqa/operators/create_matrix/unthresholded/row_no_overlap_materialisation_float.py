...
"""
Create unthresholded recurrence matrix using brute-force processing.
...
def create_matrix(settings,
    ...
    :param return_sub_matrix_data: Shall the sub matrix data be returned? (Default: False)
    ...
    # Write to buffers
    vectors_x = settings.time_series_x.get_sub_vectors_as_rows(sub_matrix.start_x,
                                                               sub_matrix.dim_x)
    ...
    vectors_y = settings.time_series_y.get_sub_vectors_as_rows(sub_matrix.start_y,
                                                               sub_matrix.dim_y)
    ...
    sub_matrix_size_byte = sub_matrix.size_byte(settings.dtype)

    if sub_matrix_size_byte > device.max_mem_alloc_size:
    ...
    if return_sub_matrix_data:
        # Read from buffer
        sub_matrix.set_empty_data_byte(settings.dtype)
