...
@dataclass(eq=False)
class PeriodicVoltageSource(TwoTerminalVoltageComponent):
    """
    A periodic voltage supply.
    ...
    def next_event(self, i: Callable[[float], np.ndarray], t1: float, t2: float) -> Optional[float]:
