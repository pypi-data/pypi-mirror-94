...
from .run_settings import RunSettings, _K8sRunSettings, _set_compute_field
...
class Input(TelemetryMixin):
    ...
    def __init__(self, dset, name: str, mode=None, owner=None):
        ...
        :param dset: The input dataset.
        :type dset: azureml.core.Dataset
    ...
    def configure(self, mode=None):
        ...
        :param mode: The mode that will be used for this input. Available options are
            'mount' and 'download'. See https://aka.ms/dataset-mount-vs-download for more details.
            Note that 'mount' only works in a linux compute, windows compute only supports 'download'.
        :type mode: str
        """
    ...
    def _configure(self, mode):
        ...
        if mode not in self._AVAILABLE_MODE:
            raise UserErrorException("Invalid mode: '{}', only 'mount' or 'download' is allowed.".format(mode))
    ...
    def _build(self):
        """Build the input to used in graph builder which is used for submission.

        TODO: Find a way to remove this method, which is not independent, highly coupled with graph builder logic.
        """
        if isinstance(self._dset, AbstractDataset):
            return self._build_dataset(self._dset, self.mode)
        elif isinstance(self._dset, Output):
            # Here we must ensure Input._build is called after Output._build in graph builder.
            # Currently a pipeline with cycle will hit the case that Input._build is called before Output._build
            # This will make the result graph incorrect.
            # However, there are another logic to check the cycle, so this will not cause real problem.
            # Due to such scenario, we cannot raise exception here when the output is not built.
            # TODO: Refine the graph build and validation logic so we could raise exception for the above case.
            return self._dset._last_build
        elif isinstance(self._dset, Input):
            # usually, the _dset should always comes from a source or a output
            # the _dset may be _Input only to describe that
            # the destination comes from a specific subgraph dummy input port
            return self._dset._build()
        # For other allowed types, directly return itself.
        elif isinstance(self._dset, self._ALLOWED_TYPES):
            return self._dset
        else:
            raise NotImplementedError("Invalid type %r for input %r." % (type(self._dset), self.name))

    def _get_telemetry_values(self):
        return self._owner._get_telemetry_values() if self._owner else {}

    @classmethod
    def _build_dataset(cls, dset: AbstractDataset, mode='mount'):
        """Build the dataset as a DatasetConsumptionConfig for submission."""
        ...
        if mode == 'mount':
            result = dset.as_named_input(name).as_mount()
        elif mode == 'download':
            result = dset.as_named_input(name).as_download()
...
class Output(TelemetryMixin):
    ...
    def _build(self, producer=None, default_datastore=None):
        if self._datastore is None:
            self._datastore = default_datastore

        self._last_build = PipelineData(self._port_name,
                                        datastore=self._datastore,
                                        output_mode=self._output_mode,
                                        output_path_on_compute=self._path_on_compute,
                                        dataset_registration=self._dataset_registration,
                                        dataset_output_options=self._dataset_output_options)
...
class Component(WorkspaceTelemetryMixin):
    ...
    * `Define a component using component specs <https://aka.ms/azureml-component-specs>`_
    ...
    def __init__(self, definition: CommandComponentDefinition, _init_params: Mapping[str, str],
        ...
        self._runsettings = RunSettings(definition.runsettings, definition.name, definition.workspace, self) if \
            definition.runsettings else None
    ...
    def _build_outputs_map(self, producer=None, default_datastore=None) -> Mapping[str, Any]:
        """Build a map from output name -> DatasetConsumptionConfig."""
        return {
            output.name: self._outputs[arg_name]._build(producer, default_datastore)
            for arg_name, output in self._definition.outputs.items()
        }

    def _build_inputs_map(self) -> Mapping[str, Any]:
        """Build a map from input name -> DatasetConsumptionConfig."""
        inputs_map = {}
        for arg_name, input in self._definition.inputs.items():
            value = self._inputs.get(arg_name)
            if value is None:
                continue
            value = value._build()
            if value is not None:
                inputs_map[input.name] = value
        return inputs_map
    ...
    def _populate_runconfig(self, use_local_compute=False):
        ...
        _set_compute_field(self._k8srunsettings, compute_type, run_config)
        ...
        if hasattr(self._runsettings, 'process_count_per_node'):
            if use_local_compute:
                ...
                if self.type == ComponentType.DistributedComponent.value:
    ...
    def _transfer_params(self, run_id, params, runconfig):
        ...
        for key, value in params.items():
            ...
            k = self._get_argument_name_by_name(key)
    ...
    def _build_arguments_for_run_config(self, run_config, run_id):
        ...
        output_params = self._transfer_params(
            ...
            run_config)
        inputs = {**input_params, **self._parameter_params}
        command = ComponentRunCommand.generate_component_command(self._definition, inputs, output_params)
        # When the component is using any command not python command,
        # by design a command string is returned instead of a python command list.
        if isinstance(command, str):  # TODO: Make sure this could be correctly handled.
            return command
        return command[2:]  # Ignore the first two command 'python xx.py'.
    ...
    def _validate(self, raise_error=False, pipeline_parameters=None, is_local=False):
        """
        ...
        if not is_local:
            ComponentValidator.validate_runsettings(runsettings=self._runsettings,
                                                    process_error=process_error)

            ComponentValidator.validate_k8srunsettings(k8srunsettings=self._k8srunsettings,
                                                       process_error=process_error)
    ...
    def _submit(self, experiment_name=None, source_dir=None, tags=None, skip_validation=False) -> Run:
        ...
        if self._definition._is_any_command():
            raise NotImplementedError(
        ...
        src = ScriptRunConfig(source_directory=source_dir, script=script,
                              arguments=arguments, run_config=run_config)
