...
class CommandExecutionBuilder:
    ...
    def build(self):
        ...
        # Generate command
        use_docker = self.component_env.image_name is not None
        run_command = ComponentRunCommand(
            self.component, self.working_dir,
            self.component_env.is_windows() if use_docker else os_name == 'nt')
        command, volumes = run_command.generate_command(use_docker=use_docker)
        ...
        if hasattr(runsettings, 'environment_variables'):
            if runsettings.environment_variables is not None:
                environment.update(json.loads(runsettings.environment_variables))
...
class ComponentRunCommand:
    ...
    def generate_command(self, use_docker, check_data_exist=True, container_input_prefix=None,
        ...
        command = self.generate_component_command(self.component._definition, inputs, output_path)
        if self.component.type == ComponentType.DistributedComponent.value:
            if self.component._definition.launcher.type == LauncherType.MPI:
                ...
                command = translate_mpi_command_by_component(command, self.component, self.is_windows)
            else:
    ...
    def resolve_command(cls, command, inputs=None, outputs=None, split=False) -> Union[str, List[str]]:
        ...
        for optional_part, optional_value in cls.OPTIONAL_PATTERN.findall(command):
            ...
            if not match:
                raise ValueError("Input pattern {inputs.xx} is not found in the optional command %r." % optional_value)
