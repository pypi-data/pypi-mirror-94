...
class ArgParseParamGenerator(BaseParamGenerator):
    ...
    def arg_type(self):
        ...
        tpl = "EnumParameter(enum={enum_class}, description=%r)" % self.description\
            if self.type == 'Enum' else "{type}Parameter(description=%r)" % self.description
        return tpl.format(type=self.type, enum_class=self.enum_class)
