...
class Airplane:
    ...
    __api_host = os.getenv("AP_TASK_RUNTIME_API_HOST")
    __api_token = os.getenv("AP_TASK_RUNTIME_TOKEN")
    ...
    def run(self, task_id, parameters):
        ...
        # Boot the new task:
        resp = requests.post(
            "%s/taskRuntime/runTask" % (self.__api_host),
            json={
                "taskID": task_id,
                "params": parameters,
            },
            headers={
                "X-Airplane-Token": self.__api_token,
            },
        )
        body = resp.json()
        run_id = body["runID"]
    ...
    def __getOutput(self, run_id):
        resp = requests.get(
            "%s/taskRuntime/getOutput/%s" % (self.__api_host, run_id),
            headers={
                "X-Airplane-Token": self.__api_token,
            },
        )
