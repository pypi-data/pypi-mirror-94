...
class OWPivot(OWWidget):
    ...
    def _add_control_area_controls(self):
        ...
        gui.comboBox(box, self, "row_feature", label=i18n.t("common.general.row"), contentsLength=12,
                     searchable=True,
                     model=DomainModel(valid_types=DomainModel.PRIMITIVE),
                     callback=self.__feature_changed)
        gui.comboBox(box, self, "col_feature", label=i18n.t("common.general.column"),
                     contentsLength=12,
                     searchable=True,
                     model=DomainModel(placeholder=__("placeholder_same_as_row"),
                                       valid_types=DiscreteVariable),
                     callback=self.__feature_changed, )
        gui.comboBox(box, self, "val_feature", label=__("box.value"),
                     contentsLength=12,
                     searchable=True,
