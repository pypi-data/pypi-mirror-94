...
class OWSelectAttributes(widget.OWWidget):
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __("box.available_var"),
                       addToLayout=False)
        ...
        box = gui.vBox(self.controlArea, __("box.target_var"), addToLayout=False)
        ...
        box = gui.vBox(self.controlArea, __("box.meta_var"), addToLayout=False)
        ...
        self.up_attr_button = gui.button(bbox, self, __("btn_up"),
        ...
        self.up_meta_button = gui.button(bbox, self, __("btn_up"),
                                         callback=partial(self.move_up, self.meta_attrs_view))
        self.move_meta_button = gui.button(bbox, self, ">",
                                           callback=partial(self.move_selected,
                                                            self.meta_attrs_view)
                                          )
        self.down_meta_button = gui.button(bbox, self, __("btn_down"),
                                           callback=partial(self.move_down, self.meta_attrs_view))

        bbox = gui.vBox(self.controlArea, __("box.additional_settings"), addToLayout=False)
        gui.checkBox(
            bbox, self, "select_new_features", __("box.select_new_features")
        )
        layout.addWidget(bbox, 3, 0, 1, 3)

        autobox = gui.auto_send(None, self, "auto_commit")
        layout.addWidget(autobox, 4, 0, 1, 3)
        reset = gui.button(None, self, i18n.t("common.btn.reset"), callback=self.reset, width=120)
