...
class BaseExperiment(CommonExperiment):
        ...
        if _in_ipython_environment() and self._log_code:
            ...
            if source_code == "":
                ...
                self._log_code_asset(
                    "experiment_creation", file_name=get_caller_file_path()
                )
            else:
                self._log_code_asset(
                    "experiment_creation", code=source_code, code_name="Jupyter Code"
                )
        ...
        if self._log_code:
            ...
            try:
                ...
                if not _in_ipython_environment():
                    experiment_creation_file = get_caller_file_path()
                    self._log_code_asset(
                        "experiment_creation", file_name=experiment_creation_file
                    )

                    script_name = sys.argv[0]
                    if script_name != experiment_creation_file and os.path.isfile(
                        script_name
                    ):
                        self._log_code_asset(
    ...
    def _log_asset_folder(
        ...
        # type: (str, Optional[int], bool, bool, str, Any, Optional[str], Optional[str], Optional[List[str]]) -> Optional[List[Tuple[str, Dict[str, str]]]]
        self.set_step(step)
