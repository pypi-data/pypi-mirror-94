...
class TwitterAPI:
    ...
    metas = [
        (data.StringVariable(__('content')), lambda doc: doc.full_text if not
         doc.retweeted else doc.text),
        (tv, lambda doc: TwitterAPI.tv.parse(doc.created_at.isoformat())),
        (data.DiscreteVariable(__('language')), lambda doc: doc.lang),
        (data.DiscreteVariable(__('location')), lambda doc: getattr(doc.place, 'country_code', None)),
        (data.ContinuousVariable(__('likes'), number_of_decimals=0),
         lambda doc: doc.favorite_count),
        (data.ContinuousVariable(__('retweets'), number_of_decimals=0),
         lambda doc: doc.retweet_count),
        (data.DiscreteVariable(__('reply_to')),
            lambda doc: '@' + doc.in_reply_to_screen_name if doc.in_reply_to_screen_name else ''),
        (data.DiscreteVariable(__('author.name')), lambda doc: doc.author.name),
        (data.StringVariable(__('author.description')), lambda doc: doc.author.description),
        (data.ContinuousVariable(__('author.status_count'), number_of_decimals=0),
         lambda doc: doc.author.statuses_count),
        (data.ContinuousVariable(__('author.favourites_count'), number_of_decimals=0),
         lambda doc: doc.author.favourites_count),
        (data.ContinuousVariable(__('author.friends_count'), number_of_decimals=0),
         lambda doc: doc.author.friends_count),
        (data.ContinuousVariable(__('author.followers_count'), number_of_decimals=0),
         lambda doc: doc.author.followers_count),
        (data.ContinuousVariable(__('author.listed_count'), number_of_decimals=0),
         lambda doc: doc.author.listed_count),
        (data.DiscreteVariable(__('author.verified')), lambda doc: str(doc.author.verified)),
        (data.ContinuousVariable(__('longitude')),
            lambda doc: coordinates_geoJSON(doc.coordinates)[0]),
        (data.ContinuousVariable(__('latitude')),
