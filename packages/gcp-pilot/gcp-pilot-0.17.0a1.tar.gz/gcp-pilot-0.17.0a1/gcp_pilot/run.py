...
class CloudRun(DiscoveryMixin, GoogleCloudPilotAPI):
    ...
    def list_routes(self, service_name: str = None, project_id: str = None) -> Generator[ResourceType, None, None]:
        params = dict(
            parent=self._namespace_path(project_id=project_id),
        )
        items = self._list(
            method=self.client.namespaces().routes().list,
            result_key='items',
            params=params,
        )
        for item in items:
            if not service_name or item['metadata']['name'] == service_name:
                yield item
