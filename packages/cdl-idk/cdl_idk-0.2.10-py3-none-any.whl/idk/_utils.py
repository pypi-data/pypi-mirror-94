...
def _correct_classname(cwd, logger):
    with open("idk.json", "r") as config_file:
        ...
        with open(config["insight_file"], "r") as ins:
            file_str = str(ins.read())
            look_for = f"class {config['insight_class']}(idk.insight.InsightGenerator)"
            if look_for not in file_str:
                logger.error(
                    f"Can't find {config['insight_class']} in {config['insight_file']} (specified in idk.json).\n" +
                    f"{30*' '}Looking for: '{look_for}'"
                )
                return False
    return True
