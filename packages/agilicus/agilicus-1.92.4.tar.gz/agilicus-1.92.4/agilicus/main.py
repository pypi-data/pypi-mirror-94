...
@click.option("--user-id", default=None)
@click.option("--search-param", default=None)
...
@click.option("--name-resolution", default=None)
@click.option(
    "--service-type", type=click.Choice(["vpn", "agent", "internet"]), default=None
)
@click.option("--protocol", default=None)
...
def list_connectors(ctx, **kwargs):
    ...
    table = PrettyTable(
        ["id", "name", "org_id", "type", "service_account_id", "application_services"]
    )
    for entry in results.connectors:
        table.add_row(
            [
                entry.metadata.id,
                entry.spec.name,
                entry.spec.org_id,
                entry.spec.connector_type,
                entry.spec.service_account_id,
...
def list_agent_connectors(ctx, **kwargs):
    ...
    for entry in results.agent_connectors:
        table.add_row(
            [
                entry.metadata.id,
                entry.spec.name,
                entry.spec.org_id,
                entry.spec.connection_uri,
                entry.spec.max_number_connections,
                entry.status.application_services,
            ]
        )
    table.align = "l"
...
@click.option(
    ...
    type=click.Choice(["ike-preshared-key", "certificate"]),
    default=None,
)
...
@click.option("--ike-cipher-encryption-algorithm", type=click.Path(["aes128", "aes256"]))
@click.option(
    "--ike-cipher-integrity-algorithm", type=click.Path(["sha256", "sha384", "sha512"])
)
@click.option(
    "--ike-cipher-diffie-hellman-group", type=click.Path(["ecp256", "ecp384", "ecp512"])
)
@click.option("--esp-cipher-encryption-algorithm", type=click.Path(["aes128", "aes256"]))
@click.option(
    "--esp-cipher-integrity-algorithm", type=click.Path(["sha256", "sha384", "sha512"])
)
@click.option(
    "--esp-cipher-diffie-hellman-group", type=click.Path(["ecp256", "ecp384", "ecp512"])
)
