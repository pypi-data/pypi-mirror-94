...
> ListIpsecConnectorResponse list_ipsec_connector(limit=limit, org_id=org_id, name=name)
...
name = 'host1_connector' # str | Query the connector by name (optional)
    ...
    try:
        # list IPsec connectors
        api_response = api_instance.list_ipsec_connector(limit=limit, org_id=org_id, name=name)
        pprint(api_response)
    except ApiException as e:
