...
def test_fn_find_finite_1():
    ...
    indices1 = pa.find_nonfinite(A)
    indices2 = pa.find_nonfinite(B)

    assert indices1.n_elem == 0
    assert indices2.n_elem == 3
    assert math.isclose(pa.accu(indices2 - pa.umat([[6],[8],[10]])), 0, abs_tol=0.0001) == True
