...
def test_fn_trace_1():
    ...
    diagonal = pa.mat([ 0.061198, 0.058956, 0.314156, -0.393139, -0.353768 ])

    assert math.isclose(pa.trace(A) - pa.accu(diagonal), 0.0, abs_tol=0.0001) == True
    assert math.isclose(pa.trace(2*A) - pa.accu(2*diagonal), 0.0, abs_tol=0.0001) == True
    assert math.isclose(pa.trace(A+A) - pa.accu(diagonal+diagonal), 0.0, abs_tol=0.0001) == True
