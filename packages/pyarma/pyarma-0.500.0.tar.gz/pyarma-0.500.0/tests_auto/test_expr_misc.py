...
# TODO: runs on undocumented behaviour
...
#     B = A[0:pa.min(pa.size(A))-1, 0:pa.min(pa.size(A))-1]

#     q = B[pa.tail_cols, 1]
#     r = B[pa.head_rows, 1]

#     B = B + q*r + B + B[:, 1]*B[2, :] + pa.inv(B.t() + B)
...
#     assert math.isclose(pa.accu(pa.abs(B - C)), 0.0, abs_tol=0.0001) == True
