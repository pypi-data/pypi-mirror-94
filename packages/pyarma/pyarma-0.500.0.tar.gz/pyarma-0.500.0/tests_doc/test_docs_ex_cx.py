...
N = cx_mat(M)                # convert to double-precision floating point
...
# Operations
A = cx_mat(5, 10, fill.randu)
B = cx_mat(5, 10, fill.randu)
C = cx_mat(10, 5, fill.randu)
...
# broadcasting
X = cx_mat(6, 5, fill.ones)
v = cx_mat(6, 1, fill.randu)
...
# set_size
A = cx_mat()
A.set_size(5,10)       # or:  A = cx_mat(5,10)

B = cx_mat()
B.set_size( size(A) )  # or:  B = cx_mat( size(A) )

# reshape
A = cx_mat(4, 5, fill.randu)
...
# submatrix
A = cx_mat(5, 10, fill.zeros)

A[ 0:2, 1:3 ]             = cx_mat(3,3, fill.randu)
# A[ span(0,2), span(1,3) ] = cx_mat(3,3, fill.randu)
# A[ 0,1, size(3,3) ]       = cx_mat(3,3, fill.randu)

B = A[ 0:2, 1:3 ].eval()
# C = A[ span(0,2), span(1,3) ].eval()
# D = A[ 0,1, size(3,3) ].eval()

A[:,1]          = cx_mat(5,1,fill.randu)
# A[span_all, 1]  = cx_mat(5,1,fill.randu)
...
X[indices] = cx_mat(4,1,fill.ones)
...
# fast cx_mat
A = cx_mat(4, 5, fill.randu)
B = cx_mat(4, 5, fill.randu)

# C = cx_mat(A,B)

# ins_row/cols
A = cx_mat(5, 10, fill.randu)
B = cx_mat(5,  2, fill.ones )
...
# swap
A = cx_mat(4, 5, fill.zeros)
B = cx_mat(6, 7, fill.ones )

A.swap(B)

# iterators
X = cx_mat(5, 6, fill.randu)
...
X = cx_mat(100, 200, fill.randu)

for val in X[40:60, 50:100]:
  print(val)

# as_col
X = cx_mat(4, 5, fill.randu)
v = X.as_col()
...
#is_vec
A = cx_mat(1, 5, fill.randu)
B = cx_mat(5, 1, fill.randu)
...
#as_scalar
r = cx_mat(1, 5, fill.randu)
q = cx_mat(5, 1, fill.randu)
...
#cross
a = cx_mat(3, 1, fill.randu)
b = cx_mat(3, 1, fill.randu)
...
#join
A = cx_mat(4, 5, fill.randu)
B = cx_mat(4, 6, fill.randu)
C = cx_mat(6, 5, fill.randu)
...
X = cx_mat(5, 6, fill.randu)
Y = cx_mat(5, 6, fill.randu)
Z = max(X,Y)

#nonzeros
B = cx_mat(100, 100, fill.eye)
b = nonzeros(B)
...
#print
A = cx_mat(123, 456, fill.randu)

print(A)

# possible output:
# 
# <pyarma.pyarma.cx_mat object at 0x123456>
# [matrix size: 123x456]
...
B = cx_mat(size(A), fill.zeros)

C = cx_mat()

C.randu(size(A))

D = cx_mat(10,20, fill.ones)
D[3,4,size(C)] = C    # access submatrix of E

E = cx_mat( size(A) + size(E) )
G = cx_mat( size(A) * 2 )
...
#sigproc
#conv
A = cx_mat(256, 1, fill.randu)

B = cx_mat(16, 1, fill.randu)
...
#conv2
A = cx_mat(256, 256, fill.randu)

B = cx_mat(16, 16, fill.randu)
...
#fft
X = cx_mat(100, 1, fill.randu)
  
Y = fft(X, 128)

#fft2
A = cx_mat(100, 100, fill.randu)
...
# #hist
# v = cx_mat(1000, 1, fill.randn) # Gaussian distribution
...
# P = cx_mat([ 0.25, 0.50, 0.75 ])

# Q = quantile(V, P)

#princomp
A = cx_mat(5, 4, fill.randu)
...
# #normpdf
# X = cx_mat(10, 1, fill.randu)
# M = cx_mat(10, 1, fill.randu)
# S = cx_mat(10, 1, fill.randu)
...
#running_stat_vec
import builtins
stats = cx_running_stat_vec()

for i in builtins.range(10000):
  sample = cx_mat(5, 1, fill.randu)
  stats(sample)
...
more_stats = cx_running_stat_vec(True)

for i in builtins.range(20):
  sample = cx_mat(1, 3, fill.randu)
