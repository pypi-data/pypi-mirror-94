...
# fmat class
A = fmat(5, 5, fill.randu)
x = A[1,2]

M = umat(5, 5, fill.ones) # initialise 5x5 matrix with ones
N = fmat(M)                # convert to double-precision floating point
...
# Operations
A = fmat(5, 10, fill.randu)
B = fmat(5, 10, fill.randu)
C = fmat(10, 5, fill.randu)
...
# broadcasting
X = fmat(6, 5, fill.ones)
v = fmat(6, 1, fill.randu)
...
# set_size
A = fmat()
A.set_size(5,10)       # or:  A = fmat(5,10)

B = fmat()
B.set_size( size(A) )  # or:  B = fmat( size(A) )
...
# submatrix
A = fmat(5, 10, fill.zeros)

A[ 0:2, 1:3 ]             = fmat(3,3, fill.randu)
# A[ span(0,2), span(1,3) ] = fmat(3,3, fill.randu)
# A[ 0,1, size(3,3) ]       = fmat(3,3, fill.randu)

B = A[ 0:2, 1:3 ].eval()
# C = A[ span(0,2), span(1,3) ].eval()
# D = A[ 0,1, size(3,3) ].eval()

A[:,1]          = fmat(5,1,fill.randu)
# A[span_all, 1]  = fmat(5,1,fill.randu)

X = fmat(5, 5, fill.randu)
...
# swap
A = fmat(4, 5, fill.zeros)
B = fmat(6, 7, fill.ones )

A.swap(B)

# iterators
X = fmat(5, 6, fill.randu)
...
X = fmat(100, 200, fill.randu)

for val in X[40:60, 50:100]:
  print(val)

# as_col
X = fmat(4, 5, fill.randu)
v = X.as_col()
...
#is_vec
A = fmat(1, 5, fill.randu)
B = fmat(5, 1, fill.randu)
C = fmat(5, 5, fill.randu)
...
#as_scalar
r = fmat(1, 5, fill.randu)
q = fmat(5, 1, fill.randu)
...
#cross
a = fmat(3, 1, fill.randu)
b = fmat(3, 1, fill.randu)
...
#join
A = fmat(4, 5, fill.randu)
B = fmat(4, 6, fill.randu)
C = fmat(6, 5, fill.randu)
...
X = fmat(5, 6, fill.randu)
Y = fmat(5, 6, fill.randu)
Z = max(X,Y)

#nonzeros
B = fmat(100, 100, fill.eye)
b = nonzeros(B)
...
#print
A = fmat(123, 456, fill.randu)

print(A)

# possible output:
# 
# <pyarma.pyarma.fmat object at 0x123456>
# [matrix size: 123x456]
...
B = fmat(size(A), fill.zeros)

C = fmat()

C.randu(size(A))

D = fmat(10,20, fill.ones)
D[3,4,size(C)] = C    # access submatrix of E

E = fmat( size(A) + size(E) )
G = fmat( size(A) * 2 )
...
#sigproc
#conv
A = fmat(256, 1, fill.randu)

B = fmat(16, 1, fill.randu)
...
#conv2
A = fmat(256, 256, fill.randu)

B = fmat(16, 16, fill.randu)
...
#fft
X = fmat(100, 1, fill.randu)
  
Y = fft(X, 128)

#fft2
A = fmat(100, 100, fill.randu)
...
#hist
v = fmat(1000, 1, fill.randn) # Gaussian distribution
...
P = fmat([ 0.25, 0.50, 0.75 ])

Q = quantile(V, P)

#princomp
A = fmat(5, 4, fill.randu)
...
#normpdf
X = fmat(10, 1, fill.randu)
M = fmat(10, 1, fill.randu)
S = fmat(10, 1, fill.randu)
