...
def add_makeup_PIL(file, face_cropper, m_type=0, col=None):
    ...
    original_open_cv_image = np.array(original_PIL)
    # Convert RGB to BGR
...
class Face_cropper:
    ...
    def crop_out_lips(self, filename, isolate_area = False):
    ...
    def isolate_mouth(self, filename, mouth_only=True, preImg=None, pre_shape=None, crop_lips=False):
        pos_origin = []
        try:
            if preImg is None:
                img = cv2.imread(filename)
...
def how_close_to_color(imgPIL, color, treshold, isPil=True):
    if isPil:
