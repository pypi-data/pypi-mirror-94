"""Module that creates bus_coordinates.csv file from bus_streams.txt"""
import os
import pandas as pd
import ast
...
bus_info = []
for d in lista:
    bus_info.append([d["Lines"], d["VehicleNumber"], d["Time"], [(d["Lat"]), (d["Lon"])]])
array1 = [_ for _, __, col, val in bus_info]
array2 = [__ for _, __, col, val in bus_info]
arrays = [array1, array2]
tuples = list(zip(*arrays))

indeks = pd.MultiIndex.from_tuples(tuples, names=['Lines', 'VehicleNumber'])

df = pd.DataFrame([{col: val} for _ , __, col, val in bus_info], index=indeks)
