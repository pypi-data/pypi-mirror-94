...
if __name__ == '__main__':
    ...
    df3['coordinates'] = df3.apply(lambda x: wcf.transform_coors(x['szer_geo'], x['dlug_geo']),
                                   axis=1)

    df3['distance_from_bus_initial'] = df3.apply(lambda x: wcf.distance(x['coordinates'], row[0]),
                                                 axis=1)
    ...
    direction, opposite_direction = wbf.check_direction(bus_stop_initial, bus_stops_two_directions, row, 3, df3)

    df3['distance_from_bus_final'] = df3.apply(lambda x: wcf.distance(x['coordinates'], row[-1]),
                                               axis=1)
    ...
    print(bus_stop_initial)
    print(bus_stop_final)
    times = []
    ...
    if bus_stop_final in direction[direction.index(bus_stop_initial):]:
        for bus_stop in direction[direction.index(bus_stop_initial):direction.index(bus_stop_final)]:
            times.append(wbf.get_time_for_bus_stop(bus_stop, previous_bus_stop_time, row, df3))
...
print(direction[direction.index(bus_stop_initial):])
print(opposite_direction[:direction.index(bus_stop_final)])
print(times)