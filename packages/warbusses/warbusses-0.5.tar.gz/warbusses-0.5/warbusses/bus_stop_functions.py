"""Module that provides functions for check_punctuality.py"""
from warbusses import coors_functions as cf
    ...
    for k, direction in enumerate(bus_stops_two_directions):
        ...
        if bus_stop_initial not in direction:
            ...
            for coordinate in checking_row:
                distances += cf.distance(next_stop_coordinates, coordinate)
...
def get_time_for_bus_stop(bus_stop, previous_bus_stop_time, row, df3):
    ...
    minimum = cf.distance(bus_stop_coordinate, row[0])
    if row.size > 6:
        for coordinate in row[0:6]:
            if cf.distance(bus_stop_coordinate, coordinate) < minimum:
                minimum = cf.distance(bus_stop_coordinate, coordinate)
                coor = coordinate
        return row[row == coor].index[0]
    else:
        for coordinate in row[0:row.size]:
            if cf.distance(bus_stop_coordinate, coordinate) < minimum:
                minimum = cf.distance(bus_stop_coordinate, coordinate)
                coor = coordinate
        return row[row == coor].index[0]
