"""Module that creates create_speed.csv and coordinates_speed.csv files"""
import pandas as pd
import os
...
for i in range(57):
    ...
    for idx, group in dfn.groupby('idx'):
        try:
            ...
            dist_diff = wcf.distance(*coors)
            dates_diff = wcf.diff_between_dates(*dates)

            result = dist_diff / dates_diff
            res_dict[idx] = result
            coord_dict[idx] = wcf.middle_coors(*coors)
        except TypeError:
            print('nie ma danych')
