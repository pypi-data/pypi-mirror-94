...
class Minimize:
    ...
    def callback(self, value):
        if self.callback is not None:
            print(
                "The callback on the {!s} Optimization was "
    ...
    def root(self, fun, x):
        ...
        while True:
            ...
            if self.iter > self.maxIter:
                print(
                    "NewtonRoot stopped by maxIters ({:d}). "
                    "norm: {:4.4e}".format(self.maxIter, norm(rt))
                )
                break
...
class ProjectedGNCG(BFGS, Minimize, Remember):
    ...
    def _startup(self, x0):
        ...
        assert (
            self.lower.shape[0] == x0.shape[0]
        ), "Lower bound must be a list or vector"
        assert (
            self.upper.shape[0] == x0.shape[0]
        ), "Upper bound must be a list or vector"
