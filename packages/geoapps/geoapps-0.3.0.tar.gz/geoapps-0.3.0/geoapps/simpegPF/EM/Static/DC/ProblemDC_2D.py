        ...
        for iky in range(self.nky):
            ...
            for src in self.survey.srcList:
                ...
                for rx in src.rxList:
                    ...
                    Jv1_temp = (
                        1.0 / np.pi * rx.evalDeriv(ky, src, self.mesh, f, df_dm_v)
                    )
                    if iky == 0:
                        # First assigment
                        Jv[src, rx] = Jv1_temp * dky[iky] * np.cos(ky * y)
                    else:
                        Jv[src, rx] += Jv1_temp * dky[iky] / 2.0 * np.cos(ky * y)
                        Jv[src, rx] += Jv0[src, rx] * dky[iky] / 2.0 * np.cos(ky * y)
        ...
        for src in self.survey.srcList:
            for rx in src.rxList:
                ...
                for iky in range(self.nky):
                    ...
                    PTv = rx.evalDeriv(ky, src, self.mesh, f, v[src, rx], adjoint=True)
                    ...
                    Jtv_temp1 = 1.0 / np.pi * (df_dmT + du_dmT).astype(float)
                    # Trapezoidal intergration
                    if iky == 0:
                        # First assigment
                        Jtv += Jtv_temp1 * dky[iky] * np.cos(ky * y)
                    else:
                        Jtv += Jtv_temp1 * dky[iky] / 2.0 * np.cos(ky * y)
                        Jtv += Jtv_temp0 * dky[iky] / 2.0 * np.cos(ky * y)
    ...
    def getADeriv(self, ky, u, v, adjoint=False):
        ...
        if adjoint:
            return (
                ...
                + ky ** 2
                * self.rhoDeriv.T
                * Utils.sdiag(u.flatten() * vol * (-1.0 / rho ** 2))
                * v
            )

        return D * ((MfRhoIDeriv(G * u)) * v) + ky ** 2 * Utils.sdiag(
            u.flatten() * vol * (-1.0 / rho ** 2)
        ) * (self.rhoDeriv * v)
...
class Problem2D_N(BaseDCProblem_2D):
    ...
    def MnSigmaDeriv(self, u):
        ...
        vol = self.mesh.vol
        return Utils.sdiag(u) * self.mesh.aveN2CC.T * Utils.sdiag(vol) * self.sigmaDeriv
    ...
    def getADeriv(self, ky, u, v, adjoint=False):
        ...
        if adjoint:
            return (
                self.MeSigmaDeriv(Grad * u).T * (Grad * v)
                + ky ** 2 * self.MnSigmaDeriv(u).T * v
            )
        return (
            Grad.T * (self.MeSigmaDeriv(Grad * u) * v)
