...
class Dipole_ky(BaseRx):
    ...
    def eval(self, kys, src, mesh, f):
        ...
        return self.IntTrapezoidal(kys, Pf, y=0.0)
    ...
    def IntTrapezoidal(self, kys, Pf, y=0.0):
        ...
        for iky in range(nky):
            phi1 = 1.0 / np.pi * Pf[:, iky]
            phi += phi1 * dky[iky] / 2.0 * np.cos(kys[iky] * y)
            phi += phi0 * dky[iky] / 2.0 * np.cos(kys[iky] * y)
            phi0 = phi1.copy()
        return phi
