...
class readFTMGdataFile:
                ...
                if "/" in dline:
                    ...
                    if "Line" in dline:
                        ...
                        if len(header_ids) == len(text_line):
                            ...
                            print(
                                "[ERROR] column id and data column mismatch or blank line found!!!"
                            )
            # print('number of stations: {0} in line: {1}'.format(len(lines[0].stations), lines[0].line_id))
            self.lines.append(lines[0])
        else:
        ...
        if limits is None:
            for line in range(len(self.lines)):
                for stn in range(len(self.lines[line].stations)):
                    ...
                    if ground:
                        z_ground = (
                            self.lines[line].stations[stn].alt_gps
    ...
    def getComponentData(self, component=None, limits=None, inducing_field=None):
        ...
        if component.lower() == "tmi":
            assert (
                inducing_field is not None
            ), "Inducing field parameters must be provided for 'TMI' data"
            ...
            for line in range(len(self.lines)):
                for stn in range(len(self.lines[line].stations)):
                    ...
                    if limits is not None:
                        ...
                        if not (limits[0] < x_stn < limits[1]) and (
                            limits[2] < y_stn < limits[3]
                        ):
                            continue
                    ...
                    if component in self.lines[line].stations[stn].comps:
                        ...
                        if component.lower() == "tmi":
                            output_comp.append(
                                self.lines[line].stations[stn].comps[component]
                                - inducing_field[0]
                            )
                        else:
                            output_comp.append(
        ...
        if inducing_field is not None:
            ...
            if limits is None:
                ...
                if inducing_field.shape[0] == 3:
                    ...
                    comps = self.getListOfAvailableComponents(
                        format_type="simpeg", force_comp=force_comp
                    )
                    # Create a MAGsurvey
                    ...
                    # create dobs
                    dobs_list = []
                    components = self.getListOfAvailableComponents(
                        force_comp=force_comp
                    )
                    for comp in components:
                        # if comp is not 'TMI':
                        print(f"component added: {comp}")
                        dobs_list.append(
                            self.getComponentData(
                                component=comp, inducing_field=inducing_field
                            )
                        )
                    ...
                    print(
                        "[ERROR] array with Inducing field parameters must be size 3!!!"
                    )
            else:
                ...
                for comp in components:
                    ...
                    print(f"component added: {comp}")
                    dobs_list.append(
                        self.getComponentData(component=comp, limits=limits)
                    )
                # now sort the data to SimPEG format
                dobs = np.vstack(dobs_list)
                size = dobs.shape[0] * dobs.shape[1]
            ...
            print(
                "[ERROR] Please include numpy array with Inducing field parameters!!!"
            )

    def getListOfAvailableComponents(
        self, keep_tmi=False, format_type="std", force_comp=None
    ):
        """
