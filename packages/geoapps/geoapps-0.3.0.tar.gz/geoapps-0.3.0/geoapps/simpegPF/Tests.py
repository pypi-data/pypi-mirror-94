...
class OrderTest(unittest.TestCase):
    ...
    def orderTest(self):
        ...
        assert len(self.expectedOrders) == len(
            self.meshTypes
...
def checkDerivative(
    ...
    print("{!s} checkDerivative {!s}".format("=" * 20, "=" * 20))
    ...
    if passTest:
        print("{!s} PASS! {!s}".format("=" * 25, "=" * 25))
        print(happiness[np.random.randint(len(happiness))] + "\n")
    else:
        print(
            "{!s}\n{!s} FAIL! {!s}\n{!s}".format("*" * 57, "<" * 25, ">" * 25, "*" * 57)
        )
        print(sadness[np.random.randint(len(sadness))] + "\n")
    ...
    if plotIt:
        ...
        ax.loglog(h, E1, "g--")
        ax.set_title(
            "Check Derivative - {!s}".format("PASSED :)" if passTest else "FAILED :(")
        )
        ax.set_xlabel("h")
        ax.set_ylabel("Error")
        leg = ax.legend(
