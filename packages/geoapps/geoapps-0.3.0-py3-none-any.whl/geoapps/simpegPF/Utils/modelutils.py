...
def surface2ind_topo(mesh, topo, gridLoc="CC", method="linear", fill_value=np.nan):
    ...
    assert gridLoc in [
        "N",
        "CC",
    ], "Value of gridLoc must be 'N' (nodal) or 'CC' (cell center)"
...
def tileSurveyPoints(locs, nRefine, minimize=True, method="cluster"):
    ...
    if method == "cluster":
        ...
        X2, Y2 = np.meshgrid(
            np.r_[tilex[1:], locs[:, 0].max()], np.r_[tiley[1:], locs[:, 1].max()]
        )
...
def minCurvatureInterp(
        ...
        if grid.ndim == 1:
            ...
            aveCC2F = sp.vstack(
                (
                    ...
                    kron3(
                        speye(grid.shape[2]),
                        av_extrap(grid.shape[1]),
                        speye(grid.shape[0]),
                    ),
                    kron3(
                        av_extrap(grid.shape[2]),
                        speye(grid.shape[1]),
                        speye(grid.shape[0]),
                    ),
                ),
    ...
    assert locs.shape[0] == data.shape[0], (
        "Number of interpolated locs " + "must match number of data"
    )
    ...
    if mesh is None:
        ...
        if ndim == 3:
            ...
            mesh = Mesh.TensorMesh([np.ones(nCx) * gridSize, np.ones(nCy) * gridSize])
            mesh.x0 = [xmin, ymin]
            # gridCy, gridCx = np.meshgrid(vectorY, vectorX)
            # gridCC = np.c_[mkvc(gridCx), mkvc(gridCy)]
        else:
            mesh = Mesh.TensorMesh([np.ones(nCx) * gridSize])
            mesh.x0 = xmin
    else:
    ...
    if method == "relaxation":
        ...
        for i in range(ndat):
            ...
            r = (locs[i, 0] - locs[:, 0]) ** 2.0 + (locs[i, 1] - locs[:, 1]) ** 2.0
            A[i, :] = r.T * (np.log((r.T + 1e-8) ** 0.5) - 1.0)
        ...
        for dd in range(data.shape[1]):
            ...
            for i in range(nC):
                ...
                r = (gridCC[i, 0] - locs[:, 0]) ** 2.0 + (
                    gridCC[i, 1] - locs[:, 1]
                ) ** 2.0
                m[i, dd] = np.sum(w[0] * r.T * (np.log((r.T + 1e-8) ** 0.5) - 1.0))
...
def transfer_to_mesh(mesh_in, mesh_out, values, return_indices=False):
    """
