...
def memProfileWrapper(towrap, *funNames):
    ...
    for f in funNames:
        if hasattr(towrap, f):
            ...
            print(f"{f!s} not found in {towrap.__name__!s} Class")
...
def setKwargs(obj, ignore=None, **kwargs):
    ...
    for attr in kwargs:
        ...
        if hasattr(obj, attr):
            ...
            raise Exception(f"{attr!s} attr is not recognized")
...
def printStoppers(obj, stoppers, pad="", stop="STOP!", done="DONE!"):
    print(pad + "{!s}{!s}{!s}".format("-" * 25, stop, "-" * 25))
    for stopper in stoppers:
        l = stopper["left"](obj)
        r = stopper["right"](obj)
        print(pad + stopper["str"] % (l <= r, l, r))
    print(pad + "{!s}{!s}{!s}".format("-" * 25, done, "-" * 25))
