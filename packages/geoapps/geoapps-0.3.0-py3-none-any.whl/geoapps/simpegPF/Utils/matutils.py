...
def xyz2pst(m, param):
    ...
    Rz = np.vstack(
        (
            np.r_[np.cos(np.deg2rad(-param[2])), -np.sin(np.deg2rad(-param[2])), 0],
            np.r_[np.sin(np.deg2rad(-param[2])), np.cos(np.deg2rad(-param[2])), 0],
            np.r_[0, 0, 1],
        )
    )
    ...
    Rx = np.vstack(
        (
            np.r_[1, 0, 0],
            np.r_[0, np.cos(np.deg2rad(-param[1])), -np.sin(np.deg2rad(-param[1]))],
            np.r_[0, np.sin(np.deg2rad(-param[1])), np.cos(np.deg2rad(-param[1]))],
        )
    )
    ...
    m_pst = np.r_[np.dot(pvec.T, m.T), np.dot(svec.T, m.T), np.dot(tvec.T, m.T)].T
