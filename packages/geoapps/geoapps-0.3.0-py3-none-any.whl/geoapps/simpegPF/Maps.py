...
class IdentityMap:
    ...
    def __init__(self, mesh=None, nP=None, **kwargs):
        ...
        if nP is not None:
            if isinstance(nP, str):
                assert nP == "*", f"nP must be an integer or '*', not {nP}"
            assert isinstance(
                nP, (int,) + (np.int64,)
    ...
    def test(self, m=None, num=4, **kwargs):
        ...
        """
        print("Testing {!s}".format(str(self)))
        if m is None:
        ...
        assert isinstance(
            self.nP, int
        ), f"nP must be an integer for {self.__class__.__name__}"
    ...
    def _assertMatchesPair(self, pair):
        ...
        ), f"Mapping object must be an instance of a {pair.__name__!s} class."
    ...
    def __mul__(self, val):
        if isinstance(val, IdentityMap):
            if (
                ...
                raise ValueError(
                    "Dimension mismatch in {!s} and {!s}.".format(str(self), str(val))
                )
            ...
            if not self.shape[1] == "*" and not self.shape[1] == val.shape[0]:
                raise ValueError(
                    "Dimension mismatch in {!s} and np.ndarray{!s}.".format(
    ...
    def __init__(self, maps, **kwargs):
        ...
        for ii, m in enumerate(maps):
            ...
            if (
                ...
                raise ValueError(
                    "Dimension mismatch in map[{!s}] ({!s}, {!s}) "
                    "and map[{!s}] ({!s}, {!s}).".format(
...
class Projection(IdentityMap):
    ...
    def __init__(self, nP, index, **kwargs):
        ...
        assert max(index) < nP, f"maximum index must be less than {nP}"
...
class SurjectUnits(IdentityMap):
    ...
    def __init__(self, index, **kwargs):
        assert isinstance(index, (list)), "index must be a list, not {}".format(
            type(index)
        )
    ...
    def P(self):
        ...
        if getattr(self, "_P", None) is None:
            ...
            P = sp.csr_matrix((val, (row, col)), shape=(len(self.index), nP)).T
...
class Tile(IdentityMap):
    ...
    def S(self):
        ...
        if getattr(self, "_S", None) is None:
            ...
            nI = self.index.sum()
            assert nI <= nP, f"maximum index must be less than {nP}"
    ...
    def P(self):
        ...
        if getattr(self, "_P", None) is None:
            ...
            if self.meshLocal._meshType == "TREE":
                ...
                indL = self.meshLocal._get_containing_cell_indexes(
                    self.meshGlobal.gridCC
                )
                ...
                local2Global = np.c_[
                    np.kron(
                        np.ones(self.nCell), np.asarray(range(self.activeLocal.sum()))
                    ).astype("int"),
                    mkvc(indx),
                ]
                Pac = Utils.speye(self.meshGlobal.nC)[:, self.actvGlobal]
                ...
                r, ind = tree.query(
                    self.meshGlobal.gridCC[self.actvGlobal], k=self.nCell
                )
                global2Local = np.c_[
                    np.kron(
                        np.ones(self.nCell), np.asarray(range(self.actvGlobal.sum()))
                    ).astype("int"),
                    mkvc(ind),
                ]

                full = np.unique(
                    np.vstack([local2Global, global2Local[:, [1, 0]]]), axis=0
                )
            ...
            global_bsw, global_tne = self.getNodeExtent(
                self.meshGlobal, np.ones(self.meshGlobal.nC, dtype="bool")
            )

            local_bsw, local_tne = self.getNodeExtent(
                self.meshLocal, np.ones(self.meshLocal.nC, dtype="bool")
            )
            ...
            if self.meshLocal.dim == 1:
                ...
                dV = np.max(
                    [
                        (
                            np.min(
                                [global_tne[full[:, 1]], local_tne[full[:, 0]]], axis=0
                            )
                            - np.max(
                                [global_bsw[full[:, 1]], local_bsw[full[:, 0]]], axis=0
                            )
                        ),
                        np.zeros(nactv),
                    ],
                ...
                dV = np.max(
                    [
                        (
                            np.min(
                                [global_tne[full[:, 1], 0], local_tne[full[:, 0], 0]],
                                axis=0,
                            )
                            - np.max(
                                [global_bsw[full[:, 1], 0], local_bsw[full[:, 0], 0]],
                                axis=0,
                            )
                        ),
                        np.zeros(nactv),
                ...
                dV *= np.max(
                    [
                        (
                            np.min(
                                [global_tne[full[:, 1], 1], local_tne[full[:, 0], 1]],
                                axis=0,
                            )
                            - np.max(
                                [global_bsw[full[:, 1], 1], local_bsw[full[:, 0], 1]],
                                axis=0,
                            )
                        ),
                        np.zeros(nactv),
            ...
            if self.meshLocal.dim == 3:
                ...
                dV *= np.max(
                    [
                        (
                            np.min(
                                [global_tne[full[:, 1], 2], local_tne[full[:, 0], 2]],
                                axis=0,
                            )
                            - np.max(
                                [global_bsw[full[:, 1], 2], local_bsw[full[:, 0], 2]],
                                axis=0,
                            )
                        ),
                        np.zeros(nactv),
            ...
            self.P_deriv = sp.block_diag(
                [
                    Utils.sdiag(1.0 / self.meshLocal.vol[self.activeLocal]) * P * self.S
                    for ii in range(self.nBlock)
                ]
            )

            self._P = sp.block_diag(
                [Utils.sdiag(1.0 / sumRow) * P * self.S for ii in range(self.nBlock)]
            )

            self._shape = (
                int(self.activeLocal.sum() * self.nBlock),
                int(self.actvGlobal.sum() * self.nBlock),
            )
    ...
    def getTreeIndex(self, tree, mesh, actvCell):
        ...
        d, indx = tree.query(mesh.gridCC[actvCell, :], k=self.nCell)
    ...
    def getNodeExtent(self, mesh, actvCell):
        ...
        bsw = mesh.gridCC - mesh.h_gridded / 2.0
        tne = mesh.gridCC + mesh.h_gridded / 2.0
...
class SelfConsistentEffectiveMedium(IdentityMap, properties.HasProperties):
    ...
    rel_tol = properties.Float(
        "relative tolerance for convergence for the fixed-point iteration", default=1e-4
    )
    ...
    def getQ(self, alpha):
        if alpha < 1.0:
            Chi = np.sqrt((1.0 / alpha ** 2.0) - 1.0)
            return (
                1.0
                / 2.0
                * (1.0 + 1.0 / (alpha ** 2.0 - 1.0) * (1.0 - np.arctan(Chi) / Chi))
            )
        elif alpha > 1.0:
            raise NotImplementedError("Aspect ratios > 1 have not been implemeted")
        elif alpha == 1:
            return 1.0 / 3.0
    ...
    def getR(self, sj, se, alpha):
        ...
        Q = self.getQ(alpha)
        return (
            se / 3.0 * (2.0 / (se + Q * (sj - se)) + 1.0 / (sj - 2.0 * Q * (sj - se)))
        )
    ...
    def __init__(self, mesh=None, nP=None, weights=None, **kwargs):
        ...
        if "nC" in kwargs:
            raise AttributeError(
                "`nC` is depreciated. Use `nP` to set the number of model " "parameters"
            )
...
class Mesh2Mesh(IdentityMap):
    ...
    def __init__(self, meshes, **kwargs):
        ...
        self.P = self.mesh2.getInterpolationMat(
            self.mesh.gridCC, "CC", zerosOutside=True
        )
...
class Mesh2MeshTopo(IdentityMap):
    ...
    def _createProjection(self):
        ...
        I = Utils.mkvc(
            np.arange(inds.shape[0]).reshape([-1, 1]).repeat(self.nIterpPts, axis=1)
        )
        J = Utils.mkvc(inds)
        P = sp.coo_matrix(
...
class InjectActiveCells(IdentityMap):
    ...
    def __init__(self, mesh, indActive, valInactive, nC=None, n_blocks=1):
        ...
        if self.n_blocks > 1:
            ...
            self.valInactive = np.kron(np.ones(self.n_blocks), self.valInactive)
        else:
            self.P = P
...
class ParametricPolyMap(IdentityMap):
    ...
    def deriv(self, m, v=None):
        ...
        g3 = Utils.sdiag(alpha * (sig2 - sig1) / (1.0 + (alpha * f) ** 2) / np.pi) * V
        ...
        if self.mesh.dim == 2:
            ...
            if self.normal == "Y":
                ...
                for i in range(np.size(self.pts)):
                    ...
                    g3[:, i] = (
                        Utils.sdiag(
                            alpha * (sig2 - sig1) / (1.0 + (alpha * f) ** 2) / np.pi
                        )
                        * fderiv
                    )
...
class AmplitudeMap(IdentityMap):
    ...
    def deriv(self, m, v=None):
        ...
        """
        A = Utils.sdiag(self._transform(m) ** -1.0)
...
class SplineMap(ParametricSplineMap):
    ...
    def __init__(self, mesh, pts, ptsv=None, order=3, logSigma=True, normal="X"):
        ...
        ParametricSplineMap.__init__(self, mesh, pts, ptsv, order, logSigma, normal)
