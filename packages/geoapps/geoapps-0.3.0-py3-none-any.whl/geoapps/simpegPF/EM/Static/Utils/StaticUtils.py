...
def plot_pseudoSection(
    ...
    for ii in range(DCsurvey.nSrc):
        ...
        if surveyType == "pole-dipole":
            ...
            LEG.append(1.0 / (2 * np.pi) * (1 / MA - 1 / MB + 1 / NB - 1 / NA))
        else:
        ...
        if dataType == "volt":
            ...
            if dataType == "appConductivity":
                ...
                print(
                    """dataType must be 'appResistivity' | 'appConductivity' | 'volt' """
                )
                break
    ...
    if scale == "log":
        cbar = plt.colorbar(
            format="$10^{%.1f}$", fraction=0.04, orientation="horizontal"
        )
    elif scale == "linear":
...
def writeUBC_DCobs(fileName, DCsurvey, dim, formatType, iptype=0):
    ...
    assert (
        (formatType == "SURFACE") | (formatType == "GENERAL") | (formatType == "SIMPLE")
