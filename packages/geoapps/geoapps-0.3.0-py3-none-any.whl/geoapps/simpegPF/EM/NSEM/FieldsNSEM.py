...
class Fields1D_ePrimSec(BaseNSEMFields):
    ...
    def _eDeriv(self, src, du_dm_v, v, adjoint=False):
        ...
        if (
            ...
            raise NotImplementedError(
                "Getting eDerivs from %s is not implemented"
    ...
    def _bDeriv(self, src, du_dm_v, v, adjoint=False):
        ...
        if (
            ...
            raise NotImplementedError(
                "Getting bDerivs from %s is not implemented"
                % self.knownFields.keys()[0]
            )
...
class Fields3D_ePrimSec(BaseNSEMFields):
    ...
    def _e_px(self, e_pxSolution, srcList):
        ...
        """
        return self._e_pxPrimary(e_pxSolution, srcList) + self._e_pxSecondary(
            e_pxSolution, srcList
        )
    ...
    def _e_py(self, e_pySolution, srcList):
        ...
        :return: electric field as defined by the sources
        """
        return self._e_pyPrimary(e_pySolution, srcList) + self._e_pySecondary(
            e_pySolution, srcList
        )
    ...
    def _b_pxPrimary(self, e_pxSolution, srcList):
        ...
        b_pxPrimary = np.zeros(
            [self.survey.mesh.nF, e_pxSolution.shape[1]], dtype=complex
        )
        for i, src in enumerate(srcList):
            bp = src.bPrimary(self.survey.prob)
            if bp is not None:
    ...
    def _b_pyPrimary(self, e_pySolution, srcList):
        ...
        b_pyPrimary = np.zeros(
            [self.survey.mesh.nF, e_pySolution.shape[1]], dtype=complex
        )
        for i, src in enumerate(srcList):
            bp = src.bPrimary(self.survey.prob)
            if bp is not None:
    ...
    def _b_px(self, eSolution, srcList):
        ...
        """
        return self._b_pxPrimary(eSolution, srcList) + self._b_pxSecondary(
            eSolution, srcList
        )
    ...
    def _b_py(self, eSolution, srcList):
        ...
        :return: magnetic flux as defined by the sources
        """
        return self._b_pyPrimary(eSolution, srcList) + self._b_pySecondary(
            eSolution, srcList
        )
    ...
    def _b_pxDeriv_u(self, src, du_dm_v, adjoint=False):
        ...
        # Primary does not depend on u
        C = sp.hstack(
            (self.mesh.edgeCurl, Utils.spzeros(self.mesh.nF, self.mesh.nE))
    ...
    def _b_pyDeriv_u(self, src, du_dm_v, adjoint=False):
        ...
        # Primary does not depend on u
        C = sp.hstack(
            (Utils.spzeros(self.mesh.nF, self.mesh.nE), self.mesh.edgeCurl)
        )  # This works for adjoint = None
        if adjoint:
            return -1.0 / (1j * omega(src.freq)) * (C.T * du_dm_v)
        return -1.0 / (1j * omega(src.freq)) * (C * du_dm_v)
