...
class Problem1D_ePrimSec(BaseNSEMProblem):
    ...
    def fields(self, m=None):
        ...
        for freq in self.survey.freqs:
            if self.verbose:
                startTime = time.time()
                print(f"Starting work for {freq:.3e}")
                sys.stdout.flush()
            A = self.getA(freq)
...
class Problem3D_ePrimSec(BaseNSEMProblem):
    ...
    def getADeriv(self, freq, u, v, adjoint=False):
        ...
        if adjoint:
            dMe_dsigV = (
                sp.hstack((self.MeSigmaDeriv(u[sol0]).T, self.MeSigmaDeriv(u[sol1]).T))
                * v
            )
        else:
            ...
            dMe_dsigV = np.hstack(
                (
                    mkvc(self.MeSigmaDeriv(u[sol0]) * v, 2),
                    mkvc(self.MeSigmaDeriv(u[sol1]) * v, 2),
                )
            )
