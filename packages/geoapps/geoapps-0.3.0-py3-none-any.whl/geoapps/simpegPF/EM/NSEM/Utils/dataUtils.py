...
def convert3Dto1Dobject(NSEMdata, rxType3D="yx"):
    ...
    for loc in uniLocs:
        ...
        # Source list
        locrecData = recData[
            np.sqrt(np.sum((rec_to_ndarr(recData[["x", "y", "z"]]) - loc) ** 2, axis=1))
            < 1e-5
        ]
        dat1DList = []
        src1DList = []
...
def resampleNSEMdataAtFreq(NSEMdata, freqs):
    ...
    for loc in uniLoc:
        ...
        ind = (
            np.sqrt(
                np.sum(
                    (rec_to_ndarr(NSEMrec[["x", "y", "z"]]) - rec_to_ndarr(loc)) ** 2,
                    axis=1,
                )
            )
            < 1.0
        )  # Find dist of 1 m accuracy
        # Make a temporary recArray and interpolate all the components
        tArrRec = np.concatenate(
            (
                simpeg.mkvc(freqs, 2),
                np.ones((len(freqs), 1)) * rec_to_ndarr(loc),
                np.nan * np.ones((len(freqs), 12)),
            ),
            axis=1,
        ).view(dNames)
        for comp in [
            ...
            int1d = sciint.interp1d(
                NSEMrec[ind]["freq"], NSEMrec[ind][comp], bounds_error=False
            )
