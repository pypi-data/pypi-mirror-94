...
class EDIimporter:
    ...
    _impUnitSI2EDI = (
        1.0 / _impUnitEDI2SI
    )  # ConvertZ[V/A] SI unit to Z[mV/km/nT] (as in EDI)
    ...
    def importFiles(self):
        ...
        for nrEDI, EDIfile in enumerate(self.filesList):
            ...
            tArrRec = (np.nan * np.ones((len(freq), len(dtRI)))).view(
                dtRI
            )  # np.concatenate((freq*np.ones((locs.shape[0],1)),locs,np.nan*np.ones((locs.shape[0],8))),axis=1).view(dtRI)
            ...
            for comp in self.comps:
                ...
                key = [
                    ...
                    for s, t in [["xx", "yy"], ["xy", "yx"], ["yx", "xy"], ["yy", "xx"]]
                    if s in comp.lower()
                ][0]
    ...
    def _transfromPoints(self, longD, latD):
        ...
        try:
            ...
            print(
                "Could not import osr, missing the gdal package\nCan not project coordinates"
            )
            raise e
