...
def MagneticDipoleWholeSpace(XYZ, srcLoc, sig, f, moment=1.0, orientation="X", mu=mu_0):
    """
    ...
    if not isinstance(orientation, str):
        if np.allclose(orientation, np.r_[1.0, 0.0, 0.0]):
            orientation = "X"
        elif np.allclose(orientation, np.r_[0.0, 1.0, 0.0]):
            orientation = "Y"
        elif np.allclose(orientation, np.r_[0.0, 0.0, 1.0]):
            orientation = "Z"
        else:
    ...
    k = np.sqrt(-1j * 2.0 * np.pi * f * mu * sig)
    kr = k * r

    front = moment / (4.0 * pi * r ** 3.0) * np.exp(-1j * kr)
    mid = -(kr ** 2.0) + 3.0 * 1j * kr + 3.0

    if orientation.upper() == "X":
        Hx = front * ((dx / r) ** 2.0 * mid + (kr ** 2.0 - 1j * kr - 1.0))
        Hy = front * ((dx * dy / r ** 2.0) * mid)
        Hz = front * ((dx * dz / r ** 2.0) * mid)

    elif orientation.upper() == "Y":
        Hx = front * ((dy * dx / r ** 2.0) * mid)
        Hy = front * ((dy / r) ** 2.0 * mid + (kr ** 2.0 - 1j * kr - 1.0))
        Hz = front * ((dy * dz / r ** 2.0) * mid)

    elif orientation.upper() == "Z":
        Hx = front * ((dx * dz / r ** 2.0) * mid)
        Hy = front * ((dy * dz / r ** 2.0) * mid)
        Hz = front * ((dz / r) ** 2.0 * mid + (kr ** 2.0 - 1j * kr - 1.0))
...
def ElectricDipoleWholeSpace(
    XYZ, srcLoc, sig, f, current=1.0, length=1.0, orientation="X", mu=mu_0
):
    XYZ = Utils.asArray_N_x_Dim(XYZ, 3)
    ...
    front = current * length / (4.0 * np.pi * sig * r ** 3) * np.exp(-1j * k * r)
