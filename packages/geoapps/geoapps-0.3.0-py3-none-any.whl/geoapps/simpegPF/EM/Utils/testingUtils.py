...
def getFDEMProblem(fdemType, comp, SrcList, freq, useMu=False, verbose=False):
    ...
    for SrcType in SrcList:
        if SrcType == "MagDipole":
            Src.append(
                EM.FDEM.Src.MagDipole([rx0], freq=freq, loc=np.r_[0.0, 0.0, 0.0])
            )
        elif SrcType == "MagDipole_Bfield":
            Src.append(
                EM.FDEM.Src.MagDipole_Bfield([rx0], freq=freq, loc=np.r_[0.0, 0.0, 0.0])
            )
        elif SrcType == "CircularLoop":
            Src.append(
                EM.FDEM.Src.CircularLoop([rx0], freq=freq, loc=np.r_[0.0, 0.0, 0.0])
            )
        elif SrcType == "RawVec":
            if fdemType == "e" or fdemType == "b":
                ...
                Src.append(
                    EM.FDEM.Src.RawVec(
                        [rx0], freq, S_m, mesh.getEdgeInnerProduct() * S_e
                    )
                )
                ...
                Src.append(
                    EM.FDEM.Src.RawVec(
                        [rx0], freq, mesh.getEdgeInnerProduct() * S_m, S_e
                    )
                )

    if verbose:
        print(f"  Fetching {fdemType!s} problem")
...
def crossCheckTest(
    ...
    mesh = prb1.mesh
    print(
        "Cross Checking Forward: {!s}, {!s} formulations - {!s}".format(
