...
def MagneticDipoleVectorPotential(
    ...
    assert np.linalg.norm(orientation, 2) == 1.0, "orientation must " "be a unit vector"

    if type(component) in [list, tuple]:
        out = list(range(len(component)))
        for i, comp in enumerate(component):
            out[i] = MagneticDipoleVectorPotential(
                srcLoc, obsLoc, comp, orientation=orientation, mu=mu
            )
        return np.concatenate(out)

    if isinstance(obsLoc, Mesh.BaseMesh):
        mesh = obsLoc
        assert component in ["Ex", "Ey", "Ez", "Fx", "Fy", "Fz"], (
            "Components" "must be in: ['Ex','Ey','Ez','Fx','Fy','Fz']"
        )
        return MagneticDipoleVectorPotential(
            srcLoc,
            getattr(mesh, "grid" + component),
            component[1],
...
def MagneticDipoleFields(
    ...
    if isinstance(orientation, str):
        assert orientation.upper() in ["X", "Y", "Z"], (
            "orientation must be 'x', "
            "'y', or 'z' or a vector"
            "not {}".format(orientation)
        )
    elif (
        not np.allclose(np.r_[1.0, 0.0, 0.0], orientation)
        or not np.allclose(np.r_[0.0, 1.0, 0.0], orientation)
        or not np.allclose(np.r_[0.0, 0.0, 1.0], orientation)
    ):
        warnings.warn(
    ...
    if isinstance(component, str):
        ...
        not np.allclose(np.r_[1.0, 0.0, 0.0], component)
        or not np.allclose(np.r_[0.0, 1.0, 0.0], component)
        or not np.allclose(np.r_[0.0, 0.0, 1.0], component)
    ):
        warnings.warn(
    ...
    assert np.linalg.norm(orientation, 2) == 1.0, (
        "orientation must be a unit " 'vector. Use "moment=X to ' "scale source fields"
    )

    if np.linalg.norm(component, 2) != 1.0:
        warnings.warn(
...
def MagneticLoopVectorPotential(
    ...
    elif not np.allclose(orientation, np.r_[0.0, 0.0, 1.0]):
        raise NotImplementedError("Only Z oriented loops implemented")

    if type(component) in [list, tuple]:
        out = list(range(len(component)))
        for i, comp in enumerate(component):
            out[i] = MagneticLoopVectorPotential(
                srcLoc, obsLoc, comp, radius, orientation, mu
            )
        return np.concatenate(out)
    ...
    if isinstance(obsLoc, Mesh.BaseMesh):
        ...
        return MagneticLoopVectorPotential(
            srcLoc, getattr(mesh, "grid" + component), component[1], radius, mu
        )
    ...
    if component == "z":
        ...
        for i in range(nSrc):
            ...
            Aphi[ind] = (
                mu
                / (np.pi * np.sqrt(m[ind]))
                * np.sqrt(radius / r[ind])
                * ((1.0 - m[ind] / 2.0) * K[ind] - E[ind])
            )
            if component == "x":
