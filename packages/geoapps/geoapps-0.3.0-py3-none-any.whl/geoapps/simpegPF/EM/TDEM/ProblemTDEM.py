...
class BaseTDEMProblem(Problem.BaseTimeProblem, BaseEMProblem):
    ...
    def fields(self, m):
        ...
        for tInd, dt in enumerate(self.timeSteps):
            ...
            # same
            if Ainv is not None and (tInd > 0 and dt != self.timeSteps[tInd - 1]):
                Ainv.clean()
                Ainv = None
    ...
    def Jvec(self, m, v, f=None):
        ...
        dun_dm_v = np.hstack(
            [
                Utils.mkvc(self.getInitialFieldsDeriv(src, v, f=f), 2)
        ...
        for tInd, dt in zip(range(self.nT), self.timeSteps):
            # keep factors if dt is the same as previous step b/c A will be the
            # same
            if Adiaginv is not None and (tInd > 0 and dt != self.timeSteps[tInd - 1]):
                Adiaginv.clean()
                Adiaginv = None
            ...
            for i, src in enumerate(self.survey.srcList):
                ...
                for projField in {rx.projField for rx in src.rxList}:
                    ...
                    df_dm_v[src, f"{projField}Deriv", tInd] = df_dmFun(
    ...
    def Jtvec(self, m, v, f=None):
        ...
        ATinv_df_duT_v = np.zeros(
            (len(self.survey.srcList), len(f[self.survey.srcList[0], ftype, 0])),
            dtype=float,
        )
        ...
        for src in self.survey.srcList:
            ...
            # initialize size
            df_duT_v[src, f"{self._fieldType}Deriv", :] = np.zeros_like(
                f[src, self._fieldType, :]
            )

            for rx in src.rxList:
                PT_v[src, f"{rx.projField}Deriv", :] = rx.evalDeriv(
                    src,
                    self.mesh,
                    self.timeMesh,
                    f,
                ...
                df_duTFun = getattr(f, f"_{rx.projField}Deriv", None)
                ...
                for tInd in range(self.nT + 1):
                    cur = df_duTFun(
                        ...
                        src,
                        None,
                        Utils.mkvc(PT_v[src, f"{rx.projField}Deriv", tInd]),
                        adjoint=True,
                    )

                    df_duT_v[src, f"{self._fieldType}Deriv", tInd] = df_duT_v[
                        src, f"{self._fieldType}Deriv", tInd
                    ] + Utils.mkvc(cur[0], 2)
                    JTv = cur[1] + JTv
        ...
        for tInd in reversed(range(self.nT)):
            ...
            if AdiagTinv is not None and (
                tInd <= self.nT and self.timeSteps[tInd] != self.timeSteps[tInd + 1]
            ):
                AdiagTinv.clean()
                AdiagTinv = None
            ...
            for isrc, src in enumerate(self.survey.srcList):
                ...
                if tInd >= self.nT - 1:
                    ...
                    ATinv_df_duT_v[isrc, :] = (
                        AdiagTinv * df_duT_v[src, f"{self._fieldType}Deriv", tInd + 1]
                    )
                elif tInd > -1:
                    ATinv_df_duT_v[isrc, :] = AdiagTinv * (
                        Utils.mkvc(df_duT_v[src, f"{self._fieldType}Deriv", tInd + 1])
                ...
                JTv = JTv + Utils.mkvc(-dAT_dm_v - dAsubdiagT_dm_v + dRHST_dm_v)
    ...
    def getInitialFields(self):
        ...
        for i, src in enumerate(Srcs):
            ifields[:, i] = ifields[:, i] + getattr(
                src, f"{self._fieldType}Initial", None
            )(self)
    ...
    def getInitialFieldsDeriv(self, src, v, adjoint=False, f=None):
        ...
        ifieldsDeriv = (
            Utils.mkvc(
                getattr(src, f"{self._fieldType}InitialDeriv", None)(
                    self, v, adjoint, f
                )
            )
            + ifieldsDeriv
...
class Problem3D_e(BaseTDEMProblem):
            ...
            if src.srcType == "Galvanic":
                ...
                ATinv_df_duT_v[isrc, :] = Grad * (
                    ...
                    * (
                        ...
                        * (
                            Utils.mkvc(
                                df_duT_v[src, f"{self._fieldType}Deriv", tInd + 1]
                            )
                            - Asubdiag.T * Utils.mkvc(ATinv_df_duT_v[isrc, :])
                        )
                    )
                ...
                un_src = f[src, ftype, tInd + 1]
                # cell centered on time mesh
                dAT_dm_v = self.MeSigmaDeriv(un_src).T * ATinv_df_duT_v[isrc, :]
    ...
    def getAdiagDeriv(self, tInd, u, v, adjoint=False):
        ...
        if adjoint:
            return 1.0 / dt * MeSigmaDeriv.T * v

        return 1.0 / dt * MeSigmaDeriv * v
    ...
    def getAsubdiagDeriv(self, tInd, u, v, adjoint=False):
        ...
        if adjoint:
            return -1.0 / dt * self.MeSigmaDeriv(u).T * v

        return -1.0 / dt * self.MeSigmaDeriv(u) * v
