...
class GravityIntegral(Problem.LinearProblem):
    ...
    def fields(self, m):
        ...
        if self.forwardOnly:
            ...
            return da.dot(
                self.G, (self.rhoMap * m).astype(np.float32)
            )  # np.array(fields, dtype='float')
    ...
    def getJtJdiag(self, m, W=None):
        ...
        return mkvc(
            np.sum((sdiag(mkvc(self.gtgdiag) ** 0.5) * dmudm).power(2.0), axis=0)
        )
        ...
        for aa in range(2):
            for bb in range(2):
                for cc in range(2):
                    ...
                    if (
                        ("gxx" in self.components)
                        or ("gzz" in self.components)
                        or ("guv" in self.components)
                    ):
                        gxx -= (
                            1e4
                            * (-1) ** aa
