...
class EM1D(Problem.BaseProblem):
    ...
    chi = Props.PhysicalProperty("Magnetic susceptibility", default=0.0)

    eta, etaMap, etaDeriv = Props.Invertible(
        "Electrical chargeability (V/V), 0 <= eta < 1", default=0.0
    )

    tau, tauMap, tauDeriv = Props.Invertible("Time constant (s)", default=1.0)

    c, cMap, cDeriv = Props.Invertible("Frequency Dependency, 0 < c < 1", default=0.5)
    ...
    def hz_kernel_vertical_magnetic_dipole(
        self, lamda, f, n_layer, sig, chi, depth, h, z, flag, I, output_type="response"
    ):
    ...
    def hz_kernel_horizontal_electric_dipole(
        self, lamda, f, n_layer, sig, chi, depth, h, z, flag, output_type="response"
    ):
    ...
    def sigma_cole(self):
        ...
        sigma_complex_tensor[:, :, :] = np.tile(
            sigma_complex.reshape((n_layer, n_frequency, 1)), (1, 1, n_filter)
        )
    ...
    def forward(self, m, output_type="response"):
        ...
        lambd[:, :], _ = get_dlf_points(self.fhtfilt, r, self.hankel_pts_per_dec)
    ...
    def getJ_height(self, m, f=None):
        ...
        if self._Jmatrix_height is not None:
            ...
            self._Jmatrix_height = (self.survey.projectFields(dudz)).reshape([-1, 1])
