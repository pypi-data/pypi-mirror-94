...
:LastEditTime: 2021-01-27 16:40:23
...
class UserBaseHandler(SevenBaseHandler):
    """
    :description: 用户信息导入到用户基础表
    """
    @filter_check_params("key")
    def get_async(self):
        """
        :description: 用户信息导入到用户基础表
        :param self：self
        ...
        user_info_model = UserInfoModel()
        record_count = user_info_model.get_total("is_auth=1")

        page_count = SevenHelper.get_page_count(page_size, record_count)
        for page_index in range(0, page_count):
            page_list, row_count = user_info_model.get_page_list("*", page_index, page_size, "is_auth=1")
            for page in page_list:
                print(page.id)
                user_base = user_base_model.get_entity("open_id=%s", params=page.open_id)
                if not user_base:
                    user_base = UserBase()
                    user_base.is_auth = 1
                    user_base.open_id = page.open_id
                    user_base.user_nick = page.user_nick
                    user_base.avatar = page.avatar
                    user_base.user_state = page.user_state
                    user_base.relieve_date = page.relieve_date
                    user_base.create_date = page.create_date
                    user_base.modify_date = page.modify_date
