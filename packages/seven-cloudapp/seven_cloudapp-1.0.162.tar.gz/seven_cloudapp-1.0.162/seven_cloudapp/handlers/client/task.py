...
:LastEditTime: 2021-02-03 12:56:32
    ...
    def get_async(self):
        ...
        for task_info in task_info_list:
            ...
            if task_info.task_type == 1:  #新人有礼
                ...
                # goods_list = task_config["goods_list"] if task_config.__contains__("goods_list") else []
                # user_goods_list = [str(i.goods_id) for i in user_goods_list] if len(user_goods_list) > 0 else []
                # for good in goods_list:
                #     if str(good["num_iid"]) in user_goods_list:
                #         good["status"] = 1
                #     else:
                #         good["status"] = 0
                # result["goods_list"] = goods_list
                # result["user_goods_list"] = user_goods_list
                result["goods_list"] = task_config["goods_list"] if task_config.__contains__("goods_list") else []
                ...
                # goods_list = task_config["goods_list"] if len(task_config["goods_list"]) > 0 else []
                # user_goods_list = [str(i.goods_id) for i in user_goods_list_model] if len(user_goods_list_model) > 0 else []
                # for good in goods_list:
                #     if str(good["num_iid"]) in user_goods_list:
                #         good["status"] = 1
                #     else:
                ...
                act_prize_list = act_prize_model.get_dict_list("act_id=%s and is_release=1 and ascription_type=1", "", "sort_index asc", "", "id,prize_name,surplus,lottery_sum", [act_id])
                if act_prize_list:
...
class WeeklySignHandler(TaskBaseHandler):
    ...
    def get_async(self):
        ...
        if send_num > 0:
            ...
            log_desc = log_title
            update_sql = f"surplus_integral=surplus_integral+{send_num},signin=\"{updata_signin}\""
            # print(update_sql)
