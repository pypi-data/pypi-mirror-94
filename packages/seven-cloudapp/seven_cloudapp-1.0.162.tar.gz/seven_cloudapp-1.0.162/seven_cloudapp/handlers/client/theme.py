...
:LastEditTime: 2020-12-30 18:13:47
...
class ThemeInfoHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if act_info:
            theme_info = theme_info_model.get_dict("id=%s", params=act_info.theme_id)
            if not theme_info:
            ...
            skin_info_list = skin_info_model.get_dict_list("theme_id=%s", params=act_info.theme_id)

            theme_info["skin_list"] = skin_info_list

            self.reponse_json_success(theme_info)
        else:
            self.reponse_json_error("NoAct", "对不起，找不到该活动")
