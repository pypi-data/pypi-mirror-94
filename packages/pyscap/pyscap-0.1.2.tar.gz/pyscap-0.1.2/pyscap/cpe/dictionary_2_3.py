...
@dataclass
class CheckType:
    """The CheckType complex type is used to define an element to hold
...
class GeneratorType:
    ...
    information about when a particular document was compiled, what version of
    the schema was used, what tool compiled the document, and what version of
    that tool was used.
    ...
    :ivar product_name: The optional product_name element specifies the
        name of the                         application used to generate
        the file.
    ...
    :ivar schema_version: The required schema_version element specifies
        the version of the                         schema that the
        document has been written against and that should be used for
        validation.
    :ivar timestamp: The required timestamp element specifies when the
        particular                         document was compiled. The
        format for the timestamp is yyyy-mm-ddThh:mm:ss. Note that the
        timestamp element does not specify when an item in the document
        was created or modified but                         rather when
        the actual XML document that contains the items was created. For
        example, a document                         might pull a bunch
        of existing items together, each of which was created at some
        point in the                         past. The timestamp in this
        case would be when this combined document was
        created.
    :ivar other_element:
    """
...
@dataclass
class NotesType:
    """The NotesType complex type defines an element that consists of one or
    more child note elements.
...
class ReferencesType:
    ...
    collection of individual references.
...
class TextType:
    """
    The TextType complex type allows the xml:lang attribute to associate a
...
@dataclass
class ItemType:
    """The ItemType complex type defines an element that represents a single
    CPE Name.
