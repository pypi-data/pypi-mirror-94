...
or between two different q values. The 3D dataset is expected to be interpolated on an orthonormal grid.  The voxels 
belonging to a slice at the defined q value are used for the calculation without further interpolation.
...
q_range = np.arange(start=0.104, stop=0.205, step=0.004)  # q values in 1/nm where to calculate the cross-correlation
    ...
    for ind_q in range(len(q_range)):
        ...
        if calc_self:
            ...
            for ind_point in range(nb_points[ind_q]):
                pool.apply_async(xcca.calc_ccf_rect, args=(ind_point, key_q1, key_q2, angular_bins, q_int),
