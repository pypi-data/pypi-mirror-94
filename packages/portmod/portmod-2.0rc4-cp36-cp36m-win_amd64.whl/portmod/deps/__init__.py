...
def resolve(
    ...
    *,
    ...
    newselected: Dict[str, Target] = dict()
    ...
    for atom, source in newselectedset.items():
        ...
        if name in newselected:
            newselected[modlist[0].CPN].pkgs.extend(modlist)
            if newselected[modlist[0].CPN].source is CMD_ATOM or source is CMD_ATOM:
                ...
                newselected[modlist[0].CPN].atom = name
            # Prefer command line as source rather than world file
            if newselected[modlist[0].CPN].source is WORLD_ATOM and source is CMD_ATOM:
                newselected[modlist[0].CPN].source = CMD_ATOM
        else:
            newselected[modlist[0].CPN] = Target(modlist, atom, source)
    ...
    formula.merge(
        weigh_clauses(
            formula.atoms,
            formula.flags,
            selected_cpn | selected_from_sets,
            deep=deep,
            depclean=depclean,
            update=update,
            newuse=newuse,
    ...
    if depclean:
        for pkg in load_all_installed():
            ...
            for flag in pkg.IUSE_EFFECTIVE:
                if flag in pkg.get_enabled_use():
    ...
    if solver.compute():
        ...
        for flag in flags:
            ...
            if re.sub(r"\[.*\]", "", atom) in enabled_final:
                usedeps.append(UseDep(Atom(atom.CP), prefix + list(atom.USE)[0], None))
