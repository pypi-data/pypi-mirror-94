...
from portmod.atom import Atom, FQAtom, InvalidAtom, QualifiedAtom
...
class Pybuild1(FullPybuild):
    ...
    * ``module``: Indicates that the installed source tree contains a
        ...
        for install_dir in self.INSTALL_DIRS:
            if check_required_use(
                ...
                if os.path.islink(source):
                    ...
                    patch_dir(source, dest, case_sensitive=not case_insensitive)
            else:
                print(
                    l10n(
                        "skipping-directory",
                        dir=magenta(
                            os.path.join(install_dir.S or "", install_dir.PATH)
                        ),
    ...
    def warn(self, string: str):
        ...
        """
        os.makedirs(os.path.join(env.WARNINGS_DIR, self.CATEGORY), exist_ok=True)
        with open(
            os.path.join(env.WARNINGS_DIR, self.CATEGORY, self.ATOM.PF), "a+"
        ) as file:
            print(string, file)
        warning(string)
    ...
    def info(self, string: str):
        ...
        args:
            string: String to display
        """
        os.makedirs(os.path.join(env.MESSAGES_DIR, self.CATEGORY), exist_ok=True)
        with open(
            os.path.join(env.MESSAGES_DIR, self.CATEGORY, self.ATOM.PF), "a+"
        ) as file:
            print(string, file)
        info(string)
