...
def weigh_clauses(
    ...
    newselectedatoms: AbstractSet[Atom],
    *,
    deep: bool,
    depclean: bool,
    update: bool,
    ...
    for group in formula.atoms:
        for atom in formula.atoms[group]:
            if (
                ...
                and Atom(atom.CPN) not in newselectedatoms
            ):
    ...
    for group in atoms:
        ...
        selected = atom.CPN in newselectedatoms if not noreplace else False
    ...
    for mod in load_all_installed():
        if not is_selected(mod.ATOM) and Atom(mod.CPN) not in newselectedatoms:
        ...
        elif Atom(mod.CPN) not in newselectedatoms and not update:
    ...
    for flag, disabledflag, atom, enabledatom, disabledatom, mod in iterate_flags():
        if (
            (deep and newuse)
            or atom.CPN in newselectedatoms
