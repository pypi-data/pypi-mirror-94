...
class GrpcHandler(AbsMilvus):
    ...
    def create_index(self, collection_name, field_name, params, timeout=None, **kwargs):
        ...
        if sync:
            if not self.wait_index_building_success(collection_name=collection_name, field_name=field_name):
                raise BaseException(status.error_code, "create index failed")
    ...
    def wait_index_building_success(self, collection_name, field_name, timeout=30, **kwargs):
        ...
        while True:
            ...
            try:
                ...
                if state == common_types.IndexState.FINISHED:
                    return True
                if state == common_types.IndexState.FAILED:
                    return False
            except BaseException:
                return False
