...
class CreateH5:
    ...
    def get_file_id(file_path):
        # This assumes the file name contains the patient ID
        items = os.path.split(file_path)
        file_id = os.path.splitext(items[1])[0]
        return file_id

    def create_file(self, files, output_file, is_train):
        mapping_logger = Logger('mapping_file', to_dir=os.path.split(output_file)[0])
        with h5py.File(output_file, 'w') as h5f:
            ...
            for file_pair in files:
                ...
                if is_train:
                    ...
                    group = h5f.create_group('{}'.format(file_id))
                    group.create_dataset('image', data=dcm_pixels)
                mapping_logger.print('{}: {}\n'.format(file_id, file_pair[0]))
