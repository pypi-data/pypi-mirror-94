...
SINGLETONS = {c for n, c in ast.__dict__.items() if isinstance(c, type) and #iteritems changed into items
...
stmt_class_names = {n for n, c in ast.__dict__.items() #changed iteritems into items
                    if isinstance(c, type) and issubclass(c, ast.stmt)}
expr_class_names = ({n for n, c in ast.__dict__.items() #changed iteritems into items
...
class MarkTokens(object):
  ...
  def _find_last_in_stmt(self, start_token):
    t = start_token
    while (not match_token(t, token.NEWLINE) and
           not match_token(t, token.OP, ';') and
...
class IC:
    ...
    def __init__(self, prefix=None, output_function=None, arg_to_string_function=None, include_context=None, include_time=None, include_delta=None):
        self.configure(
            prefix=prefix,
            output_function=output_function,
            arg_to_string_function=arg_to_string_function,
    ...
    def configure(self, prefix=None, output_function=None, arg_to_string_function=None, include_context=None, include_time=None, include_delta=None):
...
def install(ic='ic'):
    setattr(builtins, ic, icecream_only.ic)
