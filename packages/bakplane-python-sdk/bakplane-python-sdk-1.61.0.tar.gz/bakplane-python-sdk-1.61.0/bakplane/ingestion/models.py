...
class Asset:
    ...
    def to_csv_entry(self, ctx: IngestionSessionContext, csv_separator: str, null_value: str):
        parts = []
        ...
        return csv_separator.join(
            [x.strip().replace(csv_separator, "").replace("\n", "").replace("\r", "") if x is not None else null_value
             for x in parts])
...
class Descriptor:
    attribute: str
    value: str
    effective_dating: typing.List[typing.Tuple[datetime.datetime, datetime.datetime]]
    knowledge_dating: typing.List[typing.Tuple[datetime.datetime, datetime.datetime]] = None
...
class Relationship:
    ...
    @property
    def identity(self):
        return hash(self.source_identity_uid) + hash(self.target_identity_uid) + hash(self.relationship_type)
