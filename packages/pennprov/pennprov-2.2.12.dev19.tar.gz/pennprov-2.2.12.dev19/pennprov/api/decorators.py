    ...
    def __call__(self, func):
        ...
        def wrapper(key, data):
            ...
            if mprov_conn:
                ...
                if not out_keys:
                    print('EMPTY OUT KEYS FUNC and WINDOW IDS and RESULT:', func.__name__, window_ids, result)
                if self.collection:
                    mprov_conn.add_to_collection(result, self.collection)
                mprov_conn.flush()
            else:
                logging.warning("Output: %s.%s <(%s)- %s", self.out_stream_name, 'w'+str(out_keys), code_token, str(window_ids))
