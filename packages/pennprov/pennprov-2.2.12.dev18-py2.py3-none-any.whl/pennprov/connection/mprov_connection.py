...
class MProvConnection:
    ...
    def _get_qname(cls, local_part):
        ...
        :return:
        """
        return pennprov.QualifiedName(cls.namespace, local_part)

    @classmethod
    def get_token_qname(cls, token):
        # types: (str) -> pennprov.QualifiedName
        """
        ...
        """
        if token.endswith('[]'):
            print('TOKEN WITH EMPTY KEYS', token)

        if len(token) > 40:
            thash = hashlib.sha1(token.encode('utf-8'))
            return cls._get_qname(thash.hexdigest())
        else:
            return cls._get_qname(token)
    ...
    def store_subgraph(self, nodes=None, links=None):
        # type: (List[pennprov.NodeModel], List[pennprov.RelationModel]) -> None
        """
        Store the given nodes and links
        ...
        self.prov_dm_api.store_prov_dm_subgraph(
            self.get_graph(), pennprov.ProvDmSubgraph(nodes=nodes, links=links))
