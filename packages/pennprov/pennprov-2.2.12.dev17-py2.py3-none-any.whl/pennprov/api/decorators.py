...
class MProvAgg:
    def __init__(self, in_stream_name,out_stream_name,in_stream_key=['index'],out_stream_key=['index'],collection=None,connection_key=None):
        ...
        self.in_stream_part = MProvConnection.get_token_qname('no_conn_in')
        self.out_stream_part = MProvConnection.get_token_qname('no_conn_out')
        ...
        if mprov_conn:
            self.in_stream_part = mprov_conn.create_collection(self.in_stream_name)
            self.out_stream_part = mprov_conn.create_collection(self.out_stream_name)
            mprov_conn.flush()
    ...
    def __call__(self, func):
        ...
        global stored
        mprov_conn = MProvConnectionCache.get_connection(self.connection_key)
        if not stored.get(func.__name__) and mprov_conn:
            ...
            self._write_collection_relationships(mprov_conn, code_token, self.out_stream_part, self.in_stream_part)
            stored[func.__name__] = code_token
            mprov_conn.flush()
        ...
        def wrapper(key, data):
            ...
            global stored
            sig = stored.get(func.__name__, 'code not stored')
            ...
            if mprov_conn:
                try:
                    for t in self.rel_keys(data, self.in_stream_key):
                        ...
                        mprov_conn.add_to_collection(tup, self.in_stream_part)
                except:
                    pass
                #print (window_ids)
                ...
                mprov_conn.add_to_collection(result, self.out_stream_part)
