...
FIELD_NOT_FOUND = 'Field "%s" on class "%s" does not exist.'
...
CATALOG_NOT_FOUND = 'Catalog not found for %s instance.'
NO_ID = 'getObject call with no ID.'
...
class Base:
    ...
    def getField(self, name, className=None, raiseError=False):
        ...
        if r is None and raiseError:
            raise AttributeError(FIELD_NOT_FOUND % (name, class_.name))
        return r
    ...
    def getCatalog(self, raiseError=False):
        ...
        if not r and raiseError:
            raise Exception(CATALOG_NOT_FOUND % self.class_.name)
        return r
