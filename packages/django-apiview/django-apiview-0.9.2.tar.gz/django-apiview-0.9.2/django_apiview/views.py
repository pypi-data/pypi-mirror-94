...
class Apiview(object):
    def __init__(self, packer, preload_plugins=None, extra_parameters=None, enable_api_response_time_log=False):
        self.packer = packer
        self.extra_parameters = extra_parameters or {}
        self.preload_plugins = preload_plugins or []
        self.enable_api_response_time_log = enable_api_response_time_log
    ...
    def log_api_response_time(self, path, response_time):
        if self.enable_api_response_time_log:
            try:
                log = ApiResponseTimeLog()
                log.path = path
                log.response_time = int(response_time)
                log.save()
            except Exception:
                logger.exception("Save api response time log failed.")
    ...
    def __call__(self, func):
        ...
        def wrapper(request, **kwargs):
            ...
            etime = time.time()
            self.log_api_response_time(_django_apiview_request.request.path, 1000000*(etime-stime))
...
class cache(ApiviewDecorator):
    ...
    def __call__(self, func):
        entry_point = ".".join([func.__module__, func.__name__])
...
class check_aclkey(ApiviewDecorator):
    ...
    def __init__(self, aclkey=None, aclkey_field_name="aclkey"):
        ...
        self.aclkey = aclkey or settings.DJANGO_APIVIEW_ACLKEY
    
    def main(self, _django_apiview_request, func):
        aclkey = _django_apiview_request.data[self.aclkey_field_name]
        if not self._check_aclkey(aclkey):
            raise bizerror.AppAuthFailed()
        return super().main(_django_apiview_request, func)

    def _check_aclkey(self, aclkey):
