...
from ._run_sync import _make_new_loop, _run_sync
...
class CogniteGeospatialClient:
    ...
    def __init__(
        ...
        api_client = RetryApiClient(self.configuration, token_generator=token_generator, retry_for_statuses=[401])
    ...
    async def _load_layer(self, layer_name: str, layer_cache: dict):
        ...
        if layer is None:
            layer = await self.list_layers_async(layer_name)
    ...
    async def get_spatial_async(
        ...
        layer = await self.list_layers_async(spatial_item.layer)
    ...
    async def get_attributes_async(
        self, item_id: SpatialItemId, attributes: List[str], extractors: List[DataExtractor] = None
    ):
        ...
        data_request = SpatialDataRequestDTO(
            spatial_id=InternalIdDTO(id=spatial_item.id), attributes=attributes, extractors=extractors_dto
        )
    ...
    @api_exception_handler
    async def list_layers_async(self, names: Union[str, List[str]]) -> Union[Layer, List[Layer]]:
    ...
    def get_layer(self, name: str) -> Layer:
        ...
        run_func = partial(self.list_layers_async, name)
        return self._run_sync(run_func, self.timeout)
