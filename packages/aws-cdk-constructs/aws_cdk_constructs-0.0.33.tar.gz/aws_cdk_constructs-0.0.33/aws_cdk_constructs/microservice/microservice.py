...
class Microservice(core.Construct):
    ...
    def create_load_balancer(self, scope, id, app_name, environment, environments_parameters, security_group=None, internet_facing=False, load_balancer_name=None, use_cdn=False):
        ...
        if internet_facing is False:
            ...
            app_proxy_security_group = _ec2.SecurityGroup.from_security_group_id(
                self, "upstream_security_group", aws_account["app_proxy_security_group"], mutable=False
            )
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.alb_security_group = self.create_security_group(
            ...
            app_name, 
            environment, 
            app_name + '_' + main_component_name + "_alb_sg"
        )
        ...
        if create_efs:
            ...
            self._efs = _efs.FileSystem(self, main_component_name+"efs", vpc=self._vpc, encrypted=False)
        ...
        if is_not_producution:
            ASG = self._asg.node.find_child("ASG")
        ...
        if create_efs:
            ...
            self.efs.connections.allow_default_port_from(self._asg)
        ...
        if existing_efs_security_group or existing_efs_security_group_id:
            ...
            # In case `existing_efs_security_group` is provided use it, otherwise retrive the security group from AWS.
            # Important: `existing_efs_security_group_id` works only if the Security Group is already deployed. Not compabible with security groups created during the 
            # first deployment            
        ...
        if use_ldap:
            ...
            self._asg.add_security_group(ldap_access_security_group)      
