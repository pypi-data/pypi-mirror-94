...
def parse_csv():
    ...
    for line in lines[1:]:
        ...
        species_plural.append({firstline[i]: line[i] for i in range(len(firstline))})
...
def process_species(species, retried=0):
    try:
        url = "http://www.iucngisd.org/gisd/speciesname/{}".format(species['Species'].replace(' ', '+'))
        species_page = requests.get(url)
        if species_page.status_code != 200:
            logging.warning("Bad HTTP response for {}: {}".format(species['Species'], species_page.status_code))
        else:
            ...
            for ar in soup.find("div", id="ar-title").find_next_sibling("ul").find_all("li"):
                sc, nlc = ar["id"].split("*")
                loc_per_country = requests.get("http://www.iucngisd.org/gisd/function/species_location.php?sc={}&nlc={}".format(sc, nlc))
                for location in loc_per_country.json():
                    ...
                    if len(species_loc_main) > 1 or len(species_loc_imp) > 1:
                        logging.warning("specie_loc returns for {} in {} are longer than 1".format(species['Species'], location['location_name']))
...
def main():
    ...
    json_output = {}
    json_output['processing_timestamp'] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    gisd = []
