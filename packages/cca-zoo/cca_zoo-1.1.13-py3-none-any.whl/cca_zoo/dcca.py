...
class DCCA(DCCA_base):
    ...
    def update_weights(self, *args):
        if self.als:
            ...
            loss = self.update_weights_st(*args)
        else:
    ...
    def update_weights_st(self, *args):
        losses, obj = self.st_loss(*args)
        self.optimizers[0].zero_grad()
        losses[0].backward()
        self.optimizers[0].step()
        self.optimizers[1].zero_grad()
    ...
    def st_loss(self, *args):
        z = self(*args)
        z_sum = stack(z, dim=0).mean(dim=0)
        self.update_covariances(z_sum)
        covariance_inv = compute_matrix_power(self.covs[0], -0.5, self.eps)
        shared_target = matmul(z_sum, covariance_inv).detach()
        losses = [mean(norm(z - shared_target, dim=0)) for z in z]
        obj = self.objective.loss(*z)
        return losses, obj
