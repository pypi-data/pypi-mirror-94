...
class IWidget(IWidgetInterface, ABC):
    ...
    def __init__(self, position, size,
        ...
        self._pressed = False
        self._focus = False
        # We can't currently serialize neighbors, as it causes infinite loops
        # Need to develop a non-object mechanism to name neighbors
    ...
    def render_centered_text(self, surface, text, color, bg_color=None):
        ...
        text = self._font.render(text, 1, color, bg_color)
    ...
    def render_focus(self, surface):
        ...
        surface : pygame.Surface
        """
        fx, fy = self._x - 2, self._y - 2
        fw, fh = self._w + 4, self._h + 4
...
class IControlWidget(IWidget, ABC):
    ...
    def _on_move(self, touch):
        ...
        if self.dynamic is True and self.function is not None:
            return self.function(self, self._adjusted_value)
