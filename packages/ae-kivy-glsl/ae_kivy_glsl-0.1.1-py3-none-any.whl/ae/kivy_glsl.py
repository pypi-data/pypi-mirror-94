...
class ShadersMixin:
    ...
    def refresh_renderer(self, renderer: RendererType):
        ...
        ren_ctx['alpha'] = float(renderer.get('alpha', 0.69))
        ren_ctx['contrast'] = float(renderer.get('contrast', 0.69))
        ren_ctx['resolution'] = list(map(float, self.size))
        ren_ctx['tex_col_mix'] = float(renderer.get('tex_col_mix', 0.69))
        if not callable(renderer.get('time')):
