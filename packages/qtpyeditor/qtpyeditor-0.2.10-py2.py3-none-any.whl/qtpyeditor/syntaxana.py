...
def countPar(row: str) -> Tuple[int, int, int]:  # 检测三类括号的数量。
    ...
    return lparNum - rparNum, lbraceNum - rbraceNum, lbracketNum - rbracketNum  # 返回左括号数量减去右括号数量。
...
def getBracketedContent(row: str) -> Tuple[str, str, str]:  # 获取任何类型括号最外层内部的东西。（不是小括号！！！）
    ...
    if (minVal == length):  # 说明根本没括号
        ...
        return symList[pos], getInfoFromRegex(regex=regex, st=row), row[:minVal]
...
def getFuncName(row: str) -> str:  # 获取函数的名称。
    return getInfoFromRegex(regex=r'def\s(.*?)[(]', st=row)  # 注意，需要匹配函数名，其中还有个空格。
...
class Row():
    ...
    def __repr__(self) -> str:
        return 'row:' + repr(self.pos) + "\t indent:" + repr(self.indent) + "\t text:" + self.text + '\n'
...
def regularize(rawText: List[str]) -> List[Row]:
    ...
    for i, l in enumerate(f):
        l = removeStringContent(l)
        l = removeComment(l)

        if (skipLine == False):
            row, currentIndent = getIndent(l)  # 获取当前的行名和缩进，同时修剪掉行首的空格
            currentRow = Row(i, row, currentIndent)
            rowList.append(currentRow)

        else:
            currentRow.text += l.strip()  # 如果判断出这一行还没有结束，就不用获取当前的缩进，直接缀连即可。
