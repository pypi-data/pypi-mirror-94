...
class MoveMongodbColl(object):
    ...
    def init_data_db3(self):
        ...
        while True:
            lists = []
            for i in self.basemongo.find({"_id": {"$gte": self.first_id}}, {"_id": 1}).sort([("_id", 1)]).limit(1000000):
                c = c + 1
                self.first_id = i["_id"]
        ...
        for info in lists:
            ...
            try:
                try:
                    ...
                    await self.mvmc.bs2.insert_one(item)
                    print("{}插入".format(_id))
