...
class ZMQProducer(object):
    ...
    def produce(self, header, message, messageFlowPattern=[],id=0):
        while (self.messageQueue.sizeSum > self.maxMemorySize):
            self.messageQueue.loseMessage()
        self.messageQueue.append(messageFlowPattern, header, message ,id)
        ...
        while self.active:
            ...
            for consumerType, workerQueue in workers.queues.items():
                if (workerQueue):
                    ...
                    if (poped):
                        messageFlowPattern, header, payload,id = poped
                        ...
                        print ('sending ' + str(id) +' to ' + str(identity)[-4:])
                        try:
