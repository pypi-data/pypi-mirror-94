...
class Package:
    def __init__(self, element, max_count=5, max_bbox_size_in_degrees=0.1):
        ...
        if self.bbox is None:
            print("warning: element " + element.get_link() +
                  " has no known bounding box!")
        self.list = [element]
    ...
    def try_adding(self, new_element):
        if self.bbox is None:
            print(
                "warning: package has no bounding box specified! It will contain only one element!")
            return False
        if len(self.list) >= self.max_count:
            return False
        bbox_of_new_element = new_element.get_bbox()
        if bbox_of_new_element is None:
        ...
        self.bbox = {'min_lat': min_lat, 'min_lon': min_lon,
    ...
    def split_into_packages(list_of_elements, max_count=5,
                            max_bbox_size_in_degrees=0.3):
        packages = []
        returned = []
        for element in list_of_elements:
            if not Package.try_adding_to_existing_package(packages, element):
                packages.append(Package(element, max_count,
                                        max_bbox_size_in_degrees))
            full_packages = [e for e in packages if len(e.list) >= max_count]
            packages_with_remaining_space = [
                e for e in packages if len(e.list) < max_count]
