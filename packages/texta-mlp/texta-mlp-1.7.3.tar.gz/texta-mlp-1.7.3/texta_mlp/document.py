...
from .parsers import AddressParser, ContactEmailNamePairParser, ContactEmailParser, ContactPhoneParserHighPrecision, ContactPhoneParserHighRecall, ContactPhoneParserStrict
...
class Document:
    ...
    def remove_duplicate_facts_by_span(self, facts):
        ...
        for fact in facts:
            if fact.fact_type != Document.FACT_NAME_NAMEMAIL and fact.spans[0][0] not in starts and fact.spans[0][1] not in ends:
                fact.fact_value = fact.fact_value.strip("><\)\(:;-\.,\!\?")
                new_facts += [fact]
    ...
    def bound_close_ones(self, facts):
        ...
        new_bound = {'doc_path': self.doc_path, 'fact': Document.FACT_NAME_BOUNDED, 'spans': [], 'str_val': {}, "str_values": []}
        new_bounded = []
        for fact in facts:
   ...
   'str_val': "{'EMAIL': ['89179012978@mail.ru]'], 'PHONE': ['89179012978']}"},
                ...
                for j, span2 in enumerate(new_bound["spans"]):
                    if span1[0] == span2[0] and i != j:
                        if span1[1] > span2[1]:
                            ...
                            to_delete_is += [i]
            new_bound_erased_coinciding = {'doc_path': self.doc_path, 'fact': Document.FACT_NAME_BOUNDED, 'spans': [], 'str_val': {}}
            for i, span1 in enumerate(new_bound["spans"]):
    ...
    def bounded(self):
        ...
        facts = self.remove_old_bounded_and_add_phone_high_recall(facts)  # bounded uses only ContactPhoneParserHighRecall results and doesn't look at old BOUNDED facts.
