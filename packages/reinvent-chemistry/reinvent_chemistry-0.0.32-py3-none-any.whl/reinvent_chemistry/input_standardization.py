...
# TODO: cosnider removing. This is obsolete. Check for usage.
class Standardizer:
    ...
    def _valid_size(self, mol, min_heavy_atoms, max_heavy_atoms, element_list, remove_long_side_chains):
    ...
    def standardize_smiles(self, smiles, min_heavy_atoms=2, max_heavy_atoms=70, element_list=None,
        ...
        if mol and neutralise_charges:
            mol, _ = self._neutralise_charges(mol)
        if mol:
            rdmolops.Cleanup(mol)
            rdmolops.SanitizeMol(mol)
            mol = rdmolops.RemoveHs(mol, implicitOnly=False, updateExplicitCount=False, sanitize=True)
        if mol and self._valid_size(mol, min_heavy_atoms, max_heavy_atoms, element_list, remove_long_side_chains):
            return rkc.MolToSmiles(mol, isomericSmiles=False)
        return None

    def randomize_smiles(self, smiles, random_type="restricted"):
        """
    ...
    def read_smiles_file(self, file_path, ignore_invalid=True, num=-1, standardize=True, randomize=False):
        """
        Reads a SMILES file.
        ...
        actions = []
        if standardize:
            actions.append(self.standardize_smiles)
        if randomize:
            actions.append(self.randomize_smiles)

        with open(file_path, "r") as csv_file:
            for i, row in enumerate(csv_file):
                if i == num:
                    break
                line = row.rstrip().replace(",", " ").replace("\t", " ").split()
                smiles = line[0]
