...
void tophat_float(const proj_struct_float *projdata, float *u, float s, float sqr_s, const pair_struct_float *pair){
    ...
    //printf("top %f %f %f\n", sqr_s, projdata->supp_sqr[0], projdata->supp_sqr[1]);
    for(int p=projdata->nsbins-1; p>=0; p--){
...
void piecewise_float(const proj_struct_float *projdata, float *u, float s, float sqr_s, const pair_struct_float *pair){
    ...
    for(int p=0;p<projdata->nprojbins;p++){
...
void gradient_float(const proj_struct_float *projdata, float *u, float s, 
    ...
    // but CAREFUL, for functions that depend on nprojbins we will have to reset it, bc actual is 4x more than assumed in other funcs!
    ...
    int nconst = projdata->nprojbins/4;
...
int compute_amplitudes_float(int nprojbins, int nd1, int nd2, int nr1, int nr2,
            float *dd, float *dr, float *rd, float *rr, float *qq, float *amps){

    // Computer numerator of estimator
    float numerator[nprojbins];
    float qqnorm[nprojbins*nprojbins];
    for (int i=0; i<nprojbins; i++){
        ...
        for (int j=0; j<nprojbins; j++){
            qqnorm[i*nprojbins+j] = qq[i*nprojbins+j]/((float)nr1*(float)nr2);
        }
    }
...
int evaluate_xi_float(int nprojbins, float *amps,
...
int qq_analytic_float(float rmin, float rmax, int nd, float volume, int nprojbins, float *rr, float *qq, proj_method_t proj_method, int nsbins, float *sbins, char *projfn){
    proj_struct_float *projdata = malloc(sizeof(proj_struct_float));
    if(projdata == NULL){
        free(projdata);
        return EXIT_FAILURE;
...
float adaptiveSimpsons_float(float (*f)(float, integration_params_float*),     // function ptr to integrate
    ...
    float aa, bb;
    float total = 0.0;
    float hh = 5.0;// starting grid width - may need to make a parameter or adjust based on nprojbins
    aa = a;
    bb = aa+hh;
    while (aa < b){
