...
class Client(GoogleStorageClient):
    ...
    @backoff.on_exception(backoff.expo, GOOGLE_EXCEPTIONS_TO_RETRY, max_tries=3)
    @staticmethod
    def delete_blobs(blobs_list: List[storage.Blob]) -> None:
    ...
    def concatenate_sub_files(
        ...
        try:
            ...
            print(f"The blob does not exist") 
            raise FileNotFoundError(f"Error occured when trying to delete the blob")
