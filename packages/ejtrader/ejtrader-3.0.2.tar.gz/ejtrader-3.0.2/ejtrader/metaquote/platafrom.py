...
class API:
    def __init__(self, host):
        ...
        try:
            ...
            # set port timeout
            self.data_socket.RCVTIMEO = data_timeout * 9000
    ...
    def liveFeed(self, context=None):
    ...
    def HistoryFeed(self, context=None):
        """Connect to socket in a ZMQ context"""
        try:
            context = context or zmq.Context.instance()
            socket = context.socket(zmq.PULL)
...
class metatrader:
    ...
    def positionCloseBySymbol(self, symbol):
        self.api.Command(
            action="TRADE",
    ...
    def historyfast(self, symbol, chartTF, fromDate):
        
        if(chartTF == 'TICK'):
            data = json.loads(json.dumps(self.api.Command(action="HISTORY", actionType="DATA", symbol=symbol, chartTF=chartTF, fromDate=fromDate)))
            data_frame = pd.DataFrame(data['data'], columns=['date', 'bid', 'ask'])
            self.api.Command(action="RESET")
        else:
            data = json.loads(json.dumps(self.api.Command(action="HISTORY", actionType="DATA", symbol=symbol, chartTF=chartTF, fromDate=fromDate)))
