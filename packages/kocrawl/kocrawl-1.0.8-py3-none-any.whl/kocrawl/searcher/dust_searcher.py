...
class DustSearcher(BaseSearcher):
    def __init__(self):
        self.CSS = {
        ...
        for i, kind in enumerate(['fine_dust', 'ultra_dust', 'ozon']):
            ...
            for date in ['today_', 'tomorrow_', 'after_']:
                for time in ['morning_', 'afternoon_']:
                    ...
                    if kind == 'fine_dust':
                        self.new_data_dict[dict_key] = re.sub(' ', '', results[i][j][0])
    ...
    def old_today(self, location: str, date: str) -> dict:
        ...
        self.old_data_dict['today_fine_dust'] = re.sub(' ', '', ultra_dust[0])
        self.old_data_dict['today_ultra_dust'] = re.sub(' ', '', fine_dust[0])
        self.old_data_dict['today_ozon'] = re.sub(' ', '', fine_dust[1])
    ...
    def old_tomorrow(self, location: str, date: str) -> dict:
        ...
        for kind in ['fine_dust', 'ultra_dust', 'ozon']:
            for date in ['tomorrow_', 'after_']:
                for time in ['morning_', 'afternoon_']:
                    dict_key = date + time + kind
                    self.old_data_dict[dict_key] = re.sub(' ', '', result[i])
                    i += 1
