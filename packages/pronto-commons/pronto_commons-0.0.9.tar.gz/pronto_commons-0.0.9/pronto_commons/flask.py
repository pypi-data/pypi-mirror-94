...
def requires_jwt_authentication(
    jwt_secret_key: str, verify_exp=False, validate_refresh_token=False, sub: Optional[str] = None, 
    ...
    def wrap(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                token, _ = _decode_jwt_from_headers(header_name=header_name, header_type=header_type)
                decoded_token = decode_jwt(jwt_token=token, verify_exp=verify_exp, jwt_secret_key=jwt_secret_key)
            except (
