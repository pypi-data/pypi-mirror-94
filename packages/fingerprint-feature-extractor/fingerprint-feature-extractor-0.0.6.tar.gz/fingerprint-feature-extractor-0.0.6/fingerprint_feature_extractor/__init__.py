...
class FingerprintFeatureExtractor(object):
    ...
    def __cleanMinutiae(self, img, spuriousMinutiaeThresh):
        self.minutiaeTerm = skimage.measure.label(self.minutiaeTerm, connectivity=2);
        RP = skimage.measure.regionprops(self.minutiaeTerm)
        self.minutiaeTerm = self.__removeSpuriousMinutiae(RP, np.uint8(img), spuriousMinutiaeThresh);
    ...
    def extractMinutiaeFeatures(self, img, spuriousMinutiaeThresh):
        self.__skeletonize(img)

        self.__getTerminationBifurcation()

        self.__cleanMinutiae(img, spuriousMinutiaeThresh)
...
def extract_minutiae_features(img, showResult=False, spuriousMinutiaeThresh=10):
    feature_extractor = FingerprintFeatureExtractor()
    FeaturesTerm, FeaturesBif = feature_extractor.extractMinutiaeFeatures(img, spuriousMinutiaeThresh)
