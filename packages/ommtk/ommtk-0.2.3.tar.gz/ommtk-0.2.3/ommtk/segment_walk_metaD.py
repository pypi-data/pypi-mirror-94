...
class SegmentMappingSim(MetadynamicSimulation):
    ...
    def __init__(self, num_segs_per_replica = 10, seg_sim_time=.5 * unit.nanosecond, scan_bias=40,
                 scan_time = 1 * unit.nanosecond, max_scan_time=6 * unit.nanosecond,
                 prod_bias=5, num_replicas=1, total_production_time = 100 * unit.nanosecond,
                 **kwargs):
    ...
    def run(self):
        ...
        #segment_coords is a list of chosen coordinates from each replica
        segment_coords = []
        ...
        for i in range(self.num_replicas):
            ...
            self._add_reporters(total_steps=nsteps * self.num_replicas, traj_interval= .01 * unit.nanosecond)
            ...
            while all(distances < distances[0]+self.unbound_distance) and  scan_clock<self.max_scan_time:
                ...
                print('CV distance range {} to {}'.format(np.min(distances),np.max(distances)))
        ...
        del self.simulation
        del self.metaD
        seg_steps = int(self.total_production_time / (self.step_length*self.num_segs*self.num_replicas))
        ...
        nsteps = self.total_production_time/self.step_length
        self._add_reporters(total_steps = nsteps)
        self._add_FESReporter()

        #loop across all replicas
        for k,replica in enumerate(range(self.num_replicas)):
            print('starting replica {}'.format(k+1))
            ...
            for i,frame in enumerate(segment_coords[replica]):
                print('swapping in coordinates from replica {} frame {}'.format(k+1,i+1))
