...
class Detectors(Plugin):
    ...
    def loadTestsFromTestCase(self, event):
        if event.testCase.__name__ != 'TestDetectors':
            return
        for data in DATA:
...
DATA = (
    ...
    ('simplematch', 'simple_42.txt', []),
    ('simplematch', 'simple_43.txt', []),
    ('simplematch', 'simple_44.txt', [b'user@example.com']),
    ('simplematch', 'bounce_02.txt', [b'userx@example.com']),
    ('simplematch', 'bounce_03.txt', [b'userx@example.uk']),
    ('simplematch', 'yahoo_12.txt', [b'user@yahoo.com']),
    # SimpleWarning
