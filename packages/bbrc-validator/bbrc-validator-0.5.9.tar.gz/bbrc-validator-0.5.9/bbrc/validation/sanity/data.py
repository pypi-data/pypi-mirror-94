...
class HasBvecBval(ScanTest):
    ...
    by the conversion to NIfTI. Passes if the dataset has existing and non-empty
    BVEC and BVAL items. Fails otherwise."""
...
class HasBvecBvalInDWIScans(ExperimentTest):
    """Diffusion-weighted datasets should have a BVEC and a BVAL file generated
    by the conversion to NIfTI. Passes if the dataset has existing and non-empty
    BVEC and BVAL items or simply no DWI/DTI scans included. Fails otherwise."""
...
class IsFreeSurferRunnable(ExperimentTest):
    ...
    for running FreeSurfer application. Fails if no MRSession scans are suited
    for running FreeSurfer (see result/report data), passes otherwise."""
