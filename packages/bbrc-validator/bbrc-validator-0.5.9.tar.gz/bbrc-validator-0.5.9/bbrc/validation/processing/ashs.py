...
class HasCorrectItems(ExperimentTest):
    '''Passes if an ASHS resource is found and such resource has the main
    expected items.'''
...
class HasCorrectASHSVersion(ExperimentTest):
    '''This test checks the ASHS version used for processing the images. Passes
...
class ASHSSnapshot(ExperimentTest, Snapshot):
    '''This generates a snapshot of the results generated by ASHS.
...
class HasNormalSubfieldVolumes(ExperimentTest):
    ''' Compares subfield volumes with predefined boundaries. Passes if the
    number of subfields falling outside is strictly lower than 4. Fails
    otherwise.'''
...
class HasAllSubfields(ExperimentTest):
    ''' Passes if all subfields are found in the resulting segmentation. Fails
    otherwise. '''
...
class AreCAVolumesConsistent(ExperimentTest):
    ''' Validates that the volumes of the _Cornu Ammonis_ main subregions from
    ASHS hippocampal segmentation results (any mode) are consistent in size.
