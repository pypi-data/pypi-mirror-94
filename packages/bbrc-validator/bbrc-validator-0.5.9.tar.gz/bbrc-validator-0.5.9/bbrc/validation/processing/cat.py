...
class HasCorrectItems(ExperimentTest):
    '''Passes if a CAT12_SEGMENT resource is found and such resource has the
    main expected items.'''
...
class HasCorrectCATVersion(ExperimentTest):
    '''This test checks the `CAT` version used for processing the images.
    Passes if CAT12_SEGMENT outputs were created using the expected CAT version
    (i.e. `CAT version: CAT12.6 Release: 1450`).'''
...
class HasCorrectSPMVersion(ExperimentTest):
    '''This test checks the `SPM` version used for processing the images as CAT
    prerequisite. Passes if CAT12_SEGMENT outputs were created using the expected
    SPM version (i.e. `SPM version: SPM12 Release: 7487`).'''
...
class HasCorrectMatlabVersion(ExperimentTest):
    '''This test checks the `MATLAB` version on which CAT toolbox was executed.
    Passes if CAT12_SEGMENT outputs were created using the expected MATLAB
    version (i.e. `MATLAB Version: 9.5.0 (R2018b)`).'''
...
class CAT12SegmentIQRScore(ExperimentTest):
    '''This test checks the `Image Quality Rating` (IQR) score generated by
    `CAT12 Segment`. This score is a weighted average of resolution, noise and
    bias data quality metrics. Tests passes if `CAT12 Segment` IQR score is higher
    than a given threshold (i.e. 75%); fails otherwise.'''
...
class CAT12SegmentExecutionTime(ExperimentTest):
    '''This test checks the execution time of `CAT12 Segment` command. Tests
    passes if elapsed time spent processing is within an _acceptable_ range
...
class CAT12SegmentSnapshot(ExperimentTest):
    '''This test generates a snapshot of the results generated by CAT12_SEGMENT.
