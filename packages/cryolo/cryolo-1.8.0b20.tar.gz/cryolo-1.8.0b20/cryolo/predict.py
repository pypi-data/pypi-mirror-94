...
def create_parser(parser, use_gooey=True):
    ...
    filament_group.add_argument(
        ...
        default=0.95,
        type=float,
        help="Threshold value for the straightness method. The default value works good for LINE_STRAIGHTNESS. Lines with a LINE_STRAIGHTNESS lower than this threshold get splitted. For RMSD, lines with a RMSD higher than this threshold will be splitted. A good value for RMSD ars 20 percent of your filament width.",
    )
...
def main(args=None):
    ...
    if filament_mode:
        ...
        if picking_result_with_boxes:
            ...
            print("Create direction maps and trace filaments in batches")
                ...
                if is_3d == False:
                    ...
                    if outdir is not None:
                        ...
                        coords_fid_traced = os.path.join(
                            outdir, "COORDS_TRACED", os.path.basename(root_ext[0])+"_fid.coords"
                        )
                    ...
                    if "filaments_traced" in result:
                        ...
                        for fil_id, fil in enumerate(result["filaments_traced"]):
                            ...
                            fil_ids_array[3].extend([fil_id] * len(fil.boxes))
