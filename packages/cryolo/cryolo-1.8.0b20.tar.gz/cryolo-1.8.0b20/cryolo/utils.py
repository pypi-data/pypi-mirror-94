    ...
    def merge_filaments(filaments, window_size, box_distance):
        ...
        for i in sample_points:#range(int(min(dotproducts)), int(max(dotproducts))+1):
            ...
            if np.sum(window_elements) > 0:
                ...
                z3d = int(np.average(A[window_elements][:,2], weights=weights[window_elements]))
            ...
            if box_i[6] == 0:
                ...
                if other_box.shape[0] > 0:
                    ...
                    if average:
                        ...
                        boxes[boxes_data[i, 7].astype(int)].z = boxes_data[i, 1]
                        boxes[boxes_data[i, 7].astype(int)].y = boxes_data[i, 2]
