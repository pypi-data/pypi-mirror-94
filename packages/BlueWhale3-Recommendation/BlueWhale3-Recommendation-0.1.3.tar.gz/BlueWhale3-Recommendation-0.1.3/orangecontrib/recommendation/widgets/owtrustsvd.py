...
def __(key):
    return i18n.t('recommendation.owtrustsvd.' + key)
...
class OWTrustSVD(OWBaseLearner):
    ...
    inputs = [(i18n.t("imganlytic.common.trust_information"), Table, "set_trust")]
    ...
    lmbda = settings.Setting(0.12)
    bias_lmbda = settings.Setting(0.12)
    social_lmbda = settings.Setting(0.9)
    trust = None
    ...
    def add_main_layout(self):
        ...
        gui.doubleSpin(box, self, "social_lmbda", minv=1e-4, maxv=1e+4,
                       step=1e-4, label=__("row.social_regularization"), decimals=4,
                       alignment=Qt.AlignRight, controlWidth=90,
                       callback=self.settings_changed)
            ...
            if not valid_ratings:  # Check if it's valid
                ...
                if self.trust is None:
                    ...
                    if not valid_trust:  # Check if it's valid
                        self.Error.data_error(
                            __("error_trust_data_invalid_for_rate_model"))
                    else:
