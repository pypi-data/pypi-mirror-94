...
def drop_privs():
    _, _, uid, gid, gecos, root, shell = pwd.getpwnam("asterisk")
    os.setgid(gid)
    os.setuid(uid)
...
class OdooConnector:
    ...
    async def listen_http(self):
        ...
        ssl_options = {
            'certfile': __opts__.get('odoopbx_ssl_crt'),
            'keyfile': __opts__.get('odoopbx_ssl_key'),
        }
        for key, file in ssl_options.items():
            if not os.path.exists(file):
                log.error(
                    'Cannot init HTTPS, %s %s does not exists.', key, file)
                ssl_options = False
                break
            log.info(
