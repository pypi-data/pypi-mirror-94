...
from .utils import column_exists, find_ngrams_range, fixup_columns

MODELFN = "models/fl_voter_soundnames/fl_voter_soundnames_lstm.h5"
VOCABFN = "models/fl_voter_soundnames/fl_voter_soundnames_vocab.csv"
...
class FloridaRegSoundNameModel():
    vocab = None
    race = None
    model = None

    @classmethod
    def pred_fl_reg_soundname(cls, df, lname_col, fname_col):
        ...
        lsoundname = df[lname_col].apply(lambda c: doublemetaphone(c)[0])
        fsoundname = df[lname_col].apply(lambda c: doublemetaphone(c)[0])

        df['__name'] = (lsoundname + ' ' + fsoundname)
...
pred_fl_reg_soundname = FloridaRegSoundNameModel.pred_fl_reg_soundname
...
def main(argv=sys.argv[1:]):
    ...
    parser.add_argument('input', default=None,
                        help='Input file')
    parser.add_argument('-o', '--output', default='fl-pred-soundname-output.csv',
    ...
    rdf = pred_fl_reg_soundname(df, args.last, args.first)
