...
from pycarus.geometry.pcd import random_drop_points, rotate, scale, shuffle_pcd, translate
...
class PointSampler:
    ...
    def __call__(self, pcd: torch.Tensor) -> torch.Tensor:
        ...
        if self.algorithm == "random":
            weights = torch.ones(len(pcd)).float()
            indices_to_remove = torch.multinomial(weights, self.num_points, replacement=False)
            pcd_ss = pcd[indices_to_remove]
