...
def farthest_point_sampling(pcd: torch.Tensor, num_points: int) -> torch.Tensor:
    ...
    batch_size, original_num_points, point_dim = pcd_copy.shape
    xyz = pcd_copy[:, :, :3]

    distances = torch.ones((batch_size, original_num_points)) * 1e10
    farthest = torch.randint(0, original_num_points, (batch_size,))

    output_shape = (batch_size, num_points, point_dim)
    sampled_points = torch.empty(output_shape, dtype=pcd_copy.dtype).to(pcd_copy.device)
    batch_indices = torch.arange(batch_size, dtype=torch.long).to(pcd_copy.device)

    for i in range(num_points):
        sampled_points[:, i] = pcd_copy[batch_indices, farthest]
...
def shuffle_pcd(pcd: torch.Tensor) -> torch.Tensor:
    ...
    batch_size = pcd_copy.shape[0]
    batch_indices = torch.arange(batch_size, dtype=torch.long).to(pcd_copy.device)
    rand_indices = torch.randperm(len(pcd)).to(pcd_copy.device)
    shuffled_pcd = pcd_copy[batch_indices, rand_indices]
...
def jitter_pcd(pcd: torch.Tensor, sigma: float = 0.01, clip: float = 0.05) -> torch.Tensor:
    ...
    noise = torch.clip(sigma * torch.randn(pcd_copy.shape), min=-1 * clip, max=clip)
    jittered_pcd = pcd_copy + noise.to(pcd_copy.device)
...
def get_neighbouring_points(
    pcd: torch.Tensor, query_points: torch.Tensor, radius: float, max_num_neighbors: int
    ...
    points_nn = pcd[torch.arange(pcd.shape[0]), indices_nn, :]
    if batched:
