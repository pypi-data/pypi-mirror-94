...
def csr_gen (isa_file, grp_file, custom_file, custom_attr_file, work_dir,
...
def station_interface(isa_yaml, grp_yaml, attr_yaml, rvxlen, pkg_name,
    ...
    req_regs = []
    for csrname in isa_yaml:
        if csrname.upper() in additional_ifc:
            if additional_ifc[csrname.upper()] not in req_regs:
                bsvstr += textwrap.indent(additional_ifc[csrname.upper()],'    ')
                req_regs.append(additional_ifc[csrname.upper()])
    ...
    for csrname in isa_yaml:
        if attr_yaml[csrname]['action_method']['required']:
            ...
            size = attr_yaml[csrname]['size']
            arguments = attr_yaml[csrname]['action_method']['def_arguments']
            arguments = arguments.replace('xlen',str(xlen))
            arguments = arguments.replace('size',str(size))
            arguments = textwrap.indent(arguments, '    ')
            bsvstr +=('\n    '+arguments)
    bsvstr +=('\n')
...
def station_methods(isa_yaml, grp_yaml, attr_yaml, rvxlen, pkg_name,
    ...
    req_regs = []
    for csrname in isa_yaml:
        if csrname.upper() in additional_ifc_dec:
            if additional_ifc_dec[csrname.upper()] not in req_regs:
                bsvstr += textwrap.indent(additional_ifc_dec[csrname.upper()],'    ')
                req_regs.append(additional_ifc_dec[csrname.upper()])
    ...
    for csrname in isa_yaml:
        ...
        if attr_yaml[csrname]['value_method']['required']:
            if 'dec_body' in attr_yaml[csrname]['value_method']:
                bsvstr += methodv_def_temp.format(csrname, '', attr_yaml[csrname]['value_method']['dec_body'])
            else:
    ...
    for csrname in isa_yaml:
        ...
        if attr_yaml[csrname]['action_method']['required']:
            size = attr_yaml[csrname]['size']
            _str = attr_yaml[csrname]['action_method']['dec_body']
...
def station_req(isa_yaml, grp_yaml, attr_yaml, rvxlen, pkg_name,
    ...
    bsvstr += method_csr_req_def_temp
...
def csr_top(isa_yaml, grp_yaml, attr_yaml, rvxlen, pkgname, logging = False):
  ...
  endinterface:Sbread'''
    ...
    bsvstr += module_temp.format(pkgname,mod_attr_str) + '\n'
...
'''.format(misa_grp)
    ...
    for csrname in isa_yaml:
        if csrname.upper() in additional_regs:
            if additional_regs[csrname.upper()] not in req_regs:
                bsvstr +=\
                textwrap.indent(additional_regs[csrname.upper()],'    ') + '\n'
                req_regs.append(additional_regs[csrname.upper()])
    ...
    req_ifc = []
    for csrname in isa_yaml:
        if csrname.upper() in additional_topifc_dec:
            grp_name = find_group(grp_yaml, csrname.upper())
            if (grp_name, additional_topifc_dec[csrname.upper()]) not in req_ifc:
                bsvstr +='\n'+\
                textwrap.indent(additional_topifc_dec[csrname.upper()].format(grp_name),'    ')
                req_ifc.append((grp_name,additional_topifc_dec[csrname.upper()]))
 ...
 of csr: ' +csrname + ' : ' +  str(dcsr))
    ...
    for csrname in isa_yaml:
        if csrname.upper() in additional_ifc_dec:
            if additional_ifc_dec[csrname.upper()] not in req_ifc:
                bsvstr +=\
                textwrap.indent(additional_ifc_dec[csrname.upper()],'    ') + '\n'
                req_ifc.append(additional_ifc_dec[csrname.upper()])
    ...
    for csrname in isa_yaml:
        ...
        if attr_yaml[csrname]['action_method']['required'] and \
            ...
            methods = re.findall(r'\s*method\s*Action\s*(.*?)[\s(]', defs)
            for x in methods:
