...
def create_new_ts_table(name: str, dtype: Type) -> Type:
    ...
    Args:
        ...
        dtype (Type): Type of the variable to be stored in the table,
            can be any of int, float and str

    Raises:
        TypeError: If dtype is not one of int, float or str
    ...
    if dtype == int:
        ...
        raise TypeError(
            ...
            "and will not be stored! Has to be one of int|float|str."
        )
...
def write_to_ts(name: str, value: Union[int, float, str], timestamp: datetime.datetime):
    """Write a value to the timeseries identified by name

    Args:
        name (str): Identifier of timeserie
        value (Union[int, float, str]): Value to be written
