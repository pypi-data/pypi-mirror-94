...
def get_operator_expectation_value(
    ...
    **kwargs: KwargTypes,
    ...
    :param state_circuit: Circuit that generates the desired state :math:`\\left|\\psi\\right>`
    :type state_circuit: Circuit
    :param operator: Operator :math:`H`. Currently does not support free symbols for the purpose of obtaining expectation values.
    ...
    if not partition_strat:
        ...
        handles = backend.process_circuits(
            pauli_circuits, n_shots, valid_check=True, **kwargs
        )
        ...
        handles = backend.process_circuits(
            measure_circs, n_shots=n_shots, valid_check=True, **kwargs
        )
        results = backend.get_results(handles)
