...
def get_report_fun(
    ...
    for current_login in login:
        ...
        # send request to API
        headers = {
            'Authorization': f'Bearer {cur_token}',
            'Accept-Language': 'ru',
        ...
        data = send_api_request(query_body, headers, field_names, cur_token, skip_errors=skip_errors)
...
def get_report(
    ...
    for i in dates[:-2]:
        ...
        # task = execute_func_in_individual_thread(get_report_fun,
    ...
    # while reduce(lambda prev, cur: prev * cur, is_run_list(tasks)) != 1:
    #     time.sleep(5)
