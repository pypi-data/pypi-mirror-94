...
def get_client_list(token: str, agency_account: str = AGENCY_ACCOUNT):  # table: Airtable = None):
    # if table is None:
    # table = get_table()
...
def get_campaign(
    ...
    while lim != 'stopped':
        ...
        for l in range(len(logins)):
            token = token  # tech_auth(login=logins[l], token=token, agency_account=agency_account)
            # if 'error' in token:
            #     return {
            #         'res': {},
            #         'error': token['error']
