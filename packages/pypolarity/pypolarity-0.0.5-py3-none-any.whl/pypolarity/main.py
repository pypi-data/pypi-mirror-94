# README copied from sentimentV4.py

# %%
# version 4: refactor code to use cut-sent-thai that I already put on pypi and not the local copy of inference_cut_sentence
...
from thaipos import pos_tag, dict_trie, thai_words

DEPLOY=1

if DEPLOY == 1:
    from pypolarity import __file__ as data_file
    sentiment_path = os.path.join(os.path.dirname(data_file), "sentiment")
    file_list = os.listdir(sentiment_path)
else:
    sentiment_path = os.path.join('.', "sentiment")
    file_list = os.listdir('./sentiment')

print(sentiment_path)
print(file_list)
# %%
# user have to set these environment variables from the command line
USER = os.environ.get('DB_USER')
PASSWORD = os.environ.get('DB_PASSWORD')

DB_URL = "mongodb://" + USER + ":" + PASSWORD + "@titipakorn.xyz:27000"
...
for f in file_list:
    with open(os.path.join(sentiment_path, f)) as c:
...
# if the next keyword is not in the same sentence -> context is until end of sentence
    ...
    tokens = pos_tag(test_string, trie)
    test_words = list()
    for t in tokens:
        test_words.append(t[0])
    ...
    test_sentences = cut_sent(test_string)
    ...
    for i, (k, b, p) in enumerate( zip(keywords_found, type_business_found, pos_found) ):
        ...
        tokens = pos_tag(current_sentence, trie)
        words = list()
        for t in tokens:
            words.append(t[0])
        
        context = ["dummy1", "dummy2"] # to avoid having undefined context in contexts.append(context) below
