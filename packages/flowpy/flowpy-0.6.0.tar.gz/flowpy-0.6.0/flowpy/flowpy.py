...
def flowshow(flow, with_calibration_pattern=True, with_arrows=True, with_tooltip=True, max_radius=None):
    height, width, _ = flow.shape
    image_ratio = height / width

    if max_radius is None:
        max_radius = get_flow_max_radius(flow)

    if with_calibration_pattern:
        fig, axes = plt.subplots(1, 2, gridspec_kw={"width_ratios": [1, image_ratio]})
        ax_1, ax_2 = axes
    else:
        fig, axes = plt.subplots()
        ax_1 = axes

    ax_1.imshow(flow_to_rgb(flow))

    if with_arrows:
        attach_arrows(ax_1, flow)

    if with_tooltip:
        attach_coord(ax_1, flow)

    if with_calibration_pattern:
        attach_calibration_pattern(ax_2, flow_max_radius=max_radius)
