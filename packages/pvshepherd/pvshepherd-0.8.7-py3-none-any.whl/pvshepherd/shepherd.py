...
AUDIO_DEBUG_RECORD_DURATION_SEC = 3
...
class PicovoiceApp(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self._status_bar = tk.Label(master=self, text='', bd=1, relief=tk.FLAT, anchor=tk.W)
    ...
    def draw_upload_models_frame(self, board, message_thread):
        self.update_status_bar("Connected to '%s'" % board)
        old_frame = self._active_frame
        self._active_frame = UploadModelsFrame(master=self, board=board, message_thread=message_thread)
        old_frame.destroy()
        del old_frame

    def draw_control_panel_frame(self, board, message_thread, picovoice_models):
        self.update_status_bar("Connected to '%s'" % board)
        old_frame = self._active_frame
        self._active_frame = ControlPanel(self, board, message_thread, picovoice_models)
        old_frame.destroy()
        del old_frame
...
class UploadFirmwareFrame(tk.Frame, PicoFrame):
    ...
    def _check_uploading(self):
        if self._is_uploading_done:
            sleep(0.1)
            self.master.update_status_bar('Connecting to the board ...')
            self._update_thread = threading.Thread(target=self._serial_port_select)
            self._update_thread.start()
            self._check_connecting()
        else:
            self.after(10, self._check_uploading)

    @staticmethod
    def _get_ports():
        return serial.tools.list_ports.comports()

    def _serial_port_select(self):
        self._ports = self._get_ports()
        for port in self._ports:
            try:
                ...
                for _ in range(SERIAL_CONNECTION_RETRIES):
                    self._message_thread.send_message("%s 0000000000" % MessageCode.PV_MESSAGE_CODE_HANDSHAKE.value[0])
                    sleep(0.2)
    ...
    def _check_connecting(self):
        if self._is_connecting_done:
            ...
            self.after(10, self._check_connecting)
    ...
    def draw_next_frame(self):
        ...
        self.master.draw_upload_models_frame(self._board, self._message_thread)
...
class UploadModelsFrame(tk.Frame, PicoFrame):
    def __init__(self, master, board, message_thread):
        tk.Frame.__init__(self, master)
        ...
        self._message_thread.change_consumer(self)
        self.after(100, self._get_uuid)
    ...
    def draw_next_frame(self):
        self._message_thread.change_consumer(None)
        self.master.draw_control_panel_frame(self._board, self._message_thread, self._selected_models)
...
class ControlPanel(tk.Frame, PicoFrame):
    def __init__(self, master, board, message_thread, models):
        tk.Frame.__init__(self, master)
    ...
    def consume(self, code, payload):
        if code is MessageCode.PV_MESSAGE_CODE_CONTEXT.value[0]:
            self._is_responded = True
...
class MonitoringCenter(tk.Frame):
    ...
    def consume(self, code, payload):
        ...
        if code is MessageCode.PV_MESSAGE_CODE_VOLUME.value[0]:
            try:
                volume_percentage = (
...
class AudioDebugging(tk.Frame):
    ...
    def update_status_bar_recording(self, n):
        ...
        if self._is_downloading:
            if n < 0:
                ...
                self.after(3000, self.update_status_bar_downloading, 0)
                self.after(3000, self._downloading_label.start_loading)
            else:
    ...
    def update_status_bar_downloading(self, n):
        ...
        n = n + 1
        if self._is_downloading:
            self.after(AUDIO_DEBUG_RECORD_DURATION_SEC * 70, self.update_status_bar_downloading, n)
        else:
