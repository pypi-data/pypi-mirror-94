...
class TabularQLearning(Model):
    ...
    def _define_graph(self):
        ...
        with tensorflow.variable_scope(full_scope):
            ...
            self._table = tensorflow.get_variable(name="table", trainable=True,
                                                  initializer=tensorflow.random_uniform([*self._observation_space_shape, *self._agent_action_space_shape], 0, 0.1))
    ...
    def update(self,
               ...
               batch: []):
        """
        Update the table q-values of the model using a batch of samples. Update is performed using the Q-Learning
        Bellman equation to compute the model target q-values.
