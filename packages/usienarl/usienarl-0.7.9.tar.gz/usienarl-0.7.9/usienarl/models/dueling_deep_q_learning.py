    ...
    def __init__(self,
        ...
        with tensorflow.variable_scope(self.scope):
            ...
            with tensorflow.variable_scope("value_stream"):
                # Define the estimator network value stream hidden layers from the config
                value_stream_hidden_layers_output = hidden_layers_config.apply_hidden_layers(self.observations)
                # Define the value stream head with the size of one
                self._value = tensorflow.layers.dense(value_stream_hidden_layers_output, 1, None, kernel_initializer=tensorflow.contrib.layers.xavier_initializer(), name="value")
            # Define the advantage stream
            with tensorflow.variable_scope("advantage_stream"):
                # Define the estimator network value stream hidden layers from the config
                advantage_stream_hidden_layers_output = hidden_layers_config.apply_hidden_layers(self.observations)
                # Define the advantage stream head with the size of the action space (always discrete)
                self._advantage = tensorflow.layers.dense(advantage_stream_hidden_layers_output, *agent_action_space_shape, None, kernel_initializer=tensorflow.contrib.layers.xavier_initializer(), name="advantage")
            # Define the mask
            self.mask = tensorflow.placeholder(shape=[None, *agent_action_space_shape], dtype=tensorflow.float32, name="mask")
            # Define the estimator network predicted q-values given the input observations and the mask with the equation:
            # Q(s,a) = V(s) + (A(s,a) - 1/|A| * sum A(s,a')) aka the aggregation layer
            self.q_values_predictions = tensorflow.add(self._value + tensorflow.subtract(self._advantage, tensorflow.reduce_mean(self._advantage, 1, True), name="q_values_predictions"), self.mask)
...
class DuelingDeepQLearning(Model):
    ...
    def update(self,
        ...
        Dueling Double Q-Learning Bellman equation to compute the model target q-values.
        ...
        q_values_next_main_network: numpy.ndarray = session.run(self._main_network_q_values_predictions,
                                                                feed_dict={
                                                                    self._main_network_observations: observations_next_input,
                                                                    self._main_network_mask: masks
                                                                })
        q_values_next_target_network: numpy.ndarray = session.run(self._target_network_q_values_predictions,
                                                                  feed_dict={
        ...
        for sample_index in range(len(actions)):
            ...
            if episode_done:
                ...
                best_action_index: int = int(numpy.argmax(q_values_next_main_network[sample_index]))
        ...
        # Train the model and save the value of the loss and of the absolute error
