...
def include_missing_spatial_samplings(data, samplings, order=0, base_key=None):
    ...
    base_data = np.array(data[base_spatial_sampling], dtype=np.float)
...
def prepare_s2_wv(n_smpl, bands, fo, rho_bounds):
    ...
    points = np.zeros((n_params, len(["spr", "tau_a"] + bands)), dtype=np.float)
    values = np.zeros(n_params, dtype=np.float)  # cwv
...
def prepare_s2_ac(s2spr, s2cwv, s2tau, clear_areas,
    ...
    n_params = np.prod(list(n_smpl.values()))
    points = np.zeros((n_params, len(n_smpl.keys())), dtype=np.float)

    rfls = np.zeros((n_params, len(bands)), dtype=np.float)
    rhos = np.zeros(n_params, dtype=np.float)
...
def ac_interpolation(iband, band, s2img, s2spr, s2cwv, s2tau, clear_areas, points, rhos, rfls, max_pixel_processing,
    ...
    dd = max(1, np.int(np.sqrt(np.floor(np.prod(data_ac.shape) / max_pixel_processing))))
    ...
    def write_l2a(granule_path, band_fns, projection, data=None, uncert=None, msk=None, driver="JP2OpenJPEG",
        ...
        if output_bands is not None and uncert is not None:
            def mk_conv_error(nodata_value, max_value, dtype):
                scaling_value = np.int(np.floor((nodata_value - 1) / max_value))
...
class Figs(object):
    ...
    def dat_2show(data, nodata):
        ...
        :return:
        """
        bf = np.array(data, dtype=np.float)
    ...
    def _to_mpl_(self, inp):
        ...
        d1, d2 = np.array(np.floor(np.array(ar.shape) / self.ss), dtype=np.int)[:2]
        if len(ar.shape) == 2:
