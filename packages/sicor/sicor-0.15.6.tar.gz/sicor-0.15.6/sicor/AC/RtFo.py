...
def opt(self, y_rfl, pt0, rho_0=[{"method": "const", "value": 0.1},
    ...
    if len(ssai) != len(self.scale_dn):  # not all errors given, assume that first ones are given
        bf = np.ones(len(self.scale_dn), dtype=np.float)
        bf[:len(ssai)] = ssai[:]
        ssai = bf
...
class Rj2RhoJ_LinInterp(object):
    def __init__(self, **kwargs):
        ...
        self.rho = np.zeros(self.n_wvl, dtype=np.float)
        self.J_rho = np.zeros((self.n_rho_lin, self.n_wvl), dtype=np.float)
...
class RtFo(object):
    ...
    def __mk_tble__(self, grp, dims, idx=None):
        ...
        if idx is None:
            ...
            tble["idx"] = np.array([idx.index(dim) for dim in tble["dims"][:-1]], dtype=np.int)
    ...
    def get_wvl_idx(self, data_wvls, sensor_name=None):
        ...
        """
        if sensor_name is None:
            return np.array([np.argmin(np.abs(data_wvls - wv)) for wv in self.wvl], dtype=np.int)
        else:
            return np.array([np.argmin(np.abs(data_wvls - wv)) for wv in self.wvl_sensors[sensor_name]], dtype=np.int)
...
def __minimize__(pt_index, p0, pt_names, data, opt_func, zoom_factor=None, opt_range="full",
    ...
    if processes is None:
        processes = np.max([1, np.int(np.ceil(cpu_count() / 2) - 2)])

    if zoom_factor is None:
        zoom_factor = np.ones(3, dtype=np.float)
    elif len(zoom_factor) == 1:
        zoom_factor = np.array([zoom_factor, zoom_factor, 1.0], dtype=np.float)
    elif len(zoom_factor) == 2:
        zoom_factor = np.array([zoom_factor[0], zoom_factor[1], 1.0], dtype=np.float)
    else:
...
def __rezoom__(res, data_shape, order=3):
    ...
    sh0, sh1 = np.array(res.shape, dtype=np.float), np.array(res.shape, dtype=np.float)
...
class FF(object):
    def __init__(self, fo, optimize_dims_atm, weight=None, wvl_sel=slice(None)):
        ...
        self.atm_idx = np.array([idx for dim, idx in zip(self.fo.L0["dims"], self.fo.L0["idx"])], dtype=np.int)
