...
.. code:: python
    ...
    pt_ind_atm_save = np.array([ii for ii,dim in enumerate(pt_names) if dim in settings["atm_fields"]],dtype=np.int)
...
.. code:: python
    ...
    def ch4_opt(ch4,rho_1,rho_2,cwv=20.0):
        ...
        return {"pt":bf,"rho":fo.rj_to_rho_J(np.array([rho_1,rho_2],dtype=np.float))[0]}
