...
class EdgeGridHttpCaller():
    ...
    def getResult(self, endpoint, headers= None,parameters=None):
    ...
    def putResult(self, endpoint, body,headers=None, params=None):
        ...
        endpoint_result = self.session.put(parse.urljoin(self.baseurl,path), data=body, headers=headers, params=params)
        status = endpoint_result.status_code
        if self.verbose:
