...
def dump_autogen_code(
    fpath, autogen_text, codetype='python', fullprint=None, show_diff=None, dowrite=None
):
    """
    ...
    if show_diff:
        if ut.checkpath(fpath, verbose=True):
            ...
            try:
                ...
                textdiff = unicodedata.normalize('NFKD', textdiff).encode(
                    'ascii', 'ignore'
                )
                ut.print_difftext(textdiff)
...
def makeinit(mod_dpath, exclude_modnames=[], use_star=False):
    ...
    IMPORT_TUPLES = util_importer.make_import_tuples(
        mod_dpath, exclude_modnames=exclude_modnames
    )
...
def load_func_from_module(modname, funcname, verbose=True, moddir=None, modpath=None):
    ...
    try:
        ...
        if modpath is None:
            ...
            if modpath is None:
                ...
                if modpath is None:
                    raise Exception(
                        'Cannot find modname={} in moddir={}'.format(modname, moddir)
                    )
        print('modpath = {!r}'.format(modpath))
    ...
    if not isinstance(modname, six.string_types):
        ...
        if module is None:
            try:
                ...
                if moddir is not None:
                    ...
                    module = ut.import_module_from_fpath(
                        join(moddir, modname.split('.')[-1] + '.py')
                    )
                else:
                    raise
            # import inspect
        ...
        try:
            ...
            testcmd = (
                "python -c \"import utool; print(utool.auto_docstr('%s', '%s'))\""
                % (modname, funcname)
            )
            error_str = ut.formatex(
                ex2, 'ex2', tb=True, keys=['modname', 'funcname', 'testcmd']
            )
            error_str += '---' + execstr
    return func, module, error_str
...
def auto_docstr(modname, funcname, verbose=True, moddir=None, modpath=None, **kwargs):
    ...
    if error_str is None:
        try:
            ...
            ).format(modname=modname, funcname=funcname, moddir=moddir)
...
def make_example_docstr(
    ...
    def append_dependant_argnames(argnames, dependant_argnames):
        ...
        for argname in argnames:
            ...
            if (
                argname not in dependant_argnames
                and argname not in argname_list
                and argname not in import_depends_map
            ):
    ...
    inferred_defaults = [
        find_arg_defaultrepr(argname, val) for argname, val in arg_val_iter
    ]
    argdef_lines = [
        '%s = %s' % (argname, inferrepr)
        for argname, inferrepr in zip(argname_list_, inferred_defaults)
    ]
    ...
    if result_print is not None:
        if return_name != 'result':
            ...
            result_line_fmt = (
                "result = ('{return_name} = %s' % (ut.repr2({return_name}),))"
            )
...
def make_default_docstr(
    ...
    if with_args and len(argname_list) > 0:
        argheader = 'Args'
        arg_docstr = make_args_docstr(
            argname_list, argtype_list, argdesc_list, ismethod, va_name, kw_name, kw_keys
        )
    ...
    if with_ret and return_header is not None:
        if return_header is not None:
            return_doctr = make_returns_or_yeilds_docstr(
    ...
    if with_example:
        ...
        examplecode = make_example_docstr(
            funcname, modname, argname_list, defaults, return_type, return_name, ismethod
        )
