...
def show_function_usage(fname, funcname_list, dpath_list):
    ...
    funcname_list = [
        r'\b%s\b' % (funcname.strip(),) for funcname in funcname_list if len(funcname) > 0
    ]
    flagged_funcnames = []
    for funcname in funcname_list:
        found_filestr_list, found_lines_list, found_lxs_list = ut.grep(
            [funcname], dpath_list=dpath_list
        )
        total = 0
