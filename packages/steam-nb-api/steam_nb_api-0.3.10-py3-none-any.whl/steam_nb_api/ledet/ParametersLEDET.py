...
class ParametersLEDET:
    ...
    def adjust_vQ(self, ROXIE_File):
        ...
        [vQ, l] = self.quenchPropagationVelocity(I, B, T_bath, cable)
    ...
    def checkTimes(self):
        ...
        t5 = self.Inputs.tQH
        t5 = [0 if x>=999 else x for x in t5]
        t5 = max(t5)
        t6 = self.Inputs.tCLIQ
        if t6 >= 999: t6 = 0
        t7 = self.Inputs.tEE
        ...
        if any(x > t4 for x in [t1, t2, t3, t5, t6, t7]):
            ...
            self.Options.time_vector_params[0] = np.max([t1, t2, t3, t5, t6, t7]) + 1
            self.Inputs.t_PC_LUT[0] = np.max([t1, t2, t3, t5, t6, t7]) + 1
        return 1
    ...
    def writeLEDETInputsNew(self, book, sheet, variableGroup, variableLabels, verbose: bool = False):
        ...
        for attribute in variableGroup.__annotations__:
            ...
            if currentRow in variableGroup.BlankRows[1]:
                ...
                sheet1.append([None])
            if isinstance(self.getAttribute(variableGroup, attribute), list):              # only check lists, single values and nparrays are fine already
                if len(self.getAttribute(variableGroup, attribute)) > 16382:         # column limit for excel is 16384, subtract 2 for description and variable name columns
                    self.setAttribute(variableGroup, attribute, np.array(self.getAttribute(variableGroup, attribute)).reshape(-1, 1))   # change list to np array and change it from row vector to column vector
            ...
            if varType == np.ndarray:
                if self.getAttribute(variableGroup, attribute).ndim > 1:
                    for i in range(np.max([self.getAttribute(variableGroup, attribute).shape])):
