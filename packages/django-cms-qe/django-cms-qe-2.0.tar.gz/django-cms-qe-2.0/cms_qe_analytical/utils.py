...
def get_required_setting(setting, value_re, invalid_msg):
    ...
    """
    try:
        value = getattr(constance_settings, setting)

    except AttributeError:
        raise AnalyticalException("%s setting: not found" % setting)  # pylint: disable=raise-missing-from
    if not value:
        raise AnalyticalNotSet("%s setting is not set" % setting)
    value = str(value)
...
class AnalyticalNotSet(Exception):
    silent_variable_failure = True
