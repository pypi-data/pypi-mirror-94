...
class KeyboardMavic2DJI(KeyboardControlledDrone):
    ...
    def send_msg(self, msg, emitter_devices: list):
        ...
        total_time = 0
        cpu_probe: ProcessProbe = self.probes['cpu'] if 'cpu' in self.probes else None
        get_time_now = lambda: cpu_probe.probe_alive_time.get_timestamp() if cpu_probe is not None else 0
    ...
    def run(self):
        ...
        while self.step(self.basic_time_step) != -1:
            ...
            # Send messages
            transmit_time = self.send_msg(StringGenerator.get_random_message(4), emitter_devices)

            # Receive messages
            received_messages, receive_time = self.receive_msgs(receiver_devices)

            flight_time = cpu_probe.probe_alive_time.get_timestamp()
            cpu_time = cpu_probe.cpu_time.get_timestamp()
            io_time = cpu_probe.io_time.get_timestamp()
