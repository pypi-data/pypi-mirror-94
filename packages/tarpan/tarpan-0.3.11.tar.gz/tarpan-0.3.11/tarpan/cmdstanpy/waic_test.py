...
def test_compare_waic():
    ...
    assert [round(model.waic_data.waic, 0) for model in result] == \
        [361, 402, 406]

    assert [round(model.waic_data.waic_std_err, 1) for model in result] == \
        [13.3, 10.8, 11.3]

    difference = [
        None if model.waic_difference_best is None
        else round(model.waic_difference_best, 0)
        for model in result
    ]

    assert difference == [None, 42, 45]

    std_err = [
        None if model.waic_difference_best_std_err is None
        else round(model.waic_difference_best_std_err, 0)
        for model in result
    ]

    assert std_err == [None, 10, 12]

    assert [round(model.waic_data.penalty, 0) for model in result] == \
        [3, 2, 2]

    actual_weight = [
        round(model.weight, 4)
        for model in result
    ]

    assert actual_weight == [0.9999, 0, 0.0001]
...
def test_save_compare_waic_csv():
    ...
    assert row["WAIC"] == approx(361.44, abs=1)
    assert row["SE"] == approx(13.33, rel=1e-3)
    assert np.isnan(row["dWAIC"])
    assert np.isnan(row["dSE"])
    assert row["pWAIC"] == approx(3.17, abs=0.2)
    assert row["Weight"] == approx(0.99985, rel=1e-3)

    row = df.loc["Itercept"]
    assert row["WAIC"] == approx(405.93, rel=1e-3)
    assert row["SE"] == approx(11.27, abs=0.2)
    assert row["dWAIC"] == approx(45.19, abs=2)
    assert row["dSE"] == approx(11.50, abs=2)
    assert row["pWAIC"] == approx(1.58, abs=0.1)
    assert row["Weight"] == approx(0.00007, abs=0.0003)
