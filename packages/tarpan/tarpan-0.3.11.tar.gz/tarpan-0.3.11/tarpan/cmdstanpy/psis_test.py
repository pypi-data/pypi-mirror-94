...
def test_psis():
    ...
    assert result.psis == approx(126.26, rel=1e-2)
    assert result.psis_std_err == approx(12.981, rel=1e-1)
    assert result.penalty == approx(3.8938, rel=1e-1)

    assert len(result.psis_pointwise) == 50
    assert result.psis_pointwise[0] == approx(4.1495, rel=1e-2)
    assert result.psis_pointwise[49] == approx(1.9286, rel=1e-2)

    assert len(result.pareto_k) == 50
    assert result.pareto_k[0] == approx(0.093890, abs=0.05)
    assert result.pareto_k[12] == approx(0.74203, rel=1e-1)
    assert result.pareto_k[49] == approx(0.13764, abs=0.3)
...
def test_compare_psis():
    ...
    assert [round(model.psis_data.psis, 0) for model in result] == \
        [126, 128, 140]

    assert [round(model.psis_data.psis_std_err, 0) for model in result] == \
        [13, 13, 10]

    difference = [
        None if model.psis_difference_best is None
        else round(model.psis_difference_best, 0)
        for model in result
    ]

    assert difference == [None, 2, 14]

    std_err = [
        None if model.psis_difference_best_std_err is None
        else round(model.psis_difference_best_std_err, 0)
        for model in result
    ]

    assert std_err == [None, 1, 9]

    assert [round(model.psis_data.penalty, 0) for model in result] == \
        [4, 5, 3]

    actual_largest_k = [
        round(model.largest_pareto_k, 1)
        for model in result
    ]

    assert actual_largest_k == [0.7, 0.8, 0.5]

    actual_weight = [
        round(model.weight, 1)
        for model in result
    ]

    assert actual_weight == [0.7, 0.2, 0.1]
...
def test_save_compare_psis_csv():
    ...
    assert row["PSIS"] == approx(126.25, rel=1e-1)
    assert row["SE"] == approx(12.981, rel=1e-1)
    assert np.isnan(row["dPSIS"])
    assert np.isnan(row["dSE"])
    assert row["pPSIS"] == approx(3.8938, rel=1e-1)
    assert row["MaxK"] == approx(0.74203, rel=1e-1)
    assert row["Weight"] == approx(0.5665, abs=0.2)

    row = df.loc["Divorse vs Age+Marriage"]
    assert row["PSIS"] == approx(128.06, abs=5)
    assert row["SE"] == approx(12.3822, abs=1)
    assert row["dPSIS"] == approx(2.41, abs=0.1)
    assert row["dSE"] == approx(0.88, abs=0.1)
    assert row["pPSIS"] == approx(5.07, abs=0.5)
    assert row["MaxK"] == approx(0.828, abs=0.3)
    assert row["Weight"] == approx(0.226, abs=0.1)
