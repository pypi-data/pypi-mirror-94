...
DIC_NAME_OFFSHORE = {
                   "Calvados": "Calv",
                   "Dunkerque Offshore": "Dunk",
                   "Fécamp Offshore": "Feca",
                   "Golfe du Lion": "Lion",
                   "Groix": "Groix",
                   "Gruissan": "Grui",
                   "Le Tréport": "Trep",
...
DIC_REGION = {
    ...
    "OFFSHORE": [str(x) for x in range(200,221)]
}
...
EOLES_ONSHORE = {"manufacturer" :"Vestas",
                 "turbine" : "V80/2000",
                 "hub_height" : 80}
EOLES_OFFSHORE = {"manufacturer" :"Siemens",
                 "turbine" : "SWT-4.0-130",
                 "hub_height" : 120}
...
def french_wind_turbine_analysis(year, root_vwf, root_county, mode="max", EOLES=False):
    ...
    extract_year = lambda x: int(x.split("/")[0]) if "/" in x else int(x)
    

    def lim_departement(dep,map_french):

        """
        parse the files ##-nom_departement.txt 
        where xx is the departement identifier (ex Isère ## = 38)
        
        Args:
            dep (string): county file name 
            map_french (object)
        ...
        x, y = map_french(lat, lon)
        return x, y, map_french(*[float(x) for x in lines[2].split(",")[0:-1]])
    
        
    def plot_french_wind_farm():
        ...
        for county_file in os.listdir(root_county): # list of french counties formatted as ##_CountyNane.txt
                                          #  ex 38_isere.txt
            insee_code_county = county_file[0:2]
            if (
                (insee_code_county != "2A") and # exclude Corse du Sud
                (insee_code_county != "2B") and # exclude Haute Corse
                int(insee_code_county) < 97     # exclude overseas counties
            ):  

                x, y, c = lim_departement(county_file,map_french)
                plt.plot(
                    x,
                    y,
                    color="k",
                ...
                try:  # the county is equipped with wind farm
                    total_power.append(d[insee_code_county])
                    prod = round(100 * d[insee_code_county] / french_total_power, 2)
                except:
                    total_power.append(0)
                    prod = 0

                ax = plt.gca()  # get current axes instance
                if int(insee_code_county) <= 90:
                    ax.text(
                        c[0],
                        c[1],
                        str(prod) + "%",
        ...
        for x_y_name in zip(x,y,[DIC_NAME_OFFSHORE[x] for x in w_offshore["Name"]]):
            plt.annotate(x_y_name[2],(x_y_name[0],x_y_name[1]))
            
        
        _ = plt.title(
                f"year = {year}, \
                        total power = {french_total_power/1.e6:.2f} Gw, \
                        number of farms = {len(wind_farm)}, \
                        median hub height = {median_hub_height:.2f} m  \
                        {per_cent_height:.1f}% of known hub heigth \n  \
                        selection mode : {mode},  number of equipped counties : {len(df_onshore)}"
                )
        plt.axis("off")
            ...
            if int(num_county) < 96:  # select the largest county wind farm
                ...
                if EOLES:  # we choose EOLES conditions
                    selected_farm[num_county] = (
                        ...
                        total_power,
                        EOLES_ONSHORE["manufacturer"] ,
                        EOLES_ONSHORE["turbine"],
                        EOLES_ONSHORE["hub_height"],
    ...
    # build onshore farms data frame df
    df_onshore = pd.DataFrame.from_dict(selected_farm).T
    df_onshore.columns = COL_NAMES
    
    hub_height = [float(x) for x in df_onshore["Hub height"] if x != "#ND"]
    per_cent_height = 100 * len(hub_height) / len(df_onshore["Hub height"])
    median_hub_height = np.median(hub_height)
    
    

    # build offshore farms data frame df_offshore
    farm_offshore = {}
    ...
    for idx in w_offshore.index:
        farm_offshore[w_offshore.loc[idx, "Name"]] = [
            ...
            EOLES_OFFSHORE["manufacturer"] ,
            EOLES_OFFSHORE["turbine"],
            EOLES_OFFSHORE["hub_height"],
        ]
        
    df_offshore = pd.DataFrame.from_dict(farm_offshore).T
    df_offshore.columns = COL_NAMES
    df_offshore.index = [str(x) for x in (200 + np.arange(0, len(df_offshore), 1))]
    
    fig = plt.figure(figsize=(15, 15))
    plot_french_wind_farm()
    plt.show()

    return df_onshore, median_hub_height, df_offshore
...
def plot_wind_pca(F, df, wind, year):
    ...
    '''Plot the PCA results (component PC2 versus component PC1)
    
    Args:
        F (nparray): wind matrix NxM (N county, M time samples)
        df (data frame): generated by
        F (nparray): PCA load matrix 
    ...
    idx_county = lambda county : re.findall("^\d{1,3}", county)[0]
    county_name = lambda county: county if int(county )<200  else DIC_OFFSHORE[county]
    ...
    for county in wind.columns:
        c.append(inv[idx_county(county)])   
  
    plt.figure(figsize=(15, 15))

    idx_dep = {idx_county(x): i for i, x in enumerate(wind.columns)}
    for region in DIC_REGION.keys():
        idx = [
            idx_dep[idx_county(x)] for x in wind.columns if idx_county(x) in DIC_REGION[region]]
            
        if len(idx): # there are countie in the region

            X = [F[i][0] for i in idx]
            Y = [F[i][1] for i in idx]
            size = [int(df.T.values[2][i] / 7000) for i in idx]
    ...
    if not txt_optimisation:
        for idx, sample in enumerate(wind.columns):
            plt.annotate(idx_county(sample), (F[:, 0][idx], F[:, 1][idx]), size=15, c=c[idx])
...
def vre_aggregation(year, root_outputs, root, new_turbine=None):
    ...
    #  if a new turbine is used than we compute the normalize power generated
    if new_turbine is not None:
        wind_speed = new_turbine.index.values
        power = new_turbine.values
    ...
    for idx, file in enumerate(os.listdir(root_outputs)):
        ...
        if len(sub_str):  # is a valid file found?
            ...
            if new_turbine is not None: # we use a new turbine
...
def vre_computation(year, vre_new, df, root_inputs_eoles, wind_farm_type="onshore"):
    ...
    '''Charge Factor computation for offshore/onshore wind turnbine
    ...
    Args:
        ...
        root_inputs_eoles (string) : full path of the dirctory containing the files generated by ninja
        
        wind_farm_type (string) : "onshore" or "offshore"
        
    Returns:
        VRE (nparray): the wind turbine charge factor versus time with an hourly resolution
