...
class lsq_lin_cvxopt:
    ...
    def __init__(self, A, b, A_ineq=None, b_ineq=None, A_eq=None, b_eq=None,
        ...
        sol = solvers.coneqp(P, q, A_ineq, b_ineq, None, A_eq, b_eq,
                             options={'show_progress': 0}, initvals=initvals)
...
class lsq_lin:
    ...
    The generazlization to linear constraints follows the ideas presented
    in Sec. 16.5 of Nocedal & Wright, 2006 "Numerical Optimization, 2nd ed".
    In addition I introduced an initialization phase which generalizes
    the one in my lsq_box procedure. This eliminates the need to solve a
    linear-programming problem to find a feasible initial point and also
    dramatically reduces the number of iterations in the main loop.
    ...
    def __init__(self, A, b, A_ineq=None, b_ineq=None, A_eq=None, b_eq=None,
        ...
        def solve_subproblem(free):
            ...
            bad = A_ineq[free] @ x > b_ineq[free] + eps[free]
            return x, bad
        ...
        # This initialization assumes the matrix defined by the free
        # inequality constraints, and possibly the additional equality
        # ones, is full rank, but it will stop if this is not the case
        bad = True
        while np.any(bad):  # Initialization loop
            free = A_ineq @ x < b_ineq - eps
            x, bad = solve_subproblem(free)
            if verbose:
                ita += 1
        ...
        while np.any(~free) and (chi2old - chi2new > ftol*chi2new):    # Loop A
            ...
            j = np.argmin(lam)
            if lam[j] < 0:
                free[np.flatnonzero(~free)[j]] = bad = True
                while np.any(bad):    # Loop B
                    z, bad = solve_subproblem(free)
                    if np.any(bad):
                        alpha = (b_ineq[free][bad] - A_ineq[free][bad] @ x)/(A_ineq[free][bad] @ (z - x))
                        k = np.argmin(alpha)
                        x += alpha[k]*(z - x)
                        free[np.flatnonzero(free)[bad][k]] = False
                        if verbose:
...
class lsq_box:
    ...
    I also included an initialization phase, which is analogue to that
            ...
            if wd[j] > 0:
                ...
                while np.any(bad):    # Loop B
                    ...
                    if np.any(bad):
                        ...
                        alpha = (bn[bad] - x[bad])/(z[bad] - x[bad])
                        k = np.argmin(alpha)
                        x[free] += alpha[k]*(z[free] - x[free])
                        p = np.flatnonzero(bad)[k]
...
class capfit:
    ...
    status : int
        The reason for algorithm termination:
         p   * -1 : improper input parameters status
    ...
    def __init__(self, func, p1, abs_step=None, bounds=(-np.inf, np.inf),
                 rcond=None, diff_step=1e-4, fixed=None, ftol=1e-4, max_nfev=None,
                 tied=None, A_ineq=None, b_ineq=None, A_eq=None, b_eq=None,
                 linear_method='cvxopt', verbose=0, x_scale='jac', xtol=1e-4,
                 args=(), kwargs={}):
        ...
        if not bounds_only:
            ...
            if A_ineq is not None:
                ...
                assert A_ineq.shape == (b_ineq.size, p1.size), "A_ineq/b_ineq size mismatch"
            ...
            if A_eq is not None:
                ...
                assert A_eq.shape == (b_eq.size, p1.size), "A_eq/b_eq size mismatch"
        ...
        while(True):
            ...
            if bounds_only:
                h = lsq_box(A, b, bounds=bounds-p1, rcond=rcond).x
            else:
                b_ineq_p = None if A_ineq is None else b_ineq - A_ineq @ p1
                b_eq_p = None if A_eq is None else b_eq - A_eq @ p1
                if linear_method == 'lsq_lin':
                    h = lsq_lin(A, b, A_ineq, b_ineq_p, A_eq, b_eq_p, bounds=bounds-p1, rcond=rcond).x
                else:
        ...
        self.cov, self.x_err = cov_err(J1, rcond=rcond)
