...
class Backend:
    ...
    def query(self, gid):
        return gid in self.association
...
class RomsmaniaBackend(Backend):
    ...
    def get_request(self, gid, cheaders):
        ...
        return requests.get(s.find_all(attrs={"class":"wait__link"})[0].get('href'), headers=cheaders, allow_redirects=True, verify=False, stream=True)
...
def search_action(args):
    ...
    rom_identifier = 0
    if args.all:
        backends = [backend() for backend in default_backends.values()]
    else:
        backends = [default_backends.get(args.backend)()]
    ...
    for row in table:
        if row[0] == ID:
            name = row[1]
            break
    downloader(backend, ID, args.output_directory, name)
...
def downloader(backend, gid, directory, filename):
    ...
    realname = requests.utils.unquote(r.url.split('/')[-1]) if not resume else filename
    total_length = r.headers.get('content-length')
    total_length = total_length and not resume and int(total_length) or 0 # when resuming content-length is not the filesize
