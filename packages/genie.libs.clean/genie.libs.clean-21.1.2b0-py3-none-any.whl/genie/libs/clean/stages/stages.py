...
@clean_schema({
    ...
    Optional('hash_timeout'): int
})
@aetest.test
def verify_running_image(section, steps, device, origin, hash_timeout=60):
    ...
    verify_running_image:
        origin:
            images (list): Image(s) from linux server that should be running
                on the device.

            hostname (str): Linux server from the testbed file that is used to
                generate the MD5 hashes.

        hash_timeout (int, optional): Maximum time in seconds allowed for the
    ...
    """
    try:
        server = device.api.convert_server_to_linux_device(origin['hostname'])
    except AttributeError:
        section.failed("The hostname '{}' provided does not exist in the "
                       "testbed servers block".format(origin['hostname']))
    ...
    with steps.start("Generate the MD5 hash of the image(s) on {}"
        ...
        try:
            ...
            step.failed("Failed to connect to {}.".format(origin['hostname']))
