...
class InfEngineServerEngine:
    ...
    def clean(self, ignore: Path, dl_limit: int = 1):
        """ Delete useless data. """

        self._logger.info(f"Cleaning InfServer Engine...")

        count = self._clean_multidb(
            self._engine_dir / INF_ENGINE_PATH_DLDATA, ignore, dl_limit
        )

        if count:
            self._logger.info(f"Deleted {count} DLData.")

    def _clean_multidb(self, directory: Path, ignore: Path, limit: int = 0) -> int:
