...
def test_workflow_file_system_globus_path(workflow_file_system):
    """
    Given: An instance of WorkflowFileSystem
    ...
    """
    wkflow_fs, rrun_id, custom_base_path = workflow_file_system
    assert wkflow_fs.globus_path() == Path(f"{rrun_id}/")
    assert wkflow_fs.globus_path("foo.txt") == Path(f"{rrun_id}/foo.txt")
