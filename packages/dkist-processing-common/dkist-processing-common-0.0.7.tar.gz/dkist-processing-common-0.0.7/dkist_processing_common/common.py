...
class TransferInputData(SupportTaskBase):
    ...
    def run(self) -> None:
        ...
        submit_globus_transfer(
            ...
            destination_files=[
                self._scratch.globus_path("input", source_file.name) for source_file in source_files
            ],
...
class TagInputData(SupportTaskBase):
    ...
    def run(self) -> None:
        input_path = self._scratch.workflow_base_path / "input"
        input_file_paths = input_path.glob("*.fits")
        logging.info(f"Tagging {len(input_file_paths)} files")
        for f in input_file_paths:
            logging.info(f"Tagging file {f.resolve()}")
            header = fits.open(str(f))[0].header
            self._scratch.tag(str(f.resolve()), "INPUT")
            self._scratch.tag(str(f.resolve()), header["DKIST004"].upper())
...
class TransferOutputData(SupportTaskBase):
    ...
    def run(self) -> None:
        ...
        submit_globus_transfer(
            source_files=self.output_paths,
