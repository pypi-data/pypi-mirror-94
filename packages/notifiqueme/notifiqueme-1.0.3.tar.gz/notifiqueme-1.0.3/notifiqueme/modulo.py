...
class Notification:
	...
	def Send(self, number, msg, notificationType):
		...
		response = self.connection.getresponse()
		if(response.closed):
			print("Conexão com servidor estava fechada, porem foi reaberta automaticamente")
			self.Authenticate() #Abri uma nova conexão
			Send(number, msg, notificationType) #reenvia a mensagem
