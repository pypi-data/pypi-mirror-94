...
from waflib import Utils, Context, Logs, Errors, Build
from waflib.Build import BuildContext, InstallContext
import wtools
from wtools import util
...
def options(opt):
    ...
    opt.add_option('--make-install', dest='make_install', default=False, action='store_true', help='export "make install" as default')
...
class MakeFileContext(InstallContext):
...
def export(bld):
    ...
    root = MakeRoot(bld)
    root.make_install = True if bld.options.make_install else False
    targets = wtools.util.get_targets(bld)
    
    for group in bld.groups:
        for tg in group:
            if "install_task" not in tg.features:
                continue

            for t in tg.tasks:
                if isinstance(t, Build.inst):
                    if t.type == "symlink_as":
                        linkname = t.outputs[0].abspath()
                        target = os.path.join(os.path.dirname(linkname), t.install_from)
                        root.add_symlink((target,linkname))

                    if t.type == "install_as":
                        files = zip(t.inputs, t.outputs)
                        root.add_files(files)
                        
                    if t.type == "install_files":
                        files = zip(t.inputs, t.outputs)
                        root.add_files(files)
...
class Make(object):
    ...
    def populate(self, content):
        ...
        s = re.sub('==VERSION==', wtools.version, s)
        return s
...
class MakeRoot(Make):
    ...
    def get_content(self):
        cmd = 'install' if self.make_install else 'build'
        s = MAKEFILE_ROOT
        ...
        s = re.sub('==ARCH==', 'ARCH?=%s' % platform.machine(), s)
        s = re.sub('==MODULES==', self.get_modules(), s)
        s = re.sub('==MODPATHS==', self.get_modpaths(), s)
        s = re.sub('==MODDEPS==', self.get_moddeps(), s)
        s = re.sub('==INSTALL_MKDIRS==', self.get_mkdirs(), s)
        s = re.sub('==INSTALL_FILES==', self.get_install_files(), s)
        s = re.sub('==INSTALL_SYMLINKS==', self.get_symlinks(), s)
        s = re.sub('==DEFAULT_COMMAND==', cmd, s)
        return s

    def add_child(self, child):
        self.childs.append(child)

    '''list of tuples (src,dst)'''
    def add_files(self, files):
        self.files.extend(files)

    '''symlink tuple (target,linkname)'''
    ...
    def get_mkdirs(self):
        dirs = []
        if len(self.files):
            dirs = list([os.path.dirname(d.abspath()) for d in list(zip(*self.files))[1]])
        if len(self.symlinks):
            dirs.extend([os.path.dirname(d) for d in list(zip(*self.symlinks))[0]])
        if not len(dirs):
            return ''
    ...
    def get_symlinks(self):
        if not len(self.symlinks):
            return ''
        s = '\n'
        for (target,linkname) in self.symlinks:
            s += '\tln -s -r -f %s %s\n' % (target, linkname)
        s = s.rstrip('\n')
        return s
...
class MakeTarget(Make):
    def __init__(self, bld):
        super(MakeTarget, self).__init__(bld)

    def get_name(self):
        name = self.bld.path.relpath().replace('\\', '/')
        name += '/target_%s.mk' % self.bld.env.DEST_CPU
        return name
    ...
    def get_libpaths(self):
        ...
        libdir = self.bld.env.LIBDIR
        libpaths = [libdir]
        if libdir.endswith("64"):
            libpaths.append(libdir.rstrip('64'))
        libpaths = libpaths + self.bld.env.LIBPATH
        
        for lpath in libpaths:
            lp = os.path.realpath(lpath)
                    
            if lp.startswith(home):
                lp = lp.replace(home, "$(HOME)", 1)
            lpaths.append(lp)

        for tgen in self.bld.task_gen_cache_names.values():
            if set(('cshlib', 'cxxshlib')) & set(tgen.features):
                for output in tgen.link_task.outputs:
                    lpaths.append("$(OUT)/%s" % os.path.dirname(output.relpath()))
    ...
    def get_content(self):
        ...
        home = expanduser("~")
        includes = []
        for include in Utils.to_list(bld.env.INCLUDES):
            if include.startswith(home):
                include = include.replace(home, "$(HOME)", 1)
            includes.append(include)
    
        defines = []
        defs = list(set(bld.env.DEFINES))
        for d in defs:
            if d.count('"') == 2:
        ...
        s = MAKEFILE_TARGET
        s = super(MakeTarget, self).populate(s)
        #s = re.sub('OUT:=', 'OUT:=%s' % out, s) # TODO: use static 'build' path for now
        s = re.sub('BINDIR:=', 'BINDIR:=%s' % str(bld.env.BINDIR).replace('\\', '/'), s)
        s = re.sub('LIBPATH:=', 'LIBPATH:=%s' % self.get_libpaths(), s)
        ...
        s = re.sub('INCLUDES:=', 'INCLUDES:=%s' % ' '.join(includes), s)
...
class MakeChild(Make):
    ...
    def get_name(self):
        name = '%s/%s.mk' % (self.gen.path.relpath(), self.gen.get_name())
        return name.replace('\\', '/')
    ...
    def get_content(self):
        ...
        loc = getattr(Context.g_module, Context.OUT)
    ...
    def cshlib(self, bld, gen):
        ...
        v = vnum.split(".")
        s = re.sub('VMAJOR=', 'VMAJOR=%s' % v[0], s)
        if len(v) >= 2:
            s = re.sub('VMINOR=', 'VMINOR=%s' % v[1], s)
        if len(v) >= 3:
            s = re.sub('VPATCH=', 'VPATCH=%s' % v[2], s)
...
ifneq ("$(wildcard target_$(ARCH).mk)","")
 $(info --> using target architecture '$(ARCH)')
 include target_$(ARCH).mk
else
 $(error target architecture '$(ARCH)' not available)
endif
...
install: build $(install_targets)==INSTALL_MKDIRS====INSTALL_FILES====INSTALL_SYMLINKS==
...
MAKEFILE_CPROGRAM = \
'''#------------------------------------------------------------------------------
# WAFTOOLS generated makefile (CPROGRAM)
# version: ==VERSION==
# waf: ==WAFVERSION==
#------------------------------------------------------------------------------
...
MAKEFILE_CXXPROGRAM = \
'''#------------------------------------------------------------------------------
# WAFTOOLS generated makefile (CXXPROGRAM)
# version: ==VERSION==
# waf: ==WAFVERSION==
#------------------------------------------------------------------------------
...
MAKEFILE_CSTLIB = \
'''#------------------------------------------------------------------------------
# WAFTOOLS generated makefile (CSTLIB)
# version: ==VERSION==
# waf: ==WAFVERSION==
#------------------------------------------------------------------------------
...
MAKEFILE_CSHLIB = \
'''#------------------------------------------------------------------------------
# WAFTOOLS generated makefile (CSHLIB)
# version: ==VERSION==
# waf: ==WAFVERSION==
#------------------------------------------------------------------------------
...
install: build
	mkdir -p $(LIBDIR)
	cp $(OUTPUT) $(LIBDIR)/$(LIBNAME)
ifneq ($(VNUM),)
	ln -s -f -r $(LIBDIR)/$(LIBNAME) $(LIBDIR)/$(LIB)
endif
ifneq ($(VMAJOR),)
	ln -s -f -r $(LIBDIR)/$(LIBNAME) $(LIBDIR)/$(LIB).$(VMAJOR)
ifneq ($(VMINOR),)
	ln -s -f -r $(LIBDIR)/$(LIBNAME) $(LIBDIR)/$(LIB).$(VMAJOR).$(VMINOR)
endif
endif
...
ifneq ($(VMAJOR),)
	rm -f $(LIBDIR)/$(LIB).$(VMAJOR)
ifneq ($(VMINOR),)
	rm -f $(LIBDIR)/$(LIB).$(VMAJOR).$(VMINOR)
endif
endif
	rm -f $(LIBDIR)/$(LIB)
...
MAKEFILE_CXXSHLIB = \
'''#------------------------------------------------------------------------------
# WAFTOOLS generated makefile (CXXSHLIB)
# version: ==VERSION==
# waf: ==WAFVERSION==
#------------------------------------------------------------------------------
...
MAKEFILE_CXXSTLIB = \
'''#------------------------------------------------------------------------------
# WAFTOOLS generated makefile (CXXSTLIB)
# version: ==VERSION==
# waf: ==WAFVERSION==
#------------------------------------------------------------------------------
