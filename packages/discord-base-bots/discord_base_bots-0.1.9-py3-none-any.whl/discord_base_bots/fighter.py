...
class Fighter(BaseBot):
    ...
    async def __send_fight_data(self, data, message):
        ...
        await self.__send_message(string, delay=0)

    async def __send_ref_data(self, data, message):
        await self.__send_message(f"refbot {data}", delay=0)
    ...
    async def __read_reaction(self, reaction, message):
        ...
        if confirmation and isinstance(confirmation, str):
            ...
            await self.__send_message(confirmation)
        await self.__send_message(f"Health: {self.current_health}, Endurance: {self.current_endurance}")
        await self.__send_ref_data(f"bow {self.opponent}", message, delay=0)
    ...
    async def __process_end_fight_statements(self, data, message):
        ...
        for statement in statements:
            await self.__send_message(statement)
