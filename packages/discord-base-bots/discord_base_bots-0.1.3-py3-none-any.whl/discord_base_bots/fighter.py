...
class Fighter(BaseBot):
    ...
    async def execute_fight(self):
        ...
        if self.current_health <= 0:
            await self.on_loss(data, message)
        if self.current_endurance <= 0:
            await self.on_timeout(data, message)
        if self.__victory:
    ...
    #
    #############################
    async def on_victory(self, data, message):
        await message.channel.send("Victory is mine")
        await message.channel.send(f"void sacrifice {self.opponent}")

    async def on_loss(self, data, message):
        await message.channel.send("I have failed")

    async def on_timeout(self, data, message):
        await message.channel.send("I can't go on")
