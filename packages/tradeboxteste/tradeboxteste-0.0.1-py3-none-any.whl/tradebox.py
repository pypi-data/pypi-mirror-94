...
class TradeBox(gym.Env):
	def __init__(self, filename):
		...
		self.low = np.array(
		    [self.pre_min, self.qtd_min, self.csv_file['Abertura'].min() ,self.min_pre_min], dtype=np.float32
		)
		self.high = np.array(
		    [self.pre_max, self.qtd_max, self.csv_file['Abertura'].max() ,self.max_pre_max], dtype=np.float32
		)
