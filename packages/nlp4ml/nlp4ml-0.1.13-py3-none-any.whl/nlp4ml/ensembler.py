...
def nested_cross_validation(model, 
	...
	for fold in range(inner_n_splits):
		...
		print(f"Fold {fold}")
		print("Accuracy={:.4f}\tF1={:.4f}\tPrecision={:.4f}\Recall={:.4f}".format(
			acc, f1, precision, recall))
		print("Best Score={:.4f}\tParams={}".format(
			result.best_score_, result.best_params))
	print('Accuracy: %.3f (%.3f)' % (mean(results["acc"]), std(results["acc"])))