...
class GUI(object):
    ...
    def update(self):
        try:
            ...
            # header GUI
            headerGUI = self.kw.get('headerGUI', '')
            ...
            # footer GUI
            footerGUI = self.kw.get('footerGUI', '')
            ...
            def configure_size():
                ...
                GUIwidth = max([taskGUIframeInterior.winfo_reqwidth(), top.winfo_width() - yscrollbar.winfo_reqwidth() - prefFrame.winfo_reqwidth()])
                canvas.itemconfigure(interior_id,
                                     width=GUIwidth,
                                     height=taskGUIframeInterior.winfo_reqheight())
                canvas.configure(width=GUIwidth,
                                 height=GUIheight)
                top.configure(width=GUIwidth + yscrollbar.winfo_reqwidth() + prefFrame.winfo_reqwidth(),
                              height=GUIheight)
...
def _setDefault(location, default=special1):
    ...
    if default is not special1 and not (not isinstance(location, str) and np.iterable(location) and np.iterable(default) and len(default) and next(default.__iter__()) is special1):
        updated = False
...
def _Entry(parent, location, lbl=None, updateGUI=False, help='', preentry=None, postcommand=None, check=special1,
           collect=False, reveal_location=None, **kw):
    r"""
...
def _helpButton(master, help):
    if help:
        ttk.Button(master, text='?', command=lambda help=help:helpTip.showtip(help,move=False,strip=True), style='flat.TButton', takefocus=False).pack(side=tk.LEFT,expand=tk.NO,fill=tk.NONE,padx=0,pady=0)

def _urlButton(master, url):
    if url:
        if '://' not in url:
            url='http://'+url
        ttk.Button(master, text='w', command=lambda url=url:openInBrowser(url), takefocus=False, style='flat.TButton').pack(side=tk.LEFT,expand=tk.NO,fill=tk.NONE,padx=0,pady=0)
...
def CompoundGUI(pythonFile, title=None, **kw):
    ...
    tmp_parentGUI = _aux['parentGUI'], _aux['notebook'], _aux['tab_name'], _aux['tab_list'], _aux['same_row'], _aux['packing'],_aux['compoundGUIid']
    ...
    try:
        ...
        _aux['parentGUI'], _aux['notebook'], _aux['tab_name'], _aux['tab_list'], _aux['same_row'], _aux['packing'], _aux['compoundGUIid'] = tmp_parentGUI
...
@_available_to_userGUI
def Entry(location, lbl=None, comment=None, updateGUI=False, help='', preentry=None, postcommand=None, check=special1,
          multiline=False, norm=None, default=special1, delete_if_default=False, url='', kwlabel={}, **kw):
    """
    ...
    if not Lock(location,checkLock=True):
        ...
        if default is not special1:
            def writeToEntry():
                ...
                e.grab_release()
            ttk.Button(frm, text=['d','D'][not default_at_import], command=writeToEntry, style='flat.TButton',
                       takefocus=False, state=kw.get('state', 'enabled')
...
@_available_to_userGUI
def ComboBox(location, options, lbl=None, comment=None, updateGUI=False, state='readonly', help='', postcommand=None,
             check=special1, default=special1, url='', kwlabel={}, **kw):
    """
    ...
    comboBox = Combobox(frm, state=state if state!='search' else 'normal', values=tuple([k.strip('*') for k in list(options.keys())]),takefocus=False,**kw)
    ...
    if Lock(location,checkLock=True):
        ...
        if default is not special1:
            def writeToEntry():
                ...
                checkKeyPressed()
            ttk.Button(
                frm, text=['d','D'][not default_at_import], command=writeToEntry, style='flat.TButton',
                takefocus=False,state=['enabled','disabled'][state=='disabled']
                ).pack(side=tk.LEFT,expand=tk.NO,fill=tk.NONE,padx=0,pady=0)
...
def Label(lbl, align='center', **kw):
    ...
    def robust_configure_size(event=None, label=label,frm_top=frm_top):
        try:
            ...
            pass
    _aux['configure_size'].append( ( frm_top , lambda event=None,label=label,frm_top=frm_top: robust_configure_size(event=None, label=label, frm_top=frm_top)) )
    return label
...
@_available_to_userGUI
def FilePicker(location, lbl=None, comment=None, updateGUI=False, help='', postcommand=None,
               localRemote=True, transferRemoteFile=True, directory=False, action='open', tree=True,
               default=special1, url='', kwlabel={}, init_directory_location=None, init_pattern_location=None,
               favorite_list_location=None, pattern_list_location=None, reveal_location=None, **kw):
    r"""
    ...
    def askRemote():
        ...
        tmp = remoteFile(_aux['parentGUI'], transferRemoteFile, remoteFilename=remoteFilename, server=server, tunnel=tunnel,
                         init_directory_location=init_directory_location, init_pattern_location=init_pattern_location, favorite_list_location=favorite_list_location,
    ...
    e, value = _Entry(frm, location=location, lbl=lbl, updateGUI=updateGUI, help=help, postcommand=postcommand, reveal_location=reveal_location, **kw)
    e.icursor(tk.END)
    e.xview(tk.END)
    e.config(width=30)

    bttTree = ttk.Button(frm, text='Tree', takefocus=False)
    if tree and (not localRemote or localRemote is True or localRemote == 'local' or (localRemote == 'remote' and transferRemoteFile is not False)):
    ...
    if Lock(location, checkLock=True):
        ...
        if default is not special1:
            ...
            ttk.Button(frm, text=['d', 'D'][not default_at_import], command=writeToEntry, style='flat.TButton', takefocus=False).pack(side=tk.LEFT, expand=tk.NO, fill=tk.NONE, padx=0, pady=0)
...
@_available_to_userGUI
def ObjectPicker(location, lbl=None, objectType=None, objectKW={}, postcommand=None, unset_postcommand=None, kwlabel={},
                 init_directory_location=None, init_pattern_location=None, favorite_list_location=None, pattern_list_location=None,
                 **kw):
    r"""
    ...
    if what in where:
        ...
        return FilePicker(locationScratch, "Pick " + lbl, postcommand=set, updateGUI=True, kwlabel=kwlabel,
...
@_available_to_userGUI
def TreeLocationPicker(location, lbl=None, comment=None, kwlabel={}, default=special1, help='', url='', updateGUI=False, postcommand=None, check=None, base=None, **kw):
    r"""
...
@_available_to_userGUI
def CheckBox(location, lbl=None, comment=None, useInt=False, mapFalseTrue=[], updateGUI=False, help='', postcommand=None,
             default=special1, url='', kwlabel={}, **kw):
    r"""
    ...
    if Lock(location,checkLock=True):
        ...
        if default is not special1:
            def writeToEntry():
                ...
                if updateGUI:
                    OMFITaux['rootGUI'].event_generate("<<update_GUI>>")
            ttk.Button(frm, text=['d','D'][not default_at_import], command=writeToEntry, style='flat.TButton', takefocus=False,
...
@_available_to_userGUI
def ListEditor(location, options, lbl=None, default=None, unique=True, ordered=True, updateGUI=False,
               postcommand=None, only_valid_options=False, help='', url='', show_delete_button=False, max=None):
    '''
...
@_available_to_userGUI
def Slider(location, start_stop_step, lbl=None, comment=None, digits=None, updateGUI=False, help='', preentry=None, postcommand=None,
           norm=None, default=special1, url='', kwlabel={}, **kw):
    """
    ...
    e = TtkScale(frm, from_=start_stop_step[0], to=start_stop_step[1], tickinterval=start_stop_step[2], value=eval(location), digits=digits, command=command, **kw)
    #e.after(500, command)
    e.pack(side=tk.LEFT, expand=tk.YES, fill=tk.X)
...
@_available_to_userGUI
def ShotTimeDevice(postcommand=None,
                   showDevice=True, showShot=True, showTime=True, showRunID=False,
                   multiShots=False, multiTimes=False, showSingleTime=False,
                   checkDevice=None, checkShot=None, checkTime=None, checkRunID=None,
                   subMillisecondTime=False,
                   stopIfNotSet=True, updateGUI=True):
    '''
    ...
    if not np.any([True if np.iterable(showDevice) else showDevice,
    ...
    if np.iterable(showDevice) or showDevice:
        ...
        ComboBox(exp['device_location'] + "['device']", deviceList, 'Device', state=tk.NORMAL, updateGUI=updateGUI, postcommand=postcommand_mainsettings, check=checkDeviceFunction)
    ...
    if np.iterable(showShot) or showShot:
        if not multiShots:
            ...
            ComboBox(exp['shot_location'] + "['shot']", shotList, 'Shot', state=tk.NORMAL, updateGUI=updateGUI, postcommand=postcommand_mainsettings, check=checkShotFunction)
            ...
            Entry(exp['shots_location'] + "['shots']", 'Shots', updateGUI=updateGUI, postcommand=postcommandArray, check=checkShotsFunction, multiline=True)
    ...
    if np.iterable(showTime) or showTime:
        if multiTimes:
            ...
            Entry(exp['times_location'] + "['times']", 'Times' + ' [ms]',
                  updateGUI=updateGUI, postcommand=postcommandArray, check=checkTimesFunction,
                  multiline=True, help='Enter list of monotonically increasing times for analysis.\nAccepts python statements.\n'
        ...
        if (multiTimes and showSingleTime) or not multiTimes:
            ...
            ComboBox(exp['time_location'] + "['time']", timeList, 'Time' + ' [ms]', state=tk.NORMAL, updateGUI=updateGUI, postcommand=postcommand_mainsettings, check=checkTimeFunction)
            set_values['time'] = eval(exp['time_location'] + "['time']")

    if showRunID:
        Entry(exp['runid_location'] + "['runid']", 'run-ID', updateGUI=updateGUI, postcommand=postcommand_mainsettings, check=is_alphanumeric)
    ...
    def linkUnlinkSettings(linked):
        if linked == 'linked':
            ...
            for item in ['device', 'shot', 'time', 'shots', 'times', 'runid']:
                root['SETTINGS']['EXPERIMENT'][item] = OMFITexpression("""try:
...
""" % (item, item))
    ...
    b._ntimage = im
    b.pack(side=[tk.LEFT, tk.TOP][int(np.sum([np.iterable(showShot) or showShot,
                                           np.iterable(showTime) or showTime,
                                           np.iterable(showDevice) or showDevice,
                                           showRunID]) > 2)], expand=tk.NO, fill=tk.NONE)

    im = tk.PhotoImage(master=frm1, file=os.path.join(OMFITsrc, 'extras', 'graphics', 'bookmark.ppm'))
    b = ttk.Button(master=frm1, text='bookmark', image=im, command=lambda device=exp['device'], shot=exp['shot'], time=exp['time']: addBookmarksWithDescription(exp['device'], exp['shot'], exp['time']), takefocus=False)
    b._ntimage = im
    b.pack(side=[tk.LEFT, tk.TOP][int(np.sum([np.iterable(showShot) or showShot,
                                           np.iterable(showTime) or showTime,
                                           np.iterable(showDevice) or showDevice,
...
def Figure(toolbar=True, returnFigure=False, fillX=False, **kw):
    ...
    kw.setdefault('figsize',(5*2./3., 4*2./3.))
    f=pyplot.Figure(**kw)
        ...
        def showTools():
            try:
                ...
                if root['__scratch__']['showSettings']:
                    ...
                    ComboBox("root['SETTINGS']['REMOTE_SETUP']['serverPicker']",list(SERVER.listServers().keys()),'server',updateGUI=True)
                    try:
                ...
                if root['__scratch__']['showStorage']:
                    ...
                    def next_runid():
                        ...
                        while root['SETTINGS']['EXPERIMENT']['runid'] in root['__STORAGE__'].keys(filter=hide_ptrn):
                            ...
                            if len(index):
                                root['SETTINGS']['EXPERIMENT']['runid'] =re.sub(index[-1]+'$',str(int(index[-1])+1),root['SETTINGS']['EXPERIMENT']['runid'])
                            else:
                    ...
                    def reload_comment(location=None):
                        if root['SETTINGS']['EXPERIMENT']['runid'] in root['__STORAGE__'].keys(filter=hide_ptrn):
                            root['SETTINGS']['EXPERIMENT']['comment']=root['__STORAGE__'][root['SETTINGS']['EXPERIMENT']['runid']]['comment']
                    ...
                    ComboBox("root['SETTINGS']['EXPERIMENT']['runid']",runs,'Run-ID',state='readonly',check=is_string,default='',updateGUI=True,postcommand=reload_comment)
                    ...
                    if '__STORAGE__' not in root or root['SETTINGS']['EXPERIMENT']['runid'] not in root['__STORAGE__'].keys(filter=hide_ptrn):
...
def EditASCIIobject(location, lbl=None, comment=None, updateGUI=False, help='', postcommand=None, url='', **kw):
    ...
    def showText():
        ...
        _Text(parent=frm, location=scratchLocation, lbl=lbl, updateGUI=updateGUI, help=help, preentry=None, postcommand=onDone, percolator=filename.endswith('.py'), reveal_location=reveal_location, **kw)
...
class FileDialog(object):
    ...
    def __init__(self, directory=None, serverPicker='', server='localhost', tunnel='', pattern='*', default='', master=None,
                 lockServer=False, focus='filterDirs', favorite_list_location=None, pattern_list_location=None,
                 is_dir=False, title='File Browser'):
        ...
        def tab_complete():
            ...
            if os.path.abspath(os.path.split(filter_get)[0])!=os.path.abspath(selection_get) and os.path.abspath(filter_get)!=os.path.abspath(selection_get):
        ...
        if pattern_list_location is None:
            ...
            self.files = tk.Listbox(frm, exportselection=0,
            ...
            self.add_to_favorite_patterns = ttk.Button(frma,text="+", command= lambda: self.manage_list(self.pattern_list,self.filterFiles,'+'))
            self.add_to_favorite_patterns.pack(side=tk.LEFT)
            # Add remove_from_favorite_patterns button
            self.remove_from_favorite_patterns = ttk.Button(frma,text="-",command= lambda: self.manage_list(self.pattern_list,self.filterFiles,'-'))
            self.remove_from_favorite_patterns.pack(side=tk.LEFT)

            # add the files list box in frame b
            self.files = tk.Listbox(frmb, exportselection=0,
                                    yscrollcommand=(self.filesbar, 'set'))
            self.files.pack(side=tk.RIGHT, expand=tk.YES, fill=tk.BOTH)
        ...
        if favorite_list_location is None:
            ...
            self.dirs = tk.Listbox(frm, exportselection=0,
            ...
            self.add_to_favorite_dirs = ttk.Button(frma,text="+",command = lambda: self.manage_list(self.favorite_list,self.filterDirs,'+'))
            self.add_to_favorite_dirs.pack(side=tk.LEFT)
            # Add remove_from_favorite_dirs button
            self.remove_from_favorite_dirs = ttk.Button(frma,text="-",command= lambda: self.manage_list(self.favorite_list,self.filterDirs,'-'))
            self.remove_from_favorite_dirs.pack(side=tk.LEFT)

            # add the directory list
            self.dirs = tk.Listbox(frmb, exportselection=0,
                                   yscrollcommand=(self.dirsbar, 'set'))
            self.dirs.pack(side=tk.BOTTOM, expand=tk.YES, fill=tk.BOTH)
            # bind events for directory selection
            btags = self.dirs.bindtags()
    ...
    def remote_command(self, command):
        ...
        def ssh_cd(inv):
            if is_localhost(self.server0):
                return inv
            return sshOptions() + controlmaster(self.username, self.server, self.port, self.server0) + " -Y -q -p " + self.port + " " + self.username + "@" + self.server + " '" + inv + "'"
...
class SaveFileDialog(FileDialog):
    ...
    def ok_command(self):
        ...
        try:
            ...
            answer = Dialog('Overwrite %s ?' % file,
                            answers=['Yes', 'No'],
                            icon='question',
                            title='File exists',
                            parent=self.top)
...
def remoteFile(parent=None, transferRemoteFile=True, remoteFilename=None, server=None, tunnel=None,
               init_directory_location=None, init_pattern_location=None, favorite_list_location=None,
               pattern_list_location=None, is_dir=False):
    '''
    ...
    if transferRemoteFile:
        fd = LoadFileDialog(directory=directory, serverPicker=serverPicker, server=server, tunnel=tunnel, pattern=pattern, master=parent, default=default,
                            transferRemoteFile=transferRemoteFile,favorite_list_location=favorite_list_location,pattern_list_location=pattern_list_location,
                            is_dir=is_dir)
    else:
        fd = FileDialog(directory=directory, serverPicker=None, server=server, tunnel=tunnel, pattern=pattern, master=parent, default=default,
                        favorite_list_location=favorite_list_location,pattern_list_location=pattern_list_location,
                        is_dir=is_dir)
...
def remote_sysinfo(server, tunnel='', quiet=False):
    ...
    def ssh_cd(inv):
        if is_localhost(server0):
            return inv
        return sshOptions() + controlmaster(username, server, port, server0) + " -t -t -Y -q -p " + port + " " + username + "@" + server + " '" + inv + "'"

    id_ = omfit_hash(str(username + server + port))
    if id_ in OMFITaux['sysinfo'] and len(set(['id', 'sysinfo', 'shell', 'login', 'logout']).difference(set(OMFITaux['sysinfo'][id_].keys()))):
...
'''.strip()
        ...
        if not len(std_out):
            raise Exception(('Could not reach {username}@{server}:{port}\n\n'
                             '   Possible causes:\n'
        ...
        if len(login):
            ...
            std_out = []
            std_err = []
            _system(ssh_cd('\\ls -1 ' + ' '.join(login)), message='Collecting remote system info', ignoreReturnCode=True, std_out=std_out, std_err=std_err, quiet=quiet)
            login_ok = []
            for line in std_out + std_err:
                line = line.strip()
                if len(line) and 'cannot' not in line.lower() and 'no such file or directory' not in line.lower() and ': ' not in line.lower():
        ...
        try:
            ...
            if serverPicker in list(SERVER.keys()) and 'workDir' in SERVER[serverPicker] and isinstance(SERVER[serverPicker]['workDir'], str) and '$' in SERVER[serverPicker]['workDir']:
...
def manage_user_errors(command, reportUsersErrorByEmail=False, **kw):
    ...
    try:
        ...
        if (not isinstance(handled_exception,(doNotReportException,SyntaxError)) and
                reportUsersErrorByEmail and OMFIT['MainSettings']['SETUP']['error_report'] and
                OMFIT['MainSettings']['SETUP']['email'] not in developers and
                not lastSeen and
                os.environ['USER'] not in creator):
            try:
                ...
                message=('OMFIT project: {project}\n'
                         ...
                         '====================================\n'
                         '{exception}\n\n'
                         ''.format( project    = str(OMFIT.filename),
                                    directory  = OMFITsrc,
                                    commit     = repo_str,
...
def _system(command_line,
    ...
    if os.name == 'nt' or int(os.environ.get('OMFIT_WINDOWS_SYSTEM','0')):
        return _system_windows(command_line=command_line,
                               message=message,
        ...
        return _system_unix(command_line=command_line,
                            message=message,
                            fixedWidthDetails=fixedWidthDetails,
                            ignoreReturnCode=ignoreReturnCode,
                            std_out=std_out,
...
def _system_unix(command_line, message, fixedWidthDetails, ignoreReturnCode, std_out, std_err, executable, quiet, progressFunction, extraButtons, noGUI):
    import fcntl
    import select
    ...
    child = subprocess.Popen(command_line, shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=True, executable=executable, preexec_fn=os.setsid)
    ...
    if len(OMFITaux['prun_process']) or noGUI or (len(OMFITaux['pythonRunWindows']) and OMFITaux['pythonRunWindows'][-1] is None):
        ...
        if OMFITaux['rootGUI'] is not None:
            ...
            if fixedWidthDetails:
                from utils_widgets import OMFITfont
                fixedWidthDetailsText = tk.Text(top, background=ttk.Style().lookup('TFrame', 'background'), wrap=tk.NONE, font=OMFITfont(size=-2, family='Courier'))
                fixedWidthDetailsText.pack(padx=5)
                fixedWidthDetailsText.insert(1.0, tkDetails.get())
                fixedWidthDetailsText.configure(state='disabled', height=min([10, len(tkDetails.get().split('\n'))]), width=80, relief=tk.FLAT)
            if progressFunction:
            ...
            for btt in list(extraButtons.keys()):
                ttk.Button(frm, text=btt, command=lambda btt=btt: extraButtons[btt](gbl), takefocus=False).pack(side=tk.LEFT, expand=tk.NO, fill=tk.X)
...
def _system_windows(command_line, message, fixedWidthDetails, ignoreReturnCode, std_out, std_err, executable, quiet, progressFunction, extraButtons, noGUI):
    if 'DYLD_LIBRARY_PATH' in os.environ:
        print('Removed DYLD_LIBRARY_PATH from os.environ')
        del os.environ['DYLD_LIBRARY_PATH']
    if executable is None:
...
def _remote_upsync(server, local, remote, tunnel=None, ignoreReturnCode=False, quiet=False):
    ...
    if not len(server):
        ...
        mkdir=sshOptions()+controlmaster(username,server,port,server0)+" -Y -q -p "+port+" "+username+"@"+server+" 'mkdir -p "+remote+"'"
        _system(mkdir, 'Building remote directory structure...', ignoreReturnCode=True)
        upsync = "{path} {progress} --copy-links --update --inplace --compress --recursive -e '".format(**system_executable('rsync', True)) + sshOptions() + controlmaster(username, server, port, server0) + " -q -p " + port + "' {} " + username + "@" + server + ":" + remote
        upsync = _xargs(upsync, local)
        ...
        while len(upsync):
            command=upsync.pop(0)
            ret_code=_system(command, 'Up-Sync of:', fixedWidthDetails=local_str, ignoreReturnCode=True, progressFunction=progressFunction, quiet=True, std_out=std_out, std_err=std_err)
            if ret_code!=0 and not ignoreReturnCode: break
            segment+=1
...
def _remote_downsync(server, remote, local, tunnel=None, ignoreReturnCode=False, quiet=False):
    ...
    if not len(server):
        ...
        if not os.path.exists(os.path.split(local)[0]):
            os.makedirs(os.path.split(local)[0])
        downsync = "{path} {progress} --copy-links --update --inplace --compress --recursive -e '".format(**system_executable('rsync', True)) + sshOptions() + controlmaster(username, server, port, server0) + " -q -p " + port + "' " + username + "@" + server + ":'{}' " + local
        downsync = _xargs(downsync, remote)
        ...
        while len(downsync):
            command=downsync.pop(0)
            ret_code=_system(command, 'Down-Sync of:', fixedWidthDetails=remote_str, ignoreReturnCode=True, progressFunction=progressFunction, quiet=True, std_out=std_out, std_err=std_err)
            if ret_code!=0 and not ignoreReturnCode: break
            segment+=1
...
def execute(command_line, interactive_input=None, ignoreReturnCode=False, std_out=None, std_err=None, quiet=False,
            arguments='', script=None, use_bang_command='OMFIT_run_command.sh', progressFunction=None, extraButtons=None):
    """
    ...
    if isinstance(script,str):
        if not len(scriptName) and '%s' not in command_line:
            raise RuntimeError('`script` option requires that you place `%s` in the command line location where you want the script filename to appear')
    ...
    if interactive_input:
        command=str(command_line.rstrip())+ """ << __c__MATCHING_EOF__c__\n"""+ \
    ...
    msg += ' of:\n'+command_line.strip()
    msg += ' in:\n'+os.getcwd()
    if not quiet: printi(msg)

    printd(bang_command,topic='execution')

    ret_code=_system(bang_command, message='Running locally:', fixedWidthDetails=command_line,
                     ignoreReturnCode=ignoreReturnCode, std_out=std_out, std_err=std_err,
                     executable='OMFITbash', quiet=quiet, progressFunction=progressFunction, extraButtons=extraButtons) #use /bin/bash because of syntax used by interactive_input
    return ret_code

@_available_to_userTASK
def remote_execute(server, command_line, remotedir, tunnel=None, interactive_input=None, ignoreReturnCode=False,
                   std_out=None, std_err=None, quiet=False, arguments='', script=None, forceRemote=False,
                   use_bang_command='OMFIT_run_command.sh', progressFunction=None, extraButtons=None,
                   xterm=False):
    """
    ...
    if not server:
        ...
        os.chdir(remotedir)
        try:
            return execute(command_line, interactive_input=interactive_input, arguments=arguments,
                           ignoreReturnCode=ignoreReturnCode, std_out=std_out, std_err=std_err, quiet=quiet,
                           script=script, use_bang_command=use_bang_command, progressFunction=progressFunction,
                           extraButtons=extraButtons)
        finally:
            os.chdir(oldDir)
    ...
    def ssh_cd(inv, tty):
        ...
        ssh_inv = 'cat %s | ' % tmp_files[-1] + sshOptions() + controlmaster(username, server, port, server0) + tty + "-Y -q -p " + port + " " + username + "@" + server + " '%s%s -s'" % (header,shell)
    ...
    kill_command = sshOptions() + controlmaster(username, server, port, server0) + " -q -p " + port + " " + username + "@" + server + " 'kill -INT -{PID}; sleep 2; kill -KILL -{PID}'"
    ...
    ret_code = _system(ssh_cd(bang_command, tty), message='Running remotely:', fixedWidthDetails=command_line,
                       ignoreReturnCode=True, std_out=std_out, std_err=std_err, executable='OMFITbash', quiet=quiet,
                       progressFunction=progressFunction, extraButtons=extraButtons)  # use /bin/bash because of syntax used by interactive_input
...
def remote_upsync(server, local, remote, tunnel=None, ignoreReturnCode=False, keepRelativeDirectoryStructure='', quiet=False):
    ...
    if keepRelativeDirectoryStructure:
        ...
        for bd in base_dirs:
            ...
            ret_codes.append(_remote_upsync(server, individual, remote + os.sep + bd + os.sep, tunnel=tunnel, ignoreReturnCode=ignoreReturnCode, quiet=quiet))
...
def remote_downsync(server, remote, local, tunnel=None, ignoreReturnCode=False, keepRelativeDirectoryStructure='', quiet=False):
    ...
    if keepRelativeDirectoryStructure:
        ...
        for bd in base_dirs:
            individual = [str(keepRelativeDirectoryStructure) + os.sep + f for f in bases if os.path.split(f)[0] == bd]
            ret_codes.append(_remote_downsync(server, individual, local + os.sep + bd + os.sep, tunnel=tunnel, ignoreReturnCode=ignoreReturnCode, quiet=quiet))
        return 0 if not np.any(ret_codes) else True
...
class IDL(object):
    ...
    def __init__(self, module_root,
                 server=None, tunnel=None, executable=None,
                 workdir=None, remotedir=None, clean=False):
        ...
        if executable is None:
            try:
                ...
                raise OMFITexception("Server "+server +" does not have `idl` setup. You may need to update OMFIT['MainSettings']['SERVER'] to fix this.")
        ...
        def ssh_cd(inv):
            ...
            return sshOptions()+controlmaster(username,server,port,server0)+" -t -t -Y -q -p "+port+" "+username+"@"+server+" '"+source+"mkdir -p "+remotedir+"\ncd "+remotedir+"\n\n"+inv+"\n\n'"
    ...
    def upsync(self, inputs=[], keepRelativeDirectoryStructure=False, ignoreReturnCode=False, quiet=False):
        ...
        remote_upsync(server=self._OMFITserver,local=upsyncFiles,remote=remotedir,tunnel=self._OMFITtunnel,ignoreReturnCode=ignoreReturnCode,keepRelativeDirectoryStructure=keepRelativeDirectoryStructure,quiet=quiet)
    ...
    def downsync(self, outputs=[], ignoreReturnCode=False, keepRelativeDirectoryStructure=False, quiet=False):
        ...
        if outputs:
            if keepRelativeDirectoryStructure:
                remote_downsync(server=self._OMFITserver,remote=downsyncFiles,local='./',tunnel=self._OMFITtunnel,ignoreReturnCode=ignoreReturnCode,keepRelativeDirectoryStructure=remotedir,quiet=quiet)
            else:
                remote_downsync(server=self._OMFITserver,remote=downsyncFiles,local='./',tunnel=self._OMFITtunnel,ignoreReturnCode=ignoreReturnCode,keepRelativeDirectoryStructure=False,quiet=quiet)
...
def initWorkdir(module_root, server=None, tunnel=None, workdir=None, remotedir=None, clean=True, quiet=False):
    ...
    if cleanLocal and os.path.exists(workdir):
        if 'OMFIT' in workdir or (isinstance(clean, str) and 'force' in clean.lower()):
            if not quiet: printi('Clearing local directory: ' + workdir)
...
To allow clearing of local directory data, add the word "OMFIT" as part of the path, or set clean="local_force".'''.strip())
    ...
    if cleanRemote:
        if 'OMFIT' in remotedir or (isinstance(clean, str) and 'force' in clean.lower()):
            if not quiet: printi('Clearing remote directory: ' + remotedir)
            ...
            remote_execute(server, noalias + mkdir + remotedir + ';' + noalias + 'echo > ' + remotedir + os.sep + utils_base.now("touch_%Y-%m-%d__%H_%M_%S__%f") + ';' + noalias + 'rm -rf ' + remotedir + os.sep + '*', '.' + os.sep, tunnel, ignoreReturnCode=True, quiet=True, use_bang_command=False)
        else:
            raise RuntimeError('This is a safety measure. The remote working directory ' + remotedir + ' does not contain the word "OMFIT" in it.\nTo allow clearing of remote directory data, add the word "OMFIT" as part of the path, or set clean="remote_force".')
    else:
...
@_available_to_userTASK
def executable(module_root=None, inputs=[], outputs=[],
               clean=True, interactive_input=None,
               server=None, tunnel=None, executable=None, workdir=None, remotedir=None,
               ignoreReturnCode=True, std_out=None, std_err=None, quiet=False, queued=False,
               keepRelativeDirectoryStructure=True, arguments='', script=None, forceRemote=False,
               progressFunction=None, use_bang_command='OMFIT_run_command.sh', extraButtons=None,
               xterm=False, clean_after=False):
    """
    ...
    # upload inputs
    if upsyncFiles:
        remote_upsync(server=server, local=upsyncFiles, remote=remotedir, tunnel=tunnel, quiet=quiet,
                      ignoreReturnCode=ignoreReturnCode, keepRelativeDirectoryStructure=os.getcwd())
    ...
    # remote execute
    ret_code = remote_execute(server, executable, remotedir, tunnel, interactive_input=interactive_input,
                              ignoreReturnCode=ignoreReturnCode, std_out=std_out, std_err=std_err, quiet=quiet,
                              arguments=arguments, script=script, forceRemote=forceRemote,
                              use_bang_command=use_bang_command, progressFunction=progressFunction,
                              extraButtons=extraButtons, xterm=xterm)
    ...
    if outputs:
        if keepRelativeDirectoryStructure:
            remote_downsync(server=server, remote=downsyncFiles, local='./', tunnel=tunnel, quiet=quiet,
                            ignoreReturnCode=ignoreReturnCode, keepRelativeDirectoryStructure=remotedir)
        else:
            remote_downsync(server=server, remote=downsyncFiles, local='./', tunnel=tunnel, quiet=quiet,
                            ignoreReturnCode=ignoreReturnCode, keepRelativeDirectoryStructure=False)
...
@_available_to_userTASK
def remote_python(module_root=None, python_script=None, target_function=None, namespace={}, executable=None, forceRemote=False, pickle_protocol=2, clean_local=False, **kw):
    r"""
...
if __name__ == '__main__':
    ...
    if excp:
        raise(excp)
'''.format(target_function=target_function, pickle_protocol=pickle_protocol)
    ...
    _executable(module_root=module_root,
                executable=executable,
...
class manage_job(object):
    def __init__(self, module_root, qsub_job,
                 server=None, tunnel=None, remotedir=None,
    ...
    def qstat(self, quiet=True, sleep=0, std_out=None, std_err=None):
        ...
        remote_execute(self.server, 'sleep ' + str(sleep) + ';' + self.qstat_command,
                       self.remotedir, self.tunnel, std_out=std_out, std_err=std_err, quiet=quiet,
    ...
    def qdel(self, std_out=None, std_err=None):
        ...
        std_err[:] = []
        self.identify_queuing_system()
        remote_execute(self.server, self.qdel_command + ' ' + str(self.jobID),
                       self.remotedir, self.tunnel, std_out=std_out, std_err=std_err, use_bang_command=False)
    ...
    def wait(self, timeout=None, sleep=10):
        ...
        while True:
            ...
            if 'r' not in status.lower() and 'q' not in status.lower() and 'pd' not in status.lower():
                printi('Terminating job ' + self.jobID + ' wait with status: ' + status + ' (%3.1f' % (
                time.time() - t0) + ' sec)')
                break
    ...
    def wait_print(self, output_file='', error_file='', progressFunction=None, extraButtons=None):
        ...
        :param extraButtons: dictionary with key/function that is used to add extra buttons to the GUI. The function receives a dictionary with the process std_out and pid
        """
        remote_upsync(self.server, os.sep.join([OMFITsrc, '..', 'bin', 'qprint']),
                      self.remotedir, tunnel=self.tunnel, ignoreReturnCode=False, quiet=True)
        if not len(output_file):
            output_file = '._no_output_file'
        command = './qprint ' + self.jobID + ' ' + output_file + ' ' + error_file
        if extraButtons is None:
            extraButtons = {}
        retcode = remote_execute(self.server, command, self.remotedir, self.tunnel, quiet=False,
                                 use_bang_command=False, progressFunction=progressFunction,
                                 extraButtons=extraButtons, ignoreReturnCode=True, forceRemote=True)
        if retcode == 0:
            pass
        elif retcode == 9:
...
def job_array(module_root,
    ...
    batch_py = OMFITascii('batch.py', fromString='''
import sys
import os
import shutil
args = sys.argv
...
os.system(command)''')
    ...
    if batch_type=='SLURM':
        batch_sh = OMFITascii('batch.sh', fromString='''#!/bin/bash
#SBATCH -p {partition}
...
$PYTHON batch.py batch_lines $SLURM_ARRAY_TASK_ID
    '''.format(nsims=len(batch_lines.strip().split('\n')) - 1, **locals()))
...
@_available_to_userTASK
def ide(module_root, ide, script=None, inputs=[], outputs=[],
        clean=True, interactive_input=None,
        server=None, tunnel=None, executable=None, workdir=None, remotedir=None,
        ignoreReturnCode=True, std_out=None, std_err=None, keepRelativeDirectoryStructure=True,
        arguments=''):
    """
    ...
    if module_root is OMFIT:
        ...
        if ide not in serverOptions:
            raise OMFITexception('Server `%s` does not have %s. Change/edit %s server accordingly.'%(SERVER(serverOptions['server']),ide,ide))
    else:
        serverOptions=SERVER[module_root]
        if ide not in serverOptions:
            serverOptions=SERVER[ide]
            txt='Server `%s` does not have %s. Fallback on %s default `%s`'%(SERVER(SERVER[module_root]['server']),ide,ide,SERVER(serverOptions['server']))
            if ide not in serverOptions:
    ...
    return _executable(module_root, inputs=inputs, outputs=outputs,
                    clean=clean, interactive_input=interactive_input,
                    server=server, tunnel=tunnel, executable=executable, workdir=workdir, remotedir=remotedir,
                    ignoreReturnCode=ignoreReturnCode, std_out=std_out, std_err=std_err, keepRelativeDirectoryStructure=keepRelativeDirectoryStructure,
...
class archive(object):
    ...
    def store(self, remotedir, filename, quiet=False, background=False, force=False, store_command=None):
        ...
        if not force and not 'OMFIT' in str(remotedir):
            raise RuntimeError('Refusing to store. This is a safety measure. The remote directory '+remotedir+' does not contain the word "OMFIT" in it.\nAdd the word "OMFIT" as part of the path, or set force=True.')
        ...
        command="""(mkdir -p {storedir} && ls -a * | xargs zip -y -r {storedir}/{filename}) {background}""".format(storedir=self.storedir, filename=self.filename, background=background)
    ...
    def restore(self, remotedir, quiet=False, background=False, force=False, restore_command=None):
        ...
        '''
        if not force and not 'OMFIT' in str(remotedir):
            raise RuntimeError('Refusing to restore. This is a safety measure. The remote directory '+remotedir+' does not contain the word "OMFIT" in it.\nAdd the word "OMFIT" as part of the path, or set force=True.')
        ...
        command="""(unzip -o {storedir}/{filename}) {background}""".format(storedir=self.storedir, filename=self.filename, background=background)
...
ttk_styles = ['TButton', 'TCheckbutton', 'TCombobox', 'TEntry', 'TFrame', 'TLabel', 'TLabelFrame', 'TMenubutton',
              'TNotebook', 'TPanedwindow', 'Horizontal.TProgressbar ', 'Vertical.TProgressbar', 'TRadiobutton',
              'Horizontal.TScale', 'Vertical.TScale', 'Horizontal.TScrollbar', 'Vertical.TScrollbar', 'TSeparator',
              'TSizegrip', 'Treeview']
...
def update_gui_theme(location=None):
    ...
    ttk_style.configure('Treeview', rowheight=int(_defaultFont['linespace'] * OMFIT['MainSettings']['SETUP']['GUIappearance']['tree_rowheight_multiplier']))
    ...
    for key in ttk_styles:
        ...
        ttk_style.configure('small.' + key,
                            font=(_defaultFont['family'], _defaultFont['size'] - 2, _defaultFont['weight']))
        ttk_style.configure('flat.' + key, relief=tk.FLAT, width=1,
                            font=(_defaultFont['family'], _defaultFont['size'], _defaultFont['weight']))
        ...
        ttk_style.configure('flatwarning.' + key, font=(_defaultFont['family'], _defaultFont['size'], _defaultFont['weight']), relief=tk.FLAT)
    ...
    if OMFITaux.get('GUI', None) is not None:
        ...
        OMFITaux['GUI'].browser_label_text.configure(background=fbg, foreground=fg, highlightbackground=fbg, height=1)  # Updates browser entry
