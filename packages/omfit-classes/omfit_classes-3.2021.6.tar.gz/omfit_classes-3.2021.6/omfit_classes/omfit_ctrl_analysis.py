...
class ControlAnalysis(OrderedDict):
    ...
    def plot_fit_response(self, order=None, x=None, y=None, u=None, y_err=None, extra_data=None, **kw):
        """
        ...
        if fig is None:
            fig = pyplot.gcf()
        axs = np.zeros((1 + cmd, 1)).astype(object)
        axs[0, 0] = fig.add_subplot(1 + cmd, 1, 1)
        for i in range(1, 1 + cmd):
            axs[i, 0] = fig.add_subplot(1 + cmd, 1, i + 1, sharex=axs[0, 0])
        axu = axs[0, 0]
        axy = axs[int(cmd), 0]
        for ax in axs[:-1, 0]:
        ...
        if twod:
            ...
            for i, shot in enumerate(shots):
                ...
                if u is not None:
                    ...
                    axu.plot(x[:, i], u[:, i], color=c, label=ct_label, linestyle='-' if cmd else '-.')
                ...
                if y is not None:
                    ...
                    uband(x[:, i], yi, ax=axy, color=c, label=m_label)
                # Model data
                if show_model:
                    axy.plot(
                        x[:, i],
                ...
                if show_guess:
                    axy.plot(x[:, i], gy[:, i], color=c, label='Guess' if i == 0 else '', linestyle=':')
        ...
        if yunits:
            axy.set_ylabel('({})'.format(yunits))
        if uunits:
            axu.set_ylabel('({})'.format(uunits))
