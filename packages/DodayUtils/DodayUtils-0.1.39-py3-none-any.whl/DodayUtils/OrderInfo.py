...
class DodayItem:
	...
	"""
	def __init__(self, category, title, options, price, amount=1, comments="", order_no=None, discount=0):
	...
	* payment_method
	...
	def db_format_to_order_info(cls, db_data, **kwargs):
		...
		for i in range(len(sorted_db_data)):
			item_list.append(DodayItem(sorted_db_data[i]["name1"], sorted_db_data[i]["name2"], \
				...
				order_no=int(sorted_db_data[i]["order_no"])))
	...
	def json_format_to_order_info(cls, json_data, **kwargs):
		...
		for iidx in range(len(json_clone["name1"])):
			...
			elif "comments" in json_clone.keys() and json_clone["order_no"] != "None":
	...
	def toprinterargs(self, **kwargs):
		...
		for i in range(total_cnt):
			sub_item_str = str(order_data_print.optional[i])
			sub_item_str = sub_item_str.replace('\n','')
			subitemnames[i].value = sub_item_str.encode('big5')
			itemnames[i].value = order_data_print.name[i].encode('big5')
			itemcounts_list.append(order_data_print.quantity[i])
			item_price = order_data_print.price[i]*order_data_print.quantity[i]
