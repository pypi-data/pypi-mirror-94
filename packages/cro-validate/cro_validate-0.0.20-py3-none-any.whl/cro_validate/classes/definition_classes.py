...
class Definition:
	...
	def _get_obj_model_validator(self):
		...
		for name in fields:
			...
			if not Utils.Empty.isempty(field_definition.default_value):
...
class Index:
	...
	def _eval_data_type_rules(
		...
		if definition.get_data_type() not in Index._data_type_rules:
			return
	...
	def _eval_data_format_rules(
		...
		if definition.get_data_format() not in Index._data_format_rules:
			return
