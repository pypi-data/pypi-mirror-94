...
class Experiment(CnvrgJob, ChartsMixin):
    ...
    def log_artifacts(self, files: list, target=None):
        try:
            ...
            self.project.put_files(files, message="Log Artifacts Commit", payload=payload, prefix=target)
    ...
    def run(command=None, arguments: dict=None, grid: dict=None, title: str=None, project: Project=None,
        ...
        if local:
            ...
            if len(e.get("experiments")) == 1:
                exp = Experiment(e.get("experiments")[0], owner=project.owner+"/"+project.slug)
                exp.notify_running()
                exp.set_as_default()
                return exp
            else:
