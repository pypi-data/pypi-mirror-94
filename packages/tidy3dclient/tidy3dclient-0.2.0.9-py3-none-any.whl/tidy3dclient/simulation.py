...
class Simulation(object):
    ...
    def __init__(self,
        ...
        logging.info(
            "Simulation domain in number of pixels: "
        ...
        # self.Nt = np.array(run_time // self.grid.dt)
        logging.info(f"Total number of time steps: {self.Nt}.")

        # Check the size (in space and time)
        self._check_size()
    ...
    def __repr__(self):
        ...
        rep += "Number of pixels in x, y, z: %s\n" % list2str(
                    self.grid.Nxyz, "%d")
        rep += "Number of pixels including \n" +\
               "symmetries                 : %s\n"%list2str(
                    self.Nxyz_sym, "%d")
        rep += "Number of nodes            : %d\n" % self.Np
