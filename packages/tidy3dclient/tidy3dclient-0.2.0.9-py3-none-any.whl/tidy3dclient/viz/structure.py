...
def _eps_cmap(eps_r, alpha=1, cmap=None, clim=None):
    ...
    # Make eps = 1 completely transparent if overlaying
    if alpha < 1:
        newmap[0, 3] = 0
...
def viz_eps_2D(
    ...
    if ind.size == 0:
        log_and_raise(
            "Plane position outside of simulation domain.",
            RuntimeError
        )
    else:
        ind = ind[0]
...
def _structure_png(self, folder_path, aspect_ratio=None):
    ...
    for norm_ind, normal in enumerate(["x", "y", "z"]):
        ...
        self.viz_eps_2D(normal=normal, position=self.center[norm_ind], ax=ax)
