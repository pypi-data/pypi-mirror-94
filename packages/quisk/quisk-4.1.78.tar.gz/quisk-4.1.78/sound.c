		...
		if (is_cw) {	// Transmit CW
			if (QUISK_CWKEY_DOWN || quiskSpotLevel >= 0) {
				for (i = 0; i < mic_count; i++) {
					...
					if (quiskSpotLevel >= 0)
						cSamples[i] = (CLIP16 - 1) * cwEnvelope * quiskSpotLevel / 1000.0 * tuneVector * quisk_sound_state.mic_out_volume;
					else
						cSamples[i] = (CLIP16 - 1) * cwEnvelope * tuneVector * quisk_sound_state.mic_out_volume;
					tuneVector *= tx_mic_phase;
				}
			}
			else {		// key is up
...
PyObject * quisk_set_sound_name(PyObject * self, PyObject * args)	// Called from GUI thread
{
	...
	if (!PyArg_ParseTuple (args, "iiies#es#", &index, &play, &driver, utf8, &description, &l1, utf8, &device_name, &l2))
		return NULL;
	if (play) {
		strMcpy(PlaybackDevices[index]->name, description, QUISK_SC_SIZE);
		strMcpy(PlaybackDevices[index]->device_name, device_name, QUISK_SC_SIZE);
...
void quisk_open_sound(void)	// Called from GUI thread
{
	...
	strMcpy(DigitalInput.stream_description, "External Digital Input", QUISK_SC_SIZE);
	strMcpy(DigitalOutput.stream_description, "External Digital Output", QUISK_SC_SIZE);
	strMcpy(RawSamplePlayback.stream_description, "Raw Digital Output", QUISK_SC_SIZE);
	strMcpy(quisk_DigitalRx1Output.stream_description, "Digital Rx1 Output", QUISK_SC_SIZE);
	...
	for (i = 0; (pPlay = PlaybackDevices[i]); i++) {
		pPlay->started = 0;
