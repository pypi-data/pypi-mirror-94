...
class Release:
    ...
    def __init__(self, action, project, config, tokens, flags=None):
        ...
        self._countries = self._parse_available_countries()
    ...
    def _parse_available_countries(self):
    ...
    def _deploy(self, version=""):
        ...
        if not version:
            ...
            releases = self._get_releases_since(max(rel["version"] for rel in current))
            if not releases:
        ...
        commits = self._github.get_commits_since_release(self._repo, version)
        self._manage_youtrack_card(commits, version, countries)
    ...
    def _get_releases_since(self, threshold):
        ...
        return releases.sort(reverse=True)[0:4]
    ...
    def _manage_youtrack_card(self, commits, new_version, countries):
            ...
            for issue_id in issue_ids:
                try:
                    self._youtrack.comment(
                        issue_id,
                        f"Deploy in production of {self._project} in countries {countries} done with the release {new_version}",
                    )
