...
def single_predictor_aggr_multi_mod(df, feature, target, weight, predicted, distrib='gaussian', ci=True, autobin=False,
    ...
    The feature chosen to group by can be auto-binned. If the number of unique values df[feature].nunique() > n_vals
    then n_bins are built using a quantile cut. This is done to avoid to bin a predictor with already few unique values.
    The number of minimum unique values (n_vals) and the number of bins (n_bins) can be chosen at will.
    ...
    :param weight: str or None
        the name of the column weight or None. If none, the weights are a vector of ones
    ...
    :param ci: bool, default=True
        compute or not the confidence intervals. Set distrib to 'gaussian' for the Gaussian approximation
    :param autobin: bool, default=False
        autobin or not the predictor to group by
    :param n_vals: int, default=50
        the minimal number of unique values that the predictor should have to be binned, because it makes little sense
    ...
    :return:
     df_average: dataframe
        the df, in long format, with the weighted average and the confidence intervals (if computed)
     df_: dataframe
    ...
    if weight is None:
        ...
        df_[weight] = 1
        if verbose:
            print('Weight not provided, using the Gaussian approx for the CI (instead of the Poisson or Gamma CI)')
    else:
    ...
    if predicted is not None:
        ...
        if isinstance(predicted, pd.DataFrame):
            if names:
                error_message = "names should be a list (of strings) the same length of the predicted dataFrame"
                assert len(names) == len(predicted.columns), error_message
                predicted.columns = names
            else:
                predicted = predicted.add_prefix('model_')
            col_y = predicted.columns.tolist()
            df_ = pd.concat([df_.reset_index(drop=True), predicted.reset_index(drop=True)], axis=1)
        elif isinstance(predicted, pd.Series):
            if names:
                error_message = "names should be a string (a single column in predicted)"
                assert isinstance(names, str), error_message
                predicted.name = names
            col_y = predicted.columns.tolist()
            df_ = pd.concat([df_.reset_index(drop=True), predicted.reset_index(drop=True)], axis=1)
        elif isinstance(predicted, list) and all(isinstance(s, str) for s in predicted):
            df_ = pd.concat([df_.reset_index(drop=True), df[predicted].reset_index(drop=True)], axis=1)
    ...
    # e.g. if the target is a rate (wealth / person) then we need to get back the quantity,
    # group by sum them at the geomid level and then divide the result to have the rate at the geomid level
    ...
    if autobin:
        if df_[feature].nunique() > n_vals:
            df_[feature] = pd.qcut(df_[feature], q=n_bins, duplicates='drop').apply(lambda x: x.mid.round(0)).astype(
                float)
    ...
    df_.loc[:, pred_obs_cols] = df_.loc[:, pred_obs_cols].div(df_[weight].values, axis=0)
    if distrib == 'poisson':
        df_["target_std"] = np.sqrt(df_[target] / df_[weight])
    elif distrib == 'gamma':
        df_["target_std"] = df_[target] * np.sqrt(1 / df_[weight])
    else:
        print('distrib is not in ["freq", "sev"], using Gaussian approx. for the conf. int.')
        df_["target_std"] = df_[target] * np.sqrt(1 / df_['count'])

    df_average = pd.melt(df_[[feature, weight, 'count'] + pred_obs_cols].copy(),
    ...
    if ci:
        if distrib == 'poisson':
            df_average["target_std"] = np.sqrt(df_average['target'] / df_average[weight])
        elif distrib == 'gamma':
            df_average["target_std"] = df_average['target'] * np.sqrt(1 / df_average[weight])
        else:
            print('distrib is not in ["freq", "sev"], using Gaussian approx. for the conf. int.')
            df_average["target_std"] = df_average['target'] * np.sqrt(1 / df_average['count'])
        ...
        upper_bound = df_average["ci_up"].quantile(.975)
        df_average["ci_up"] = df_average["ci_up"].clip(upper=upper_bound)
        df_average = df_average.reset_index()[["model", feature, 'target', "ci_low", "ci_up", "target_std"]]
...
def prepare_geo_data(df, cols_to_plot, target, predicted=None, dissolve_on=None, distrib='gaussian',
    ...
    If weight is provided, then the target (and predicted if not None) are multiplied: target*weight, in order
    to compute the correct average value for each different geoid.

    If dissolve it set to True, the geometries are "dissolved" to go at the upper level, e.g. dissolve counties to
    go at the state level (a state being larger and containing several counties).


    :param df: pd.DataFrame
        the data set, predictors + dependent variable
    :param cols_to_plot: list of str of None
        the columns to plot with the target (e.g. predicted values or any other columns than target)
    :param target: str
    ...
    :param predicted: str or pandas data frame or None, default: None
        if dataframe, should be the same length of df. It could be predicted values, not joined to df
    ...
    if dissolve_on is not None:
        if dissolve_on not in df.columns:
            raise KeyError("the key `dissolve_on` you try to dissolve on is not in df.columns")

        df_average, df_ = single_predictor_aggr_multi_mod(feature=dissolve_on, df=df, weight=weight,
                                                          predicted=predicted, target=target,
                                                          ci=False, autobin=autobin, distrib=distrib,
                                                          n_vals=50, n_bins=n_bins)

        geo_df = geom_merc.dissolve(by=dissolve_on).reset_index()
        geo_df = pd.merge(geo_df, df_.fillna(0), how='left', left_on=[dissolve_on], right_on=[dissolve_on])
    else:
        df_average, df_ = single_predictor_aggr_multi_mod(feature='geoid', df=df, weight=weight,
                                                          predicted=predicted, target=target,
                                                          ci=False, autobin=autobin, distrib=distrib,
                                                          n_vals=50, n_bins=n_bins)
        ...
        geo_df = pd.merge(geo_df, df_.fillna(0), how='left', left_on=[geoid], right_on=['geoid'])
...
def plot_on_map(df, target, dissolve_on=None, distrib='gaussian', plot_uncertainty=True, plot_weight=True,
    ...
    If weight is provided, then weighted average, at the geoid level, of the target is computed. The uncertainty
    (on the weighted average) is computed accordingly to the distribution (gaussian, Poisson or Gamma)
    ...
    :param dissolve_on: str
        ...
        (e.g. moving from counties to states)
    :param distrib: str, default='gaussian'
        the distribution for computing the confidence intervals. Either 'gaussian', 'poisson' or 'gamma'.
    ...
    :param cmap: matplotlib cmap or None
        Please, use ONLY scientific color maps. No jet, no rainbow! If you have any doubt, keep the default
        e.g. http://www.fabiocrameri.ch/colourmaps.php
    :param normalize: bool
        should the other illustrated columns normalized to the target? If True, the vmin and vmax will be
    ...
    if plot_uncertainty and plot_weight:
        ncols = 3
    elif (plot_uncertainty and plot_weight is False) or (plot_uncertainty is False and plot_weight):
        ncols = 2
    else:
        ncols = 1
...
def facet_map(df, cols_to_plot, target, predicted=None, dissolve_on=None,
    ...
    If weight is provided, then weighted average, at the geoid level, of the target is computed.
    ...
    # Create figure and axes (this time it's 9, arranged 3 by 3)
    f, axs = plt.subplots(nrows=n_rows, ncols=ncols, figsize=figsize, facecolor=facecolor)
...
def facet_map_interactive(df, cols_to_plot, target, predicted=None, dissolve_on=None,
    ...
    :param normalize: bool
        should the other illustrated columns normalized to the target? If True, the vmin and vmax will be
        the same for all the panels and computed w.r.t the target colum,
    ...
    if (tiles_src is not None) and (tiles_src not in list(hv.element.tiles.tile_sources.keys())):
        raise ValueError("`tiles_src` should be a string and one "
                         "of {}".format(list(hv.element.tiles.tile_sources.keys())))
    elif (tiles_src is not None) and (tiles_src in list(hv.element.tiles.tile_sources.keys())):
    ...
    for i, col in enumerate([target] + cols_to_plot):
        ...
        if autobin:
            if normalize:
                ser, bins = pd.qcut(geo_df[target], q=n_bins, retbins=True, labels=None, precision=2)
                geo_df[col] = pd.cut(geo_df[col], bins=bins, labels=np.unique(ser),
                                     include_lowest=True).apply(lambda x: x.mid).astype(float)
                # pd.cut(geo_df[col], q=n_bins, duplicates='drop').apply(lambda x: x.mid).astype(float)
            else:
                geo_df[col] = pd.qcut(geo_df[col], q=n_bins,
                                      duplicates='drop', precision=2).apply(lambda x: x.mid).astype(float)
                # .apply(lambda x: x.mid).astype(float)

            hv_plot_list.append(
                tiles * gv.Polygons(geo_df, vdims=[hv.Dimension(col)],
            ...
            hv_plot_list.append(
                tiles * gv.Polygons(geo_df, vdims=[hv.Dimension(col, range=(vmin, vmax))],
                                    crs=ccrs.GOOGLE_MERCATOR).opts(
                    **plot_opts))

    # Display the figure
    hvl = hv.Layout(hv_plot_list).opts(opts.Tiles(width=figsize[0], height=figsize[1])).cols(2)
    return hvl
