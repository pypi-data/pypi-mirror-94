...
def authenticate_proxy(auth_info, response, **args):
    scheme = response.headers.get('Proxy-Authenticate', '')
    context = HttpNegotiateAuthSSPIContext(username=auth_info.username, password=auth_info.password, domain=auth_info.domain)
    return _retry_using_http_Negotiate_auth(context, response, scheme, args)


def _retry_using_http_Negotiate_auth(context,response, scheme, args):
    if 'Proxy Authorization' in response.request.headers:
        return response

    if context._host is None:
        targeturl = urlparse(response.request.url)
        context._host = targeturl.hostname
        try:
            context._host = socket.getaddrinfo(context._host, None, 0, 0, 0, socket.AI_CANONNAME)[0][3]
        except socket.gaierror as e:
            _logger.info('Skipping canonicalization of name %s due to error: %s', context._host, e)

    targetspn = '{}/{}'.format(context._service, context._host)

    # Set up SSPI connection structure
    pkg_info = win32security.QuerySecurityPackageInfo(scheme)
    clientauth = sspi.ClientAuth(scheme, targetspn=targetspn, auth_info=context._auth_info)
    ...
    try:
        ...
        request.headers['Proxy-Authorization'] = '{} {}'.format(scheme,
    ...
    if response2.status_code != 407:
        if response2.status_code == 200:
            # Kerberos may have succeeded; if so, finalize our auth context
            final = response2.headers.get('Proxy-Authenticate')
            if final is not None:
                try:
                    ...
                    error, auth = clientauth.authorize(sec_buffer)
                    _logger.debug('Kerberos Authentication succeeded - error={} authenticated={}'.format(error,
    ...
    try:
        error, auth = clientauth.authorize(sec_buffer)
        proxy_auth = '{} {}'.format(scheme,base64.b64encode(auth[0].Buffer).decode('ASCII'))
...
class HttpNegotiateAuthSSPIContext():
    _auth_info = None
    _service = 'HTTP'
    _host = None
    _delegate = False
