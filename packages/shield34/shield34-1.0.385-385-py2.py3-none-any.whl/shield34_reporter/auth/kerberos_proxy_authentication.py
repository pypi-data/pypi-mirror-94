...
class HTTPKerberosAuth(AuthBase):
    ...
    object."""
    def __init__(
            self, mutual_authentication=OPTIONAL,
    ...
    def authenticate_user(self, response, **kwargs):
        ...
        proxy_url = kwargs['proxies'][scheme]
        if not proxy_url.startswith(scheme):
    ...
    def handle_407(self, response, **kwargs):
        ...
        log.debug("handle_407(): Handling: 401")
    ...
    def handle_response(self, response, **kwargs):
        ...
        if response.status_code == 407 and num_407s < 2:
            ...
            log.debug("handle_response() has seen %d 401 responses", num_407s)
            num_407s += 1
...
def authenticate_proxy(auth_info, response, **args):
    ...
    try:
        auth.pos = response.request.body.tell()
    except AttributeError:
        pass
    ...
    return auth.handle_response( response, **args)