...
def get_route_push_value(device, address, expected_table_name):
    ...
    for route in route_table_list:
        if expected_table_name == route.q.get_values('table-name') and \
            address == route.q.get_values('rt-destination'):
            mpls_label = route.q.get_values('mpls-label',0)
...
def get_route_table_first_label(device, table, address):
    ...
    mpls_label= out.q.contains(address).get_values('mpls_label',0)
