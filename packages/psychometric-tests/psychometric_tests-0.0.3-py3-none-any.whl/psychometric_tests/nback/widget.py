...
class nBack_Widget(QtWidgets.QWidget):
    def __init__(self, setup_info):
        ...
        self.settings_file = str(defs.project_root() / 'nback.ini')
        ...
        self.save_path = Path(
            setup_info['Save Directory']) / 'NBack_ID{}-s{}-{}.csv'.format(
            *setup_info.values())
        ...
        self.stimulus_font = QtGui.QFont(defs.font, 64, QtGui.QFont.Bold)
        self.answer_font = QtGui.QFont(defs.font, 48, QtGui.QFont.Normal)
        self.stimulus_font.setStyleHint(QtGui.QFont.SansSerif)
    ...
    def gui_save(self, settings):
        ...
        settings.setValue('Widget/stimulus_font', self.stimulus_font)
    ...
    def gui_restore(self, settings):
        ...
        if settings.value('Widget/stimulus_font') is not None and \
                settings.value('Widget/answer_font') is not None:
            self.stimulus_font = settings.value('Widget/stimulus_font')
