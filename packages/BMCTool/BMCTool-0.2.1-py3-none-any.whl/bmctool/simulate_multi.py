...
def simulate_multi(config_file: (str, Path) = None, seq_file: (str, Path) = None):
    ...
    # vary_vals = np.linspace(-0.4, 0.4, 3)  # dB0
    # vary_vals = np.linspace(0.7, 1.3, 3)  # rB1
    # vary_vals = np.linspace(0.05, 7, 3)  # T1
    # vary_vals = np.linspace(0.15, 20, 10)  # R1
    vary_vals = np.linspace(0.01, 2, 3)  # T2
    ...
    for val in vary_vals:
        ...
        sim_params.update_water_pool(r2=1/val)
...
if __name__ == '__main__':
    ...
    for spec in specs:
        plt.plot(offsets, spec, marker='o', linestyle='--')
    plt.title(f'T2')
