...
model_content = """
envprotecc.vortex.Vortex.SECRETS: TaintSource[Secret] = ...
def envprotecc.vortex.Vortex.taint_arg(self, arg: TaintSink[Endpoint]): ...
def envprotecc.vortex.Vortex.taint_arg(self, arg: TaintInTaintOut[LocalReturn]): ...
def envprotecc.vortex.Vortex.register_endpoint(self, func: TaintSink[Endpoint]): ...
"""
...
def init(config_path):
    ...
    os.mkdir(stubs_path)
    os.mkdir(os.path.join(stubs_path, "envprotecc"))
    with open(os.path.join(stubs_path, "envprotecc", "vortex.pyi"), "w") as fp:
