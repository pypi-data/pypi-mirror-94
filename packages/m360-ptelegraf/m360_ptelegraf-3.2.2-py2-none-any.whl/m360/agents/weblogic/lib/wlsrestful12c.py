...
class WLS(object):
    ...
    def get(self, url, **kwargs):
        ...
        if url in self._cache:
            ...
            if (now-timestamp) <= EXPIRED_CACHE_SEGS:
                ...
                return self._handle_response(self._cache[url]['response'])
        response = self.session.get(url, timeout=self.timeout,verify=False)
        self._cache[url]={'time':time.time(),'response':response}
    ...
    def _handle_response(self, response):
        logger.debug(
            ...
            response.request.body,extra={"monitor":"Weblogic","modulo":"NA"}
        )
        logger.debug(
            'Recieved response:\nHTTP %s\n%s\n\n%s',
            response.status_code,
            '\n'.join(["{0}: {1}".format(k, v) for k, v in response.headers.items()]),
            response.content.decode(),extra={"monitor":"Weblogic","modulo":"NA"}
        )
...
class WLSObject(object):
    ...
    def __init__(self, name, url, wls):
        ...
        arrurl = url.split('/')
        self_link=url
        if "JMSServers" in arrurl and "JMSRuntime" not in arrurl:
            newself_link = []
            for uri in arrurl:
                if uri == "JMSServers":
                    newself_link.append("JMSRuntime")
                newself_link.append(uri)
            self_link = "/".join(newself_link)
