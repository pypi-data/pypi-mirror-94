...
from m360.agents.internal.instance import Instance
from m360.agents.internal.settings import Settings


class Monitor (base.Monitor):
    def __init__(self,host,user=None, settings=None,outputs=None,service=None,queue=None,loop=False):
        instances = []
        m_outputs = []
        if settings is None:
            settings = []
        ...
        for name, conf in settings.items():
            if name.startswith("outputs."):
                ...
                instances.append(Instance(instance_name, host, conf,queue))
            else:
