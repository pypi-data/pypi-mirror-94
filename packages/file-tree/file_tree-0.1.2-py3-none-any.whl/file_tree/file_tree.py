...
class FileTree:
    ...
    def add_subtree(self, sub_tree: "FileTree", precursor: Optional[Union[str, Sequence[str]]]=(None, ), parent_template: Optional[str]="") -> None:
        ...
        for name in precursor:
            ...
            self.placeholders.update({add_string + key: value for key, value in sub_tree_fill.placeholders.items()})
    ...
    def update_glob(self, template_key: str, inplace=False) -> "FileTree":
        """Updates any undefined placeholders based on the which files exist on disk for template

        Args:
            template_key (str): key of the template to use
            inplace (bool): if True change the placeholders in-place (and return the FileTree itself); 
                by default a new FileTree is returned with the updated values without altering this one.
        ...
        return self.update(inplace, **new_placeholders)
