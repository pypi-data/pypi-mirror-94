...
class Entry(GLXCurses.Widget, GLXCurses.Editable, GLXCurses.Movable):
    ...
    def _draw_entry(self):
        ...
        self.preferred_width = estimated_preferred_width
        self.preferred_height = 1
    ...
    def _draw_the_good_entry(self, color=None):
        ...
        # Normal
        self.add_string(self.y_offset, self.x_offset, message_to_display, color)
        ...
        if self.can_focus:
            if (
                ...
                # Cursor print
                try:
                    self.subwin.move(self.y_offset, self.x_offset + self.get_position())
                    curses.curs_set(1)
                    # self.add_string(
                    #     self.y_offset,
                    ...
                    #     color=self.style.color(
                    #         bg=self.style.get_color_text('black', 'STATE_NORMAL'),
                    #         fg=self.style.get_color_text('bg', 'STATE_SELECTED')
        ...
        if self.get_completion() is not None:
            ...
            if self.has_prelight:
                color = self.style.color(
                    ...
                    bg=self.style.attribute_to_rgb("bg", "STATE_PRELIGHT"),
                )

            elif self.state["NORMAL"]:
                color = self.style.color(
                    fg=self.style.attribute_to_rgb("text", "STATE_NORMAL"),
                    bg=self.style.attribute_to_rgb("bg", "STATE_NORMAL"),
                )
            else:
                color = self.style.color(
                    fg=self.style.attribute_to_rgb("text", "STATE_NORMAL"),
            ...
            if self.y_offset >= y > self.y_offset - self.preferred_height:
                if (self.x_offset - 1) + self.width >= x > (self.x_offset - 1):
                    ...
                    if event == curses.BUTTON1_TRIPLE_CLICKED:
                        self.select_region(start_pos=0, end_pos=len(self.get_text()))
                        self._grab_focus()
