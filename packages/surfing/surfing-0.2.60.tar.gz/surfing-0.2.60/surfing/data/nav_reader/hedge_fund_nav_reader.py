...
class HedgeFundNAVReader:
    ...
    @staticmethod
    def _read_for_HuangChengZhiYuanNo3WithoutV(file_path: str) -> pd.DataFrame:
    ...
    def read_navs_and_dump_to_db(self):
        ...
        for name, comp_date in data.items():
            ...
            try:
                if 'SGM473' in name:
                    df = HedgeFundNAVReader._read_for_HuangChengZhiYuanNo3WithoutV(file_path)
                elif '华澄致远三号' in name:
    ...
    def _dump_to_db(self, df: pd.DataFrame):
        ...
        if now_df is not None and not now_df.empty:
            ...
            now_df = now_df.drop(columns=['update_time', 'create_time', 'is_deleted']).sort_values(by=['fund_id', 'datetime']).drop_duplicates(subset=['fund_id', 'datetime'], keep='last')
            now_df = now_df.astype({'net_asset_value': 'float64', 'adjusted_net_value': 'float64', 'v_net_value': 'float64'})
            ...
            df = df[df._merge == 'left_only'].drop(columns=['_merge', 'insert_time_y', 'calc_date_y', 'adjusted_net_value_y']).rename(columns={'insert_time_x': 'insert_time', 'calc_date_x': 'calc_date', 'adjusted_net_value_x': 'adjusted_net_value'})
            if df.empty:
                return
