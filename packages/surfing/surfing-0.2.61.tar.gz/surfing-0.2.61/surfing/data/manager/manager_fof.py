...
from ..view.basic_models import FOFManually, HedgeFundNAV, FOFInvestorPosition
...
class FOFDataManager(metaclass=Singleton):
    ...
    def _calc_virtual_net_value():
        ...
        asset_allocation = asset_allocation[asset_allocation.fof_id == FOFDataManager._FOF_ID]
        ...
        for row in asset_allocation.itertuples(index=False):
            init_water_line.loc[row.datetime, row.fund_id] = acc_nav.at[row.confirmed_date, row.fund_id]
    ...
    def _insert_errors_to_db_from_file(self, path: str = ''):
        ...
        manually = BasicDataApi().get_fof_manually([FOFDataManager._FOF_ID]).drop(columns=['update_time', 'create_time', 'is_deleted'])
        ...
        if is_from_excel:
            ...
            for date in self._date_list:
                try:
                    ...
                    investor_pos_list.append(scale_data.loc[scale_data.share.isnull(), ['fof_id', 'investor_id', 'amount', 'unit_total']])
            ...
            self._fof_investor_pos = pd.concat(investor_pos_list).rename(columns={'unit_total': 'shares'}).reset_index(drop=True)
    ...
    def dump_fof_nav_and_pos_to_db(self) -> bool:
        ret = False
        ...
        if self._fof_investor_pos is not None:
            now_df = FOFDataManager().get_fof_investor_position([FOFDataManager._FOF_ID])
            if now_df is not None:
                now_df = now_df.drop(columns=['update_time', 'create_time', 'is_deleted'])
                # merge on all columns
                df = self._fof_investor_pos.merge(now_df, how='left', on=['fof_id', 'investor_id', 'amount', 'shares'], indicator=True, validate='one_to_one')
                df = df[df._merge == 'left_only'].drop(columns=['_merge', 'datetime_y']).rename(columns={'datetime_x': 'datetime'})
            else:
                df = self._fof_investor_pos
            print(df)
            if not df.empty:
            ...
            print('[dump_fof_nav_and_pos_to_db] no investor position, should calc it first')

        return ret

    def _gather_asset_info_of_fof(self, nav_df: pd.DataFrame):
        fof_aa = FOFDataManager.get_fof_asset_allocation([FOFDataManager._FOF_ID])
        ...
        for fund_id in fund_list:
            try:
                aa_info = fof_aa[(fof_aa.fund_id == fund_id) & (fof_aa.status == FundStatus.DONE)]
                if aa_info.empty:
                    continue
                aa_insentive_info = fof_aa[(fof_aa.fund_id == fund_id) & (fof_aa.status == FundStatus.INSENTIVE_DONE)]
                ...
                fund_data = {
                    ...
                    'total_ret': f'{round((fund_auv.array[-1] / fund_auv.array[0] - 1) * 100, 2)}%',
                ...
                last_date_data = fund_auv[fund_auv.index <= last_date].iloc[[-1]]
                ...
                fund_auv = fund_auv[fund_auv.index >= aa_info.sort_values(by='confirmed_date').confirmed_date.array[0]]
                res_status = Calculator.get_stat_result(fund_auv.index, fund_auv.array)
                mv = (aa_info.unit_total.sum() - aa_info.share.sum() - aa_insentive_info.share.sum()) * fund_nav.net_asset_value.array[-1]
                holding_data = {
                    'start_date': res_status.start_date.isoformat(),
                    'days': (fund_auv.index.array[-1] - res_status.start_date).days,
                    'mv': mv,
                    # TODO 这里需要考虑正常赎回的情况
                    'total_ret': f'{round((mv / aa_info.amount.sum() - 1) * 100, 2)}%',
                    'avg_cost': round(aa_info.amount.sum() / (aa_info.unit_total.sum() - aa_insentive_info.share.sum()), 4),
                    'sharpe': round(res_status.sharpe, 6),
                    'annualized_ret': f'{round(res_status.annualized_ret * 100, 2)}%',
                    'mdd': f'{round(res_status.mdd * 100, 2)}%',
                    'annualized_vol': round(res_status.annualized_vol, 6),
                }
                latest_nav_date = nav_df.loc[[fund_id], 'datetime'].iat[-1]
                self._wechat_bot.send_hedge_fund_info(self._FOF_ID, fund_id, hedge_info.at[fund_id, 'brief_name'], latest_nav_date, fund_data, holding_data, ret_data_with_last_date)
            except Exception as e:
                traceback.print_exc()
                self._wechat_bot.send_hedge_fund_info_failed(self._FOF_ID, fund_id, e)

    def _gather_fof_info(self):
        try:
            # fof_info = FOFDataManager.get_fof_info([FOFDataManager._FOF_ID]).set_index('fof_id')
            fof_sa = FOFDataManager.get_fof_scale_alteration([FOFDataManager._FOF_ID])
            fof_nav = FOFDataManager.get_fof_nav(FOFDataManager._FOF_ID)
            ...
            fof_data = {
                ...
                'mv': (fof_sa.unit_total.sum() - fof_sa.share.sum()) * nav,
                'nav': round(nav, 4),
                'total_ret': f'{round(((fof_nav.array[-1] / fof_nav.array[0]) - 1) * 100, 2)}%',
                'sharpe': round(res_status.sharpe, 6),
                'annualized_ret': f'{round(res_status.annualized_ret * 100, 2)}%',
                'mdd': f'{round(res_status.mdd * 100, 2)}%',
                'annualized_vol': round(res_status.annualized_vol, 6),
            }
            self._wechat_bot.send_fof_info(FOFDataManager._FOF_ID, fof_nav.index.array[-1], fof_data)
        except Exception as e:
            traceback.print_exc()
            self._wechat_bot.send_fof_info_failed(self._FOF_ID, e)
    ...
    def service_start(self):
        ...
        if nav_df is not None:
            self._gather_asset_info_of_fof(nav_df)
        try:
            self.calc_nav()
            ret = self.dump_fof_nav_and_pos_to_db()
            if ret:
                self._gather_fof_info()
        except Exception as e:
            traceback.print_exc()
    ...
    def get_investor_return(fof_id: str, *, investor_id_list: Tuple[str] = (), start_date: Optional[datetime.date] = None, end_date: Optional[datetime.date] = None) -> Optional[pd.DataFrame]:
        ...
        for investor_id, datas in investor_mvs.items():
            ...
            investor_returns[investor_id] = {'v_nav': avg_v_nav, 'total_rr': total_rr, 'amount': datas.amount.sum(), 'shares': total_share}
    ...
    @staticmethod
    def virtual_backtest(hedge_fund_ids: Dict[str, float], start_date: datetime.date, end_date: Optional[datetime.date] = None, benchmark_ids: Tuple[str] = ()) -> Optional[Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, List[str]]]:
        '''
        对私募基金做虚拟回测
        ...
        try:
            ...
            # 获取私募基金净值数据
            fund_nav = BasicDataApi().get_hedge_fund_nav(list(hedge_fund_ids.keys()))
            if fund_nav is None:
                return
            # 这里首选复权净值
            ...
            fund_nav = FOFDataManager._do_calc_v_net_value(adj_nav, adj_nav, adj_nav.loc[adj_nav.index >= start_date, :].bfill().iloc[0, :], fund_info.incentive_fee_ratio.fillna(DEFAULT_INCENTIVE_RATIO), fund_info.v_nav_decimals.fillna(DEFAULT_DECIMAL))
            # water_line = pd.Series({'SGM473': 1, 'SGM992': 1, 'SJE335': 1, 'SLC213': 1, 'SNG191': 1, 'SNH765': 1, 'SR9762': 1.1146})
            # fund_nav = FOFDataManager._do_calc_v_net_value(adj_nav, adj_nav, water_line, fund_info.incentive_fee_ratio.fillna(DEFAULT_INCENTIVE_RATIO), fund_info.v_nav_decimals.fillna(DEFAULT_DECIMAL))
            # fund_nav = FOFDataManager._do_calc_v_net_value(adj_nav, adj_nav, fund_info.water_line.fillna(INIT_NAV), fund_info.incentive_fee_ratio.fillna(DEFAULT_INCENTIVE_RATIO), fund_info.v_nav_decimals.fillna(DEFAULT_DECIMAL))
