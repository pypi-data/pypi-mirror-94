...
def GARCHjdiff_lncondassetprob(y,u_prev,u_this,dt,rho,sigma,mu,alpha,xi,lamb,gamm,omeg,lncp):
...
#            print(2,vol_tmp)
    ...
    mix_yret[:,:,:]-=np.tile(np.tile(coeff_dt+coeff_iu*iu_tmp*dt \
               +coeff_invsqrtu * dt / np.sqrt(iu_tmp) \
               +coeff_sqrtu * dt * np.sqrt(iu_tmp) \
               +coeff_dsqrtu*dsqrtu_tmp,(Nobs,1)).T,(Nmix,1,1)).T
