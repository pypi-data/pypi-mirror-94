...
from mercuryclient.types.cibil.request import CibilRequest, Applicant, Identity, Address


class CibilMixinTest(TestCase):
    def setUp(self):
        ...
        self.request_obj = CibilRequest(
            ...
            applicant=Applicant(
                ...
                gender="FEMALE",
                identities=[Identity(id_number="ABCDEFGH", id_type="PAN_CARD")],
                addresses=[
                    Address(
        ...
        self.request_dict = {
            ...
            "applicant": {
                ...
                "date_of_birth": "2000-01-01",
                "gender": "FEMALE",
                "identities": [{"id_number": "ABCDEFGH", "id_type": "PAN_CARD"}],
                "addresses": [
                    {
                        "address_line_1": "Random Address",
                        "city": "Random City",
        ...
        self.sleep_mock = mock.patch("mercuryclient.mixins.cibil.time.sleep").start()
        self.sleep_mock.return_value = None
        self.addCleanup(self.get_api_mock.stop)

    def test_request_cibil_report(self):
        mock_response = mock.MagicMock()
        mock_response.status_code = 201
        self.post_api_mock.return_value = ("random_string", mock_response)
        self.client.request_cibil_report(self.request_obj, "some_profile")
    ...
    def test_request_api_succeeds_if_status_code_success(self):
        ...
        request_id = self.client.request_cibil_report(self.request_obj, "some_profile")
        self.assertEqual(request_id, "random_string")

    def test_response_cibil_report(self):
        mock_response = mock.MagicMock()
        mock_response.status_code = 200
        self.get_api_mock.return_value = ("random_string", mock_response)
        self.client.get_cibil_response("random_string")
    ...
    def test_response_api_succeeds_if_status_code_success(self):
        ...
        request_id, result = self.client.get_cibil_response("random_string")
        self.assertEqual(request_id, "random_string")
    ...
    def test_entire_request_response_flow(self):
        ...
        response = self.client.fetch_cibil_report(self.request_obj, "some_profile")
    ...
    def test_entire_request_response_flow_failure(self):
        ...
        with self.assertRaises(Exception):
            self.client.fetch_cibil_report(self.request_obj, "some_profile")
