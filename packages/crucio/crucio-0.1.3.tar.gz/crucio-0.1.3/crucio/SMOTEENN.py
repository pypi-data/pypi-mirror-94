...
class SMOTEENN:
    ...
    def undersample(self,target,majority_class,minority_class):
        '''
    ...
    def balance(self, df : pd.DataFrame, target : str) -> pd.DataFrame:
        ...
        self.undersample(target,majority_class,minority_class)
        #find difference 
        ...
        for _ in range(difference):
            ...
            neighbours_indexes = self.get_k_neighbours(example) 
            ...
            alpha = random.random()
            new_example = example+alpha*(selected_neighbour-example)#check this out
            #add it to df
    ...
    def get_k_neighbours(self,example):
        '''
