...
class ResPartner(models.Model):
    ...
    def _name_search(
        ...
        # Partial VATS (DNI formatted) are not found in res_partner's name_search
        vat_results = self.env['res.partner'].search([('vat', 'ilike', name)])
        if vat_results:
            result.extend(models.lazy_name_get(vat_results))
