...
def retry(exceptions=Exception, tries=3, backoff_factor=1, check=None, retry_log=None):
    """
...
def wait_for_event(event, duration, check, step=0.1):
    ...
    if not event.is_set():
        raise Exception("Thread could not be created")


def requires_random_bind(address: str = None):
    """
    :param address:
    :return: False if address does not start with tcp://, False if address contains a port number. True otherwise
    """
