...
class otaku(Base):
    ...
    def extract(self, id):
        ...
        if (eps := soup.findAll(class_="episodelist")[1:]):
            ...
            for li in eps[0].findAll("li"):
                ch[li.a.text] = self.re.search(r"tv/([^>]+)", li.a.attrs["href"]).group(1)
            id = self.choice(ch.keys())
            return self.extract(ch[id])
        else:
            dls = {}
    ...
    def search(self, query, page=1):
        ...
        for li in ul.findAll("li"):
            result = {
                "title": li.a.text,
                "id": self.re.search(r"tv/(.+?)/?$", li.a.attrs["title"]).group(1)
            }
            yield result
